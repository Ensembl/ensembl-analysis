...
sub new {
  ...
  my ( $genes, $ignorewarnings, $adaptor, $slice ) = rearrange(
    [ qw ( GENES
        IGNOREWARNINGS
        ADAPTOR
        SLICE
...
sub check_Clustering {
  ...
  if ( scalar(@clusters) < scalar(@$genes) ) {
    $self->add_Error( "Reclustering reduced number of genes from " . scalar(@$genes) . " to " . scalar(@clusters) . "\n" );
  }
  elsif ( scalar(@clusters) > scalar(@$genes) ) {
    $self->add_Error( "Reclustering increased number of genes from " . scalar(@$genes) . " to " . scalar(@clusters) . "\n" );
  }
  return \@clusters;
} ## end sub check_Clustering
  ...
  if ( $cluster1->overlaps($cluster2) ) {
         ...
         ( $cluster1->start > $cluster2->start && $cluster1->end > $cluster2->end ) )
    {
      $self->add_Error( "Interlocking genes on the $strand_name strand (bounds " .
                        $cluster1->start . "-" . $cluster1->end . " and " . $cluster2->start . "-" . $cluster2->end . ")\n" );
    }
    else {
      ...
      my @containing_exons        = sort { $a->start <=> $b->start } @containing_exons_unsort;
      my @contained_exons         = sort { $a->start <=> $b->start } @contained_exons_unsort;

      my ( $left_exon_first, $right_exon_first ) = $self->find_enclosing_Exons( \@containing_exons, $contained_exons[0] );
      my ( $left_exon_last,  $right_exon_last )  = $self->find_enclosing_Exons( \@containing_exons, $contained_exons[$#contained_exons] );
      if ( $left_exon_first != $left_exon_last || $right_exon_first != $right_exon_last ) {
    ...
    } ## end else [ if ( ( $cluster1->start...))]
  } ## end if ( $cluster1->overlaps...)
} ## end sub check_for_interlock
    ...
    if ( scalar(@gene_exons) ) {
      ...
      foreach my $exon (@gene_exons) {
        if ( $strand != $exon->strand ) {
          $self->add_Error( "Gene " . $gene->dbID . " has exons on different strands\n" );
        }
        if ( $exon->strand == 1 ) {
          ...
          $self->add_Error( "Gene " . $gene->dbID . " has an unstranded exon (" . $exon->dbID . ")\n" );
        }
        if ( exists( $exon_gene_map{ $exon->dbID } ) ) {
          $self->add_Error( "Seen exon " .
                   $exon->dbID . " more than once (in gene " . $gene->dbID . " and gene " . $exon_gene_map{ $exon->dbID }->dbID . ")\n" );
        }
...
FEXON: foreach my $f_exon (@sorted_forward_exons) {
    ...
    for ( my $i = 0; $i < $n_r_exon; $i++ ) {
      ...
      if ( $r_exon->overlaps($f_exon) ) {
        $self->add_Error(
            "Overlapping exons on two strands for exons " . $f_exon->dbID . " (gene = " . get_gene_id( $exon_gene_map{ $f_exon->dbID } ) .
        ...
        foreach my $f_trans ( @{ $f_cluster->get_Transcripts } ) {
          foreach my $r_trans ( @{ $r_cluster->get_Transcripts } ) {
            ...
            if ( $self->_all_exons_overlap( \@f_exons, \@r_exons, "forward" ) ) {
              $self->add_Error( "Transcript with all (" .
                                scalar(@f_exons) . ") overlapping exons on other strand for " .
                                get_transcript_id($f_trans) . " and " . get_transcript_id($r_trans) . " (forward)\n" );
            }
            if ( $self->_all_exons_overlap( \@r_exons, \@f_exons, "reverse" ) ) {
              $self->add_Error( "Transcript with all (" .
                                scalar(@r_exons) . ") overlapping exons on other strand for " .
                                get_transcript_id($f_trans) . " and " . get_transcript_id($r_trans) . " (reverse)\n" );
            }
          }
        }
      }
    }
  } ## end foreach my $f_cluster (@forward_clusters)
} ## end sub find_StrandOverlap_Transcripts
