...
sub get_chrlengths_v20 {
  ...
  if ( $coordsystem ne 'toplevel' ) {
    ...
    my $query =
      "select seq_region.name, seq_region.length as mce from seq_region,coord_system where" .
    ...
    if ($ignore_haplotypes) {
      $query .=
" and seq_region.seq_region_id not in (select seq_region_id from assembly_exception where exc_type in ('HAP','PATCH_NOVEL','PATCH_FIX'))";
    }
  ...
  } ## end if ( $coordsystem ne 'toplevel')
  else {
    ...
    foreach my $slice (@slices) {
      ...
      if ( $slice->coord_system->version eq $type ) {
        ...
        print STDERR "WARNING: " . $slice->seq_region_name . " " .
          $slice->length . " " . $slice->coord_system->version . " is NOT on requested assembly version but is toplevel, will skip it!\n";
      }
    }
  }
  return \%chrhash;
} ## end sub get_chrlengths_v20
