...
sub new {
  ...
  my ( $transcript,     $minshortintronlen,  $maxshortintronlen, $minlongintronlen, $minshortexonlen, $maxshortexonlen,
       $minlongexonlen, $maxexonstranscript, $mintranslationlen, $ignorewarnings,   $slice,           $adaptor )
    = rearrange(
    [ qw ( TRANSCRIPT
        MINSHORTINTRONLEN
        MAXSHORTINTRONLEN
...
sub output {
  ...
  print "\n===\n";
  print "Transcript " . $transcript->dbID . " " . ( defined( $transcript->stable_id ) ? $transcript->stable_id : "" ) . "\n";
  ...
  foreach my $exon ( @{ $transcript->get_all_Exons() } ) {
    printf "  Exon %9d %9d %9d %9d %9d %9d %9d", $exon->dbID, $exon->start + $vcoffset, $exon->end + $vcoffset, $exon->strand,
      ( $exon->end - $exon->start + 1 ), $exon->phase, $exon->end_phase;
...
sub check {
  ...
  if ( $numexon == 0 ) {
    ...
    $self->add_Error( "Unusually large number of exons (" . $numexon . ")\n", 'manyexons' );
  }
...
EXON: foreach my $exon (@sortedexons) {
    if ( $exon != $exons[ $exnum++ ] ) {
      $self->add_Error( "Incorrect exon ranks (first at exon " . $exons[ $exnum++ ]->dbID .
                          ")\n" . "NOTE: Further error checking (except translations) " . "done with resorted exons.\n",
                        'exonrank' );
      last EXON;
    }
  }
  ...
  if ( defined( $self->transcript->translation ) ) {
    ...
    for ( my $i = 1; $i < scalar(@exons); $i++ ) {
      ...
      if ( $exon->phase != $prev_exon->end_phase ) {
        $self->add_Error( "EndPhase/Phase mismatch (" . $prev_exon->end_phase .
                            " and " . $exon->phase . ") between exons " . $prev_exon->dbID . " and " . $exon->dbID . "\n",
                          'endphase_phase' );
      }

      if ( $exon->phase != $calc_phases[$i] ) {
        $self->add_Error( "Phase wrong for " . $exon->dbID . " is " . $exon->phase . " should be " . $calc_phases[$i] . "\n",
                          'startphase' );
      }
      ...
      if ( $exon->end_phase != $calc_endphase && $i != $#exons ) {
        $self->add_Error( "EndPhase wrong for " . $exon->dbID . " is " . $exon->end_phase . " should be " . $calc_endphase . "\n",
                          'endphase' );
      }
      $prev_exon = $exon;
    }
    if ( $zero_phase_count/scalar(@exons) < 0.20 && scalar(@exons) > 8 ) {
      $self->add_Warning(
                "Suspiciously high number of non phase zero exon (num phase zero = $zero_phase_count nexon = " . scalar(@exons) . ")\n" );
    }
  } ## end if ( defined( $self->transcript...))
} ## end sub check_Phases
...
sub check_Translation {
  ...
  eval { $pepseq = $self->transcript->translate; };
  if ( defined($pepseq) ) {
...
sub check_Structure {
  ...
  foreach my $exon (@$sortedexons) {
    ...
    if ( $exlen >= $self->minshortexonlen && $exlen <= $self->maxshortexonlen ) {
      $self->add_Error( "Short exon (" . $exlen . " bases) for exon " . $exon->dbID . "\n", 'shortexon' );
    }
    elsif ( $exon->length >= $self->minlongexonlen ) {
      $self->add_Error( "Long exon (" . $exlen . " bases) for exon " . $exon->dbID . "\n", 'longexon' );
    }
    if ( defined $prev_exon ) {
      ...
      elsif ( ( $exon->strand == 1 && $exon->start < $prev_exon->end ) || ( $exon->strand == -1 && $exon->end > $prev_exon->start ) ) {
    ...
    $prev_exon = $exon;
  } ## end foreach my $exon (@$sortedexons)
...
EXON: foreach my $exon (@$exons) {
    ...
    $rank++;
  } ## end EXON: foreach my $exon (@$exons)
  if ( !$foundstart ) {
    $self->add_Error( "Didn't find translation->start_exon (" . $trans_start_exon->dbID . ")\n" );
  }
  if ( !$foundend ) {
    $self->add_Error( "Didn't find translation->end_exon (" . $trans_end_exon->dbID . ")\n" );
  }
...
sub check_Intron {
  ...
  if ( $prev_exon->overlaps($exon) ) {
    $self->add_Error( "Overlapping exons in transcript for exons " . $prev_exon->dbID . " and " . $exon->dbID . "\n", 'overlapexon' );
    return;
  }
  ...
  if ( $intlen >= $self->minshortintronlen && $intlen <= $self->maxshortintronlen ) {
    $self->add_Error( "Short intron (" . $intlen . " bases) between exons " . $prev_exon->dbID . " and " . $exon->dbID . "\n",
                      'shortintron' );
  }
  elsif ( $intlen >= $self->minlongintronlen ) {
    $self->add_Error( "Long intron (" . $intlen . ") between exons " . $prev_exon->dbID . " and " . $exon->dbID . "\n", 'longintron' );
  }
  ...
  if ( $intlen >= 2 ) {
    ...
    if ( substr( $intseq, 0, 2 ) ne "GT" ) {
      $self->add_Warning( "Non consensus 5' intron splice site sequence (" .
                            substr( $intseq, 0, 2 ) . ") after exon " . $prev_exon->dbID . " (intron length = " . $intlen . ")\n",
                          'noncons5' );
    }
    if ( substr( $intseq, -2, 2 ) ne "AG" ) {
      $self->add_Warning( "Non consensus 3' intron splice site sequence (" .
                            substr( $intseq, -2, 2 ) . ") before exon " . $exon->dbID . " (intron length = " . $intlen . ")\n",
                          'noncons3' );
    }
  }
} ## end sub check_Intron
