...
GetOptions( 'dbhost|host|h:s'       => \$host,
            ...
            'help!'                 => \$help, ) or
  perldocs("Failed to get options");
...
if ($gene_database_name) {
  ...
  if ( $dnadbname ne "" ) {
    ...
    my $dnadbase =
      new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $dnahost, -user => $user, -port => $dnaport, -dbname => $dnadbname, -pass => $pass, );
    $db->dnadb($dnadbase);
  }
}
...
foreach my $chr ( @{ sort_chr_names($chrhash) } ) {
  ...
  my $chrend = ( defined($specend) && $specend < $chrhash->{$chr} ) ? $specend : $chrhash->{$chr};
  ...
  my $cgc = new ContigGenesChecker( -slice => $slice, -genes => $genes, -ignorewarnings => $ignorewarnings, -adaptor => $db, );
  $cgc->check;
...
GENE: foreach my $gene (@$genes) {
    ...
    TRANSCRIPT: foreach my $transcript (@trans) {
        $total_transcripts++;
        my $tc = new TranscriptChecker( -transcript         => $transcript,
      ...
      } ## end TRANSCRIPT: foreach my $transcript (@trans)
    } ## end if ($check_transcripts)
