...
my $all_contigs_filename = "contig.fa"
  ; # filename for the fasta file containing all the downloaded contigs once concatenated (IT MUST BE THE SAME AS THE ONE DEFINED IN ensembl-personal/genebuilders/scripts/get_ENA_sequences.pl)
...
GetOptions(
  ...
  , # local path where the 'ftp_species_path' contents will be downloaded together with the dumpped out tables after creating the reference db. The script will create it if necessary.
...
if ( ( $skip < 1 and $until >= 1 ) or $only == 1 ) {
  download_ftp_dir( $ftp_server, $ftp_species_path . $primary_assembly_dir_name, $local_path . $primary_assembly_dir_name, $verbose );
}
...
if ( ( $skip < 3 and $until >= 3 ) or $only == 3 ) {
  ...
  download_ftp_contigs( $ena, $ena_tax, $ena_id, $dir_assembly_scripts,
                        $local_path . $primary_assembly_dir_name,
                        $local_path . $contigs_dir_name,
                        $all_contigs_filename, $verbose );
  ...
  &fix_ENA_contig_headers( $local_path . $contigs_dir_name . "/" . $all_contigs_filename, $verbose );
}

# create the reference db where the downloaded assembly will be loaded
print("\n===== STEP 4: CREATE REFERENCE DB =====\n") if ($verbose);
create_ref_db( $dbhost, $dbport, $dbuser, $dbpass, $dbname, $dir_enscode, $local_path, $verbose )
  if ( ( $skip < 4 and $until >= 4 ) or $only == 4 );

print("\n===== STEP 5: LOAD SEQ_REGIONS =====\n") if ($verbose);
my @agp_filepaths = ();
@agp_filepaths = load_seq_regions( $dbhost, $dbport, $dbuser, $dbpass, $dbname, $dir_enscode,
                                   $local_path . $primary_assembly_dir_name,
...
load_assembly_info( $dbhost, $dbport, $dbuser, $dbpass, $dbname, $dir_enscode, $verbose, $local_path . $primary_assembly_dir_name,
                    @agp_filepaths )
  if ( ( $skip < 6 and $until >= 6 ) or $only == 6 );

# run top level script
print("\n===== STEP 7: RUN TOP LEVEL SCRIPT =====\n") if ($verbose);
&top_level( $dbhost, $dbport, $dbuser, $dbpass, $dbname, $dir_enscode, $verbose ) if ( ( $skip < 7 and $until >= 7 ) or $only == 7 );
...
&check_top_level( $dbhost, $dbport, $dbuser, $dbpass, $dbname, $dir_enscode, $local_path . $primary_assembly_dir_name, $verbose )
  if ( ( $skip < 8 and $until >= 8 ) or $only == 8 );

my $db =
  Bio::EnsEMBL::DBSQL::DBAdaptor->new( -host => $dbhost, -port => $dbport, -user => $dbuser, -pass => $dbpass, -dbname => $dbname );
...
sub download_ftp_dir {
  ...
  if ( system( "wget $wget_verbose -nH -P $local_dir $ftp_server$ftp_path/../../$ass_report_dir" .
                 "_assembly_report.txt -O $local_dir/assembly_report.txt" ) )
  {
    throw(
"Could not download *_assembly_report.txt file from $ftp_server$ftp_path/../../ to $local_dir. Please, check that both paths are valid."
    );
  }
  else {
...
`cat $all_contigs_filepath | perl -e 'while (<>) {if ( /\\>\\S+\\|\\S+\\|(\\S+).*/ ) {print ">\$1\n";} elsif ( /\\>(\\S+)/ ) {print ">\$1\n";} else {print;}}' > $all_contigs_filepath.fixed`;
    ...
    if ( $contigs_after_mv != $contigs_before_mv ) {
      throw(
"The number of contigs before fixing the headers is different from the number of contigs after the fix. Please check your files $all_contigs_filepath and $all_contigs_filepath.fixed"
      );
    }
    else {
  ...
  } ## end if ( -e $all_contigs_filepath)
  else {
    throw("File $all_contigs_filepath cannot be found.");
  }
} ## end sub fix_ENA_contig_headers
...
sub create_ref_db {
  ...
  if ( $num_tables_created != $num_tables_to_create ) {
    throw(
"The reference database main tables could not be created. The number of created tables differs from the number of tables to be created. Number of created tables=$num_tables_created; Number of tables to be created=$num_tables_to_create"
    );
  }
  else {
  ...
  my $num_pip_tables_created =
    `mysqlshow --count -h$host -P$port -u$user -p$pass $dbname | grep rows | awk '{print \$1}'` - $num_tables_created;
  ...
  if ( $num_pip_tables_created != $num_pip_tables_to_create ) {
    throw(
"The reference database pipeline-related tables could not be created. The number of created tables differs from the number of tables to be created. Number of created tables=$num_pip_tables_created; Number of tables to be created=$num_pip_tables_to_create"
    );
  }
  else {
...
`perl $enscode$populate_production_tables_script -h $host -P $port -u $user -p $pass -d $dbname -mh ens-staging1 -md ensembl_production -mu ensro -mP 3306 -dp $dump_path`;
  ...
  my $num_populated_tables =
    int(`mysqlshow --count -h$host -P$port -u$user -p$pass $dbname | awk '{if (int(\$6) > 0) print \$6}' | wc -l`);
  ...
  if ( $num_populated_tables == $NUM_PRODUCTION_DB_TABLES ) {
    ...
    throw(
"The production db tables could not be populated. Number of populated tables=$num_populated_tables. Expected number of populated tables=$NUM_PRODUCTION_DB_TABLES."
    );
  }
} ## end sub create_ref_db
...
# Note that 'ls' is executed with -r (reverse) option so that the 'assembled_chromosomes' dir is the last one to be processed in the foreach loop. In this way, we will get the seq_regions loaded in the desired order as stated in the 'sop_assembly_loading' document after the genebuilder convention meeting. Note that the loading order will change if the dir names do.
  ...
  if ( basename( $agp_filenames_arr[ @agp_filenames_arr - 1 ] ) =~ /chr/ ) {
    ...
    $rank = 3;
    print(
"Chromosome agp file found. Chromosome level will be of rank 1, scaffold level will be of rank 2 and contig level will be of rank 3.\n"
    ) if ($verbose);
  }
  else {
...
`perl $enscode/ensembl-pipeline/scripts/load_seq_region.pl -dbhost $host -dbuser $user -dbpass $pass -dbport $port -dbname $dbname -coord_system_name contig -coord_system_version $ass_name -rank $rank -default_version -fasta_file $contigs_path -sequence_level -noverbose > $ass_path/load_seq_region_$contigs_path_basename.out`;
  ...
  if ( $num_contigs != $num_dna ) {
    ...
    print(
      "The number of 'seq_region' table loaded contigs ($num_contigs) is the same as the number of 'dna' table rows ($num_dna). Great!\n")
      if ($verbose);
  }
...
`perl $enscode/ensembl-pipeline/scripts/load_agp.pl -dbhost $host -dbuser $user -dbpass $pass -dbport $port -dbname $dbname -assembled_name $assembled -component_name $component -agp_file $curr_agp_path >& $curr_agp_path_outfile`;
    ...
    $num_items_file[$i] =
      int(`grep -v "#" $curr_agp_path | grep -v "\\WN\\W" | grep -c -v "\\WU\\W"`); #skipping gaps as not stored in db like in load_agp.pl
    ...
    if ( $num_items_loaded != $num_items_file[$i] ) {
      throw(
"num:$num_items_db sum:$last_item_db , The number of items loaded into the assembly table ($num_items_loaded) differs from the number of items in file ($num_items_file[$i]) for file $curr_agp_path"
      );
    }
    else {
      print(
"The number of items loaded into the assembly table ($num_items_loaded) is the same as the number of items in file ($num_items_file[$i]) for file $curr_agp_path. Great!\n"
      ) if ($verbose);
    }
  ...
  } ## end for ( my $i = 0; $i < @agp_filepaths...)

  # check meta table new rows for assembly.mapping
  my $numMetaRows =
    int(`mysql -h$host -P$port -u$user -p$pass -D$dbname -NB -e'select count(*) from meta where meta_key = "assembly.mapping"'`);
  if ($chr_level_exists) {
    if ( $numMetaRows == 3 ) {
...
sub top_level() {
  ...
  my $numTopLevel =
    int(`mysql -h$host -P$port -u$user -p$pass -D$dbname -NB -e'select count(*) from seq_region_attrib where attrib_type_id = 6'`)
    ;    # 6 corresponds to Top Level
  if ( $numTopLevel > 0 ) {
...
`perl $enscode/ensembl-analysis/scripts/sequence_dump.pl -dbhost $host -dbuser $user -dbpass $pass -dbport $port -dbname $dbname -toplevel -onefile -output_dir $ass_path/ > $ass_path/sequence_dump.out`;
 ...
 #  }
  ...
  if ( $numTopLevel != $numTopLevelDownloaded ) {
    throw(
"The number of dumped top level sequences ($numTopLevel) does not match the number of downloaded top level sequences ($numTopLevelDownloaded)."
    );
  }
  else {
    print(
"The number of dumped top level sequences ($numTopLevel) matches the number of downloaded top level sequences ($numTopLevelDownloaded). Great!\n"
    ) if ($verbose);
  }
  ...
  if ( $numTopLevelCombined != $sumTopLevel ) {
    throw(
"The number of combined top level sequences ($numTopLevelCombined) does not match the sum of dumped top level and downloaded top level sequences ($sumTopLevel)."
    );
  }
  else {
    print(
"The number of combined top level sequences ($numTopLevelCombined) matches the sum of dumped and downloaded top level sequences ($sumTopLevel). Great!\n"
    ) if ($verbose);
  }
  ...
  if ( $numTopLevelFastanrdb != $numTopLevel ) {
    warning(
"The number of sequences in file $ass_path/fastanrdb.out is $numTopLevelFastanrdb but it should be $numTopLevel. It means that we have duplicates.\n"
    );
    print(
"The number of sequences in file $ass_path/fastanrdb.out is $numTopLevelFastanrdb but it should be $numTopLevel. It means that we have duplicates.\n"
    );
    ...
    print(
"The number of sequences in file $ass_path/fastanrdb.out is $numTopLevelFastanrdb, which is good since it is the expected number, but there might be odd cases where duplicates can be present. More checks will be run.\n"
    );
  }
  ...
  if ( $numIds != $numTopLevel*2 ) {
    throw(
"The number of identifiers in headers in $ass_path/fastanrdb.out ($numIds) does not match the number_of_top_level_sequences*2 ($numTopLevel*2)"
    );
  }
  else {
    print(
"The number of identifiers in headers in $ass_path/fastanrdb.out ($numIds) matches the number_of_top_level_sequences*2 ($numTopLevel*2). Great!\n"
    ) if ($verbose);
  }
...
sub add_meta_table_values {
  ...
  if ( defined $genebuilder_id ) {
    ...
    warning
      "Could not find /ensembl-production_private/release_coordination/docs/genebuilder_id.txt, please add your id in the meta table!";
  }
  ...
  if ( -e $local_path . $primary_assembly_dir_name . '/assembly_report.txt' ) {
    open( RF, $local_path . $primary_assembly_dir_name . '/assembly_report.txt' ) ||
      die("Could not open $local_path$primary_assembly_dir_name/assembly_report.txt");
...
sub add_seq_region_synonym {
  ...
  if ( -e $local_path . $primary_assembly_dir_name . '/assembled_chromosomes/chr2acc' ) {
    open( RF, $local_path . $primary_assembly_dir_name . '/assembled_chromosomes/chr2acc' ) ||
      die( "Could not open " . $local_path . $primary_assembly_dir_name . '/assembled_chromosomes/chr2acc' );
    my $sth_select = $db->dbc->prepare(
'SELECT sr.seq_region_id FROM seq_region sr, coord_system cs WHERE cs.coord_system_id = sr.coord_system_id AND sr.name = ? AND cs.rank = 1'
    );
  ...
  foreach my $file ( 'component_localID2acc', 'scaffold_localID2acc' ) {
    ...
    open( RF, $local_path . $primary_assembly_dir_name . '/' . $file ) ||
      die( "Could not open " . $local_path . $primary_assembly_dir_name . '/' . $file );
...
} ## end sub add_seq_region_synonym
...
sub copying_core_config_files {
  ...
  foreach my $example_file ( '/Bio/EnsEMBL/Pipeline/Config/General.pm', '/Bio/EnsEMBL/Pipeline/Config/BatchQueue.pm' ) {
    print 'Copying ', $configdir, $example_file, "...\n" if ($verbose);
    copy( $dir_enscode . '/ensembl-pipeline/modules' . $example_file . '.example', $configdir . $example_file );
  }
  my $databases_cfg =
    Bio::EnsEMBL::Analysis::Tools::ConfigWriter->new( -modulename => 'Bio::EnsEMBL::Analysis::Config::Databases', -is_example => 1, );
...
} ## end sub copying_core_config_files
