...
if ( scalar( keys %contigs_id ) < 100 ) {
  ...
  if ($use_ena) {
    ...
    } ## end foreach my $a_wgs_id (@wgs_ids)
  } ## end if ($use_ena)
  else {
...
} ## end else [ if ( scalar( keys %contigs_id...))]
...
if ( -e $clones_file ) {
  ...
  if ( scalar(@clones) != scalar(@res) ) {
    ...
    close(WF);
    warning( "You had " .
             scalar(@clones) . " agp sequence entries and only " .
             scalar(@res) . " mfetched sequences, have a look at $missing\n" );
    ...
    if ( scalar(@missing_ids) != scalar(@res_ena) ) {
      ...
      close(WFILE);
      throw( "You had " .
             scalar(@missing_ids) . " missing sequences from mfetch and only " .
             scalar(@res_ena) . " were fetched from ENA SVArchive, have a look at $missing_ena\n" );
    }
    else {
      system(
        'cat ' . $ena_archive_file . ' ' . $clones_file . ' ' . $fastadir . '/' . $file . '.fa  | sed \'/no match/d\' > ' . $contigfile );
    }    #end if number ENA SVArchive sequences check
  } ## end if ( scalar(@clones) !=...)
  else {
    system( 'cat ' . $clones_file . ' ' . $fastadir . '/' . $file . '.fa > ' . $contigfile );
  }    # end if number mfetch sequences check
} ## end if ( -e $clones_file )
else {
...
if ($remove_non_atg) {
  ...
  throw("Could not remove the non ATGC letters from $contigfile")
    if ( system("sed -i '/>/ ! y/BDEFHIJKLMOPQRSUVWXYZ/NNNNNNNNNNNNNNNNNNNNN/' $contigfile") );
  throw( 'Oooops!!! missing some stuff after removing non ATCG letters... Count shoud be ' . $count )
    if ( `grep -c \\> $contigfile` != $count );
}
