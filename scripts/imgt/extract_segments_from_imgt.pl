...
my ( @good_tax_strings, @bad_tax_strings, @moltype,         @dataclass,  @good_keywords, @bad_keywords,  @tagstrings,
     $kill_list,        %kill_list,       $gene_name_in_id, $translate,  $keep_pseudos,  $keep_partials, $keep_stops,
     $guess_frame,      $n_stop_allow,    $c_stop_allow,    $min_length, $flanking_seq );

$min_length   = 0;
$n_stop_allow = 0;
$c_stop_allow = 0;

&GetOptions( 'taxstring=s@'       => \@good_tax_strings,
...
my $outio = Bio::SeqIO->new( -format => 'fasta', -fh => \*STDOUT );

my $seqio = Bio::SeqIO->new( -format => 'IMGT_embl', -fh => \*ARGV );
...
SEQ: while ( my $seq = $seqio->next_seq ) {
  ...
  if (@good_tax_strings) {
    ...
    pop @taxonomy;
    push @taxonomy, lc( $seq->species->species );
  ...
  foreach my $val ( $seq->get_SeqFeatures ) {
    ...
    if ( $val->primary_tag =~ /^(gene)|(\S+\-GENE)$/ ) {
      ...
      next;
    } ## end if ( $val->primary_tag...)

    if ( exists $all_tags{ $val->primary_tag } ) {
      my $seg = { code => $val->primary_tag, location => $location, codon_start => 1, pseudo => $entry_is_pseudo, partial => 0, };

      if ( $val->annotation->get_Annotations("gene") ) {
        my ($tag_val) = $val->annotation->get_Annotations("gene");
        $seg->{gene} = $tag_val->value if $tag_val->value !~ /\s+/;
      }
      if ( $val->annotation->get_Annotations("partial") or $location->isa("Bio::Location::Fuzzy") ) {
        $seg->{partial} = 1;
      }
    ...
    } ## end if ( exists $all_tags{...})
  } ## end foreach my $val ( $seq->get_SeqFeatures)

  @gene_regions = sort { $a->{location}->start <=> $b->{location}->start; } @gene_regions;
    ...
    if ( exists $segments{$tagstring} ) {
      foreach my $seg ( @{ $segments{$tagstring} } ) {
        ...
        foreach my $greg (@gene_regions) {
          if ( $seg->{location}->start >= $greg->{location}->start and $seg->{location}->end <= $greg->{location}->end ) {
      ...
      if ( not( grep { not exists $segments{$_} } @bits ) ) {
        ...
        if ( not @gene_regions ) {
          ...
          foreach my $bit ( keys %bits ) {
            ...
            } ## end if ( exists $segments{...})
          } ## end foreach my $bit ( keys %bits)
        } ## end else [ if ( not @gene_regions)]

      GENE: foreach my $gid ( keys %seg_groups ) {
          # check completeness
          my @segs = sort { $a->{location}->start <=> $b->{location}->start; } @{ $seg_groups{$gid} };
      ...
      } ## end if ( not( grep { not exists...}))
    } ## end elsif ( $tagstring =~ /:/) [ if ( exists $segments{...})]
  } ## end TAG: foreach my $tagstring (@tagstrings)
...
SEGLIST: foreach my $seglist (@segment_lists) {
    ...
    foreach my $seg (@$seglist) {
      ...
      if ( $seg->{partial} ) {
        ...
        foreach my $loc ( $seg->{location}->each_Location ) {
          if ( $loc->start_pos_type ne 'EXACT' or $loc->end_pos_type ne 'EXACT' ) {
            $is_partial = 1;
          }
        }
      }
  ...
  } ## end SEGLIST: foreach my $seglist (@segment_lists)
  ...
  CHECK: foreach my $olist (@nr_seg_lists) {
      ...
      foreach my $loc (@locs) {
        foreach my $oloc (@olocs) {
          if ( $loc->start <= $oloc->end and $loc->end >= $oloc->start ) {
            $overlaps = 1;
            last CHECK;
          }
  ...
  } ## end foreach my $list (@segment_lists)

  @nr_seg_lists = sort { $a->[0]->{location}->start <=> $b->[0]->{location}->start; } @nr_seg_lists;
...
SEGLIST: foreach my $seglist (@nr_seg_lists) {
    ...
    my $id = $seq->primary_id . "." . $segs[0]->{location}->start . "-" . $segs[-1]->{location}->end;
    ...
    if ($translate) {
      ...
      if ( $pepseq->seq !~ /\*/ or $keep_stops or $pepseq->seq !~ /^.{$n_stop_allow}.*\*.*.{$c_stop_allow}$/ ) {
        $outio->write_seq($pepseq) if $min_length <= $pepseq->length;
      }
      elsif ($guess_frame) {
        my %frame;
        map { $frame{$_} = 1 } ( 0, 1, 2 );
        ...
        foreach my $fr ( keys %frame ) {
          ...
          if ( $pep->seq !~ /\*/ or $keep_stops or $pep->seq !~ /^.{$n_stop_allow}.*\*.*.{$c_stop_allow}$/ ) {
            push @good_seqs, $pep;
          }
        }
        if ( scalar(@good_seqs) == 1 ) {
          $outio->write_seq( $good_seqs[0] ) if $min_length <= $good_seqs[0]->length;
        }
      }
    } ## end if ($translate)
    else {
      $outio->write_seq($newseq) if $min_length <= $newseq->length;
    }

  } ## end SEGLIST: foreach my $seglist (@nr_seg_lists)
} ## end SEQ: while ( my $seq = $seqio->...)
...
sub adjust_for_bad_codon_boundaries {
  ...
  return 1;
} ## end sub adjust_for_bad_codon_boundaries
