...
my ( @good_tax_strings, @bad_tax_strings, @good_keywords, @bad_keywords,  $kill_list,  %kill_list,
     $gene_name_in_id,  $translate,       $keep_pseudos,  $keep_partials, $keep_stops, $min_length );
...
SEQ: while ( my $seq = $seqio->next_seq ) {
  ...
  foreach my $val ( $seq->get_SeqFeatures ) {
    ...
    if ( exists $tags{ $val->primary_tag } ) {
      ...
      my $seg = { code => $val->primary_tag, location => $location, pseudo => $entry_is_pseudo, partial => 0, codon_start => 0, };
  ...
  foreach my $seg (@segments) {
    ...
    foreach my $greg (@gene_regions) {
      if ( $seg->{location}->start <= $greg->{location}->end and $seg->{location}->end >= $greg->{location}->start ) {
...
SEGLIST: foreach my $seglist (@segment_lists) {
    ...
    my $complete = 0;
    foreach my $end_pair ( [ $seglist->[0], $seglist->[-1] ], [ $seglist->[-1], $seglist->[0] ] ) {
      my ( $f, $l ) = @$end_pair;
      if ( $f->{code} eq 'CL' or
...
SEGLIST: foreach my $orig_seglist (@complete_seg_lists) {
    ...
    foreach my $seglist (@nov_seglists) {
      ...
      my @locs = sort { $a->start <=> $b->start; } map { $_->{location}->each_Location } @$seglist;
        ...
        if ( grep { exists( $_->{untranslated} ) } @$seglist ) {
          ...
          foreach my $loc (@locs) {
            foreach my $stop (@stops) {
              if ( $stop->start >= $loc->start and $stop->end <= $loc->end ) {
                $contains_utr = 1;
                last;
              }
            }
      ...
      my $id = $seq->primary_id . "." . $locs[0]->start . "-" . $locs[-1]->end;
      ...
      my $newseq = Bio::PrimarySeq->new( -seq => $seqstring, -id => $id, -description => join( "; ", @this_desc ) );
      ...
      if ($translate) {
        ...
        if ( $pepseq->seq !~ /\*/ or $keep_stops ) {
          $outio->write_seq($pepseq) if not defined $min_length or $min_length <= $pepseq->length;
        }
      }
      else {
        $outio->write_seq($newseq) if not defined $min_length or $min_length <= $newseq->length;
      }
    } ## end foreach my $seglist (@nov_seglists)
  } ## end SEGLIST: foreach my $orig_seglist (@complete_seg_lists)
...
sub deal_with_overlapping_segments {
  ...
  my @segs = sort { $a->{location}->start <=> $b->{location}->start or $a->{location}->end <=> $b->{location}->end; } @$segs;
...
SEG: foreach my $seg (@segs) {
    foreach my $grp (@seg_groups) {
      foreach my $os (@$grp) {
        if ( $os->{location}->start <= $seg->{location}->end and $os->{location}->end >= $seg->{location}->start ) {
          push @$grp, $seg;
          next SEG;
        }
      }
  ...
  foreach my $grp (@seg_groups) {
    ...
    } ## end elsif ( scalar(@$grp) == ... [ if ( scalar(@$grp) > 2)])
  } ## end foreach my $grp (@seg_groups)
  ...
  if ( grep { $_->{code} eq 'CH-S' } @segs and grep { $_->{code} =~ /^M(1|2)?$/ } @segs ) {
...
} ## end sub deal_with_overlapping_segments
