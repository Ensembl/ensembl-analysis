...
def fetch_db_data(query,database,host,port,user,password):
  registry_db_url = 'mysql://' + user + ':' + password + '@' + host + ':' + port + '/' + database
  print(registry_db_url)
  try:
    engine = db.create_engine(registry_db_url)
    with engine.connect() as connection:
      rows = connection.execute(db.text(query))
...
def fetch_db_data1(query,database,host,port,user,password):
  try:
...
def update_assembly_sheet(assembly_db_data,meta_db_data,existing_sheet_records,assembly_sheet,gettime,worksheet_name):
  ...
  for gca in assembly_db_dict:
    ...
    # If it does exist we need to check if an update is required. There are only a few columns this might pertain to
      ...
      print("Processing genbank accession: " + gca)
      ...
      if contig_N50 != sheet_contig_N50_val:
        ...
        row_update_index = assembly_sheet.find(gca).row
        print("Row index before method call : " + str(row_update_index) + "Sheet index : " + str(sheet_contig_N50_index) + " Sheet value : " + str(contig_N50)) 
...
def update_cell_val(assembly_sheet,row_index,col_offset,val):
  col_offset += 1
  print("Row index : " + str(row_index) + ", Column index : " + str(col_offset)+ " and Value : " + str(val))
