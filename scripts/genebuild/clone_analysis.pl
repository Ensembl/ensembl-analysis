...
if ( !($dbhost) || !($dbuser) || !($dbname) ) {
  ...
  print STDERR "-dbhost $dbhost -dbuser $dbuser -dbpass $dbpass -dbname" . " $dbname -dbport $dbport\n";
  usage();
}

if ( !$file ) {
  print STDERR "You need to pass a file name which either represents a " .
    "analysis config file to be read and stored in the database or " . "written based on the analysis objects in the database\n";
  usage();
}
...
unless ($not_pipeline) {
  ...
  $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $dbhost, -user => $dbuser, -pass => $dbpass, -dbname => $dbname, -port => $dbport, );
}
...
sub clone_analysis {
  ...
  unless ($not_pipeline) {
    $new_analysis = new Bio::EnsEMBL::Pipeline::Analysis( -logic_name      => $new_logic_name,
  ...
  } ## end unless ($not_pipeline)

  # If it's not a pipeline db make a normal analysis object, without an input id type
  else {
    $new_analysis = new Bio::EnsEMBL::Analysis( -logic_name      => $new_logic_name,
                                                -db              => $analysis_to_clone->db(),
                                                -db_version      => $analysis_to_clone->db_version(),
                                                -db_file         => $analysis_to_clone->db_file(),
                                                -program         => $analysis_to_clone->program(),
  ...
  unless ( $not_pipeline || $no_rules ) {
    ...
    my $new_rule     = Bio::EnsEMBL::Pipeline::Rule->new( -goalanalysis => $new_analysis, -adaptor => $db, );
...
sub clone_analysis_config {
  foreach my $module_name (@config_list) {
    ...
    my $config_writer = new Bio::EnsEMBL::Analysis::Tools::ConfigWriter( -MODULENAME => $path, );
    ...
    print "\n";
  } ## end foreach my $module_name (@config_list)

} ## end sub clone_analysis_config
