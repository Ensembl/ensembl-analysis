...
my $paf_logic_name         = '';
&GetOptions(
  \%opt, 'dbhost|host|h=s', 'dbname|db|D=s', 'dbuser|user|u=s', 'dbpass|pass|p=s', 'dbport|port|P=i', 'config=s',
  ...
  'verbose!', 'acc_input_file=s', 'filename_acc_no_desc=s', 'rerun!', 'run_multiple_jobs!',
...
if ( ( $opt{no_mfetch_sprot_dat} and !$opt{no_mfetch_trembl_dat} ) or ( !$opt{no_mfetch_sprot_dat} and $opt{no_mfetch_trembl_dat} ) ) {
  throw(
"If you want to run on no_mfetch mode, you must provide two filepaths. One of the following arguments is missing: no_mfetch_sprot_dat, no_mfetch_trembl_dat\n"
  );
}
...
my $dba = Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor->new( -dbname => $opt{dbname},
                                                         -host   => $opt{dbhost},
                                                         -user   => $opt{dbuser},
                                                         -pass   => $opt{dbpass},
                                                         -port   => $opt{dbport}, );
...
my $statement =
  ...
  "from analysis a, protein_align_feature pf " . "where pf.analysis_id = a.analysis_id group by logic_name ";
$sth = $dba->prepare($statement);
$sth->execute();
$result = $sth->fetchall_arrayref();

print "These analysis have entries in the protein_align_feature-table : \n";
for my $row (@$result) {
...
if ( $opt{rerun} ) {
  ...
  $statement =
    "select hit_name, logic_name from analysis a,  protein_align_feature pf " .
  ...
  if ( $opt{run_multiple_jobs} ) {
    ...
    for ( my $nr = 0; $nr < $files; $nr++ ) {
      ...
      my $cmd = "bsub -M 500 -R 'select[mem>500] rusage[mem=500]' -oo " .
        $opt{output_dir} . "/acc_$nr.log -J cfp_$nr $script_call -rerun -acc_input_file $opt{acc_input_file}.acc_$file_name ";
  ...
  } ## end if ( $opt{run_multiple_jobs...})
} ## end else [ if ( $opt{rerun} ) ]
...
if ( $opt{no_mfetch_sprot_dat} and $opt{no_mfetch_trembl_dat} ) {
  ( $entries, $not_found ) =
    @{ $obj->get_Entry_Fields_no_mfetch( $opt{no_mfetch_sprot_dat}, $opt{no_mfetch_trembl_dat}, \@hit_ids_investigate, \@fields_to_fetch )
    };
}
else {
...
for my $lg ( @{ get_logic_names_for_classification() } ) {
  ...
  unless ($ana) {
    ...
    my $upload_ana =
      Bio::EnsEMBL::Pipeline::Analysis->new( -logic_name => $lg, -input_id_type => "classifyProteins", -module => "BlastGenscanPep", );
...
for my $lg (@logic_names) {
  ...
  foreach my $hit_name ( keys %filtered_hits ) {
    ...
    if ( $opt{no_mfetch_sprot_dat} and $opt{no_mfetch_trembl_dat} ) {
      printf( SQL "update protein_align_feature " . "set analysis_id = %d " . "where hit_name like '%s%' and analysis_id = %d;\n",
              $analysis_id, $hit_name, $orig_analysis_id );
    }
    else {
      printf( SQL "update protein_align_feature " . "set analysis_id = %d " . "where hit_name = '%s' and analysis_id = %d;\n",
              $analysis_id, $hit_name, $orig_analysis_id );
    }
  } ## end foreach my $hit_name ( keys...)
}    # end logic names
close(SQL);

if ( $inserted_new_analysis >= 1 ) {
  warning( "Script ended successfully but please be aware that one or more new analyses " . "have been added to this database, " .
           $opt{dbname} . " at " . $opt{dbhost} . ". Please synchronise the analysis tables across all databases where necessary." );
}
...
ACC: for my $acc ( keys %$entries ) {
    ...
    if ($expression_to_match) {
      ...
      } ## end for my $string_match (@$expr_to_match)
    } ## end if ($expression_to_match)
    else {
  ...
  return \%result;
} ## end sub get_and_exclude_protein_hits_by_field
...
sub get_analysis {
  my ( $dba, $logic_name ) = @_;
  my $ana = $dba->get_AnalysisAdaptor()->fetch_by_logic_name($logic_name);
  if ($ana) {
    print "fetched analysis $logic_name --> " . $ana->dbID . "\n";
    return $ana;
  }
  warning( "Analysis with logic_name : $logic_name could not be found in : " . $dba->dbname . " \@ " . $dba->host . "\n" );
  return undef;
}
  ...
  for my $k ( keys %$PROTEIN_CLASSIFICATION ) {
    ...
    for my $p ( @{ $conf{PHYLUM} }, @{ $conf{EXCLUDED_PHYLUM} } ) {
      ...
      if ( length($p) > 0 && !exists( $phylum{$p} ) ) {
        throw( "The phylum : $p for [$k] " . "can't be found in the phylum dictionary.\n" .
               "Make sure you've spelled it correctly " . "- if the dictionary is incomplete, please add it...\n" );
      }
    }
