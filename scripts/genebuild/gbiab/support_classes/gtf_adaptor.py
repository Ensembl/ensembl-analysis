...
class GeneAdaptorGTF:
    def __init__(self, gtf_file, sequence_fasta_file=None):
        ...
        if not os.path.exists(gtf_file):
            raise Exception(
                "GTF file does not exist on specified path. Path specified:\n" + gtf_file
            )
    ...
    def load_all_genes(self, location_name_constraints=None):
        ...
        for line in gtf_lines:
            ...
            if (
                location_name_constraints is not None
                and location_name not in location_name_constraints
            ):
                continue
    ...
    def build_and_group_exons(self, exon_data):
        ...
        for record_entry in exon_data:
            ...
            exon = Exon(
                int(start), int(end), strand, location_name, self.sequence_fasta_file
            )
    ...
    def write_genes_to_file(genes, output_file):
        ...
        for gene_idx, gene in enumerate(genes):
            ...
            gene_attribute_string = GeneAdaptorGTF.create_attribute_string(
                gene_attributes
            )
            gtf_out.write(
                GeneAdaptorGTF.create_feature_entry(gene, "gene", gene_attribute_string)
            )
            ...
            for transcript_idx, transcript in enumerate(transcripts):
                ...
                if transcript.public_identifier is not None:
                    ...
                    transcript_attributes[
                        "transcript_id"
                    ] = transcript.internal_identifier

                transcript_attribute_string = GeneAdaptorGTF.create_attribute_string(
                    transcript_attributes
                )
                gtf_out.write(
                    GeneAdaptorGTF.create_feature_entry(
                        transcript, "transcript", transcript_attribute_string
                    )
                )
                ...
                for exon_idx, exon in enumerate(exons):
                    ...
                    exon_attributes["transcript_id"] = transcript_attributes[
                        "transcript_id"
                    ]
                    ...
                    exon_attribute_string = GeneAdaptorGTF.create_attribute_string(
                        exon_attributes
                    )
                    gtf_out.write(
                        GeneAdaptorGTF.create_feature_entry(
                            exon, "exon", exon_attribute_string
                        )
                    )
