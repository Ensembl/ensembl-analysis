...
class Sequence:
  ...
  def __init__(self, start=None, end=None, strand=None, location_name=None, sequence=None, fasta_file=None, bedtools_path=None):
    self.start = start
    self.end = end
    self.strand = strand
  ...
  def get_sequence(self,fasta_file):
    start = self.start
    end = self.end
    length = end - start + 1
    strand = self.strand
    ...
    with tempfile.NamedTemporaryFile(mode='w+t', delete=False) as bed_temp_file:
      bed_temp_file.writelines(location_name + "\t" + str(start) + "\t" + str(length))
      bed_temp_file.close()
