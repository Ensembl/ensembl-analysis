...
GetOptions( 'infile:s' => \$infile, 'blastfile:s' => \$blastfile, 'threshold:s' => \$threshold, 'regex:s' => \$regex, );

# usage
if ( !defined $infile || !defined $blastfile ) {
...
my $seqouta = new Bio::SeqIO( -file => ">$infile" . "_keep",    -format => 'fasta' );
my $seqoutb = new Bio::SeqIO( -file => ">$infile" . "_discard", -format => 'fasta' );
