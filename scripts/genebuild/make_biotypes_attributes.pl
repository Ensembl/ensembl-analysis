...
} ## end SLICE: while ( my $slice = shift...)
...
sub create_Attribute {
  ...
  CODE: foreach ( my $offset = 15; $offset > 0; $offset-- ) {
      ...
      throw( "Still have a " . $code . " which is too long from " . $reason . " " . "using " . $real_offset ) if ( length($code) >= 16 );
      #print "Executing sql\n";
      $sth->execute($code);
  ...
  my $attrib = Bio::EnsEMBL::Attribute->new( -code => $code, -name => $reason, -description => $reason, -value => $reason, );
  return $attrib;
} ## end sub create_Attribute
