...
if ( -e $output_file ) {
  print "\nProtein file '" . $output_file . "' already exists, these " . "entries will be appended to the end of the file.\n";
...
my $output_io = Bio::SeqIO->new( -format => 'fasta', -file => ">>" . $output_file, );
...
SEQ: while ( my $seq = $io->next_seq ) {
    ...
    if ( !$id ) {
      warn( $regex . " failed to parse an id out of " . $parseable_string );
      next SEQ;
    }
    if ($x_string) {
      if ( $seq->seq =~ /$x_string/ ) {
  ...
  } ## end SEQ: while ( my $seq = $io->next_seq)
