...
my %opt = ( outfile               => "protein_classification.sql",
...
my $paf_logic_name         = '';
&GetOptions(
  \%opt, 'config=s', 'delete',    # avoid interactive mode and overwrite files
  'test!', 'output_dir=s', 'outfile=s', 'paf_logic_name=s', 'filename_acc_no_desc=s', 'verbose!', 'paf_file=s', 'rerun!', );
...
if (0) {
  if ( -e $opt{all_entries_hash_file} ) {
    ...
    sleep(5);
  } ## end if ( -e $opt{all_entries_hash_file...})
} ## end if (0)
...
for my $acc (@not_found) {
  ...
  } ## end else [ if ( scalar(@$not_found...))]
} ## end for my $acc (@not_found)
#print scalar(keys %all_entries)  . " in all entries \n" ;
