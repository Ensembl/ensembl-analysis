...
print STDERR "\nReading from Bio::EnsEMBL::Analysis::Config::Databases && " .
  "Bio::EnsEMBL::Analysis::Config::ExonerateTags.\nLooking at database " . $$DATABASES{'REFERENCE_DB'}{'-dbname'} .
  ":" . $$DATABASES{'REFERENCE_DB'}{'-host'} . ":" . $$DATABASES{'REFERENCE_DB'}{'-port'} . "\n\n";
...
sub setup {
       ...
       !$$DATABASES{'REFERENCE_DB'}{'-pass'} )
  {
    throw "\nDatabase parameters missing from file " .
      $$DATABASES{'REFERENCE_DB'}{'-host'} . "Bio::EnsEMBL:Analysis::Config::GeneBuild::Databases\n";
  }
  ...
  foreach my $configvar (@configvars) {
    ...
    if ( !$configvarref ) {
      throw "\nAnalysis parameters $configvar missing from file " . "Bio::EnsEMBL:Analysis::Config::ExonerateTags\n";
    }
    if ( $configvar =~ m/DIR/ ) {
      if ( !-e $configvarref ) {
        throw "\nCouldn t find/access file or directory " . "from file Bio::EnsEMBL:Analysis::Config::ExonerateTags\n";
      }
    }
  }

  #CONNECT TO PIPELINE DATABASE
  my $pipedb = new Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor( -host   => $$DATABASES{'REFERENCE_DB'}{'-host'},
                                                             -port   => $$DATABASES{'REFERENCE_DB'}{'-port'},
                                                             -user   => $$DATABASES{'REFERENCE_DB'}{'-user'},
                                                             -pass   => $$DATABASES{'REFERENCE_DB'}{'-pass'},
                                                             -dbname => $$DATABASES{'REFERENCE_DB'}{'-dbname'} ) or
    throw "cant connect to pipedatabase.";
  return ($pipedb);
} ## end sub setup
...
sub save_ditags {
  ...
  my $ans        = "n";
  my %seqs;
  my $ditag_adaptor = $db->get_ditagAdaptor or throw("Couldn t get DitagAdaptor. Check analysis tables.");

  my $tmpfile   = $config{TMPDIR} . '/ditag_inserts.' . $tagtype . '.sql';
  my $queryfile = $config{QUERYFILES}{$tagtype};

  open( DITAGS, "<" . $queryfile ) or throw( "Couldn t open ditag file " . $queryfile );
  open( TOFILE, ">" . $tmpfile )   or throw( "Couldn t open output file " . $tmpfile );
  ...
  foreach my $tag_sequence ( keys %seqs ) {
    print TOFILE $ditag_adaptor->print_creation( $seqs{$tag_sequence}->{'probe_name'},
  ...
  if ($write) {
    #LOAD TO DATABASE
    my $cmd =
      "mysql -h $$DATABASES{'REFERENCE_DB'}{'-host'} " .
      " -P$$DATABASES{'REFERENCE_DB'}{'-port'} " . " -u$$DATABASES{'REFERENCE_DB'}{'-user'} " .
      " -p$$DATABASES{'REFERENCE_DB'}{'-pass'} " . " -D$$DATABASES{'REFERENCE_DB'}{'-dbname'} " . " < $tmpfile ";
...
sub make_input_ids {
  ...
  for ( my $i = 1; $i <= $number_of_chunks; $i++ ) {
    ...
    print SQL "INSERT into input_id_analysis " . "values ('$iid', '$input_type', $ana_id, now(), '', '', 0);\n";
  }
  ...
  if ($write) {
    ...
    print STDERR "INPUT IDS CREATED IN " . $$DATABASES{'REFERENCE_DB'}{'-dbname'} .
      ":" . $$DATABASES{'REFERENCE_DB'}{'-host'} . ":" . $$DATABASES{'REFERENCE_DB'}{'-port'} . "\n";
    if ( !defined $delete ) {
      print STDERR "REMOVE FILE? ";
      $ans = <>;
