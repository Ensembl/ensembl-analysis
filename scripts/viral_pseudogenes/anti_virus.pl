...
if ( $regfile && $compara ) {
  ...
  my $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( '-host' => $dbhost, '-user' => 'ensro', '-dbname' => $dbname, '-port' => $dbport, );
  push @dbas, $db;
}
...
foreach my $db (@dbas) {
  ...
  foreach my $domain ( keys %$domains ) {
    push @genes, @{
      sql(
        "SELECT DISTINCT(transcript.gene_id)
  ...
  TRANS: foreach my $transcript ( @{ $gene->get_all_Transcripts } ) {
      ...
      if ($check_domains) {
        ...
        foreach my $pf ( @{ $transcript->translation->get_all_ProteinFeatures } ) {
          next unless $pf->analysis->logic_name eq "Superfamily" or $pf->analysis->logic_name eq "Pfam";
      ...
      } ## end if ($check_domains)
      else {
        $bad_trans++;
      }

      # call the transcript dodgy if all the domains it has are bad or dodgy ones
      if ( $total_domains{ $transcript->dbID } ==
           ( $bad_domains{ $transcript->dbID }{'count'} + $dodgy_domains{ $transcript->dbID }{'count'} ) &&
           $total_domains{ $transcript->dbID } > 1 )
      {
        $dodgy_trans++;
      }
    } ## end TRANS: foreach my $transcript ( @{...})
    ...
    $genelist->{$gene_id}{'unknown'}  = \%unknown_domains;
  } ## end GENE: foreach my $gene_id (@$genes)
...
sub write_output {
  ...
  foreach my $gene_id ( keys %$genelist ) {
    ...
    print HTML "<TR><TD>$gene_num<TD><a href= 'http://www.ensembl.org/$URL/geneview?gene=" .
      $gene->stable_id . ";db=core'>" . $gene->stable_id . ' </a>'
      if $html;
    ...
    foreach my $transcript ( @{ $gene->get_all_Transcripts } ) {
      ...
      $count++;
      print HTML "<TD><a href= 'http://www.ensembl.org/$URL/transview?;db=core;transcript=" .
        $transcript->stable_id . ";db=core'>" . $transcript->stable_id . ' </a>'
        if $html;
      ...
      if ( my $translation = $transcript->translation ) {
        ...
        print HTML "<TD><a href= 'http://www.ensembl.org/$URL/protview?transcript=" .
          $transcript->stable_id . ";db=core'>" . $translation->stable_id . ' </a>'
          if $html;
      ...
      print SUMMARY "  cdna length / genomic span : \t"
        .
        sprintf( "%.2f",
                 ( $transcript->coding_region_end - $transcript->coding_region_start )/
      ...
      foreach my $sf (@sfs) {
        if ( $sf->isa("Bio::EnsEMBL::DnaPepAlignFeature") ) {
          ...
          if ($html) {
            print HTML
              '<a href= http://srs.ebi.ac.uk/srsbin/cgi-bin/wgetz?-newId+-e+[libs%3d{IMGTLIGM%20SWALL%20REFSEQ%20EMBL%20REFSEQP}-all:' .
              $sf->hseqname . ']+-vn+2>' . $sf->hseqname . '</a><br>'
              unless $done{ $sf->hseqname };
          }
          $done{ $sf->hseqname } = 1;
