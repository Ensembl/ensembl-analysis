...
my ( $dbname, $dbhost,      $dbuser,      $dbport,      $tgdbname,   $tgdbhost,   $tgdbuser,   $tgdbport, $tgdbpass,
     $path,   @tg_biotypes, %tg_biotypes, $tg_biotypes, $dna_dbuser, $dna_dbhost, $dna_dbname, $dna_dbport );
...
my $qy_db = Bio::EnsEMBL::DBSQL::DBAdaptor->new( '-dbname' => $dbname, '-host' => $dbhost, '-user' => $dbuser, '-port' => $dbport, );

my $tg_db = Bio::EnsEMBL::DBSQL::DBAdaptor->new( '-dbname' => $tgdbname,
                                                 '-host'   => $tgdbhost,
                                                 '-user'   => $tgdbuser,
                                                 '-port'   => $tgdbport,
                                                 '-pass'   => $tgdbpass );
...
if ( $qyDB_has_DNA == 0 || $tgDB_has_DNA == 0 ) {
  ...
  my $dnadb = Bio::EnsEMBL::DBSQL::DBAdaptor->new( -dbname => $dna_dbname,
                                                   -host   => $dna_dbhost,
                                                   -user   => $dna_dbuser,
                                                   -port   => $dna_dbport,
                                                   -path   => $path );
...
if ($tg_biotypes) {
  ...
  print "This is your array of transcript biotypes which can be overwritten by Ig models should there be exon overlap : ",
    join( " - ", @tg_biotypes ), "\n";
...
if (@ARGV) {
  foreach my $slid (@ARGV) {
    ...
    my $tl_sl = $qy_db->get_SliceAdaptor->fetch_by_region( 'toplevel', $sl->seq_region_name );
...
if ($sub_biotypes) {
  ( $genes_to_delete_hash, $stable_id_event_hash ) = &compare_new_genes_with_current_genes( \@genes, 1 );
}
else {
  ( $genes_to_delete_hash, $stable_id_event_hash ) = &compare_new_genes_with_current_genes( \@genes, 0 );
}
########################################################
...
$verbose and
  printf( STDERR "Done (%s removed, %d added)\n%s\n", scalar( keys %$genes_to_delete_hash ), scalar(@genes), &gene_stats_string );
...
sub compare_new_genes_with_current_genes {
  ...
  foreach my $sr_id ( keys %by_slice ) {
    ...
    my $tg_tl_slice = $tg_db->get_SliceAdaptor->fetch_by_region( 'toplevel', $sr_id );

    foreach my $g (@g) {
      # Need to fully load the genes
      $g->load();
      my $oslice = $tg_db->get_SliceAdaptor->fetch_by_region( 'toplevel', $sr_id, $g->start, $g->end );
        ...
        foreach my $t ( @{ $g->get_all_Transcripts } ) {
          ...
          foreach my $og (@ogenes) {
            ...
            PAIR: foreach my $oe ( @{ $ot->get_all_Exons } ) {
                foreach my $e (@exons) {
                  if ( $e->strand == $oe->strand and $e->overlaps($oe) ) {
                    $has_exon_overlap = 1;
                    last PAIR;
                  }
                }
              ...
              } ## end if ($has_exon_overlap)
            } ## end foreach my $ot ( @{ $og->get_all_Transcripts...})
          } ## end foreach my $og (@ogenes)
        } ## end foreach my $t ( @{ $g->get_all_Transcripts...})
      } ## end if (@ogenes)
    } ## end foreach my $g (@g)
  } ## end foreach my $sr_id ( keys %by_slice)

  return ( \%genes_to_remove, \%stable_id_event );
} ## end sub compare_new_genes_with_current_genes
...
sub check_if_DB_contains_DNA {
  ...
  if ( $dna_array[0] > 0 ) {
    print "Your DB " . $db->dbc->dbname . " contains DNA sequences. No need to attach a " . "DNA_DB to it.\n" if ($verbose);
    return 1;
  }
  else {
