...
my ( $dbname, $dbhost, $dbuser, $dbport, $dbpass, $index_dir, $index_file, $guess_gene_names, $all_gene_names, );
...
my $qy_db = Bio::EnsEMBL::DBSQL::DBAdaptor->new( '-dbname' => $dbname,
                                                 '-host'   => $dbhost,
                                                 '-user'   => $dbuser,
                                                 '-port'   => $dbport,
                                                 '-pass'   => $dbpass );
...
my %biotypes =
  ( 'V_segment' => 'V gene segment', 'D_segment' => 'D gene segment', 'J_segment' => 'J gene segment', 'C_segment' => 'C gene segment', );

my $index = new Bio::DB::Flat::OBDAIndex( -index_dir => $index_dir, -dbname => $index_file, -format => 'IMGT_embl', );
  ...
  foreach my $g (@genes) {
    ...
    foreach my $t ( @{ $g->get_all_Transcripts } ) {
      ...
      printf( "%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
              ...
              "IMGT/LIGM_DB", $f->hseqname, $f->hseqname, "", "XREF" );
    }
    ...
    foreach my $hseqname ( keys %support ) {
      ...
      foreach my $reg (@greg) {
        if ( not exists $gnames{ $reg->{gene} } ) {
          $gnames{ $reg->{gene} } = { name => $reg->{gene}, count => 0, percent => 0, };
        }
...
} ## end foreach my $sr_id ( keys %genes_by_slice)
  ...
  } keys %gene_to_name )
{
  ...
  my @names;
  if ( @{ $gene_to_name{$gid} } ) {
    if ( $guess_gene_names or scalar( @{ $gene_to_name{$gid} } ) == 1 ) {
      push @names, $gene_to_name{$gid}->[0]->{name};
    }
    elsif ($all_gene_names) {
      foreach my $nh ( @{ $gene_to_name{$gid} } ) {
        push @names, sprintf( "%s(%d,%.1f%s)", $nh->{name}, $nh->{count}, $nh->{av_percent}, $nh->{supported} ? "*" : "" );
      }
    }
  }
...
} ## end foreach my $gid ( sort { $all_genes...})
...
sub extract_seq_info {
  ...
  foreach my $val ( $seq->get_SeqFeatures ) {
    if ( $val->primary_tag =~ /^(gene)|(\S+\-GENE)|(\S\-REGION)$/ and $val->annotation->get_Annotations("gene") ) {
      ...
      if ( $tag_val->value !~ /\s+/ ) {
        push @gene_regs, { start => $val->location->start, end => $val->location->end, gene => $tag_val->value, };
      }
    }
  }
  ...
  for ( my $i = 0; $i < @gene_regs; $i++ ) {
    ...
    if ( not defined $max_end_upstream or $this_gene->{end} > $max_end_upstream ) {
  ...
  return { description => $seq->description, keywords => [ $seq->get_keywords ], gene_list => \@gene_regs, };
} ## end sub extract_seq_info
...
sub find_overlapping_genes {
  ...
  for ( my $j = $left - 1; $j >= 0; $j-- ) {
    ...
    elsif ( $list->[$j]->{start} <= $en and $list->[$j]->{end} >= $st ) {
      push @overlapping, $list->[$j];
    }
  }

  return @overlapping;
} ## end sub find_overlapping_genes
...
} ## end sub find_best_name_and_desc
