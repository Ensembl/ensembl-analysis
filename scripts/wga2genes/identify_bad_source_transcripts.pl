...
my ( $dbname, $dbhost, $dbuser, $dbport, $dbpass, $look_for_dodgy, $print_gsi, );
...
foreach my $sl ( sort { $a->length <=> $b->length } @slices ) {
  ...
  foreach my $g (@usable_genes) {
    ...
    foreach my $t ( @{ $g->get_all_Transcripts } ) {
      ...
      if ( length( $t->translateable_seq ) % 3 != 0 ) {
        ...
        if ($look_for_dodgy) {
          ...
          push @trans, { start => $cst, end => $cen, tran => $t, };
        }
      }
    ...
    if (@trans) {
      ...
      push @genes, { start => $gst, end => $gen, gene => $g, transcripts => \@trans, };
    }
  } ## end foreach my $g (@usable_genes)
  ...
  foreach my $g (@genes) {
    if ( not @clusters or $clusters[-1]->{end} < $g->{start} - 1 ) {
      push @clusters, { start => $g->{start}, end => $g->{end}, genes => [$g], };
    }
    else {
    ...
    if ( @{ $c->{genes} } > 1 ) {
      ...
      foreach my $g ( @{ $c->{genes} } ) {
        ...
        foreach my $og ( @{ $c->{genes} } ) {
          ...
          if ( $og->{start} > $g->{start} and $og->{end} < $g->{end} ) {
            push @inside, $og;
          }
        }
        ...
        if (@inside) {
          ...
          foreach my $t ( @{ $g->{transcripts} } ) {
            ...
            foreach my $in (@inside) {
              if ( $in->{start} < $t->{start} or $in->{end} > $t->{end} ) {
                $all_inside = 0;
                last;
              }
            }
      ...
      } ## end foreach my $g ( @{ $c->{genes...}})
    } ## end if ( @{ $c->{genes} } ...)
  } ## end foreach my $c (@clusters)
} ## end foreach my $sl ( sort { $a->...})
