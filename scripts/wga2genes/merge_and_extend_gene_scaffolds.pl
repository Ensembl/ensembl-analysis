...
my ( $dbname,       $dbuser,     $dbhost,      $dbport,    $verbose,   $agp_outfile, $agp_outfh,
     $gene_outfile, $gene_outfh, $log_outfile, $log_outfh, @agp_files, @gene_files );
...
my $DB = Bio::EnsEMBL::DBSQL::DBAdaptor->new( -dbname => $dbname, -host => $dbhost, -user => $dbuser, -port => $dbport );
...
if ( defined $agp_outfile ) {
  open $agp_outfh, ">$agp_outfile" or die "Could not open $agp_outfile for writing\n";
}
else {
  $agp_outfh = \*STDOUT;
}
if ( defined $gene_outfile ) {
  open $gene_outfh, ">$gene_outfile" or die "Could not open $gene_outfile for writing\n";
}
else {
  $gene_outfh = \*STDOUT;
}
if ( defined $log_outfile ) {
  open $log_outfh, ">$log_outfile" or die "Could not open $log_outfile for writing\n";
}
else {
  $log_outfh = \*STDERR;
}
...
for ( my $cl_cnt = 0; $cl_cnt < @clusters; $cl_cnt++ ) {
  ...
  my ( $target_slices, $target_slice_maps ) = &fetch_slices_and_component_maps( $DB, $gene_scaffolds );
  ...
  my $extended_chains =
    &extend_gene_scaffold_components( $chains, \%complex_gene_scaffolds, \%simple_gene_scaffolds, $target_slices, $target_slice_maps );

  # make a map to/from Gene scaffold coords and component coords
  my $orig_map = Bio::EnsEMBL::Mapper->new( 'scaffold', 'genescaffold' );

  my $new_map = Bio::EnsEMBL::Mapper->new( 'scaffold', 'genescaffold' );

  foreach my $gs_id ( keys %$gene_scaffolds ) {
    my $gs = $gene_scaffolds->{$gs_id};
    foreach my $seg ( @{ $gs->{components} } ) {
      $orig_map->add_map_coordinates( $seg->to->id, $seg->to->start,   $seg->to->end, $seg->to->strand,
                                      $gs_id,       $seg->from->start, $seg->from->end, );
    }
  }
  ...
  foreach my $ch (@$extended_chains) {
    ...
    for ( my $i = 0; $i < @coords; $i++ ) {
      ...
      $new_map->add_map_coordinates( $c->id, $c->start, $c->end, $c->strand, $new_gs_name, $gs_start, $last_end, );

      if ( $c->id =~ /^$FAKE_SCAFFOLD_PREFIX/ ) {
        printf( $agp_outfh "%s\t%d\t%d\t%d\tN\t%d\n", $new_gs_name, $gs_start, $last_end, $line_count++, $last_end - $gs_start + 1 );
      }
      else {
        printf( $agp_outfh "%s\t%d\t%d\t%d\tW\t%s\t%d\t%d\t%s\n",
                $new_gs_name, $gs_start, $last_end, $line_count++, $c->id, $c->start, $c->end, $c->strand > 0 ? "+" : "-" );
      }
    ...
    foreach my $gs_id (@gs_ids) {
      foreach my $line ( @{ $gene_scaffolds->{$gs_id}->{annotation} } ) {
        ...
        my ($loc_in_orig) = $orig_map->map_coordinates( $line->[0], $line->[3], $line->[4], $line->[5], 'genescaffold' );

        my ($loc_in_new) =
          $new_map->map_coordinates( $loc_in_orig->id, $loc_in_orig->start, $loc_in_orig->end, $loc_in_orig->strand, 'scaffold' );
  ...
  } ## end foreach my $ch (@$extended_chains)
  ...
  foreach my $gs_id ( keys %simple_gene_scaffolds ) {
    ...
    foreach my $cmp (@cmps) {
      my ($loc_in_orig) = $orig_map->map_coordinates( $gs_id, $cmp->from->start, $cmp->from->end, 1, 'genescaffold' );

      my ($loc_in_new) = $new_map->map_coordinates( $loc_in_orig->id, $loc_in_orig->start, $loc_in_orig->end, 1, 'scaffold' );
  ...
  } ## end foreach my $gs_id ( keys %simple_gene_scaffolds)
  ...
  foreach my $gs_id ( keys %simple_gene_scaffolds ) {
    ...
    foreach my $line ( @{ $simple_gene_scaffolds{$gs_id}->{annotation} } ) {
      ...
      my ($mapped_reg) = $orig_map->map_coordinates( $line->[0], $line->[3], $line->[4], 1, 'genescaffold' );
...
sub single_linkage_cluster {
  ...
  return @clusts;
} ## end sub single_linkage_cluster
...
sub identify_plugged_gaps {
  ...
  foreach my $gs_id ( keys %{$gene_scaffolds} ) {
    ...
    my @comps = sort { $a->from->start <=> $b->from->start; } @{ $gene_scaffolds->{$gs_id}->{components} };
    ...
    foreach my $l (@lines) {
      ...
      if ($in_gap) {
        push @gaps, { start => $start, end => $end, };
      }
    }

    foreach my $gap ( sort { $a->{start} <=> $b->{start} } @gaps ) {
      if ( @{ $plugged_gaps{$gs_id} } and $plugged_gaps{$gs_id}->[-1]->{end} >= $gap->{start} ) {
  ...
  foreach my $gs_id ( keys %plugged_gaps ) {
    ...
    foreach my $reg ( @{ $plugged_gaps{$gs_id} } ) {
      ...
      my $fake_q = Bio::EnsEMBL::Mapper::Coordinate->new( $gs_id,     $reg->{start}, $reg->{end}, 1 );
      my $fake_t = Bio::EnsEMBL::Mapper::Coordinate->new( $fake_name, $fake_start,   $fake_end,   1 );
      my $pair = Bio::EnsEMBL::Mapper::Pair->new( $fake_q, $fake_t );

      push @{ $gene_scaffolds->{$gs_id}->{components} }, $pair;
    }

    $gene_scaffolds->{$gs_id}->{components} =
      [ sort { $a->from->start <=> $b->from->start; } @{ $gene_scaffolds->{$gs_id}->{components} } ];
  }
} ## end sub identify_plugged_gaps
...
sub make_gene_scaffold_chains {
  ...
  $go_further = 0 if scalar( keys %{$gene_scaffolds} ) <= 1;
  ...
  if ($go_further) {
    ...
    GS:
        ...
        foreach my $comp (@comps) {
          if ( not defined $strand ) {
            ...
            if ( $comp->to->strand == $strand ) {
              if ( $strand > 0 and $comp->to->start > $tend ) {
                $tend = $comp->to->end;
              }
              elsif ( $strand < 0 and $comp->to->end < $tstart ) {
                $tstart = $comp->to->start;
              }
              else {
                next GS;
      ...
      if ( scalar(@extents) > 1 ) {
        ...
        } ## end foreach my $sl (@sub_lists)
      } ## end if ( scalar(@extents) ...)
    ...
    } ## end foreach my $shared_sid (@shared_sids)
  } ## end if ($go_further)
  ...
  OC: foreach my $o_c (@merged_chains) {
    ...
    } ## end OC: foreach my $o_c (@merged_chains)
    if ( not $merged ) {
      push @merged_chains, $c;
    }
  } ## end foreach my $c (@all_chains)
...
} ## end sub make_gene_scaffold_chains
...
sub _generate_sub_lists {
  ...
  if ( scalar(@list) == 2 ) {
    ...
    return @s_lists;
  } ## end else [ if ( scalar(@list) == ...)]
} ## end sub _generate_sub_lists
...
sub sort_gene_scaffolds_by_common_component {
  ...
  my @summaries;
  foreach my $gs_id (@$gs_ids) {
    my $extents = &get_gene_scaffold_component_extents( $gene_scaffolds->{$gs_id} );

    if ( exists( $extents->{$s_id}->{ori}->{1} ) and exists( $extents->{$s_id}->{ori}->{-1} ) ) {
    ...
    push @summaries, { gs_id => $gs_id, start => $extents->{$s_id}->{start}, end => $extents->{$s_id}->{end}, ori => $ori, };
  }

  for ( my $i = 0; $i < @summaries; $i++ ) {
    for ( my $j = 0; $j < $i; $j++ ) {
      if ( $summaries[$i]->{start} <= $summaries[$j]->{end} and $summaries[$i]->{end} >= $summaries[$j]->{start} ) {
        # overlap
  ...
  return 1;
} ## end sub sort_gene_scaffolds_by_common_component
...
sub extend_gene_scaffold_components {
  my ( $chains, $gene_scaffolds, $simple_gene_scaffolds, $target_slices, $target_slice_maps ) = @_;
  ...
  foreach my $gs_id ( keys %$simple_gene_scaffolds ) {
    ...
    foreach my $cmp ( @{ $simple_gene_scaffolds->{$gs_id}->{components} } ) {
      ...
      $min_start = $cmp->to->start if not defined $min_start or $cmp->to->start < $min_start;
      $max_end   = $cmp->to->end   if not defined $max_end or $cmp->to->end > $max_end;
    }
    push @{ $non_break_regions{$id} }, { start => $min_start, end => $max_end, };
  }
  ...
  foreach my $chain (@$chains) {
    ...
    foreach my $gs_id (@$chain) {
      ...
      foreach my $pair ( @{ $gene_scaffolds->{$gs_id}->{components} } ) {
        ...
        if ( @merged_coords and $merged_coords[-1]->strand == $this_c->strand and $merged_coords[-1]->id eq $this_c->id ) {

          if ( $this_c->strand == 1 ) {
            if ( $this_c->start > $merged_coords[-1]->end ) {
              my $can_merge = 1;
              foreach my $o_c ( @{ $coords_by_scaffold{ $this_c->id } } ) {
                if ( $o_c->start > $merged_coords[-1]->end and $o_c->end < $this_c->start ) {
                  $can_merge = 0;
                  last;
                }
              }
            ...
            if ( $this_c->end < $merged_coords[-1]->start ) {
              my $can_merge = 1;
              foreach my $o_c ( @{ $coords_by_scaffold{ $this_c->id } } ) {
                if ( $o_c->start > $this_c->end and $o_c->end < $merged_coords[-1]->start ) {
                  $can_merge = 0;
                  last;
                }
              }
        ...
        if ( not $merged ) {
          push @merged_coords, Bio::EnsEMBL::Mapper::Coordinate->new( $this_c->id, $this_c->start, $this_c->end, $this_c->strand );
        }
      } ## end foreach my $pair ( @{ $gene_scaffolds...})
    } ## end foreach my $gs_id (@$chain)

    push @merged_chains, { members => $chain, merged_coords => \@merged_coords, };
  } ## end foreach my $chain (@$chains)
  ...
  foreach my $sid ( keys %coords_by_scaffold ) {
    my @sorted_els = sort { $a->start <=> $b->start; } @{ $coords_by_scaffold{$sid} };
    ...
    my @bits = $map->map_coordinates( $sid, 1, $tsl->length, 1, 'scaffold' );
    my $last_end = 0;
    for ( my $i = 0; $i < @bits; $i++ ) {
      my $bit = $bits[$i];
      ...
      elsif ( $i > 0 and $bits[ $i - 1 ]->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {

        push @contig_breaks, Bio::EnsEMBL::Mapper::Gap->new( $last_end, $this_start );
      }
    ...
    for ( my $i = 1; $i < @sorted_els; $i++ ) {
      ...
      foreach my $break (@scaffold_gaps) {
        if ( $break->start <= $this_el->start and $break->end >= $prev_el->end ) {
          ...
          foreach my $reg (@non_break_regions) {
            if ( $break->start >= $reg->{start} and $break->end <= $reg->{end} ) {
              $bad_break = 1;
              last;
            }
          }
          push @relevant_gaps, $break if not $bad_break;
        }
      }
      ...
      foreach my $break (@contig_breaks) {
        if ( $break->start < $this_el->start and $break->end > $prev_el->end ) {
          ...
          push @relevant_breaks, $break if not $bad_break;
        }
      }
      ...
      if (@relevant_gaps) {
        ...
        printf( $log_outfh "EXTEND: Split in scaffold gap (%s/%d-%d -> %d-%d)\n",
                $sid, $prev_el->end, $this_el->start, $split_reg_start, $split_reg_end );
      }
        ...
        if (@non_break_regions) {
          for ( my $i = 1; $i < @non_break_regions; $i++ ) {
            ...
            if ( $okay_reg_start >= $split_reg_start and $okay_reg_end <= $split_reg_end ) {
    ...
    } ## end for ( my $i = 1; $i < @sorted_els...)

    $sorted_els[-1]->end( $tsl->length );
  } ## end foreach my $sid ( keys %coords_by_scaffold)

  return \@merged_chains;
} ## end sub extend_gene_scaffold_components
...
sub fetch_slices_and_component_maps {
  ...
  foreach my $gs_id ( keys %$gene_scaffolds ) {
    foreach my $comp ( @{ $gene_scaffolds->{$gs_id}->{components} } ) {
      ...
      if ( not exists( $slices{$sid} ) ) {
        $slices{$sid} = $db->get_SliceAdaptor->fetch_by_region( 'toplevel', $sid );

        # get components here
        my @seq_lev = @{ $slices{$sid}->project('seqlevel') };

        my $map = Bio::EnsEMBL::Mapper->new( 'seqlevel', 'scaffold' );

        foreach my $comp (@seq_lev) {
          $map->add_map_coordinates( $comp->to_Slice->seq_region_name,
                                     $comp->to_Slice->start,
                                     $comp->to_Slice->end, $comp->to_Slice->strand,
                                     $sid, $comp->from_start, $comp->from_end );
  ...
  } ## end foreach my $gs_id ( keys %$gene_scaffolds)

  return ( \%slices, \%slice_maps );
} ## end sub fetch_slices_and_component_maps
...
sub is_simple_gene_scaffold {
  ...
  return 1;
} ## end sub is_simple_gene_scaffold
...
my ( @gs_fhs, @gene_fhs, %gs_file_index, %gene_file_index );
...
sub index_gene_scaffold_files {
  ...
  for ( my $i = 0; $i < @files; $i++ ) {
    ...
    for ( my $curpos = tell $fh; $_ = <$fh>; $curpos = tell $fh ) {
      ...
      if ( defined $gs_id ) {
        if ( not exists $gs_file_index{$gs_id} ) {
          $gs_file_index{$gs_id} = { fh_index => $i, from => $curpos, to => $curpos, };
        }
        else {
    ...
    } ## end for ( my $curpos = tell...)
  } ## end for ( my $i = 0; $i < @files...)

  return ( \%gs_index, \%s_index );
} ## end sub index_gene_scaffold_files
    ...
    for ( my $curpos = tell $fh; $_ = <$fh>; $curpos = tell $fh ) {
      ...
      if ( defined $gs_id ) {
        if ( not exists $gene_file_index{$gs_id} ) {
          $gene_file_index{$gs_id} = { fh_index => $i, from => $curpos, to => $curpos, };
        }
        else {
          if ( defined $last_gs_id and $gs_id ne $last_gs_id ) {
...
} ## end sub index_gene_annotation_files
  ...
  foreach my $gs_id (@gs_ids) {
    ...
    for ( my $curpos = tell $fh; $_ = <$fh>; $curpos = tell $fh ) {
      ...
      /^(\S+)\s+(\d+)\s+(\d+)\s+\S+\s+\S+\s+(\S+)\s+(\d+)\s+(\d+)\s+(\S+)/ and do {
        my ( $this_gs_id, $gs_start, $gs_end, $scaf_id, $scaf_start, $scaf_end, $scaf_ori ) = ( $1, $2, $3, $4, $5, $6, $7 );
        ...
        ###
        my $asm_unit = Bio::EnsEMBL::Mapper::Coordinate->new( $gs_id,   $gs_start,   $gs_end,   1 );
        my $cmp_unit = Bio::EnsEMBL::Mapper::Coordinate->new( $scaf_id, $scaf_start, $scaf_end, $scaf_ori );
        my $pair = Bio::EnsEMBL::Mapper::Pair->new( $asm_unit, $cmp_unit );

        push @{ $gene_scaffolds->{$gs_id}->{components} }, $pair;

        }
    } ## end for ( my $curpos = tell...)
  } ## end foreach my $gs_id (@gs_ids)
} ## end sub fetch_gene_scaffold_entries
