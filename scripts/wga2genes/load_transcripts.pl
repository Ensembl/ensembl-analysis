...
my ( $dbname,          $dbhost,        $dbuser,   $dbport,      $dbpass,     $test, $verbose,
     $gene_logic_name, $sf_logic_name, %ug_feats, %transcripts, %attributes, @out_genes, );
...
my ( $gs_cs, $prev_tl_cs ) = sort { $a->rank <=> $b->rank } @{ $db->get_CoordSystemAdaptor->fetch_all };
...
while (<>) {
  ...
  if (/^\#\#\-ATTRIBUTE\s+(.+)/) {
    ...
    if ( defined($tid) and defined($code) and defined($val) ) {
      ...
      push @{ $attributes{$tid} }, { code => $code, value => $val, name => $name, desc => $desc, };
    }
    next;
  } ## end if (/^\#\#\-ATTRIBUTE\s+(.+)/)
  elsif (/^\#/) {
  ...
  my ( $seq_name, $source, $type, $start, $end, $strand ) = ( $l[0], $l[1], $l[2], $l[3], $l[4], $l[5] );
  ...
  if ( $l[2] eq 'Exon' ) {
    ...
    push @{ $transcripts{$seq_name}->{$transcript_id} },
      { exon_id => $exon_id, start => $start, end => $end, strand => $strand, phase => $phase, end_phase => $end_phase, };
...
foreach my $target_id ( keys %transcripts ) {
  ...
  if ( $target_id =~ /^GeneScaffold/ ) {
    $slice = $db->get_SliceAdaptor->fetch_by_region( $gs_cs->name, $target_id );
  }
  else {
    $slice = $db->get_SliceAdaptor->fetch_by_region( $prev_tl_cs->name, $target_id );
  }

  foreach my $transcript_id ( keys %{ $transcripts{$target_id} } ) {
    my $tran = Bio::EnsEMBL::Transcript->new( -analysis => $gene_analysis, -biotype => 'protein_coding' );
    ...
    foreach my $ex_h ( @{ $transcripts{$target_id}->{$transcript_id} } ) {
      ...
      $tran->add_Exon($exon);
    } ## end foreach my $ex_h ( @{ $transcripts...})
  ...
  } ## end foreach my $transcript_id (...)

  foreach my $clust ( &cluster_by_exon_overlap(@transcripts) ) {
    # check for exon-overlap between result genes;
    my $gene = Bio::EnsEMBL::Gene->new( -analysis => $gene_analysis, -biotype => 'protein_coding' );
...
sub cluster_by_exon_overlap {
  ...
  return @final_clusts;
} ## end sub cluster_by_exon_overlap
