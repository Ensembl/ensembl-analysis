...
my ( $dbname, $dbhost, $dbuser, $dbport, $dbpass, $verbose, $seq_level, $direct_level, $asm_coord_sys_name, $asm_coord_sys_version,
     $cmp_coord_sys_name, $test, %agps, %comp_slices, );
...
foreach my $gs_id ( keys %agps ) {
  ...
  if ($test) {
    ...
    eval { $seq_region_id = $db->get_SliceAdaptor->get_seq_region_id($slice); };
    if ($@) {
  ...
  foreach my $comp ( @{ $agp->{components} } ) {
    if ( not exists $comp_slices{ $comp->to->id } ) {
      $comp_slices{ $comp->to->id } = $db->get_SliceAdaptor->fetch_by_region( $cmp_coord_sys_name, $comp->to->id );
    }

    if ($seq_level) {
      my $bit_slice =
        $db->get_SliceAdaptor->fetch_by_region( $cmp_coord_sys_name, $comp->to->id, $comp->to->start, $comp->to->end, $comp->ori );
      ...
      foreach my $bit (@bits) {
        ...
        if ($test) {
          printf( "ASSEMBLY: %s %d %d %s %d %d %d\n",
                  $slice->seq_region_name, $gs_start, $gs_end, $bit->to_Slice->seq_region_name,
                  $bit->to_Slice->start, $bit->to_Slice->end, $bit->to_Slice->strand );
        }
        else {
          $ass_sth->execute( $seq_region_id, $db->get_SliceAdaptor->get_seq_region_id( $bit->to_Slice ),
                             $gs_start, $gs_end, $bit->to_Slice->start, $bit->to_Slice->end, $bit->to_Slice->strand );
        }
      }
    ...
    if ($direct_level) {
      if ($test) {
        printf( "ASSEMBLY: %s %d %d %s %d %d %d\n",
                $slice->seq_region_name, $comp->from->start, $comp->from->end, $comp->to->start, $comp->to->end, $comp->ori );
      }
      else {
        $ass_sth->execute( $seq_region_id, $db->get_SliceAdaptor->get_seq_region_id( $comp_slices{ $comp->to->id } ),
                           $comp->from->start, $comp->from->end, $comp->to->start, $comp->to->end, $comp->ori );
      }
    }
  } ## end foreach my $comp ( @{ $agp->...})
} ## end foreach my $gs_id ( keys %agps)
...
if ($seq_level) {
  ...
  my $mapstring = sprintf( "%s:%s|%s", $asm_coord_sys_name, $asm_coord_sys_version, $cs->name );
  if ($test) {
...
if ($direct_level) {
  my $mapstring = sprintf( "%s:%s#%s:%s", $asm_coord_sys_name, $asm_coord_sys_version, $cmp_coord_sys_name, $cmp_coord_sys_version );
  if ($test) {
    printf( "META: %s %s\n", "assembly.mapping", $mapstring );
  }
  else {
...
sub read_agp_files {
  ...
  while (<>) {
    ...
    if ( $l[4] ne 'N' ) {
      ...
      my $pair = Bio::EnsEMBL::Mapper::Pair->new( $from, $to, $l[8] eq '-' ? -1 : 1 );

      push @{ $agp_hash->{ $l[0] }->{components} }, $pair;
      if ( not exists $agp_hash->{ $l[0] } or $agp_hash->{ $l[0] }->{length} < $l[2] ) {
...
sub write_coord_system {
  ...
  if ( not defined $cs ) {
    $cs = Bio::EnsEMBL::CoordSystem->new( -name => $new_name, -version => $new_version, -rank => 1, -default => 1 );
