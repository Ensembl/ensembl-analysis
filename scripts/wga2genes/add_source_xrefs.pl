...
my ( $dbname,     $dbhost,     $dbuser,     $dbport,     $dbpass,     $stable_id_map_file,
     @src_dbname, $src_dbhost, $src_dbuser, $src_dbport, $src_dbpass, $verbose, );
...
if ( defined $stable_id_map_file ) {
  ...
  foreach my $src_dbname (@src_dbname) {
    my $src_db = Bio::EnsEMBL::DBSQL::DBAdaptor->new( '-dbname' => $src_dbname,
                                                      '-host'   => $src_dbhost,
                                                      '-user'   => $src_dbuser,
                                                      '-port'   => $src_dbport,
                                                      '-pass'   => $src_dbpass );
    ...
    foreach my $ens_pep_name (@ens_pep_names) {
      ...
      if ( defined $stable_id_map ) {
        if ( exists $stable_id_map->{$ens_pep_name} ) {
          ( $src_p_id, $src_p_ver, $src_t_id, $src_t_ver, $src_g_id, $src_g_ver ) = @{ $stable_id_map->{$ens_pep_name} };
        }
      }
      elsif (@src_dbs) {
...
sub find_source_transcript_and_gene {
  ...
  foreach my $db (@src_dbs) {
    ...
    $src_p = $db->get_TranslationAdaptor->fetch_by_stable_id($ensp_id);
    next unless defined $src_p;

    $src_db = $db;

    if ( defined $src_p ) {
      $src_t = $src_db->get_TranscriptAdaptor->fetch_by_translation_stable_id( $src_p->stable_id );

      if ( defined $src_t ) {
        $src_g = $src_db->get_GeneAdaptor->fetch_by_transcript_stable_id( $src_t->stable_id );
      }
    }
  }
...
sub read_map_file {
  ...
  while (<F>) {
    ...
    if ( not exists $idmap{ $l[0] } or $l[1] > $idmap{$pid}->[1] ) {
      $idmap{$pid} = \@l;
    }
  }
  close(F);
