...
&GetOptions( 'db:s' => \$blast_db, 'write!' => \$write, 'chr:s' => \$chromosome, 'thresholds:s' => \$t, 'sql:s' => \$file, );
...
sub label_singletons {
  ...
  for ( my $i = 0; $i < $#thresholds; $i++ ) {
    if ( ( $hcov <= $thresholds[$i] or $qcov <= $thresholds[$i] ) &&
         ( $hcov >= $thresholds[ $i + 1 ] && $qcov >= $thresholds[ $i + 1 ] ) )
    {
      print SQL "UPDATE gene SET biotype = '" .
        $thresholds[ $i + 1 ] . "-" . $thresholds[$i] . ":" . $biotype . "' WHERE gene_id = " . $gene->dbID . ";\n";
      return;
    }
  }
  print SQL "UPDATE gene SET biotype = '0-" . $thresholds[-1] . ":" . $biotype . "' WHERE gene_id = " . $gene->dbID . ";\n";
} ## end sub label_singletons
