...
my $sdb = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $host, -user => 'ensro', -port => $port, -dbname => $dbname, );

my $final_db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host   => $final_host,
                                                   -user   => $fdbuser,
                                                   -port   => $final_port,
                                                   -dbname => $final_dbname,
                                                   -pass   => $fdbpass, );
...
if ($dna_dbname) {
  my $dna_db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $dna_host, -user => 'ensro', -port => $dna_port, -dbname => $dna_dbname );
  $sdb->dnadb($dna_db);
}
if ( -e $chrlist ) {
