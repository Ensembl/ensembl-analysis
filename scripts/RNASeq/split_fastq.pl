...
unless ($update) {
  ...
  print
"Once the bsubs have finished run the script again with the -update flag to check that the chunking has worked and to update the meta file with the new fastq files\n";
  exit;
}
...
foreach my $file (@files) {
  ...
  open( my $fh, "$command 2>&1 |" ) || throw("Error counting chunks");
...
sub count_reads {
  ...
  eval {
    open( my $fh, "$command 2>&1 |" ) || throw("Error counting reads");
    while (<$fh>) {
      chomp;
      if ( $_ =~ /(\d+) \S+/ ) {
        $total_reads = $1/4;
