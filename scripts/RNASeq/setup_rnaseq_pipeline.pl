...
my $repeatf_sth = $dba->prepare( "SELECT COUNT(*) FROM repeat_feature" );
...
my $repeatc_sth = $dba->prepare( "SELECT COUNT(*) FROM repeat_consensus" );
...
my $dna_sth = $dba->prepare( "SELECT COUNT(*) FROM dna" );
$dna_sth->execute; my $dna_rows = $dna_sth->rows;
throw("Db $dbname has unpopulated repeat_feature table\n") unless $dna_rows > 0; 

my $mc_sth = $dba->prepare( "SELECT COUNT(*) FROM meta_coord" );
