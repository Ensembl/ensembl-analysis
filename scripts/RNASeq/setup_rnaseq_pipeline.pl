...
my $multiple_cpus = $RNASEQCONFIG->{MULTIPLE_CPUS};
$rgt = 'ID' unless $rgt;
my %id_groups;
my %ids_by_tissue;
my %tissue_by_id;
...
foreach my $db_config_name ($dbname, $RNASEQCONFIG->{ROUGHDB}, $RNASEQCONFIG->{REFINEDDB}, $RNASEQCONFIG->{BLASTDB}) {
  my $dba = new Bio::EnsEMBL::DBSQL::DBAdaptor( %{ $databases{$db_config_name} }, );
  throw("Db $db_config_name not found in Databases.pm\n") unless $dba;
  $database_hash{$db_config_name} = $dba;
}
...
my $pipelinea =
  new Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor(-dbconn => $dba->dbc);
...
unless ($update_analyses) {
  unless ($use_gsnap) {
    if (   !$init and (( $bwa2bam_count >= 1 && $bwa2bam_count == $submit_bwa2bam_count )
    ...
    if (   !$init and  (( $gsnap_count >= 1 && $gsnap_count == $submit_gsnap_count )
         or ( $force_stage eq "bwa_complete" )) )
    {
  ...
  unless ( $stage eq "Initialization" ) {
    ...
    if ( !$init and ((    $submit_chromosome_count > 0
...
foreach my $row (@rows) {
  ...
  my $submit_gsnap =
    new Bio::EnsEMBL::Pipeline::Analysis(
                                    ...
                                    -input_id_type => 'GSNAP_' . uc($ln), );

  my $submit_bwa2bam =
    new Bio::EnsEMBL::Pipeline::Analysis(
                                  -logic_name => "submit_" . $ln . "_bwa2bam",
                                  -input_id_type => 'BWA2BAM_' . uc($ln), );
  my $refine =
...
$is_example = 0 if ($force_stage or $use_existing);
...
$bq_cfg->root_value('DEFAULT_RETRIES', 5);
...
if ($use_gsnap) {
    ...
    $bwa_cfg->default_value('SAMPE_OPTIONS', $bwa_cfg->default_value('SAMPE_OPTIONS').' -T -t '.$multiple_cpus) if ($multiple_cpus and $bwa_cfg->default_value('SAMPE_OPTIONS') !~ /-t/);
...
$bq_cfg->add_analysis_to_batchqueue('refine_all',
  { output_dir => $output_dir .'/refine_all_pipeline',
    ...
    memory     => $RNASEQCONFIG->{MEMORY_REFINE},
    resource   => 'rusage[myens_'.$ref_load.'tok=25, myens_'.$refine_load.'tok=25]',
    retry_resource   => 'rusage[myens_'.$ref_load.'tok=25, myens_'.$refine_load.'tok=25]',
  }) unless ($bq_cfg->analysis_from_batchqueue('refine_all'));
$bq_cfg->add_analysis_to_batchqueue('bam2genes',
  { output_dir => $output_dir .'/bam2genes_pipeline',
    batch_size => $slice_batches,
    memory     => $RNASEQCONFIG->{MEMORY_BAM2GENES},
  ...
  }) unless ($bq_cfg->analysis_from_batchqueue('bam2genes'));
$bq_cfg->add_analysis_to_batchqueue('bam2genes_wait') unless ($bq_cfg->analysis_from_batchqueue('bam2genes_wait'));
$bq_cfg->add_analysis_to_batchqueue('bam2introns',
  { output_dir => $output_dir .'/bam2introns_pipeline',
    batch_size => $rough_batches,
    memory     => $RNASEQCONFIG->{MEMORY_BAM2INTRONS},
    resource   => 'rusage[myens_'.$ref_load.'tok=25]',
    retry_resource   => 'rusage[myens_'.$ref_load.'tok=25]',
  }) unless ($bq_cfg->analysis_from_batchqueue('bam2introns'));
$bq_cfg->add_analysis_to_batchqueue('bam2introns_wait') unless ($bq_cfg->analysis_from_batchqueue('bam2introns_wait'));
$bq_cfg->add_analysis_to_batchqueue('sam2bam',
  { output_dir => $output_dir .'/sam2bam_pipeline',
    memory     => $RNASEQCONFIG->{MEMORY_SAM2BAM},
    resource   => 'rusage[myens_'.$ref_load.'tok=25]',
    retry_resource   => 'rusage[myens_'.$ref_load.'tok=25]',
    batch_size => 1,
  }) unless ($bq_cfg->analysis_from_batchqueue('sam2bam'));
$bq_cfg->add_analysis_to_batchqueue('sam2bam_wait') unless ($bq_cfg->analysis_from_batchqueue('sam2bam_wait'));
$bq_cfg->add_analysis_to_batchqueue('rnaseqblast',
  { output_dir => $output_dir .'/rnaseqblast_pipeline',
    batch_size => $slice_batches,
    memory     => $RNASEQCONFIG->{MEMORY_BLASTRNASEQ},
  ...
  }) unless ($bq_cfg->analysis_from_batchqueue('rnaseqblast'));
  ...
  if ($use_gsnap) {
    ...
    $bq_cfg->add_analysis_to_batchqueue('gsnap_' . $row->{ID},
        { output_dir => $output_dir .'/'.$row->{ID}.'_pipeline',
          memory     => $RNASEQCONFIG->{MEMORY_GSNAP},
          queue      => 'long',
          resource   => 'rusage[myens_'.$ref_load.'tok=25]',
          retry_resource   => 'rusage[myens_'.$ref_load.'tok=25]',
        }) unless ($bq_cfg->analysis_from_batchqueue('gsnap_' . $row->{ID}));
  }
  else {
    my %bwa_hash = (
                      INDIR  => $input_dir,
    ...
    $bwa_hash{OPTIONS} .= ' -I' if ($row->{PL} =~ /\+/);
    $bwa_hash{OPTIONS} .= ' -t '.$multiple_cpus if ($multiple_cpus);
    ...
    my %bwa2bam_bq = (
        output_dir => $output_dir .'/'.$row->{ID}.'_pipeline',
        memory     => $RNASEQCONFIG->{MEMORY_BWA},
        resource   => 'rusage[myens_'.$ref_load.'tok=25]',
        retry_resource   => 'rusage[myens_'.$ref_load.'tok=25]',
        batch_size => 1,
    );
    my %bwa_bq = (
        output_dir => $output_dir .'/'.$row->{ID}.'_pipeline',
        memory     => $RNASEQCONFIG->{MEMORY_BWA2BAM},
        resource   => 'rusage[myens_'.$ref_load.'tok=25]',
        retry_resource   => 'rusage[myens_'.$ref_load.'tok=25]',
        batch_size => 1,
    );
    if ($multiple_cpus) {
        $bwa_bq{resource} .= ' span[hosts=1]';
        $bwa_bq{retry_resource} .= ' span[hosts=1]';
        $bwa_bq{sub_args} = "-n $multiple_cpus";
        $bwa_bq{retry_sub_args} = "-n $multiple_cpus";
        $bwa2bam_bq{resource} .= ' span[hosts=1]';
        $bwa2bam_bq{retry_resource} .= ' span[hosts=1]';
        $bwa2bam_bq{sub_args} = "-n $multiple_cpus";
        $bwa2bam_bq{retry_sub_args} = "-n $multiple_cpus";
    }
    $bq_cfg->add_analysis_to_batchqueue('bwa_' . $row->{ID},
        \%bwa_bq) unless ($bq_cfg->analysis_from_batchqueue('bwa_' . $row->{ID}));
    $bq_cfg->add_analysis_to_batchqueue('bwa_' . $row->{ID} . '_wait') unless ($bq_cfg->analysis_from_batchqueue('bwa_' . $row->{ID} . '_wait'));
    $bq_cfg->add_analysis_to_batchqueue('bwa2bam_' . $row->{ID},
        \%bwa2bam_bq) unless ($bq_cfg->analysis_from_batchqueue('bwa2bam_' . $row->{ID}));
  }
  ...
  if ($RNASEQCONFIG->{SINGLE_TISSUE} && !$seen{$tissue_by_id{$row->{ID}}}) {
      ...
      $bq_cfg->add_analysis_to_batchqueue('refine_' . $tissue_by_id{$row->{ID}},
          { output_dir => $output_dir .'/refine_'.$tissue_by_id{$row->{ID}}.'_pipeline',
            ...
            resource   => 'rusage[myens_'.$ref_load.'tok=25]',
            retry_resource   => 'rusage[myens_'.$ref_load.'tok=25]',
          }) unless ($bq_cfg->analysis_from_batchqueue('refine_' . $tissue_by_id{$row->{ID}}));
  }
