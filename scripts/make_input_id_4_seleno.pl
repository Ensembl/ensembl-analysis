...
my $db =
  new Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor( -dbname => $dbname, -user => $user, -pass => $pass, -port => $port, -host => $host );
...
while ( my ( $chr, $version, $start, $end, $strand ) = $sth->fetchrow_array ) {
  ...
  if ( $slice_chr eq $chr && $start < $slice_end && $end > $slice_start && $slice_strand == $strand ) {
    ...
    print "insert into input_id_analysis values ('toplevel:" .
      $version . ":$chr:$start:$end:1','" . $input_id_type . "'," . $analysis_id . ", now(), '', '', '0');\n";
    $slice_chr    = $chr;
    $slice_start  = $start;
    $slice_end    = $end;
    $slice_strand = $strand;
