...
for ( my $i = 0; $i < $num_patches; $i++ ) {
  ...
  if ( $align_by_component or $fix_overlap or $new_align_session or $check_repeat_masked ) {
    ...
    #run for each patch
    $command =
      "bsub -R'select[mem>4000] rusage[mem=4000]' -M4000 -o " .
      $out_dir . $patch_slice->seq_region_name . "_nonident.out -e " . $out_dir . $patch_slice->seq_region_name .
      "_nonident.err " . $perl . " " . $script . " --dbhost " . $host . " --dbport " . $port . " --dbuser " .
      $user . " --dbpass " . $pass . " --dbname " . $dbname . " --assembly " . $assembly . " --altdbname " . $dbname . " --altassembly " .
      $assembly . " --chromosomes " . $ref_slice->seq_region_name . " --altchromosomes " . $patch_slice->seq_region_name .
      " --logpath " . $out_dir . " --logfile " . $patch_slice->seq_region_name . "_nonident.log --force-stage";

    if ( ( $patch_slice->seq_region_name eq 'HG1472_PATCH' ) or
         ( $patch_slice->seq_region_name eq 'HG858_PATCH' ) or
         ( $patch_slice->seq_region_name eq 'CHR_HG1342_HG2282_PATCH' ) )
    {
      $command =
        "bsub -R'select[mem>10000] rusage[mem=10000]' -M10000 -o " .
        $out_dir . $patch_slice->seq_region_name . "_nonident.out -e " . $out_dir . $patch_slice->seq_region_name .
        "_nonident.err " . $perl . " " . $script . " --dbhost " . $host . " --dbport " . $port . " --dbuser " . $user .
        " --dbpass " . $pass . " --dbname " . $dbname . " --assembly " . $assembly . " --altdbname " . $dbname . " --altassembly " .
        $assembly . " --chromosomes " . $ref_slice->seq_region_name . " --altchromosomes " . $patch_slice->seq_region_name .
        " --logpath " . $out_dir . " --logfile " . $patch_slice->seq_region_name . "_nonident.log --force-stage";
    }
    print $command. "\n";
    my $cmd_rtn = `$command`;
    print $cmd_rtn. "\n";
  } ## end elsif ($align_non_ident)  [ if ( $align_by_component...)]
} ## end for ( my $i = 0; $i < $num_patches...)
...
if ($align_by_component) {
  ...
  $command =
    "bsub -R\"select[mem>3800] rusage[mem=3800]\" -M3800 -o " . $out_dir . "component_ident.out -e " . $out_dir .
    "component_ident.err " . $perl . " " . $script . " --host " . $host . " --port " . $port . " --user " . $user . " --pass " . $pass .
    " --dbname " . $dbname . " --altdbname " . $dbname . " --assembly " . $assembly . " --altassembly " . $assembly . " --chromosomes " .
    $ref_chromosomes . " --altchromosomes " . $patch_chromosomes . " --logpath " . $out_dir . " --logfile component_ident.log";
  ...
  $command =
    "bsub -o " . $out_dir . "new_align_session.out -e " . $out_dir . "new_align_session.err " .
    $perl . " " . $script . " --host " . $host . " --port " . $port . " --user " . $user . " --pass " . $pass . " --dbname " . $dbname .
    " --altdbname " . $dbname . " --assembly " . $assembly . " --altassembly " . $assembly . " --chromosomes " . $ref_chromosomes .
  ...
  $command =
    "bsub -o " . $out_dir . "fix_overlap.out -e " . $out_dir . "fix_overlap.err " .
    $perl . " " . $script . " --dbhost " . $host . " --dbport " . $port . " --dbuser " . $user . " --dbname " . $dbname .
    " --altdbname " . $dbname . " --dbpass " . $pass . " --assembly " . $assembly . " --altassembly " . $assembly . " --chromosomes " .
    $ref_chromosomes . " --altchromosomes " . $patch_chromosomes . " --logpath " . $out_dir . " --logfile fix_overlap.log --force-stage";
