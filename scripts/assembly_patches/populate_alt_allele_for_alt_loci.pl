...
foreach my $alt_seq_mapping (@alt_seq_mappings) {
  print $alt_seq_mapping->seq_region_name() . " " . $alt_seq_mapping->seq_region_start() . " " . $alt_seq_mapping->seq_region_end() .
    " " . $alt_seq_mapping->hseqname() . " " . $alt_seq_mapping->hstart() . " " . $alt_seq_mapping->hend() . "\n";

  # get the genes on the source region (usually hap/patch)
  my $ae_slice = $sa->fetch_by_region( 'toplevel',                           $alt_seq_mapping->seq_region_name(),
                                       $alt_seq_mapping->seq_region_start(), $alt_seq_mapping->seq_region_end(),
                                       $alt_seq_mapping->seq_region_strand() );
  ...
  my $pa_slice = $sa->fetch_by_region( 'toplevel',
                                       $alt_seq_mapping->hseqname(),
                                       $alt_seq_mapping->hstart(),
                                       $alt_seq_mapping->hend(),
                                       $alt_seq_mapping->hseq_region_strand() );
      ...
      if ( $ae_gene_key eq $pa_gene_key ) {
        ...
        if ( $pa_gene_group and $ae_gene_group ) {
          if ( $pa_gene_group->dbID() != $ae_gene_group->dbID() ) {
            throw( "FOUND alt allele relationship between gene IDs " . $pa_gene->dbID() . " and " . $ae_gene->dbID() .
" but both of them are part of different alt allele groups so the creation of a new alt allele group is not allowed because a gene is only expected to be part of ONE alt allele group."
            );
          }
          else {
            print "FOUND already existing alt_allele relationship for: " .
              $ae_gene->stable_id() . " " . $ae_gene_key . " and " . $pa_gene->stable_id() . " " . $pa_gene_key . "\n";
          }
        }
        elsif ($pa_gene_group) {
          print "FOUND new alt_allele relationship for: " . $ae_gene->stable_id() . " " . $ae_gene_key . " and " . $pa_gene->stable_id() .
            " " . $pa_gene_key . " Adding the former to the latter's alt allele group " . $pa_gene_group->dbID() . ".\n";
          $pa_gene_group->add_member( $ae_gene->dbID(), \%alt_gene_flags );
          $aaga->update($pa_gene_group);
        }
        elsif ($ae_gene_group) {
          print "FOUND new alt_allele relationship for: " . $ae_gene->stable_id() . " " . $ae_gene_key . " and " . $pa_gene->stable_id() .
            " " . $pa_gene_key . " Adding the latter to the former's alt allele group " . $ae_gene_group->dbID() . ".\n";
          $ae_gene_group->add_member( $pa_gene->dbID(), \%primary_gene_flags );
          $aaga->update($ae_gene_group);
        }
        else {
          my $new_group = Bio::EnsEMBL::AltAlleleGroup->new(
                                     -MEMBERS => [ [ $pa_gene->dbID(), \%primary_gene_flags ], [ $ae_gene->dbID(), \%alt_gene_flags ] ] );
          my $new_group_id = $aaga->store($new_group);
          print "FOUND new alt_allele relationship for: " . $ae_gene->stable_id() . " " . $ae_gene_key .
            " and " . $pa_gene->stable_id() . " " . $pa_gene_key . " Creating a new alt allele group " . $new_group_id . ".\n";
        }
      }    #if
    }    #foreach pa_gene
  }    #foreach ae_gene
} ## end foreach my $alt_seq_mapping...
print $fh "DONE\n";
close($fh);
...
sub get_gene_key {
  ...
  my $string =
    ( $gene->seq_region_start() - $region_start ) . ":" . $gene->biotype() . ":" . $gene->length() . ":" . $gene->seq_region_strand();
