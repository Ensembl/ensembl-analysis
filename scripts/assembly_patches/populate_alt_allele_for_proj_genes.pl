...
my $old_db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $old_host, -user => $old_user, -port => $old_port, -dbname => $old_dbname );
...
foreach my $alt_allele_group ( @{$alt_allele_groups} ) {
  ...
  foreach my $member ( @{ $alt_allele_group->get_all_members } ) {
    ...
    print $fh "Alt_allele_group " . $alt_allele_group->dbID .
      " includes gene with dbID " . $gene->dbID . " on slice " . $gene->slice->seq_region_name . " ($flag_list )\n";
  }
}
...
foreach my $proj_gene (@projected_genes) {
  ...
  if ( scalar(@parent_stable_ids) != 1 ) {
    ...
    warning(
          "Expected only one parent. Got the following parents for projected gene " . $proj_gene->stable_id . ":\n  @parent_stable_ids" );
  ...
  my $orig_gene;
  print $fh "Found " . ( scalar(@possible_orig_genes) ) .
    " possible genes for " . $proj_gene->stable_id . " on slice " . $parent_gene->feature_Slice->name . "\n";
  my $count_matched = 0;
  ...
  if ( get_gene_info($orig_gene) eq get_gene_info($proj_gene) ) {
    ...
    if ( exists $ref_genes_to_allele_group_id{ $orig_gene->dbID } ) {
      ...
      if ($member_found) {
        print $fh "oh no not necessary because it's already there in " .
          $alt_allele_group->dbID . " so I'm not doing to update the group\n";
      }
      elsif ( !( $alt_allele_gene_ids{ $proj_gene->dbID } ) ) {
        ...
        print $fh " with dbID " . $alt_allele_group_id .
          " as I see that gene " . $orig_gene->stable_id . " (dbID " . $orig_gene->dbID . ") on primary already stored\n";
      }
    } ## end if ( exists $ref_genes_to_allele_group_id...)
    else {
    ...
    } ## end else [ if ( exists $ref_genes_to_allele_group_id...)]
  } ## end elsif ( !( $alt_allele_gene_ids... [ if ( get_gene_info($orig_gene...))]))
} ## end foreach my $proj_gene (@projected_genes)
...
sub get_gene_info {
  my ($gene) = @_;
  my $string =
    $gene->slice->seq_region_name . ":" . $gene->seq_region_start . ":" . $gene->seq_region_end . ":" . $gene->seq_region_strand . ":";
