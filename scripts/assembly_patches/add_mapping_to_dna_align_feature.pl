...
my $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $dbhost, -user => $dbuser, -port => $dbport, -dbname => $dbname );
...
for ( my $i = 0; $i < $num_patches; $i++ ) {
  ...
  if ($patch_cmp_in_mapping) {
    $get_mapping_sth = $db->dbc->prepare(
"select cmp_seq_region_id, cmp_start, cmp_end, asm_seq_region_id, asm_start, asm_end, ori from assembly, seq_region, coord_system where cmp_seq_region_id = ? and asm_seq_region_id = seq_region_id and seq_region.coord_system_id=coord_system.coord_system_id and coord_system.name='chromosome'"
      ) ||
      die "Couldn't get mapping";
  }
  elsif ($ref_cmp_in_mapping) {
    $get_mapping_sth = $db->dbc->prepare(
"select asm_seq_region_id, asm_start, asm_end, cmp_seq_region_id, cmp_start, cmp_end, ori from assembly, seq_region, coord_system where asm_seq_region_id = ? and cmp_seq_region_id = seq_region_id and seq_region.coord_system_id=coord_system.coord_system_id and coord_system.name='chromosome'"
      ) ||
      die "Couldn't get mapping";
  }
  #get mapping
...
MAPPING: while ( $get_mapping_sth->fetch() ) {
    ...
    print SQL
"insert into dna_align_feature (analysis_id, seq_region_id, seq_region_start, seq_region_end, seq_region_strand, hit_start, hit_end, hit_strand, hit_name, cigar_line,external_db_id)
values(" . $mapping_analysis_id . ", " . $patch_chrom_id . ", " . $start . ", " .
      $end . ", 1, " . $pri_start . ", " . $pri_end . ", " . $ori . ", '" . $hit_name . "', '" . $length . "M', $external_db_id);\n";

  } ## end MAPPING: while ( $get_mapping_sth->...)
