...
my $mapdb = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $host, -user => $user, -pass => $pass, -port => $port, -dbname => $dbname );
  ...
  foreach my $ref_gene (@ref_genes) {
    ...
    if ($alt_gene) {
      ...
      foreach my $transcript ( @{ $alt_gene->get_all_Transcripts } ) {
        print "Looking at transcript " .
          $transcript->stable_id . " dbID: " . $transcript->dbID . " region: " . $transcript->seq_region_name . "\n";
        ...
        my $parent_stable_id = $transcript->stable_id
          ; # at this stage just after the projection, the stable IDs are duplicated so the transcript stable ID is the same as the parent transcript stable ID
        my $parent_exon_key = "";
        ...
        my $parent_sid_att = Bio::EnsEMBL::Attribute->new(
          ...
          -NAME => "parent_sid",
          -DESCRIPTION =>
"The parent stable ID to identify a projected transcript's parent transcript. For internal statistics use only since this method does not work in all cases.",
  ...
  } ## end foreach my $ref_gene (@ref_genes)
