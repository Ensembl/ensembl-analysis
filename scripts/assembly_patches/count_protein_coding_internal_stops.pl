...
my $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $host, -user => $dbuser, -port => $port, -dbname => $dbname, -pass => $dbpass, );

# The DNA is in the ref db
my $dnadb =
  Bio::EnsEMBL::DBSQL::DBAdaptor->new( -dbname => $dnadbname, -host => $dnadbhost, -user => $dbuser, -pass => $dbpass, -port => $port );
