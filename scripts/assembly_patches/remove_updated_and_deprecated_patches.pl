...
my $get_all_synonyms_sth =
  $dba->dbc->prepare( 'SELECT synonym from seq_region, seq_region_synonym, coord_system, external_db WHERE seq_region.name IN ' .
'(SELECT seq_region.name from seq_region, seq_region_attrib, attrib_type WHERE attrib_type.code IN ("patch_fix","patch_novel") AND attrib_type.attrib_type_id = seq_region_attrib.attrib_type_id AND seq_region_attrib.seq_region_id = seq_region.seq_region_id)'
  . ' AND seq_region.coord_system_id=coord_system.coord_system_id AND coord_system.name = "'
  . $central_coord_system .
'" AND seq_region.seq_region_id=seq_region_synonym.seq_region_id AND seq_region_synonym.external_db_id = external_db.external_db_id AND external_db.db_name = "INSDC"'
  ) ||
...
my $get_name_sth = $dba->dbc->prepare(
    "select name from seq_region,seq_region_synonym where synonym = ? and seq_region_synonym.seq_region_id = seq_region.seq_region_id") ||
  die "Could not prepare to get name";

if ( $get_name_sth == 0 ) {
  throw("Could not prepare to get name");
}
my $get_synonym_sth = $dba->dbc->prepare(
"select synonym from seq_region_synonym, seq_region, coord_system where seq_region. name = ? and seq_region.coord_system_id=coord_system.coord_system_id and coord_system.name = ? and seq_region.seq_region_id=seq_region_synonym.seq_region_id"
  ) ||
...
my $get_seq_region_id_sth = $dba->dbc->prepare(
"select seq_region_id from seq_region, coord_system where seq_region. name = ? and seq_region.coord_system_id=coord_system.coord_system_id and
coord_system.name = ?" ) ||
...
sub remove_patch {
  ...
  my $scaf_comp;
  my $get_region_components_sth = $dba->dbc->prepare("select distinct cmp_seq_region_id from assembly where asm_seq_region_id = ?") ||
    die "Couldn't get components";
...
COMP: while ( $get_region_components_sth->fetch() ) {
    ...
    my $asm_id;
    my $get_asm_ids_sth = $dba->dbc->prepare("select distinct asm_seq_region_id from assembly where cmp_seq_region_id = ?") ||
      die "Couldn't get asm ids";
