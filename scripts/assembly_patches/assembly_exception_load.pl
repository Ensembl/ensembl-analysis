...
my $dba =
  new Bio::EnsEMBL::DBSQL::DBAdaptor( '-host' => $host, '-user' => $user, '-pass' => $pass, '-dbname' => $dbname, '-species' => "load" );

my $sth = $dba->dbc->prepare("select max(seq_region_id) from seq_region") || die "Could not get max seq_region_id";
...
if ( defined $count_assembly_exception_id && $count_assembly_exception_id > 0 ) {
  $sth = $dba->dbc->prepare("select max(assembly_exception_id) from assembly_exception") || die "Could not get max assembly_exception_id";
  ...
  warning(
     "The assembly_exception table is empty. This is OK if it's the first ever patch release for a species, otherwise there is a problem."
  );
...
my $get_seq_region_id_sth =
  $dba->dbc->prepare("select seq_region_id, length from seq_region where coord_system_id = ? and name like ?") ||
  die "Could not prepare get_seq_region_id_sth";

my $get_seq_id_sth = $dba->dbc->prepare(
"select sr.seq_region_id from seq_region sr , coord_system cs where sr.name like ? and cs.coord_system_id = sr.coord_system_id and cs.attrib like '%sequence%'"
);
...
SCAF: while (<TXT>) {
  if (/^#/) {
    ...
    foreach my $name (
      qw(alt_scaf_name alt_scaf_acc parent_type parent_name ori alt_scaf_start alt_scaf_stop parent_start parent_stop alt_start_tail alt_stop_tail)
      )
    {
    ...
    my ( $tmp_id, $tmp_len );
    $get_seq_region_sth->execute($alt_name) || die "Could not execute get seq_region for $alt_name";
    $get_seq_region_sth->bind_columns( \$tmp_id, \$tmp_len ) || die "Could not bind get_seq_region";
    ...
    if ( !defined($coord_sys) ) {
      die "Could not get coord_system_id for " .
        $arr[ $key_to_index{'parent_type'} ] . " col index " . $key_to_index{'parent_type'} . "\n";
    }
    ...
    my $new_length =
      ...
      $arr[ $key_to_index{'alt_scaf_start'} ] + 1 +
      $arr[ $key_to_index{alt_start_tail} ] +
      $arr[ $key_to_index{alt_stop_tail} ];
    ...
    if ( exists $name_to_type{$alt_name} && defined $name_to_type{$alt_name} ) {
      if ( $name_to_type{$alt_name} =~ /fix/i ) {
        print SQL
          "insert into seq_region_attrib (seq_region_id, attrib_type_id, value) values ($max_seq_region_id, $patch_fix, NOW());\n";
        $hap_type = "'PATCH_FIX'";
      }
      elsif ( $name_to_type{$alt_name} =~ /novel/i ) {
        print SQL
          "insert into seq_region_attrib (seq_region_id, attrib_type_id, value) values ($max_seq_region_id, $patch_novel, NOW());\n";
...
# note there is no need to insert anything into the seq_region_attrib because the only attrib for haps has already been inserted above (non_ref)
    ...
    print SQL
"insert into assembly_exception (assembly_exception_id, seq_region_id, seq_region_start, seq_region_end, exc_type, exc_seq_region_id, exc_seq_region_start, exc_seq_region_end, ori)\n";
    print SQL "\tvalues($max_assembly_exception_id, $max_seq_region_id, ", $arr[ $key_to_index{'parent_start'} ], ",",
      ( $new_length + $arr[ $key_to_index{'parent_start'} ] - 1 ), ",", "$hap_type, ", $seq_region_id, ", ",
      $arr[ $key_to_index{'parent_start'} ], ", ", $arr[ $key_to_index{'parent_stop'} ], ",", " 1);\n\n";
    ...
    print SQL "               values(" . $name_to_seq_id{$alt_name} .
      ", " . $name_to_sc_seq_id{$alt_name} . ", " . $asm_start . ", " . $asm_end . ", $cmp_start, $cmp_end, $strand);\n\n";
    ...
    if ( defined( $name_to_seq_id{$contig} ) ) {
      ...
      if ( !defined($cmp_seq_id) ) {
        ...
        if ( $out =~ /no match/ ) {
          ...
          $out = system(
"wget -o $contig.wget.log -O $contig.fa 'http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&id=$contig&rettype=fasta&retmode=text' > ./$contig.fa"
          );
        }
        else {
        ...
        $cmp_seq_id = $name_to_seq_id{$contig};
      } ## end if ( !defined($cmp_seq_id...))
      else {
        print "$contig already stored :-)\n";
      }
    } ## end else [ if ( defined( $name_to_seq_id...))]
    ...
    print SQL "insert into assembly (asm_seq_region_id, cmp_seq_region_id, asm_start, asm_end, cmp_start , cmp_end, ori) \n";
    print SQL "               values(" .
      $sc_seq_id . ", " . $cmp_seq_id . ", " . $p_start . ", " . $p_end . ", $c_start, $c_end, $strand);\n\n";
    ...
    print SQL "insert into assembly (asm_seq_region_id, cmp_seq_region_id, asm_start, asm_end, cmp_start , cmp_end, ori) \n";
    print SQL "               values(" .
      $seq_id . ", " . $cmp_seq_id . ", " . $asm_start . ", " . $asm_end . ", $cmp_start, $cmp_end, $strand);\n\n";
  } ## end if ( $type =~ /^[ADFGOPW]$/)
  else {
    print "GAP of $contig length\n";
  }
} ## end MAP: while (<MAPPER>)
...
sub load_seq_region {
  ...
  my ( $tmp_id, $tmp_len );
  $get_seq_region_sth->execute($name) || die "Could not execute get seq_region for $name";
  $get_seq_region_sth->bind_columns( \$tmp_id, \$tmp_len ) || die "Could not bind get_seq_region";
  $get_seq_region_sth->fetch();
  #    || die "Could not fetch seq_region id for name $name";;
  ...
  print SQL
    "insert into seq_region (seq_region_id, name, coord_system_id, length) values ($max_seq_region_id, '$name', $contig_coord_id, " .
    length($$seq) . ");\n";
