...
my $mapdb = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $host, -user => $user, -port => $port, -dbname => $dbname );
...
foreach my $patch_exc (@patches) {
  ...
  foreach my $band (@ref_bands) {
    ...
    print SQL "insert into karyotype(seq_region_id, seq_region_start, seq_region_end, band, stain) values(" .
      $patch_slice->get_seq_region_id . ", " . $patch_start . ", " . $patch_end . ", '" . $band->name . "', '" . $band->stain . "');\n";
  } ## end foreach my $band (@ref_bands)
} ## end foreach my $patch_exc (@patches)
