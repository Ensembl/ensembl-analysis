...
my $ftp = Net::FTP->new( 'ftp.ncbi.nlm.nih.gov', Debug => 0 ) or die "Can't connect to NCBI FTP: $@";
...
foreach my $patches_ftpdir (@patches_ftpdirs) {
  ...
  open( $remote_file_handle, '>', \$content );
  $ftp->get( 'alt_scaffold_placement.txt', $remote_file_handle ) or die "get failed ", $ftp->message;
  ...
  foreach my $patch ( keys %ftp_filename ) {
    close $remote_file_handle;
    open( $remote_file_handle, '>', \$content );
    $ftp->get( $ftp_filename{$patch}, $remote_file_handle ) or die "get failed ", $ftp->message;
...
my $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $dbhost, -user => $dbuser, -pass => $dbpass, -port => $dbport, -dbname => $dbname );

my $sa = $db->get_SliceAdaptor();

my $analysis = new Bio::EnsEMBL::Analysis( -logic_name => "grc_alignment_import", -db_version => $patch_release );
...
for ( my $i = 0; $i < $num_patches; $i++ ) {
  ...
  foreach my $string ( @{ $align_str{$patch_name} } ) {
    ...
    my $daf = new Bio::EnsEMBL::DnaDnaAlignFeature( -slice          => $slice,
                                                    -start          => $start,
                                                    -end            => $end,
                                                    -strand         => $strand,
                                                    -analysis       => $analysis,
  ...
  } ## end foreach my $string ( @{ $align_str...})
    ...
    if ($store) {
      ...
      if ($@) {
        ...
        print "Written " . $dna_align_feat->hseqname . " on chr " . $dna_align_feat->slice->name .
          " strand " . $dna_align_feat->hstrand . " with start " . $dna_align_feat->start . " end " . $dna_align_feat->end . "\n"
          if $verbose;
      }
    }
    else {
