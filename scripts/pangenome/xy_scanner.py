...
def run_minimap(genome_index, marker_file, output_dir):
    ...
    subprocess.run(
        ["minimap2", "--secondary=no", genome_index, marker_file, "-o", temp_path]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--genome_index", help="Path to the genome index file (mmi)", required=True
    )
    parser.add_argument(
        "--x_marker_file", help="Number of threads to use", required=True
    )
    parser.add_argument(
        "--y_marker_file", help="Number of threads to use", required=True
    )
    parser.add_argument(
        "--output_dir",
        help="Path to output dir to write minimap output to",
        required=False,
    )
    parser.add_argument(
        "--output_file_name",
        help="File name for the result, will be written to output dir",
        required=False,
    )
