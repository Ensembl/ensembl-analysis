...
# # #
my $production_db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host   => $production_host,
                                                        -user   => $production_user,
                                                        -port   => $production_port,
                                                        -dbname => $production_dbname );

my $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $host, -user => $user, -port => $port, -pass => $pass, -dbname => $dbname );
if ($dnadbname) {
...
my $known_biotypes = { 'protein_coding'                     => 'coding',
...
sub get_biotype_groups {
  ...
  my $sql =
"select distinct name, biotype_group from biotype where is_current = 1 and object_type in ('gene','transcript') and db_type like '%core%' order by biotype_group,name;";
...
sub giveMeBasicAnnotationTranscripts
{    # as a parameter I give a gene object and it returns a reference of a list with the gene's basic annotation transcript objects.
  ...
  {    # if I dont find any (nor coding neither non-coding) I consider these biotypes which are in the UCSC the problem category!
    ...
    elsif ( scalar @ncProblemlengths > 1 )
    { #if we ended up having more than one transcript with same biggest length,I iterate through the list to get only 1,the one with the biggest genomic length
      $maxGenLength = 0;
  ...
  } ## end if ( scalar @basicAnnotationTranscripts...)
  if ( scalar(@basicAnnotationTranscripts) == 0 ) {
...
{    # i call it with a gene object and it returns a reference of a list with the basic coding transcript objects of the gene
  ...
  foreach my $transcript (@transcripts) {    # pushes the protein-coding full length transcripts in the basicAnnotationTranscripts array
    ...
    if ( $known_biotypes->{ $transcript->biotype } eq 'coding' ) {
      ...
      foreach my $attribute (@attributes) {
        if ( ( $attribute->code eq "cds_end_NF" and $attribute->value == 1 ) or
             ( $attribute->code eq "cds_start_NF" and $attribute->value == 1 ) or
             ( $attribute->code eq "mRNA_end_NF"  and $attribute->value == 1 ) or
             ( $attribute->code eq "mRNA_start_NF" and $attribute->value == 1 ) )
        {                                    # all these attributes signify non full-length transcripts
  ...
  elsif ( scalar(@basicCodingAnnotation) == 0 )
  {      # if NOT--->it pushes the protein_coding (from the broader pc category) longest CDS partial length transcript
    ...
    if ( scalar(@cdsLongest) > 0 ) {    # if we have more than 1 transcript with same biggest CDS we have to do some further filtering..
      ...
      { # if we have more than one with the biggest length(exons+introns) we get the one with the longest genomic length (if there is more than one transcript with the longest genomic length we get the first one that we find!)
    ...
    } ## end if ( scalar(@cdsLongest...))
    elsif ( scalar(@cdsLongest) == 1 ) {

      push( @basicCodingAnnotation, $cdsLongest[0] );
    }

  } ## end elsif ( scalar(@basicCodingAnnotation... [ if ( scalar(@basicCodingAnnotation...))]))

  return \@basicCodingAnnotation;

}    # end of method

sub returnBasicNonCodingAnnotation
{    # i call it with a gene object and it returns a reference of a list with the basic non-coding transcript objects of the gene
  ...
  foreach my $transcript (@transcripts) {    # pushes non-coding , well caracterized, FULL length transcripts
    ...
    if ( $known_biotypes->{ $transcript->biotype } eq 'noncoding' ) {
      ...
      my @attributes = @{ $transcript->get_all_Attributes };
      foreach my $attribute (@attributes) {
        if ( ( $attribute->code eq "mRNA_end_NF" and $attribute->value == 1 ) or
             ( $attribute->code eq "mRNA_start_NF" and $attribute->value == 1 ) )
        {
  ...
  if ( scalar(@basicNonCodingAnnotation) == 0 )
  {    # if there are no nc-well caracterized-FULL LENGTH transcripts, I get the transcripts that are non-coding biggest length

    foreach my $transcript (@transcripts) {

      if ( $known_biotypes->{ $transcript->biotype } eq 'noncoding' or
           $known_biotypes->{ $transcript->biotype } eq 'noncoding_second_choice' )
      {
  ...
  } ## end if ( scalar(@basicNonCodingAnnotation...))
################################

  return
    \@basicNonCodingAnnotation
    ; # the array may have 0, or 1 transcript! 0: if there is not such transcript with nc biotype, 1: the longest(exons) tr or the genomic longest (introns+exons) tr

}    # end of method!

sub returnBasicPseudogeneAnnotation
{    # i call it with a gene object and it returns a reference of a list with the basic coding transcript objects of the gene
  ...
  if ( $transcribed_pseudogene == 1 ) {
    @basicPseudogeneAnnotation = @{
      filter_transcribed_pseudogene( $gene->get_all_Transcripts,
                                     [                               'transcribed_processed_pseudogene',
                                                                     'transcribed_unprocessed_pseudogene',
                                                                     'transcribed_unitary_pseudogene' ] ) };
  }

  if ( scalar(@basicPseudogeneAnnotation) > 1 && $transcribed_pseudogene != 1 )
  {    # if the transcripts are non-coding-well caracterized-full length I am finished!
    ...
    warning( "Pseudogene " . $gene->stable_id . " should have exactly one transcript and not " .
             ( scalar(@basicPseudogeneAnnotation) ) . " transcripts" );
  }

  return \@basicPseudogeneAnnotation;

} ## end sub returnBasicPseudogeneAnnotation

sub getGivenCoverUntilExonsCovered
{ # I call it with a set of transcripts and it returns the longest transcripts covering the maximum number of exons until exon_coverage% of all exons (based on exonic length) are covered
  my ( $gene, $exon_coverage, @basicTranscripts ) = @_;

  my @uncoveredExons = @{ $gene->get_all_Exons() };
  my @sortedBasicTranscripts =
    sort { getScoreExonsCoverAndLength( $b, @uncoveredExons ) cmp getScoreExonsCoverAndLength( $a, @uncoveredExons ) } @basicTranscripts;
  ...
  while ( scalar(@sortedBasicTranscripts) >= 1 ) {
    ...
    @sortedBasicTranscripts =
      sort { getScoreExonsCoverAndLength( $b, @uncoveredExons ) cmp getScoreExonsCoverAndLength( $a, @uncoveredExons ) }
      @sortedBasicTranscripts;
  }
...
UNIQUE: foreach my $geneExon (@uniqueExons) {
    ...
    foreach my $transcriptExon (@allTranscriptExons) {
      if ( $transcriptExon->start() == $geneExon->start() and $transcriptExon->end() == $geneExon->end() ) {
        $found = 1;
        last;
      }
    }
...
UNIQUE: foreach my $exon (@exons) {
    foreach my $transcriptExon ( @{ $transcript->get_all_Exons() } ) {
      if ( $transcriptExon->start() == $exon->start() and $transcriptExon->end() == $exon->end() ) {
        $numExonsCovered++;
        next UNIQUE;
      }
    }
  ...
  my $length_score_percentage =
    min( ( $MAX_TRANSCRIPT_LENGTH - 1 )/$MAX_TRANSCRIPT_LENGTH, $transcript->length()/$MAX_TRANSCRIPT_LENGTH );
...
UNIQUE: foreach my $geneExon (@uniqueExons) {
    foreach my $transcriptExon (@allTranscriptExons) {
      ...
      if ( $transcriptExon->start() <= $geneExon->end() and $transcriptExon->end() >= $geneExon->start() ) {
      ...
      if ( ( $max_coverage == $geneExon->length() ) or ( $transcriptExon == $allTranscriptExons[-1] ) ) {
...
} ## end sub transcriptExonsCoverGenePercentage
...
sub filter_transcribed_pseudogene {
  ...
  return \@preferred;
} ## end sub filter_transcribed_pseudogene
...
} ## end sub sort_by_biotype_exons_and_length
