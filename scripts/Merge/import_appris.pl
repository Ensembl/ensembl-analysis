  ...
  foreach my $gene ( @{ $slice->get_all_Genes } ) {
    ...
    foreach my $transcript ( @{ $gene->get_all_Transcripts } ) {
      ...
      if ( exists $appris_results{ $gene->stable_id }{ $transcript->stable_id } ) {
        ...
        $stable_id_in_file++;
        if ($write) {
          store_attrib( $aa, $transcript,
                        $appris_results{ $gene->stable_id }{ $transcript->stable_id }{'attrib_type_code'},
                        $appris_results{ $gene->stable_id }{ $transcript->stable_id }{'attrib_value'} );
          print STDERR "  writing gene " . $gene->stable_id . " transcript" . $transcript->stable_id .
            " APPRIS " . $appris_results{ $gene->stable_id }{ $transcript->stable_id }{'attrib_type_code'} . "\n";
        }
      }
      else {
