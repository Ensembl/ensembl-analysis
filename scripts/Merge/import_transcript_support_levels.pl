      ...
      if ( exists $support_levels{ $transcript->stable_id } ) {
        ...
        if ( $transcript->version == $support_levels{ $transcript->stable_id }{'version'} ) {
          ...
          if ($write) {
            ...
            print STDERR "  writing " . $transcript->stable_id .
              " version " . $transcript->version . " TSL " . $support_levels{ $transcript->stable_id }{'support_level'} . "\n";
          }
        }
        else {
          ...
          $transc_updated++;
          $stable_id_in_file++;
          warning( "Transcript annotation mismatch " . $transcript->stable_id . " version in Ensembl=" .
                   $transcript->version . " vs version in file=" . $support_levels{ $transcript->stable_id }{'version'} );
          if ($write) {
            store_attrib( $aa,
                          $transcript,
                          $code,
                          $support_levels{ $transcript->stable_id }{'support_level'} .
                            " (assigned to previous version " . $support_levels{ $transcript->stable_id }{'version'} . ")" );
            print STDERR "  writing " . $transcript->stable_id .
              " version " . $transcript->version . " TSL " . $support_levels{ $transcript->stable_id }{'support_level'} . "\n";
          }
        }
...
sub store_attrib {
  ...
  my $attrib = Bio::EnsEMBL::Attribute->new( -NAME => $name, -CODE => $code, -VALUE => $value, -DESCRIPTION => $description );
