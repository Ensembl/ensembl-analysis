...
my $compdb = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $comphost, -user => $compuser, -port => $compport, -dbname => $compdbname );
...
if ($outdbname) {
  $outdb = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $outhost, -user => $outuser, -port => $outport, -pass => $outpass,
                                               -dbname => $outdbname, );
...
foreach my $chr ( @{ sort_chr_names($chrhash) } ) {
  ...
  print FP "GENE INFO: Chr $chr " .
    scalar(@genes) . " genes " . $ntrans . " trans " .
  ...
  foreach my $uncl (@$unclustered) {
    ...
    foreach my $gene ( @{ $uncl->get_Genes } ) {
      ...
      print FP " " . get_gene_id($gene) . " " .
        ...
        scalar( @{ $gene->get_all_Exons } ) . " " . $max_exons_in_trans . " " . $n_trans_with_stop .
        " " . $n_trans_with_fshift . " " . $total_stops_in_gene . " " . $gene->biotype . " " . $gene->start . " " . $gene->end;
    } ## end foreach my $gene ( @{ $uncl...})
    ...
    print FP "\n";
  } ## end foreach my $uncl (@$unclustered)
  ...
  printf FP "CLUSTER INFO: Chr %10.10s %4d clusters %4d unclustered %4d $set1_name" .
    "_only_uncl %4d $set2_name" . "_only_uncl %4d $set1_name" . "_redund_cds %4d $set2_name" .
    "_redund_cds %4d twoways\n", $chr, scalar(@$clusters), scalar(@$unclustered), $n_chr_set1_only_unclustered,
    $n_chr_set2_only_unclustered, $n_set1_redund, $n_set2_redund, scalar(@twoways);
...
# print "Comparing " . get_exon_id($exon) . " "  . $exon->start . " " . $exon->end . " with " . get_exon_id($comp_exon) .  "  " .$comp_exon->start . "  " . $comp_exon->end . "\n";
              ...
              {
                # print "Failed \n";
                print "Comparing " . get_exon_id($exon) . " " . $exon->start .
                  " " . $exon->end . " with " . get_exon_id($comp_exon) . "  " . $comp_exon->start . "  " . $comp_exon->end . "\n";
                next COMPTRANS;
              }
              if ($usephase) {
                if ( ( $exon->phase != $comp_exon->phase ) && !$first_e ) {
                  print FP "Phase diff (e_count = $e_count) for " .
                    get_exon_id($exon) . " (" . $exon->phase . ") and exon " . get_exon_id($comp_exon) . "(" . $comp_exon->phase . ")\n";
                  next COMPTRANS;
                }
                if ( ( $exon->end_phase != $comp_exon->end_phase ) && !$first_e && !$last_e ) {
                  print FP "End phase diff for " . get_exon_id($exon) .
                    " (" . $exon->end_phase . ") and exon " . get_exon_id($comp_exon) . "(" . $comp_exon->end_phase . ")\n";
                  next COMPTRANS;
                }
              }
              $e_count++;
            ...
            my $has_noncons_splice =
              ( Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptUtils::are_splice_sites_canonical( $trans, $translate ) == 1 ) ?
              0 :
              1;
            ...
            printf FP "MATCH:\t%s\t%-40.40s\t%-40.40s\t%-40.40s\t%-40.40s\t%1d\t%1d\t%1d\t%1d\t%3d\t%3d\t%3d", $chr, get_gene_id($gene),
              get_transcript_id($trans), get_gene_id($comp_gene), get_transcript_id($comp_trans), $is_complete, $has_stops,
              $has_frameshifts, $has_noncons_splice, scalar( @{ $trans->get_all_Exons } ), scalar( @{ $comp_trans->get_all_Exons } ),
            ...
            $n_chr_match_trans++;
            $cluster_match = 1;
          } ## end COMPTRANS: foreach my $comp_trans ( @{...})
        } ## end TRANS: foreach my $trans ( @{ $gene...})
      } ## end foreach my $comp_gene (@comp_genes)
      foreach my $trans ( @{ $gene->get_all_Transcripts } ) {
        if ( !$hadmatch{$trans} ) {
          my $is_complete     = is_complete_cds($trans);
          my $has_stops       = contains_stops($trans);
          ...
          print FP "UNMATCHED TRANSCRIPT: ", get_transcript_id($trans), " ", $trans->slice->seq_region_name, " ", $trans->start, " ",
            $trans->end, " ", $is_complete, " ", $has_stops, " ", $has_frameshifts, " ", $has_noncons_splice, " ", $is_ccds, " ",
            get_gene_id($gene), "\n";
        }
      }
    ...
    print FP " $start $end";
    print FP " $in_ccds " .
      scalar(@genes) . " $nclusttrans $nclustcodingtrans $nclustfulllentrans " .
      scalar(@comp_genes) . " $nclustcomptrans $nclustcompcodingtrans $nclustcompfulllentrans";
    ...
    print FP "\n";
  } ## end CLUSTER: foreach my $cluster (@twoways)

  printf FP
"SUMMARY: Chr %10.10s %4d genes %4d trans %4d comp_genes %4d comp_trans %4d clusters %4d unclustered %4d twoways %4d matched %4d good matches %4d matched_clusters\n",
    $chr, scalar(@genes), $ntrans, scalar(@comp_genes), $ncomptrans, scalar(@$clusters), scalar(@$unclustered), scalar(@twoways),
    $n_chr_match_trans, $n_chr_complete_translating_match_trans, $n_chr_match_cluster;
  if ($outdb) {
    my $out_slice;
    if ( $schema == 20 ) {
      $slicename = "$coordsystem:$path:$chr:$chrstart:$chrend:1";
...
} ## end foreach my $chr ( @{ sort_chr_names...})
...
sub remove_redundant_cds_from_gene {
  ...
  COMPTRANS:
    for ( my $j = $i + 1; $j < scalar(@trans_array); $j++ ) {
      ...
      foreach my $exon (@exons) {
        my $comp_exon = shift @comp_exons;
        if ( $exon->start != $comp_exon->start || $exon->end != $comp_exon->end || $exon->strand != $comp_exon->strand ) {
          next COMPTRANS;
        }
      }
      $trans_array[$j] = undef;
...
} ## end sub remove_redundant_cds_from_gene
...
sub remove_redundant_cds_from_cluster {
  ...
  TRANS:
    ...
    } ## end TRANS: for ( my $i = 0; $i < scalar...)
    push @unique_trans_array, @{ $gene->get_all_Transcripts };
  } ## end foreach my $gene (@genes)

  return $n_removed;
} ## end sub remove_redundant_cds_from_cluster
...
} ## end sub is_significant_overlap
...
sub check_for_gene_duplication {
  ...
  print "Number of exon clusters = " . scalar(@exonclusters) . " max number of exons in transcript = " . $max_num_exons_transcript . "\n";
...
sub find_Longest_CDS_of_Gene {
  ...
  if ( !defined($longest) ) {
    print "Didn't find a longest transcript for gene " . get_gene_id($gene) . " type " . $gene->biotype . "\n";
  }
  else {
    print " longest transcript is " . get_transcript_id($longest) . "\n";
  }
  return ( $longest, $maxlen, $nexonmax );
} ## end sub find_Longest_CDS_of_Gene
...
sub cluster_Genes {
  ...
  } ## end foreach my $gene (@sorted_genes)
