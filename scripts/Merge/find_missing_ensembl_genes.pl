...
# genome bits
my @default_ensembl_genetypes =
  ( 'IG_C_gene', 'IG_D_gene', 'IG_J_gene', 'IG_LV_gene', 'Mt_rRNA', 'Mt_tRNA', 'processed_pseudogene', 'protein_coding', 'pseudogene' );
my @genetypes = ();
my @default_merged_genetypes = ( '3prime_overlapping_ncrna', 'antisense',         'IG_C_gene',              'IG_D_gene',
                                 'IG_J_gene',                'IG_LV_gene',        'IG_V_gene',              'IG_V_pseudogene',
                                 'lincRNA',                  'miRNA',             'misc_RNA',               'Mt_rRNA',
                                 'Mt_tRNA',                  'non_coding',        'polymorphic_pseudogene', 'processed_pseudogene',
...
print STDERR
"Ensembl database: name $ensembldbname host $ensemblhost port $ensemblport, \nMerged database: name $mergeddbname host $mergedhost port $mergedport\n";
...
my $ensembldb =
  new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $ensemblhost, -user => $ensembluser, -port => $ensemblport, -dbname => $ensembldbname );
my $ensemblsa = $ensembldb->get_SliceAdaptor();

my $mergeddb =
  new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $mergedhost, -user => $mergeduser, -port => $mergedport, -dbname => $mergeddbname );
...
sub get_genomic_location {
  ...
  my $genomic_location = $genomic_fields[0] .
    ":" . $genomic_fields[1] . ":" . $genomic_fields[2] . ":" . $cluster->start . ":" . $cluster->end . ":" . $genomic_fields[5];
