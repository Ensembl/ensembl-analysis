...
my $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -dbname => $dbname, -host => $dbhost, -user => $user, -pass => $pass, -port => $port );
...
if ($dnadbname) {
  ...
  my $dnadb = Bio::EnsEMBL::DBSQL::DBAdaptor->new( -dbname => $dnadbname,
                                                   -host   => $dnadbhost,
                                                   -user   => $user,
                                                   -pass   => $pass,
                                                   -port   => $dnadbport );
...
$gene_prefixes_str = join( "|", @GENE_PREFIXES ) if ($affix);
my $gene_suffixes_str = "";
$gene_suffixes_str = join( "|", @GENE_SUFFIXES ) if ($affix);
my $transcript_prefixes_str = "";
$transcript_prefixes_str = join( "|", @TRANSCRIPT_PREFIXES ) if ($affix);
my $transcript_suffixes_str = "";
$transcript_suffixes_str = join( "|", @TRANSCRIPT_SUFFIXES ) if ($affix);
...
foreach my $slice (@slices) {
  ...
  foreach my $gene (@genes) {
    ...
    } ## end if ( ( exists( $gene_trans_type_combination...)))
