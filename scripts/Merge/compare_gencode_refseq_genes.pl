...
my $refseqdb =
  new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $refseqhost, -user => $refsequser, -port => $refseqport, -dbname => $refseqdbname, );
...
my $sql =
"select biotype_group,name from biotype where is_current=1 and object_type='gene' and db_type like '%core%' order by biotype_group,name";
...
$sql =
"select biotype_group,name from biotype where is_current=1 and object_type='gene' and db_type like '%otherfeatures%' order by biotype_group,name";
...
foreach my $ensembl_gene (@ensembl_genes) {
  ...
  print STDERR "Got " .
    scalar(@$refseq_genes) . " refseq genes on " . $refseq_slice->name . " (Ensembl gene " . $ensembl_gene->stable_id . ")\n";
  ...
  if ( scalar(@$refseq_genes) < 1 ) {
    ...
    foreach my $id ( keys %$matches ) {
      if ( $ensembl_biotype_groups{ $ensembl_gene->biotype } eq $refseq_biotype_groups{ $matches->{$id}->biotype } ) {
        ...
        $attribute = make_attrib( "Overlapping RefSeq Gene ID " .
                                  $matches->{$id}->stable_id . " matches and has similar biotype of " . $matches->{$id}->biotype );
      }
      else {
        print STDERR "  nonmatched biotype ensembl " . $ensembl_gene->biotype . " (" . $ensembl_biotype_groups{ $ensembl_gene->biotype } .
          ") vs refseq " . $matches->{$id}->biotype . " (" . $refseq_biotype_groups{ $matches->{$id}->biotype } . ")\n";
        $attribute = make_attrib(
           "Overlapping RefSeq Gene ID " . $matches->{$id}->stable_id . " matches but different biotype of " . $matches->{$id}->biotype );
      }
    }
  }
...
} ## end foreach my $ensembl_gene (@ensembl_genes)
...
sub make_attrib {
  ...
  my $attribute =
    Bio::EnsEMBL::Attribute->new( -CODE => 'refseq_compare', -NAME => 'refseq_compare', -DESCRIPTION => '', -VALUE => $comment );
  return $attribute;
}
...
REFSEQ: foreach my $refseq_gene ( @{$refseq_genes} ) {
    ...
    if ( $ensembl_gene->strand != $refseq_gene->strand ) {
      print STDERR "    strand_mismatch " .
        $ensembl_gene->stable_id . " " . $ensembl_gene->strand . " vs " . $refseq_gene->stable_id . " " . $refseq_gene->strand . "\n";
      $match_strand = 0;
      next REFSEQ;
    }
    else {
...
# BUT note that havana will often annotate one nice long transcript and many short ones so the median is also not always good to use
    ...
    if ( $ensembl_genomic_span_length*$factor < $refseq_genomic_span_length ) {
      print STDERR "    refseq_median_length_longer " . $ensembl_gene->stable_id .
        " " . $ensembl_genomic_span_length . " vs " . $refseq_gene->stable_id . " " . $refseq_genomic_span_length . "\n";
      $match_span_length = 0;
    }
    elsif ( $ensembl_genomic_span_length > $refseq_genomic_span_length*$factor ) {
      print STDERR "    ensembl_median_length_longer " . $ensembl_gene->stable_id .
        " " . $ensembl_genomic_span_length . " vs " . $refseq_gene->stable_id . " " . $refseq_genomic_span_length . "\n";
      $match_span_length = 0;
    }
    else {
      print STDERR "    median_lengths_similar " . $ensembl_gene->stable_id .
        " " . $ensembl_genomic_span_length . " vs " . $refseq_gene->stable_id . " " . $refseq_genomic_span_length . "\n";
      $match_span_length = 1;
    }
    ...
    if ( $ensembl_median_gene_length*$factor < $refseq_median_gene_length ) {
      print STDERR "    refseq_median_length_longer " . $ensembl_gene->stable_id .
        " " . $ensembl_median_gene_length . " vs " . $refseq_gene->stable_id . " " . $refseq_median_gene_length . "\n";
    ...
    if ( $ensembl_gene->seq_region_start + ( $ensembl_gene_length/$threshold ) < $refseq_gene->seq_region_start ) {
      print STDERR "    ensembl_start_earlier " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->seq_region_start . " vs " . $refseq_gene->stable_id . " " . $refseq_gene->seq_region_start . "\n";
      $match_start = 0;
    }
    elsif ( $ensembl_gene->seq_region_start - ( $ensembl_gene_length/$threshold ) > $refseq_gene->seq_region_start ) {
      print STDERR "    ensembl_start_later " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->seq_region_start . " vs " . $refseq_gene->stable_id . " " . $refseq_gene->seq_region_start . "\n";
      $match_start = 0;
    }
    else {
      print STDERR "    ensembl_start_similar " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->seq_region_start . " vs " . $refseq_gene->stable_id . " " . $refseq_gene->seq_region_start . "\n";
      $match_start = 1;
    }

    if ( $ensembl_gene->seq_region_end + ( $ensembl_gene_length/$threshold ) < $refseq_gene->seq_region_end ) {
      print STDERR "    ensembl_ends_earlier " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->seq_region_end . " vs " . $refseq_gene->stable_id . " " . $refseq_gene->seq_region_end . "\n";
      $match_end = 0;
    }
    elsif ( $ensembl_gene->seq_region_end - ( $ensembl_gene_length/$threshold ) > $refseq_gene->seq_region_end ) {
      print STDERR "    ensembl_ends_later " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->seq_region_end . " vs " . $refseq_gene->stable_id . " " . $refseq_gene->seq_region_end . "\n";
      $match_end = 0;
    }
    else {
      print STDERR "    ensembl_end_similar " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->seq_region_end . " vs " . $refseq_gene->stable_id . " " . $refseq_gene->seq_region_end . "\n";
      $match_end = 1;
    }
    ...
    if ( $ensembl_gene->external_name ne $refseq_xrefs_shortlist[0]->display_id ) {
      ...
      print STDERR "    name_mismatch " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->external_name . " vs " . $refseq_gene->stable_id . " " . $refseq_xrefs_shortlist[0]->display_id . "\n";
#print STDERR "    name_mismatch ".$ensembl_gene->stable_id." ".$ensembl_gene->external_name." vs ".$refseq_gene->stable_id." ".$refseq_gene->external_name."\n";
      ...
      print STDERR "    names_match " . $ensembl_gene->stable_id .
        " " . $ensembl_gene->external_name . " vs " . $refseq_gene->stable_id . " " . $refseq_xrefs_shortlist[0]->display_id . "\n";
#print STDERR "    names_match ".$ensembl_gene->stable_id." ".$ensembl_gene->external_name." vs ".$refseq_gene->stable_id." ".$refseq_gene->external_name."\n";
  ...
  } ## end REFSEQ: foreach my $refseq_gene ( @...)

  return ( $consider_as_matched, \%matched_genes );
} ## end sub confirm_refseq_in_ensembl
