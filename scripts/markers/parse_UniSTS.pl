...
my %opt = ( file => 'UniSTS.sts', );

&GetOptions( \%opt, 'file=s', 'species=s', 'outfile=s', );
...
for my $id (@species_entries) {
  ...
  for my $org ( keys %uni_orgs ) {
    ...
    if ( ( $uni_orgs{ $opt{species} }{len} =~ m/\d/ ) && ( $uni_orgs{ $opt{species} }{name} !~ m/-/ ) ) {
    ...
    unless ( $len_org eq $name_org ) {
           ...
           $uni_orgs{$len_org}{lprim} eq $uni_orgs{ $opt{species} }{lprim} )
      {
        ...
        print O "$id\t$uni_orgs{$opt{species}}{rprim}\t$uni_orgs{$opt{species}}{lprim}\t" .
          "$len\t$name\t$uni_orgs{$opt{species}}{chr}\t$uni_orgs{$opt{species}}{acc}\t" . "$opt{species}\n";
      }
    }
  }
} ## end MARKER: for my $id (@species_entries)
close(O);
print "**done**\n";
