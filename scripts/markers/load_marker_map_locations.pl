...
my $db =
  new Bio::EnsEMBL::DBSQL::DBAdaptor( '-host' => $host, '-user' => $user, '-dbname' => $dbname, '-pass' => $pass, '-port' => $port, );
...
while (<FH>) {
  ...
  my $en = { id => $l[0], name => $l[1], chr => $l[2], pos => $l[3], lod => $lod, };
...
my $check_map_loc_sql =
"select marker_id from marker_map_location where marker_id = ? and  map_id = ? and chromosome_name = ? and marker_synonym_id = ? and  position = ? and lod_score = ?";
my $check_map_loc_sth = $db->dbc->prepare($check_map_loc_sql);

my $map_loc_sql =
  "insert into marker_map_location(marker_id, map_id, chromosome_name, marker_synonym_id, position, lod_score ) values (?,?,?,?,?,?)";
my $map_loc_sth = $db->dbc->prepare($map_loc_sql);

my $map_loc_sql_null =
  "insert into marker_map_location(marker_id, map_id, chromosome_name, marker_synonym_id, position ) values (?,?,?,?,?)";
...
if ($write) {
  foreach my $id ( keys %data ) {
    ...
    my ( $name, $chr, $pos, $lod ) = ( $en->[0]->{name}, $en->[0]->{chr}, $en->[0]->{pos}, $en->[0]->{lod} );
    $marker_id_sth->execute($id);
    my ($marker_id) = $marker_id_sth->fetchrow;
    if ( !$marker_id ) {
      throw( "Marker " . $id . " " . $name . " has failed to produce an internal id " . "cannot continue" );
    }
