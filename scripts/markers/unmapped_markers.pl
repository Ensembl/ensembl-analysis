...
unless ( $host && $user && $dbname && $port ) {
  die(
    "unmapped_markers.pl - removes duplicated and unaligned markers from the database and populates the unmapped feature table
-host    $host
-user    $user
-dbname  $dbname
-port    $port
...
my $db = new Bio::EnsEMBL::DBSQL::DBAdaptor( -host => $host, -user => $user, -port => $port, -dbname => $dbname, -pass => $pass, );
...
if ( !$dry_run ) {
  ...
  # marker feature
  sql(
    "DELETE marker_feature from marker_feature LEFT JOIN marker
  ...
  sql(
    "DELETE marker_feature from marker_feature 
  WHERE map_weight > $max_duplicates", $db );
  # marker synonym
  sql(
    "DELETE marker_synonym from marker_synonym LEFT JOIN marker
  ...
  foreach my $marker (@$array_ref) {
    ...
    foreach my $synonym ( @{ $marker->get_all_MarkerSynonyms } ) {
      ...
      if ( $type eq 'duplicates' ) {
        $umo = Bio::EnsEMBL::UnmappedObject->new( -type       => 'Marker',
                                                  -analysis   => $analysis,
