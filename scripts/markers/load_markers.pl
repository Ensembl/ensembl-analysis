...
if ( !$host || !$name || !$user ) {
  throw( "Need -dbhost $host -dbuser $user and -dbname $name to run " . " use -help for docs" );
}

my $db =
  new Bio::EnsEMBL::DBSQL::DBAdaptor( '-host' => $host, '-user' => $user, '-dbname' => $name, '-pass' => $pass, '-port' => $port, );

my $query =
  "insert into marker(left_primer, right_primer, min_primer_dist, " . "max_primer_dist, priority, type) values (?,?,?,?,1,'est')";
...
my $display_id_sql = "select marker_synonym_id from marker_synonym " . "where name = ? and source = ? and marker_id = ?";
...
if ($fieldlength) {
  while (<FP>) {
    ...
    my ( $display_id, $lprim, $rprim, $dist, $name, $junk, $acc, $species ) = @line;
    if ($name) {
      my @names = split /\;/, $name;
  ...
  if ( $maxlen > $fieldlength ) {
    print STDERR "\nERROR: Length of name field in table marker_synonym has length " .
      ...
      "mysql -u$user -p$pass -h$host -P$port -D$name -e" . "'alter table marker_synonym modify name varchar(" .
      ( $maxlen + 1 ) . ");'\n\n";
    exit 0;
  }
}
...
MARKER: while (<FP>) {
  my ( $display_id, $lprim, $rprim, $dist, $name, $junk, $acc, $species ) = split /\t/, $_;
  my %names;
  my @dists = split /-/, $dist;
  #insert marker
  if ( scalar(@dists) > 1 ) {
  ...
  if ( !$dis_syn_id && $write ) {
    print "SQL = " . $display_id_sql . " " . $display_name . " " . $source . " didnt " . "get an id\n";
    exit;
  }
