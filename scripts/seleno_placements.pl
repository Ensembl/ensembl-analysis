...
&GetOptions(
  \%opt, 'write!', 'input_id=s', 'verbose!', 'dbhost=s', 'dbuser=s', 'dbpass=s', 'dbport=s', 'dbname=s', 'dnahost=s', 'dnauser=s',
  'dnapass=s', 'dnaport=s', 'dnadbname=s', 'skip_ids=s@',    # ids to skip for mouse it is P63300
...
my $blessed_seq_io = Bio::SeqIO->new( '-file' => "<$opt{sp_file}", '-format' => 'swiss', );
...
read_swissprot_file_for_sec_positions_and_sequence( $blessed_seq_io, \@sequence_array, \%sequence_hash, \%sec_location_hash );
...
$db = Bio::EnsEMBL::DBSQL::DBAdaptor->new( -user   => $opt{dbuser},
                                           -dbname => $opt{dbname},
                                           -pass   => $opt{dbpass},
                                           -host   => $opt{dbhost},
                                           -port   => $opt{dbport}, );
...
if ( $opt{dnadbname} ) {
  my $dnadb = Bio::EnsEMBL::DBSQL::DBAdaptor->new( -user   => $opt{dnauser},
...
align_protein_subseqs_against_genomic_sequence( $db, \%alignment_hash, \%sequence_hash, \%sec_location_hash, $protein_residue_window );
...
sub read_swissprot_file_for_sec_positions_and_sequence {
  ...
  } ## end while ( my $sequence = $blessed_seq_io...)

  print "done gathering protein sequences\n";
} ## end sub read_swissprot_file_for_sec_positions_and_sequence

sub find_best_exonerate_output {
  my ( $exonerate_output_file, $alignment_hash ) = @_;

  open( EXONERATE, "<$exonerate_output_file" ) or die "can find exonerate output file: $exonerate_output_file\n";
...
LINE: while (<EXONERATE>) {
    chomp;
    #ryo string:%S %pi %ql %tl %g, which means the fields will be:
    my ( $queryid, $qstart, $qend, $qorient, $targetid, $tstart, $tend, $tstrand, $score, $pid, $querylength, $tlength, $gene_orient ) =
      split;
    ...
    my @align_vector =
      ( $queryid, $qstart, $qend, $qorient, $targetid, $tstart, $tend, $tstrand, $score, $pid, $querylength, $tlength, $gene_orient );
  ...
  } ## end LINE: while (<EXONERATE>)
} ## end sub find_best_exonerate_output
  ...
  foreach my $key ( keys %alignment_hash ) {
    ...
    foreach my $sec_location (@sec_pos_array) {
      my $stop_codon_location =
        get_stop_codon_location( $filename_stem, $tstrand, $long_protein_seq, $queryid, $sec_location, $genome_seq_filename,
                                 $sequence_string, $genome_seq );
    ...
    my $genewise_runnable =
      new Bio::EnsEMBL::Pipeline::Runnable::Genewise( $edited_genomic_bioseq, $protein_bioseq, $slice, undef, $reverse );
      ...
      foreach my $old_transcript ( @{ $old_gene->get_all_Transcripts } ) {
        ...
        foreach my $transcript ( @{ $gene->get_all_Transcripts } ) {
          ...
          EXON:
            ...
            my $seq_edit = Bio::EnsEMBL::SeqEdit->new( -CODE    => '_selenocysteine',
                                                       -NAME    => 'Selenocysteine',
                                                       -DESC    => 'Selenocysteine',
          ...
          } ## end foreach my $sec_location (@sec_pos_array)
        } ## end foreach my $transcript ( @{...})

      } ## end foreach my $old_transcript ...
    } ## end foreach my $old_gene ( $genewise_runnable...)
  } ## end foreach my $key ( keys %alignment_hash)
} ## end sub align_protein_subseqs_against_genomic_sequence
...
sub run_exonerate {
  ...
  my $command =
    "exonerate-0.9.0 --querytype protein --targettype dna " . "--bestn 1 " .
    "--ryo \"%S %V\\n\" " . "--query $query_seq --target $target_seq " . "--showalignment false --showvulgar false --showsugar false";
  ...
  while (<EXONERATE_COMMAND>) {
    ...
    my ( $queryid, $qstart, $qend, $qorient, $targetid, $tstart, $tend, $tstrand, $score, @align_components ) = split;
...
sub get_stop_codon_location {
  ...
  my $protein_subseq =
    new Bio::Seq( -seq => $protein_seq->subseq( $prot_start_coord, $prot_end_coord ), -accession => $queryid, -id => $queryid );
...
} ## end sub get_stop_codon_location
