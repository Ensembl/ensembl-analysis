...
GetOptions( 'chromosome:s'     => \$slicename,
            'verbose!'         => \$verbose,
...
sub check_positions {
  ...
  foreach my $df ( @{ $df_cluster->{'features'} } ) {
    ...
    print "\t" . $df->ditag_id . ":  " . $df->start . " - " . $df->end . ", " . $df->strand . " (" . $totalstart . "-" . $totalend . ")\n"
      if $verbose;
  } ## end foreach my $df ( @{ $df_cluster...})

  if ( ( ( $minus_start > 4 ) && ( $plus_end > 4 ) ) ||
       ( ( $minus_end > 4 )   && ( $plus_start > 4 ) ) ||
       ( ( $minus_start > 4 ) && ( $minus_end > 4 ) )  ||
       ( ( $plus_start > 4 )  && ( $plus_end > 4 ) ) )
  {
    print STDERR "evil pos: " . $minstart . "-" . $maxend . " [" . $df_cluster->{'start'} . "-" . $df_cluster->{'end'} . "] " .
      ( scalar @ditagids ) . "\n";
