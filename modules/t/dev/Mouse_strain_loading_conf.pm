...
########################
    ...
    'output_path'               => '/lustre/scratch109/ensembl/rn6/mouse_strains_1/mus_musculus/CBA_J_v1/',
    'primary_assembly_dir_name' => 'Primary_Assembly',
    'wgs_id'                    => 'LVXO',
    'assembly_name'             => 'CBA_J_v1',
    'assembly_accession'        => 'GCA_001624475.1',
    'full_ftp_path' =>
'ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/vertebrate_mammalian/Mus_musculus/all_assembly_versions/GCA_001624475.1_CBA_J_v1//GCA_001624475.1_CBA_J_v1_assembly_structure',
...
########################
    ...
    'pipeline_db' => { -dbname => $self->o('farm_user_name') . '_' . $self->o('species_name') . '_' . $self->o('strain') . '_pipe',
    ...
    'reference_db' => { -dbname => $self->o('farm_user_name') . '_' . $self->o('species_name') . '_' . $self->o('strain') . '_core',
                        -host   => $self->o('reference_db_server'),
                        -port   => $self->o('port'),
                        -user   => $self->o('user_w'),
                        -pass   => $self->o('password'), },

    'production_db' => { -host => 'ens-staging1', -port => 3306, -user => 'ensro', -dbname => 'ensembl_production', },

    'taxonomy_db' => { -host => 'ens-livemirror', -port => 3306, -user => 'ensro', -dbname => 'ncbi_taxonomy', },
...
###############################################################################

    {
      ...
      -parameters => { 'full_ftp_path'             => $self->o('full_ftp_path'),
                       'output_path'               => $self->o('output_path'),
                       'primary_assembly_dir_name' => $self->o('primary_assembly_dir_name'), },
      -rc_name   => 'default',
      -flow_into => { 1 => ['find_contig_accessions'], },
      -input_ids => [ {} ], },

    {
      ...
      -parameters => { 'contigs_source'            => $self->o('contigs_source'),
                       'wgs_id'                    => $self->o('wgs_id'),
                       'output_path'               => $self->o('output_path'),
                       'primary_assembly_dir_name' => $self->o('primary_assembly_dir_name'), },
      -rc_name   => 'default',
      -flow_into => { 1 => ['download_contigs'], }, },

    {
      ...
      -parameters => { 'contigs_source'            => $self->o('contigs_source'),
                       ...
                       'primary_assembly_dir_name' => $self->o('primary_assembly_dir_name'), },
      -rc_name   => 'default',
      -flow_into => { 1 => ['populate_production_tables'], }, },

    {
      # Creates a reference db for each species
      -logic_name => 'create_core_db',
      -module     => 'Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveCreateDatabase',
      -parameters => { 'target_db'        => $self->o('reference_db'),
...

    {
      ...
      -parameters => { 'coord_system_version'      => $self->o('assembly_name'),
                       'target_db'                 => $self->o('reference_db'),
                       'output_path'               => $self->o('output_path'),
                       'enscode_root_dir'          => $self->o('enscode_root_dir'),
                       'primary_assembly_dir_name' => $self->o('primary_assembly_dir_name'), },
      -rc_name   => 'default',
      -flow_into => { 1 => ['load_assembly_info'], }, },
...

    {
      ...
      -parameters => { 'taxon_id'                  => $self->o('taxon_id'),
...
###############################################################################
    ...
    { -logic_name => 'run_repeatmasker',
      ...
      -parameters => { target_db          => $self->o('reference_db'),
                       ...
                       commandline_params => '-nolow -species "' .
                         $self->o('repeatmasker_library') . '" -engine "' . $self->o('repeatmasker_engine') . '"', },
      -rc_name       => 'repeatmasker',
      -flow_into     => { -1 => ['run_repeatmasker_himem'], 4 => ['run_dust'], },
      -hive_capacity => 900, },
...

    {
      ...
      -parameters => { 'coord_system_name'  => 'toplevel',
                       'target_db'          => $self->o('reference_db'),
...
###############################################################################

    {
      ...
      -parameters =>
        { target_db => $self->o('reference_db'), logic_name => 'dust', module => 'HiveDust', dust_path => $self->o('dust_path'), },
...

    {
      ...
      -parameters =>
        { target_db => $self->o('reference_db'), logic_name => 'trf', module => 'HiveTRF', trf_path => $self->o('trf_path'), },
      -rc_name       => 'simple_features',
      -flow_into     => { 4 => ['run_eponine'], },
      -hive_capacity => 900,
      -batch_size    => 20, },
...

    {
      ...
      -parameters =>
        { target_db => $self->o('reference_db'), logic_name => 'cpg', module => 'HiveCPG', cpg_path => $self->o('cpg_path'), },
      -rc_name       => 'simple_features',
      -flow_into     => { 4 => ['run_trnascan'], },
      -hive_capacity => 900,
      -batch_size    => 20, },
...
##############################################################################

    {
      ...
      -rc_name => 'genscan',
      -flow_into =>
        { 4 => ['create_prediction_transcript_ids'], -1 => ['decrease_genscan_slice_size'], -2 => ['decrease_genscan_slice_size'], },
      -hive_capacity => 900,
      -batch_size    => 20, },

    {
      ...
      -module     => 'Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveSubmitAnalysis',
      -parameters => { split_slice => 1, slice_size => 100000, },
      -flow_into    => { 4 => ['run_genscan_short_slice'], },
      -rc_name      => 'default',
      -can_be_empty => 1, },
    ...
    { -logic_name => 'run_genscan_short_slice',
      ...
      -parameters => { target_db                  => $self->o('reference_db'),
                       ...
                       repeat_masking_logic_names => [ 'repeatmasker_repbase_' . $self->o('repeatmasker_library') ], },
      -rc_name       => 'genscan',
      -flow_into     => { 4 => ['create_prediction_transcript_ids'], -1 => ['failed_genscan_slices'], -2 => ['failed_genscan_slices'], },
...
##############################################################################

    {
      ...
      -flow_into            => { -1 => ['run_uniprot_blast_himem'], 4 => ['run_vertrna_blast'], },
      -rc_name              => 'blast',
...

    {
      ...
      -flow_into            => { -1 => ['run_vertrna_blast_himem'], 4 => ['run_unigene_blast'], },
      -rc_name              => 'blast',
      -failed_job_tolerance => 0.5,
      -hive_capacity        => 900,
      -batch_size           => 20, },
...

    {
      ...
      -flow_into            => { -1 => ['run_unigene_blast_himem'], },
      -rc_name              => 'blast',
      -failed_job_tolerance => 0.5,
      -hive_capacity        => 900,
      -batch_size           => 20, },
...

    {
      ...
      -module     => 'Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveCreateDatabase',
      -parameters => { 'source_db'   => $self->o('reference_db'),
                       'user_w'      => $self->o('user_w'),
                       'pass_w'      => $self->o('password'),
                       'create_type' => 'backup',
    ...
    { -logic_name => 'assembly_loading_report',
      ...
      -parameters => {
                  cmd => 'perl ' .
                    $self->o('loading_report_script') . ' -user ' . $self->o('user_r') . ' -host ' . $self->o( 'reference_db', '-host' ) .
                    ' -port ' . $self->o( 'reference_db', '-port' ) . ' -dbname ' . $self->o( 'reference_db', '-dbname' ) .
                    ' -report_type assembly_loading' . ' > ' . $self->o('output_path') . '/loading_report.txt' },
      -rc_name   => 'default',
    ...
    { -logic_name => 'email_loading_report',
      ...
      -parameters => { email   => $self->o('email_address'),
                       subject => 'AUTOMATED REPORT: assembly loading and feature annotation for ' .
                         $self->o( 'reference_db', '-dbname' ) . ' completed',
                       text => 'Assembly loading and feature annotation have completed for ' .
                         $self->o( 'reference_db', '-dbname' ) . ". Basic stats can be found below",
    ...
    { -logic_name => 'set_repeat_types',
      ...
      -parameters => {
                   cmd => 'perl ' . $self->o('enscode_root_dir') . '/ensembl/misc-scripts/repeats/repeat-types.pl' .
                     ' -user ' . $self->o('user_w') . ' -pass ' . $self->o('password') . ' -host ' . $self->o( 'reference_db', '-host' ) .
                     ' -port ' . $self->o( 'reference_db', '-port' ) . ' -dbpattern ' . $self->o( 'reference_db', '-dbname' ) },
      -rc_name   => 'default',
    ...
    { -logic_name => 'wait_for_annotation_file',
      ...
      -parameters => {},
      -rc_name    => 'default',
      -flow_into  => { 1 => ['load_annotation_file'], }, },
    ...
    { -logic_name => 'load_annotation_file',
      ...
      -parameters => {
        cmd => 'perl ' . $self->o('gtf_parsing_script') .
          ' -user ' . $self->o('user_w') . ' -pass ' . $self->o('password') . ' -host ' . $self->o( 'reference_db', '-host' ) .
          ' -port ' . $self->o( 'reference_db', '-port' ) . ' -dbname ' . $self->o( 'reference_db', '-dbname' ) . ' -dna_user ' .
          $self->o('user_r') . ' -dna_host ' . $self->o( 'reference_db', '-host' ) . ' -dna_port ' . $self->o( 'reference_db', '-port' ) .
          ' -dna_dbname ' . $self->o( 'reference_db', '-dbname' ) . ' -gtf_file ' . $self->o('output_path') . '/annotation_file.gtf' },
    ...
    { -logic_name => 'load_external_db_ids_and_optimise_af_tables',
      -module     => 'Bio::EnsEMBL::Hive::RunnableDB::SystemCmd',
      -parameters => {
        cmd => 'perl ' . $self->o('enscode_root_dir') .
          '/ensembl-analysis/scripts/genebuild/load_external_db_ids_and_optimize_af.pl' . ' -output_path ' . $self->o('output_path') .
          '/optimise_daf_paf' . ' -uniprot_filename ' . $self->o('uniprot_index') . ' -dbuser ' . $self->o('user_w') . ' -dbpass ' .
          $self->o('password') . ' -dbhost ' . $self->o( 'reference_db', '-host' ) . ' -dbname ' . $self->o( 'reference_db', '-dbname' ) .
          ' -prod_dbuser ' . $self->o('user_r') . ' -prod_dbhost ' . $self->o( 'production_db', '-host' ) . ' -prod_dbname ' .
          $self->o( 'production_db', '-dbname' ) . ' -prod_dbport ' . $self->o( 'production_db', '-port' ) . ' -verbose' },
    ...
    { -logic_name => 'annotation_loading_report',
      -module     => 'Bio::EnsEMBL::Hive::RunnableDB::SystemCmd',
      -parameters => {
         cmd => 'perl ' . $self->o('loading_report_script') . ' -user ' . $self->o('user_r') .
           ' -host ' . $self->o( 'reference_db', '-host' ) . ' -port ' . $self->o( 'reference_db', '-port' ) . ' -dbname ' .
           $self->o( 'reference_db', '-dbname' ) . ' -report_type annotation' . ' > ' . $self->o('output_path') . '/annotation_report.txt'
      },
      -rc_name   => 'default',
    ...
    { -logic_name => 'email_annotation_report',
      ...
      -parameters => { email   => $self->o('email_address'),
                       ...
                       text    => 'Loading of annotation has been completed for ' .
                         $self->o( 'reference_db', '-dbname' ) . ". Basic stats can be found below",
...
sub pipeline_wide_parameters {
  ...
  return { %{ $self->SUPER::pipeline_wide_parameters() },    # inherit other stuff from the base class
  };
}
...
sub resource_classes {
  ...
  return { 'default' => {
              LSF => '-q normal -M900 -R"select[mem>900] rusage[mem=900,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           },
           'default_himem' => {
              LSF => '-q normal -M2900 -R"select[mem>2900] rusage[mem=2900,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           },
           'repeatmasker' => {
              LSF => '-q normal -M2900 -R"select[mem>2900] rusage[mem=2900,myens_build' .
           ...
           'repeatmasker_himem' => {
              LSF => '-q normal -M5900 -R"select[mem>5900] rusage[mem=5900,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           },
           'simple_features' => {
              LSF => '-q normal -M2900 -R"select[mem>2900] rusage[mem=2900,myens_build' .
           ...
           'genscan' => {
              LSF => '-q normal  -W 180 -M2900 -R"select[mem>2900] rusage[mem=2900,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           },
           'genscan_short' => {
              LSF => '-q normal -W 60 -M5900 -R"select[mem>5900] rusage[mem=5900,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           },
           'blast' => { LSF => '-q normal -M2900 -n 3 -R "select[mem>2900] rusage[mem=2900,myens_build' . $reference_db_server_number .
                          'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . '] span[hosts=1]"' },
           'blast_himem' => {
                           LSF => '-q normal -M5900 -n 3 -R "select[mem>5900] rusage[mem=5900,myens_build' . $reference_db_server_number .
                             'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . '] span[hosts=1]"' },
           'optimise_daf_paf' => {
              LSF => '-q normal -M9000 -R "select[mem>9000] rusage[mem=9000,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           },
           'load_annotation' => {
              LSF => '-q normal -M9900 -R"select[mem>9900] rusage[mem=9900,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           }, };
...
sub master_config_settings {
  ...
  my $master_config_settings = {
    ...
    HiveBlast => {
      Default => { BLAST_PARSER  => 'Bio::EnsEMBL::Analysis::Tools::BPliteWrapper',
                   PARSER_PARAMS => { -regex => '^(\w+)', -query_type => undef, -database_type => undef, },
                   BLAST_FILTER  => undef,
                   FILTER_PARAMS => {},
                   BLAST_PARAMS => { -unknown_error_string => 'FAILED', -type => 'wu', } },

      HiveBlastGenscanPep => {
           BLAST_PARSER => 'Bio::EnsEMBL::Analysis::Tools::FilterBPlite',
           PARSER_PARAMS =>
             { -regex => '^(\w+\W\d+)', -query_type => 'pep', -database_type => 'pep', -threshold_type => 'PVALUE', -threshold => 0.01, },
           BLAST_FILTER  => 'Bio::EnsEMBL::Analysis::Tools::FeatureFilter',
           FILTER_PARAMS => { -min_score => 200, -prune => 1, }, },

      HiveBlastGenscanVertRNA => {
          BLAST_PARSER => 'Bio::EnsEMBL::Analysis::Tools::FilterBPlite',
          PARSER_PARAMS =>
            { -regex => '^(\w+\W\d+)', -query_type => 'pep', -database_type => 'dna', -threshold_type => 'PVALUE', -threshold => 0.001, },
          BLAST_FILTER  => 'Bio::EnsEMBL::Analysis::Tools::FeatureFilter',
          FILTER_PARAMS => { -prune => 1, }, },

      HiveBlastGenscanUnigene => { BLAST_PARSER  => 'Bio::EnsEMBL::Analysis::Tools::FilterBPlite',
                                   PARSER_PARAMS => { -regex          => '\/ug\=([\w\.]+)',
                                                      -query_type     => 'pep',
                                                      -database_type  => 'dna',
...
} ## end sub master_config_settings
