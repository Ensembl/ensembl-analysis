...
my $time = time;
eval {
  Bio::EnsEMBL::Analysis::Tools::Utilities::execute_with_wait('sleep t 2&>1 /dev/null', undef, 2);
};
$time = time()-$time;
...
$time = time;
eval {
  Bio::EnsEMBL::Analysis::Tools::Utilities::execute_with_wait('sleep t 2&>1 /dev/null', 'Doh', 2);
};
$time = time()-$time;
