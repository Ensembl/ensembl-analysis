...
package Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveCreateTissueJobsFromRegistry;
...
sub fetch_input {
  my $self = shift;
  my $registry_adaptor = new Bio::EnsEMBL::Analysis::Hive::DBSQL::TranscriptomicRegistryAdaptor(
  ...
  my @results = $registry_adaptor->fetch_samples_run_by_sample_name($self->param('SM'),$self->param('taxon_id'));
  my @output_ids;
  ...
  foreach my $result (@results) {
    ...
    #say "ID is $sample_id and name is $sample_name";
    push (@samples, [$sample_id,$sample_name]);
  }
