...
package Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveSeedRedPipeline;
...
sub run {
  ...
  my $clade_group        = $self->param('clade_group');
  say "Fetching GCAs by constraints...";
  my $inital_gca_list       = $assembly_registry_dba->fetch_gca_by_constraints_clade_group($clade_group,$min_contig_n50,$min_scaffold_n50,$min_total_length,undef,$max_version_only);
...
sub write_output {
  ...
  foreach my $gca (@{$gcas_to_process}) {
    ...
    $processing_hash->{'species'} = $species_name;
...
sub post_cleanup {
  ...
  sleep($sleep_length_hours * 84600);
...
sub find_gcas_to_process {
  ...
  foreach my $gca (@{$inital_gca_list}) {
    ...
    my $sql = "insert into repeat_library_status (assembly_id,assembly_accession,date_started,library_status,library_source) values(?,?,?,?,?)";
