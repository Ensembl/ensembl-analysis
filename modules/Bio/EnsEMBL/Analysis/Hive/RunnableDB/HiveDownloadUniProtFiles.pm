...
sub build_query {
  ...
  my %pe_code = ( '1' => 'evidence+at+protein+level',
  ...
  unless ( $self->param_is_defined('file_name') &&
           $self->param_is_defined('dest_dir') &&
  ...
  $full_query .=
    $pe_string . $taxonomy_string .
    $fragment_string . $mito . "&compress=" . $compress . "&format=" . $format . "\" > " . $dest_dir . "/" . $file_name;
  if ( $compress eq 'yes' ) {
    $full_query .= ".gz";
  }
