...
sub fetch_input {
  ...
  foreach my $target (@target_list) {
    ...
    } ## end else [ if ( ref $target eq 'ARRAY')]
  } ## end foreach my $target (@target_list)
  ...
  if ( -e $query and -d $query ) {
    ...
    if ( $self->USE_KILL_LIST ) {
      $query_file =
        filter_killed_entries( $query_file, $self->KILL_TYPE, $self->REFDB, $self->KILLLISTDB, $self->KILL_LIST_FILTER, $self->input_id );
    ...
    throw(
         "When your input ids are not filenames, you must define " . "IIDREGEXP in config to enable inference of chunk number and total" )
  ...
  if ( defined $self->PROGRAM && defined $self->analysis->program_file ) {
    if ( $self->PROGRAM ne $self->analysis->program_file ) {
      ...
      warning( "CONFLICT: You have defined -program in your config file and " . "-program_file in your analysis table." );
    }
  }
...
sub REFDB {
  ...
  if ( defined $value ) {
    my $dba = new Bio::EnsEMBL::DBSQL::DBAdaptor( %$value );
...
sub filter_killed_entries {
  ...
  my $kill_list_object = Bio::EnsEMBL::KillList::HiveKillList->new( -TYPE          => $mol_type,
                                                                    -GB_REF_DB     => $input_db,
                                                                    -KILL_LIST_DB  => $killlist_db,
                                                                    -FILTER_PARAMS => $filter_params );
  my %kill_list = %{ $kill_list_object->get_kill_list() };

  my $seqin = new Bio::SeqIO( -file => "<$orig_query_filename", -format => "Fasta", );

  my $filtered_seqout_filename = "/tmp/$inputID" . "_filtered";
  print "Filename for my filtered sequence: $filtered_seqout_filename.\n";

  my $seqout = new Bio::SeqIO( -file => ">$filtered_seqout_filename", -format => "Fasta" );
