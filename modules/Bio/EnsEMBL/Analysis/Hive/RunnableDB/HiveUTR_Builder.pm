...
package Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveUTR_Builder;
...
sub forward_genewise_clusters{
  ...
  if (!defined($self->param('_forward_genewise_clusters'))) {
    $self->param('_forward_genewise_clusters',[]);
  }

  if (defined $cluster_ref && scalar(@{$cluster_ref})) {
    push(@{$self->param('_forward_genewise_clusters')},@{$cluster_ref});
    # store them sorted
    @{$self->param('_forward_genewise_clusters')} = sort { $a->start <=> $b->start } @{$self->param('_forward_genewise_clusters')};
  }

  return $self->param('_forward_genewise_clusters');
}
...
sub reverse_genewise_clusters{
  ...
  if (!defined($self->param('_reverse_genewise_clusters'))) {
    $self->param('_reverse_genewise_clusters',[]);
  }

  if (defined $cluster_ref && scalar(@{$cluster_ref})) {
    push(@{$self->param('_reverse_genewise_clusters')},@{$cluster_ref});
    # store them sorted
    @{$self->param('_reverse_genewise_clusters')} = sort { $a->start <=> $b->start } @{$self->param('_reverse_genewise_clusters')};
  }

  return $self->param('_reverse_genewise_clusters');
}
...
sub cdna_genes {
  ...
  if (!defined($self->param('_cdna_genes'))) {
    $self->param('_cdna_genes',[]);
  }

  if (defined $genes && scalar(@{$genes})) {
    push(@{$self->param('_cdna_genes')},@{$genes});
  }

  return ($self->param('_cdna_genes'));
}
...
sub prune {
  ...
  if (!defined($self->param('_prune_genes'))) {
    $self->param('_prune_genes',undef);
  }

  if (defined $bool) {
    $self->param('_prune_genes',$bool);
  }

  return ($self->param('_prune_genes'));
}
...
sub ests {
  ...
  if (!defined($self->param('_ests'))) {
    $self->param('_ests',[]);
  }

  if (defined $ests && scalar(@{$ests})) {
    push(@{$self->param('_ests')},@{$ests});
  }

  return($self->param('_ests'));
}
...
sub ditags {
  ...
  if (!defined($self->param('_ditags'))) {
    $self->param('_ditags',[]);
  }

  if (defined $ditags && scalar(@{$ditags})) {
    push(@{$self->param('_ditags')}, @{$ditags});
  }

  return($self->param('_ditags'));
}
...
sub gw_genes {
  my ($self, $genes) = @_;
  if (!defined($self->param('_gw_genes'))) {
    $self->param('_gw_genes',[]);
  }

  if (defined $genes && scalar(@{$genes})) {
    push(@{$self->param('_gw_genes')},@{$genes});
  }

  return $self->param('_gw_genes');
}
...
sub blessed_genes {
  ...
  if (!defined($self->param('_blessed_genes'))) {
    $self->param('_blessed_genes',[]);
  }
  ...
  OLDGENE:
	...
	push(@{$self->param('_blessed_genes')}, $newgene);

      }
    }
  }

  return $self->param('_blessed_genes');
}
...
sub combined_genes {
  ...
  if (!defined($self->param('_combined_genes'))) {
    $self->param('_combined_genes',[]);
  }
  if (defined $genesref) {
    push(@{$self->param('_combined_genes')}, $genesref);
  }
#  if (defined $genesref && scalar(@{$genesref})) {
#    push(@{$self->{'_combined_genes'}},@{$genesref});
#  }

  return $self->param('_combined_genes');
}
...
sub unmatched_genes {
  ...
  if (!defined($self->param('_unmatched_genes'))) {
    $self->param('_unmatched_genes',[]);
  }

  if(@genes){
    push(@{$self->param('_unmatched_genes')},@genes);
  }


  return $self->param('_unmatched_genes');
}
...
sub kill_list {
  ...
  if($kill_list){
    $self->param('_kill_list',$kill_list);
  }

  return $self->param('_kill_list');
...
sub merged_unmerged_pairs {
  ...
  if (!defined($self->param('_merged_unmerged_pairs'))) {
    $self->param('_merged_unmerged_pairs',{});
  }

  if ($unmerged_gene && $merged_gene) {
    $self->param('_merged_unmerged_pairs')->{$merged_gene} = $unmerged_gene;
  }

  # hash ref
  return $self->param('_merged_unmerged_pairs');
}
...
sub modified_unmodified_pairs {
  ...
  if (!defined($self->param('_modified_unmodified_pairs'))) {
    $self->param('_modified_unmodified_pairs',{});
  }

  if ($unmodified_gene && $modified_gene) {
    $self->param('_modified_unmodified_pairs')->{$modified_gene} = $unmodified_gene;
  }

  # hash ref
  return $self->param('_modified_unmodified_pairs');
}
...
sub output{
  ...
  if (!defined($self->param('_output'))) {
    $self->param('_output',[]);
  }

  if(@genes){
    push(@{$self->param('_output')},@genes);
  }
...
sub _cdna_slice {
  ...
  if($slice){
    $self->param('_cdna_slice',$slice);
  }

  return $self->param('_cdna_slice');
}
...
sub _cdna_evidence {
  ...
  if($cdna_evidence){
    $self->param('_cdna_evidence',$cdna_evidence);
  }

  return $self->param('_cdna_evidence');
}
...
sub _known_pairs {
  ...
  if (defined $hashref) {
    $self->param('_known_pairs',$hashref);
  }

  return $self->param('_known_pairs');
}
...
sub CDNA_DB {
  ...
  if ($val){
     $self->param('_cdna_dbs',$val);
  }

  return $self->param('_cdna_dbs');
}
...
sub EST_DB {
  ...
  if ($est_db){
    $self->param('_est_db',get_db_adaptor_by_string($est_db,1));
  }

  return $self->param('_est_db');
}
...
sub DITAG_DB {
    ...
    if ($ditag_db){
      $self->param('_ditag_db',$ditag_db);
    }

    if((defined $self->param('_ditag_db')) && (!$self->param('_ditag_db')) && (scalar @{$self->DITAG_TYPE_NAMES})){
      throw("You have defined DITAG_TYPE_NAMES, but no DITAG_DB parameters.\n");
    }
    return $self->param('_ditag_db');
}
...
sub BLESSED_DB {
  ...
  if ($blessed_db){
    $self->param('_blessed_db',$blessed_db);
  }

  return $self->param('_blessed_db');
}
...
sub OUTPUT_DB {
  ...
  if ($output_db){
    $self->param('_output_db',$output_db);
  }
  if(!$self->param('_output_db')){
    $self->throw("Please define database parameters for output db.\n");
  }
...
sub INPUT_GENETYPES {
  ...
  if($input_genetypes){
    $self->param('_input_genetypes',$input_genetypes);
  }

  return $self->param('_input_genetypes');
}
...
sub BLESSED_GENETYPES {
  ...
  if (defined $value) {
    $self->param('_blessed_genetypes',$value);
  }
  return $self->param('_blessed_genetypes');
}
...
sub VERBOSE {
  ...
  if (defined $value) {
    $self->param('_verbose',$value);
  }
  return $self->param('_verbose');
}
...
sub MAX_EXON_LENGTH {
  ...
  if (defined $value) {
    $self->param('_max_exon_length',$value);
  }
...
sub MAX_CDS_EXTEND {
  ...
  if (defined $value) {
    $self->param('_max_cds_extend',$value);
  }
  return $self->param('_max_cds_extend');
}
...
sub DITAG_TYPE_NAMES {
  ...
  if (defined $value) {
    $self->param('_ditag_type_names',$value);
  }
  return $self->param('_ditag_type_names');
}
...
sub cDNA_GENETYPE {
  ...
  if (defined $value) {
    $self->param('_cdna_genetype',$value);
  }
  return $self->param('_cdna_genetype');
}
...
sub EXTEND_ORIGINAL_BIOTYPE {
  ...
  if (defined $value) {
    $self->param('_extend_original_biotype',$value);
  }
  return $self->param('_extend_original_biotype');
}
...
sub EXTEND_BIOTYPE_OF_UNCHANGED_GENES {
  ...
  if (defined $value) {
    ...
    $self->param('_extend_biotype_of_unchanged_genes',$value);
  }
  return $self->param('_extend_biotype_of_unchanged_genes');
}
...
sub KNOWN_UTR_GENETYPE {
  ...
  if (defined $value) {
    $self->param('_known_utr_genetype',$value);
  }
  return $self->param('_known_utr_genetype');
}
...
sub UTR_GENETYPE {
  ...
  if (defined $value) {
    $self->param('_utr_genetype',$value);
  }
  return $self->param('_utr_genetype');
}
...
sub FILTER_ESTS {
  ...
  if (defined $value) {
    $self->param('_filter_ests',$value);
  }
  return $self->param('_filter_ests');
}
...
sub MAX_INTRON_LENGTH {
  ...
  if (defined $value) {
    $self->param('_max_intron',$value);
  }
  return $self->param('_max_intron');
}
...
sub EST_GENETYPE {
  ...
  if (defined $value) {
    $self->param('_est_genetype',$value);
  }
  return $self->param('_est_genetype');
}
...
sub BLESSED_UTR_GENETYPE {
  ...
  if (defined $value) {
    $self->param('_blessed_utr_genetype',$value);
  }
  return $self->param('_blessed_utr_genetype');
}
...
sub LOOK_FOR_KNOWN {
  ...
  if (defined $value) {
    $self->param('_look_for_known',$value);
  }
  return $self->param('_look_for_known');
}
...
sub DITAG_WINDOW {
  ...
  if (defined $value) {
    $self->param('_ditag_window',$value);
  }
  return $self->param('_ditag_window');
}
...
sub KNOWNUTR_FILE {
  ...
  if (defined $value) {
    $self->param('_knownutr_file',$value);
  }
  return $self->param('_knownutr_file');
}
...
sub INPUT_GENES {
  ...
  return $self->param('INPUT_GENES');
}

sub ABINITIO_SETS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('ABINITIO_SETS',$arg);
  }
  return $self->param('ABINITIO_SETS');
}

sub SIMGW_SETS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('SIMGW_SETS',$arg);
  }
  return $self->param('SIMGW_SETS');
}

sub EST_SETS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('EST_SETS',$arg);
  }
  return $self->param('EST_SETS');
}

sub AB_INITIO_LOGICNAMES {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('AB_INITIO_LOGICNAMES',$arg);
  }
  return $self->param('AB_INITIO_LOGICNAMES');
}

sub OUTPUT_DATABASE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('OUTPUT_DATABASE',$arg);
  }
  return $self->param('OUTPUT_DATABASE');
}

sub FILTER_SINGLETONS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('FILTER_SINGLETONS',$arg);
  }
  return $self->param('FILTER_SINGLETONS');
}

sub FILTER_NON_CONSENSUS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('FILTER_NON_CONSENSUS',$arg);
  }
  return $self->param('FILTER_NON_CONSENSUS');
}

sub ADD_UTR {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('ADD_UTR',$arg);
  }
  return $self->param('ADD_UTR');
}

sub MIN_CONSENSUS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('MIN_CONSENSUS',$arg);
  }
  return $self->param('MIN_CONSENSUS');
}

sub UTR_PENALTY {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('UTR_PENALTY',$arg);
  }
  return $self->param('UTR_PENALTY');
}

sub END_EXON_PENALTY {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('END_EXON_PENALTY',$arg);
  }
  return $self->param('END_EXON_PENALTY');
}

sub EST_OVERLAP_PENALTY {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('EST_OVERLAP_PENALTY',$arg);
  }
  return $self->param('EST_OVERLAP_PENALTY');
}

sub SHORT_INTRON_PENALTY {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('SHORT_INTRON_PENALTY',$arg);
  }
  return $self->param('SHORT_INTRON_PENALTY');
}

sub SHORT_EXON_PENALTY {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('SHORT_EXON_PENALTY',$arg);
  }
  return $self->param('SHORT_EXON_PENALTY');
}

sub GOOD_PERCENT {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('GOOD_PERCENT',$arg);
  }
  return $self->param('GOOD_PERCENT');
}

sub GOOD_BIOTYPE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('GOOD_BIOTYPE',$arg);
  }
  return $self->param('GOOD_BIOTYPE');
}

sub BAD_BIOTYPE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('BAD_BIOTYPE',$arg);
  }
  return $self->param('BAD_BIOTYPE');
}

sub SMALL_BIOTYPE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('SMALL_BIOTYPE',$arg);
  }
  return $self->param('SMALL_BIOTYPE');
}

sub RNASEQ_INTRON_NAME {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('RNASEQ_INTRON_NAME',$arg);
  }
  return $self->param('RNASEQ_INTRON_NAME');
}

sub RNASEQ_INTRON_DB {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('RNASEQ_INTRON_DB',$arg);
  }
  return $self->param('RNASEQ_INTRON_DB');
}

sub INPUT_DBS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('INPUT_DBS',$arg);
  }
