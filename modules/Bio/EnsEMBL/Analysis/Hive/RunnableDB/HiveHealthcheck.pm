...
sub coding_supporting_evidence_presence {
  ...
  my @sql_queries = (
    'SELECT t.* FROM transcript t LEFT JOIN transcript_supporting_feature tsf ON t.transcript_id = tsf.transcript_id WHERE t.biotype NOT LIKE "%RNA" AND t.biotype NOT LIKE "ribozyme" AND tsf.transcript_id IS NULL',
  );
