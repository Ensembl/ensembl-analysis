...
sub clean_headers {
  ...
  if ( $self->param_is_defined('skip_broken_headers') &&
       ( $self->param('skip_broken_headers') eq "1" ||
         $self->param('skip_broken_headers') eq "yes" ||
         $self->param('skip_broken_headers') eq "YES" ) )
  {
  ...
  if ( $self->param_is_defined('use_killlist') &&
       ( $self->param('use_killlist') eq "1" || $self->param('use_killlist') eq "yes" || $self->param('use_killlist') eq "YES" ) )
  {
  ...
  my $seqin = new Bio::SeqIO( -file => "<$input_file", -format => "Fasta", );

  my $seqout = new Bio::SeqIO( -file => ">$output_file", -format => "Fasta" );
...
sub match_against_source {
  ...
  if ( $header_source eq 'uniprot' ) {
    ...
    throw( "You have entered a source that is not supported. The code must be updated to " . "deal with it. Source:\n" . $header_source );
  }
