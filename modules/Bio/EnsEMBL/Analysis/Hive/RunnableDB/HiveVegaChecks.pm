...
sub param_defaults {
  return {
    ...
    write        => '',
    affix        => 0
    , # perform the checks by using the biotypes with or without the prefixes and suffixes like weird_, _Ens, _hav, ... ; without affixes by default
...
sub run {
  ...
  my $TXT_GENE_TO_CODING =
    "Gene contains at least one '\%protein_coding\%' transcript and no '\%polymorphic_pseudogene\%' transcripts. Gene biotype should be";
  my $TXT_GENE_TO_PROCESSED_TRANSCRIPT =
"Gene contains no '\%polymorphic_pseudogene\%' transcripts, no '\%protein_coding\%' transcripts and at least one '\%processed_transcript\%' transcript. Gene biotype should be";
  ...
  my $TXT_GENE_TRANS_BIOTYPE_NOT_ALLOWED =
    "Gene and transcript biotypes combination is not allowed. Please, correct this problem and run this script again.";
  ...
  my $gene_prefixes_str = "";
  $gene_prefixes_str = join( "|", @GENE_PREFIXES ) if ( $self->param('affix') );
  my $gene_suffixes_str = "";
  $gene_suffixes_str = join( "|", @GENE_SUFFIXES ) if ( $self->param('affix') );
  my $transcript_prefixes_str = "";
  $transcript_prefixes_str = join( "|", @TRANSCRIPT_PREFIXES ) if ( $self->param('affix') );
  my $transcript_suffixes_str = "";
  $transcript_suffixes_str = join( "|", @TRANSCRIPT_SUFFIXES ) if ( $self->param('affix') );
  ...
  foreach my $slice (@slices) {
    ...
    foreach my $gene (@genes) {
      ...
      my $g_biotype_basename =
        get_biotype_basename( $g_biotype, $gene_biotypes_str, $gene_prefixes_str, $gene_suffixes_str );    # remove any prefix and suffix

      if ( ( exists( ${ $self->param('gene_trans_type_combination') }{$g_biotype_basename} ) ) or
           ( $self->param('biotypes_extension') and
             exists( ${ $self->param('gene_trans_type_combination_extension') }{$g_biotype_basename} ) ) )
      {
        ...
        if ( $self->param('biotypes_extension') and
             exists( ${ $self->param('gene_trans_type_combination_extension') }{$g_biotype_basename} ) )
        {
          $g_allowed_transcript_biotypes_str .= "|" if ( !( $g_allowed_transcript_biotypes_str eq "" ) );
          $g_allowed_transcript_biotypes_str .= join( "|",
                                                      @{${ $self->param('gene_trans_type_combination_extension') }{$g_biotype_basename} }
          );
        }
        ...
        foreach my $trans (@transcripts) {
          ...
          if ( $t_biotype =~ /^($transcript_prefixes_str)($transcript_biotypes_str)($transcript_suffixes_str)$/ ) {
            ...
            if ( !$trans->translation ) {    # transcript has no translation
              if ( $t_biotype =~ /^($transcript_prefixes_str)($transcript_coding_biotypes_str)($transcript_suffixes_str)$/ )
              {                              # but its biotype is protein coding (including polymorphic_pseudogene)
                                             # transcript biotype should be processed_transcript, which means it is non-coding
                my ( $t_biotype_prefix, $t_biotype_suffix ) =
                  get_biotype_affix( $t_biotype, $transcript_coding_biotypes_str, $transcript_prefixes_str, $transcript_suffixes_str );
                my $new_suggested_biotype = $t_biotype_prefix . 'processed_transcript' . $t_biotype_suffix;

                print_gene_transcript_info( $gene, $trans, $TXT_NO_TRANSLATION_TRANSCRIPT_TO_PROCESSED_TRANSCRIPT,
          ...
          # we want to keep track of the presence of processed_transcript/non-coding transcripts to decide about the gene biotype later on
            ...
            elsif ( $trans->translate->seq =~ /\*/ and $t_biotype !~ /TR_gene/ and $t_biotype !~ /IG_gene/ )
            {                                   # transcript has translation with stop codon
              if ( $t_biotype !~ /.*polymorphic.*/ ) {    # but its biotype is not polymorphic or polymorphic_pseudogene
                ...
                my ( $t_biotype_prefix, $t_biotype_suffix ) =
                  get_biotype_affix( $t_biotype, $transcript_biotypes_str, $transcript_prefixes_str, $transcript_suffixes_str );
              ...
              if ( ( $t_biotype =~ /^($transcript_prefixes_str)($transcript_non_coding_biotypes_str)($transcript_suffixes_str)$/ ) )
              {       # but its biotype is non-coding
          ...
          } ## end if ( $t_biotype =~ /^($transcript_prefixes_str)($transcript_biotypes_str)($transcript_suffixes_str)$/)
        ...
        if ( $polymorphic_transcript > 0 ) {
          if ( $g_biotype !~ /.*polymorphic.*/ ) {
            my ( $g_biotype_prefix, $g_biotype_suffix ) =
              get_biotype_affix( $g_biotype, $gene_biotypes_str, $gene_prefixes_str, $gene_suffixes_str );
          ...
          if ( ( $g_biotype =~ /^($gene_prefixes_str)($gene_non_coding_biotypes_str)($gene_suffixes_str)$/ ) or
               ( $g_biotype =~ /.*polymorphic.*/ ) )
          {    # if gene biotype is protein coding excluding polymorphic (or "is non-coding or polymorphic")
            my ( $g_biotype_prefix, $g_biotype_suffix ) =
              get_biotype_affix( $g_biotype, $gene_biotypes_str, $gene_prefixes_str, $gene_suffixes_str );
  ...
  print(
"The number of genes whose biotype $should_be_or_has_been updated to '\%processed_transcript\%' is: $num_processed_transcript_genes\n\n"
  );

  print(
     "The number of transcripts whose biotype $should_be_or_has_been updated to '\%protein_coding\%' is: $num_protein_coding_transcript\n"
  );
  print(
"The number of transcripts whose biotype $should_be_or_has_been updated to '\%polymorphic_pseudogene\%' is: $num_polymorphic_transcript\n"
  );
  print(
"The number of transcripts whose biotype $should_be_or_has_been updated to '\%processed_transcript\%' is: $num_processed_transcript\n\n"
  );
  ...
  if ( $self->param('sql_output') ) {
    ...
    print( "The SQL file " . $self->param('sql_output') .
           " that $should_be_or_has_been run to apply the suggested updates has$not_sql_output been written.\n" );
  }
  return 1;
} ## end sub run
...
sub print_gene_transcript_info {
  ...
  my $g_id        = $gene->dbID;
  my $g_stable_id = "undefined";
  $g_stable_id = $gene->stable_id if ( $gene->stable_id );
  my $g_biotype   = $gene->biotype;
  my $t_id        = $trans->dbID;
  my $t_stable_id = "undefined";
  $t_stable_id = $trans->stable_id if ( $trans->stable_id );
  my $t_biotype = $trans->biotype;

  my $str_to_print =
"Gene ID: $g_id, Gene stable_id: $g_stable_id, Gene biotype: $g_biotype, Transcript ID: $t_id, Transcript stable_id: $t_stable_id, Transcript biotype: $t_biotype. $message";
