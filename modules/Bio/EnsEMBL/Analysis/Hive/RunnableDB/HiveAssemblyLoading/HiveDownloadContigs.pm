...
sub fetch_input {
  ...
  unless ( $self->param('wgs_id') && $self->param('output_path') && $self->param('contigs_source') ) {
    $self->throw( "Must pass in the following parameters:\n" .
                  "wgs_id e.g AAEX for Dog" . "output_path e.g /path/to/work/dir\n" . "contigs_source e.g. 'ENA' or 'NCBI'" );
  }
...
sub fix_contig_headers {
  ...
  if ( $source eq 'ncbi' ) {
    ...
    unless ( $contig_count1 == $contig_count2 ) {
      $self->throw( "The contig count in contigs_unfixed_header.fa (" .
                    $contig_count1 . ") did not match the count in contigs.fa (" . $contig_count2 . "). They should match" );
    }

  } ## end if ( $source eq 'ncbi')
  elsif ( $source eq 'ena' ) {

  }
} ## end sub fix_contig_headers
