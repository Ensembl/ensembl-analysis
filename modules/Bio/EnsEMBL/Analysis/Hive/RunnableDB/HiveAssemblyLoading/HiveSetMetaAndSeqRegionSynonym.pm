...
sub backup_tables {
  ...
  for my $table ( 'seq_region', 'meta' ) {
    ...
    my $cmd =
      "mysqldump" .
      " -h" . $dbhost . " -P" . $dbport . " -u" . $dbuser . " -p" . $dbpass . " " . $dbname . " " . $table . " > " . $backup_file;
...
sub set_seq_region_synonyms {
  ...
  if ($chromo_present) {
    ...
    my $sth_select = $target_dba->dbc->prepare(
'SELECT sr.seq_region_id FROM seq_region sr, coord_system cs WHERE cs.coord_system_id = sr.coord_system_id AND sr.name = ? AND cs.rank = 1'
  ...
  foreach my $file ( 'component_localID2acc', 'scaffold_localID2acc' ) {
    ...
    if ( $insert_count == 0 ) {
      $self->throw( "The insert/update count after parsing " .
                    $file . " was 0, this is probably wrong. File used:\n" . $path_to_files . "/" . $file );
    }
  ...
  } ## end foreach my $file ( 'component_localID2acc'...)

} ## end sub set_seq_region_synonyms
