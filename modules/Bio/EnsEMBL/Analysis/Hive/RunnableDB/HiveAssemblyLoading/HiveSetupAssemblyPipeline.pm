...
sub parse_ftp_link_file {
  ...
  while (<FTP_LINK_FILE>) {
    ...
    my $output_hash = { 'output_path'               => $output_path,
...
"ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/vertebrate_mammalian/Propithecus_coquereli/all_assembly_versions/GCA_000956105.1_Pcoq_1.0/" );
    ...
    } ## end foreach my $assembly_line (...)

    unless ($taxon_id) {
      $self->throw(
            "Failed to find and parse 'Taxid' line from report file. File used:\n" . $gca_and_assembly_version . "_assembly_report.txt" );
    }
    ...
    if ( scalar( @code_types == 0 ) ) {
      $self->throw(
         "Failed to parse any potential 4 letter wgs code from the accessions in " . $gca_and_assembly_version . "_assembly_report.txt" );
    }

    if ( scalar( @code_types > 1 ) ) {
      $self->throw( "Failed because of multiple potential 4 letter wgs code from the accessions in " .
                    $gca_and_assembly_version . "_assembly_report.txt, " . " codes parsed:\n" . @code_types );
    }
  ...
  } ## end while (<FTP_LINK_FILE>)
  close FTP_LINK_FILE;
  close REPORT_FILE;

  $self->param( 'output_array', $output_array );
} ## end sub parse_ftp_link_file
