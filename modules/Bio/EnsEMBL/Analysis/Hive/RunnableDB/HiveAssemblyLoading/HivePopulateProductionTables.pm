...
sub populate_production_db_tables {
  ...
  my $populate_script =
    $self->param('enscode_root_dir') . "/ensembl-production/scripts/production_database/populate_production_db_tables.pl";
  my $cmd =
    "perl " . $populate_script . " -h " . $target_host . " -u " . $target_user . " -p " . $target_pass . " -d " . $target_dbname .
    " -mh " . $production_host . " -md " . $production_dbname . " -mu " . $production_user . " -dp " . $self->param('output_path');
...
} ## end sub populate_production_db_tables
