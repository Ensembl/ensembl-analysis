...
sub run {
  ...
  my $path_to_files = $output_path."/".$primary_assembly_dir_name."/AGP/";

  load_assembly($core_db,$path_to_files,$enscode_dir);
...
sub load_assembly {
  my ($core_db,$path_to_files,$enscode_dir) = @_;
  ...
  unless(-e $enscode_dir."/ensembl-pipeline/scripts/load_agp.pl") {
    throw("Could not find the load_agp script in enscode dir. Path checked:\n".$enscode_dir."/ensembl-pipeline/scripts/load_agp.pl");
  }
  ...
  if(-e $path_to_files."/scaf_all.agp") {
    ...
    if($return) {
      throw("The load agp script returned a non-zero exit code. Commandline used:\n".$cmd);
    }
    ...
    unless($assembly_count_file == $assembly_count_diff) {
      throw("Difference in count between scaff_all.agp and entries loaded into the database:\n".
    ...
    if ($num_warnings > 0) {
      warning("There are some warning messages in the output file that should be checked (grep 'WARN'):\n".$path_to_files."/scaf_all.agp");
    }

    $num_warnings = int(`grep "You are already using" $path_to_files/scaf_all.agp | wc -l`);
    if ($num_warnings > 0) {
      warning("There are some warning messages in the output file that should be checked (grep 'You are already using'):\n".$path_to_files."/scaf_all.agp");
    }

    my $exception = int(`grep "EXCEPTION" $path_to_files/scaf_all.agp | wc -l`);
    if($exception) {
      throw("Exception found in output file:\n".$path_to_files."/scaf_all.agp");
    }

  } else {
    throw("No scaffold to contig agp file exists, expected the following file:\n".$path_to_files."/scaf_all.agp");
  }
  ...
  if(-e $path_to_files."/comp_all.agp") {
    ...
    unless($assembly_count_file == $assembly_count_diff) {
      throw("Difference in count between comp_all.agp and entries loaded into the database:\n".
  ...
  if(-e $path_to_files."/chr_all.agp") {
    ...
    unless($assembly_count_file == $assembly_count_diff) {
      throw("Difference in count between chr_all.agp and entries loaded into the database:\n".
  ...
  if($chromo_present) {
    unless($chromo_present == 2) {
      warning("Chromosomes present, but you only have one mapping file instead of two");
    }
  }
  ...
  if ($chromo_present) {
    if ($num_meta_rows == 3) {
      ...
      warning($num_meta_rows." new rows found in 'meta' table for 'assembly.mapping' meta_key. 3 expected.");
    }
  } else { # if there is no chromosome level
    if ($num_meta_rows == 1) {
      say "1 new row found in 'meta' table for 'assembly.mapping' meta_key. Great!";
    } else {
      throw($num_meta_rows." new rows found in 'meta' table for 'assembly.mapping' meta_key. 1 expected");
    }
  }
