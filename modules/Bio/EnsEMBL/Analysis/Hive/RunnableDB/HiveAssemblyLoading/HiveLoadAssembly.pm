...
sub run {
  ...
  my $path_to_files =
    $self->param('output_path') . "/" . $self->param('species_name') . "/" . $self->param('primary_assembly_dir_name') . "/AGP/";
...
sub load_assembly {
  ...
  unless ( -e $enscode_dir . "/ensembl-pipeline/scripts/load_agp.pl" ) {
    $self->throw(
          "Could not find the load_agp script in enscode dir. Path checked:\n" . $enscode_dir . "/ensembl-pipeline/scripts/load_agp.pl" );
  }
  ...
  if ( -e $path_to_files . "/scaf_all.agp" ) {
    ...
    my $cmd =
      ...
      " -dbpass " . $dbpass . " -dbport " . $dbport . " -dbname " . $dbname . " -assembled_name " . $assembled . " -component_name " .
      $component . " -agp_file " . $path_to_files . "/scaf_all.agp" . "  > " . $path_to_files . "/load_assembly_scaffold_contig.out";
    ...
    if ( $num_warnings > 0 ) {
      $self->warning(
        "There are some warning messages in the output file that should be checked (grep 'WARN'):\n" . $path_to_files . "/scaf_all.agp" );
    }
  ...
  if ( -e $path_to_files . "/comp_all.agp" ) {
    ...
    my $cmd =
      "perl " . $enscode_dir . "/ensembl-pipeline/scripts/load_agp.pl" . " -dbhost " . $dbhost . " -dbuser " . $dbuser .
      " -dbpass " . $dbpass . " -dbport " . $dbport . " -dbname " . $dbname . " -assembled_name " . $assembled . " -component_name " .
      $component . " -agp_file " . $path_to_files . "/comp_all.agp" . "  > " . $path_to_files . "/load_assembly_chromosome_contig.out";
    my $return = system($cmd);
  ...
  if ( -e $path_to_files . "/chr_all.agp" ) {
    ...
    my $cmd =
      "perl " . $enscode_dir . "/ensembl-pipeline/scripts/load_agp.pl" . " -dbhost " . $dbhost . " -dbuser " . $dbuser .
      " -dbpass " . $dbpass . " -dbport " . $dbport . " -dbname " . $dbname . " -assembled_name " . $assembled . " -component_name " .
      $component . " -agp_file " . $path_to_files . "/chr_all.agp" . "  > " . $path_to_files . "/load_assembly_chromosome_scaffold.out";
    my $return = system($cmd);
