...
Bio::EnsEMBL::Analysis::Hive::RunnableDB::RecoverRNASeqTissuesModels -
...
package Bio::EnsEMBL::Analysis::Hive::RunnableDB::RecoverRNASeqTissuesModels;
...
=head2 new
  ...
  Arg [1]   : Bio::EnsEMBL::Analysis::Hive::RunnableDB::RecoverRNASeqTissuesModels
  Function  : instatiates a RecoverRNASeqTissuesModels object and reads and checks the config
  file
  Returntype: Bio::EnsEMBL::Analysis::Hive::RunnableDB::RecoverRNASeqTissuesModels
  Exceptions
  Example   :
...
sub fetch_input {
  ...
  $self->throw("No input id") unless defined($self->param('iid')) ;
...
# Should replace with hrdb call
sub _cache_db {
    my ($self, $dbname, $dbadaptor) = @_;

    $self->param('_cache_db')->{$dbname} = $dbadaptor;
    if(defined $dbadaptor) {
      $self->param('_cache_db')->{$dbname} = $dbadaptor;
    } elsif(defined $dbname) {
      return $self->param('_cache_db')->{$dbname};
    } else {
      return $self->param('_cache_db');
    }
}
...
sub _get_db {
    ...
    return values %{$self->_cache_db} unless $dbname;
    return $self->_cache_db($dbname) if ($self->_cache_db($dbname));
    return undef;
}
...
sub merged_genes {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_merged_genes',$arg);
  }
  return $self->param('_merged_genes');
}
...
sub tissues_genes {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_tissues_genes',$arg);
  }
  return $self->param('_tissues_genes');
}
...
sub IGNORE_STRAND {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_IGNORE_STRAND',$arg);
  }
  return $self->param('_IGNORE_STRAND');
}
...
sub CHECK_TWOWAY {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_CHECK_TWOWAY',$arg);
  }
  return $self->param('_CHECK_TWOWAY');
}

sub CHECK_STOP {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_CHECK_STOP',$arg);
  }
  return $self->param('_CHECK_STOP');
}

sub CHECK_FRAMESHIFT {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_CHECK_FRAMESHIFT',$arg);
  }
  return $self->param('_CHECK_FRAMESHIFT');
}

sub CHECK_SINGLE {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_CHECK_SINGLE',$arg);
  }
  return $self->param('_CHECK_SINGLE');
}

sub CHECK_ORPHAN {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_CHECK_ORPHAN',$arg);
  }
  return $self->param('_CHECK_ORPHAN');
}

sub DELETE_BIOTYPE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_DELETE_BIOTYPE',$arg);
  }
  return $self->param('_DELETE_BIOTYPE');
}

sub MERGED_SET_DATABASE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_MERGED_SET_DATABASE',$arg);
  }
  return $self->param('_MERGED_SET_DATABASE');
}

sub TISSUES_UNIPROT_DATABASE {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_TISSUES_UNIPROT_DATABASE',$arg);
  }
  return $self->param('_TISSUES_UNIPROT_DATABASE');
}

sub MERGED_BIOTYPES {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_MERGED_BIOTYPES',$arg);
  }
  return $self->param('_MERGED_BIOTYPES');
}

sub TISSUES_SET_DATABASE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_TISSUES_SET_DATABASE',$arg);
  }
  return $self->param('_TISSUES_SET_DATABASE');
}

sub MERGED_UNIPROT_DATABASE {
  my ($self, $arg) = @_ ;
  if (defined $arg) {
    $self->param('_MERGED_UNIPROT_DATABASE',$arg);
  }
  return $self->param('_MERGED_UNIPROT_DATABASE');
}

sub TISSUES_BIOTYPES {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_TISSUES_BIOTYPES',$arg);
  }
  return $self->param('_TISSUES_BIOTYPES');
}

sub OUTPUT_DATABASE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_OUTPUT_DATABASE',$arg);
  }
  return $self->param('_OUTPUT_DATABASE');
}

sub INTRON_BAM_FILE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_INTRON_BAM_FILE',$arg);
  }
  return $self->param('_INTRON_BAM_FILE');
}

sub NUM_GROUPS {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_NUM_GROUPS',$arg);
  }
  return $self->param('_NUM_GROUPS');
}

sub GOOD_BIOTYPE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_GOOD_BIOTYPE',$arg);
  }
  return $self->param('_GOOD_BIOTYPE');
}

sub INTRON_ALLOWANCE {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_INTRON_ALLOWANCE',$arg);
  }
  return $self->param('_INTRON_ALLOWANCE');
}

sub ABUNDANCE_THRESHOLD {
  my ($self, $arg) = @_ ;
  if(defined $arg) {
    $self->param('_ABUNDANCE_THRESHOLD',$arg);
  }
  return $self->param('_ABUNDANCE_THRESHOLD');
}
