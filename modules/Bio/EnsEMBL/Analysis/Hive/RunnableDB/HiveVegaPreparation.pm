...
sub run {
  ...
  if ( not $self->param('output_path') or
       not $self->param('check_vega_met_stop_dir') or
       not $self->param('dbhost')                  or
       not $self->param('dbname')                  or
       not $self->param('dbuser')                  or
       not $self->param('dnadbhost')               or
       not $self->param('dnadbname') )
  {
  ...
  readthrough_transcripts_tagged( $self->param('dbhost'), $self->param('dbport'), $self->param('dbuser'), $self->param('dbpass'),
                                  $self->param('dbname') ) if ( $self->param('skip') < 1 or $self->param('only') == 1 );

  methionine_to_stop_codon( $self->param('dbhost'),                  $self->param('dbport'),
                            $self->param('dbuser'),                  $self->param('dbpass'),
                            $self->param('dbname'),                  $self->param('dnadbhost'),
                            $self->param('dnadbport'),               $self->param('dnadbname'),
                            $self->param('check_vega_met_stop_dir'), $self->param('output_path')
  ) if ( $self->param('skip') < 2 or $self->param('only') == 2 );

  set_ncrna_host_gene_attribute( $self->param('dbhost'), $self->param('dbport'), $self->param('dbuser'), $self->param('dbpass'),
                                 $self->param('dbname') ) if ( $self->param('skip') < 3 or $self->param('only') == 3 );

  truncate_tsf_table( $self->param('dbhost'), $self->param('dbport'), $self->param('dbuser'), $self->param('dbpass'),
                      $self->param('dbname') ) if ( $self->param('skip') < 4 or $self->param('only') == 4 );

  add_attribute_to_GAGE_cluster( $self->param('dbhost'), $self->param('dbport'), $self->param('dbuser'), $self->param('dbpass'),
...
sub readthrough_transcripts_tagged {
  ...
  my $readthrough_transcripts_sql =
"update transcript_attrib ta, transcript t set ta.attrib_type_id = (select attrib_type_id from attrib_type where code = 'readthrough_tra') where (ta.value like '%read%through%') and ta.transcript_id = t.transcript_id;";
  ...
  my $num_readthrough_transcripts_sql =
"select count(*) from transcript_attrib ta,transcript t, attrib_type at where code = 'readthrough_tra' and (ta.value like '%read%through%') and ta.transcript_id = t.transcript_id;";
...
sub methionine_to_stop_codon {
  ...
  my $dump_ids_sql =
"select distinct(tr.transcript_id) from translation tr, transcript t, transcript_attrib ta, gene g where t.gene_id = g.gene_id and t.transcript_id = ta.transcript_id and tr.transcript_id = t.transcript_id;";
  ...
  run_command(
"perl $check_vega_met_stop_dir/check_vega_met_stop.pl -dbname $dbname -host $dbhost -dna_dbname $dnadbname -dna_host $dnadbhost -dna_port $dnadbport < $ids_file > $full_ids_file"
  );

  my $num_wrong_ids   = int( run_command("awk '\$3 != \"ok\" { nok++ } END { print nok }' $full_ids_file") );
  ...
  warning(
"Methionine to stop codon check: Please check manually that the number of wrong transcripts $num_wrong_ids is similar to the one in previous release (if exists). Some of the previous releases numbers are: e71: 29260; e70: 29381 e69: 56594; e68: 53703"
  );
} ## end sub methionine_to_stop_codon

sub set_ncrna_host_gene_attribute {
  my ( $dbhost, $dbport, $dbuser, $dbpass, $dbname ) = @_;

  my $ncrna_host_sql =
"update gene_attrib ga, gene g set ga.attrib_type_id = (select attrib_type_id from attrib_type where code = 'ncrna_host') where ga.gene_id = g.gene_id and (value like 'transcribed%' or value = 'ncrna_host');";
  my $check_sql_1 = "select count(*) from gene_attrib where (value like 'transcribed%' or value = 'ncrna_host');";
  my $check_sql_2 =
"select count(*) from gene g, gene_attrib ga where ga.attrib_type_id = (select attrib_type_id from attrib_type where code = 'ncrna_host') and ga.gene_id = g.gene_id;";
...
sub truncate_tsf_table {
  ...
  my $num_tsf =
    run_command( "mysql -h$dbhost -P$dbport -u$dbuser -p$dbpass -D$dbname -NB -e'select count(*) from transcript_supporting_feature;'",
                 "\n===== STEP 4: Truncate transcript_supporting_feature table =====\n" );

  run_command(
"mysql -h$dbhost -P$dbport -u$dbuser -p$dbpass -D$dbname -NB -e'rename table transcript_supporting_feature to $tsf_bak_name; create table transcript_supporting_feature like $tsf_bak_name;'"
  );
...
sub add_attribute_to_GAGE_cluster {
  ...
  my $num_att = run_command(
"mysql -h$dbhost -P$dbport -u$dbuser -p$dbpass -D$dbname -NB -e\"select count(*) from transcript,gene_attrib ga where value = 'gene_cluster_GAGE' and transcript.gene_id=ga.gene_id;\"",
...
END
  ...
  my $check_sql =
    "select count(*) from transcript_attrib where attrib_type_id=(select attrib_type_id from attrib_type where code = 'gene_cluster');";
  run_command( "mysql -h$dbhost -P$dbport -u$dbuser -p$dbpass -D$dbname -NB -e\"$check_sql\"", "", $num_att );
}
