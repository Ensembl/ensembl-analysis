...
sub run {
  ...
  my $vertrna = $self->param('vertrna');
  my $refseq = $self->param('refseq');
  my $infile = $self->param('infile');
  my $findN_prog = $self->param('findN_prog');
  my $reasons_file = $self->param('reasons');
...
sub find_reasons {
  ...
  while (<IN>) {
    ...
    if ($tmp[0]=~/rpp/){
      ...
      } elsif ($tmp[3] < $cov){ #so best coverage too low:
        $cdnas{$tmp[6]}{"Low coverage"} = $tmp[3];
      } elsif ($tmp[5] < $pid){ #do we only want to record highest pid hit?
  ...
  open(REFSEQ, "<", $refseq) or die("can't read $refseq.\n");

  while (my $entry = <REFSEQ>){
    if ($entry =~ /^>/){
      my $id = "";
  ...
  #check the database to see if any of these cdnas has a hit in the cdna db
  ...
  $q1 = $db->dbc->prepare($sql) or die "sql error";
  $q1->execute();
  ...
  my $uoa = $db->get_UnmappedObjectAdaptor();
