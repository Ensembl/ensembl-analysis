...
sub project_exon {
  ...
  say "Processing source exon: ".$exon->stable_id;
  my $exon_header = $exon->stable_id.'.'.$exon->version;
  $exon_seq = $exon->seq->seq;
  my $exon_seq_object = Bio::Seq->new(-display_id => $exon_header, -seq => $exon_seq);

  $self->make_runnables($exon_seq_object, $exon_align_slices, $annotation_features);
...
#      $self->param('_branch_to_flow_on_fail', -3);
 ...
 #     }
      ...
      my $preliminary_exons = $runnable->output;
      my $selected_exons;
      unless(scalar(@{$preliminary_exons})) {
        next;
      }

#      if(scalar(@{$preliminary_exons}) == 1) {
#      $selected_exons = $preliminary_exons;
#      } else {
#        $selected_exons = $self->select_best_transcripts($preliminary_trs);
#      }
...
sub make_runnables {
  my ($self, $exon_seq, $exon_slices, $annotation_features) = @_;
  ...
  $runnable->target_seqs($exon_slices);
  $runnable->query_seqs([$exon_seq]);
