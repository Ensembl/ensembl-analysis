  ...
  foreach my $block (@$gen_al_blocks) {
    ...
    if ( not exists $self->target_slices->{ $tga->dnafrag->name } ) {
      $self->target_slices->{ $tga->dnafrag->name } =
        $target_transcript_dbc->get_SliceAdaptor->fetch_by_region( 'toplevel', $tga->dnafrag->name );
    }
...
sub run {
  ...
  foreach my $chain ( @{ $self->genomic_align_block_chains } ) {
    ...
    my $gene_scaffold =
      Bio::EnsEMBL::Analysis::Tools::WGA2Genes::GeneScaffold->new( -genomic_align_blocks => $chain,
...
sub hive_set_config {
  ...
  my $analysis = new Bio::EnsEMBL::Analysis( -logic_name => $self->param('logic_name'), -module => $self->param('module'), );
...
sub hrdb_get_dba {
  ...
  if ( defined $non_standard_db_adaptor ) {
    if ( $non_standard_db_adaptor eq 'compara' ) {
      $dba = Bio::EnsEMBL::Compara::DBSQL::DBAdaptor->new( %$connection_info );
    ...
    if ($dna_db_name) {
      ...
      unless ( $core_db_asm eq $dna_db_asm ) {
        warning(
            "You try to add  a DNA_DB with assembly $dna_db_asm to " . "a core/cdna/otherfeatures DB with assembly $core_db_asm ...\n\t" .
              "that's incompatbile. I will not add dna_database " . $dnadb->dbname . " to core " . $dba->dbname . "\n" );

        $dna_db_and_core_db_are_compatible = 0;
      }

      unless ( $core_db_species eq $dna_db_species ) {
        warning( "You try to add a DNA_DB with species " .
                 $dna_db_species . " to " . "a core database with species: '" . $core_db_species . "' - this does not work. \n" .
      ...
      if ($dna_db_and_core_db_are_compatible) {
        $dba->dnadb($dnadb);
        print "\nAttaching DNA_DB " . $dnadb->dbname . " to " . $dba->dbname . "\n";
      }
    } ## end if ($dna_db_name)
  ...
  } ## end else [ if ( defined $non_standard_db_adaptor)]

  $dba->dbc->disconnect_when_inactive(1);
  return $dba;

} ## end sub hrdb_get_dba
