...
package Bio::EnsEMBL::Analysis::Hive::RunnableDB::HISAT2;

use warnings;
use strict;
use feature 'say';

use Bio::EnsEMBL::Analysis::Runnable::HISAT2;
...
=head2 fetch_input
 ...
 Arg [1]    : None
 Description: Fetch parameters for HISAT2
 Returntype : None
...
sub fetch_input {
  ...
  foreach my $input_id (@$input_ids) {
    ...
    my $runnable = Bio::EnsEMBL::Analysis::Runnable::HISAT2->new
    (
     -analysis       => $self->create_analysis,
     -program        => $program,
     -samtools_path  => $self->param('samtools_path'),
     -options        => $self->param('short_read_aligner_options'),
     -outdir         => $self->param('output_dir'),
     -genome         => $self->param('genome'),
     -genome_index   => $self->param('genome_dir')."/hsindex",
