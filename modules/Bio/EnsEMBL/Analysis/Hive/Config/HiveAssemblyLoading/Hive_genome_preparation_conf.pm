...
########################
    ...
    'pipeline_db' => { -host   => $self->o('pipe_db_server'),
                       -port   => $self->o('port'),
                       -user   => $self->o('user'),
    ...
    'reference_db' =>
      { -host => $self->o('reference_db_server'), -port => $self->o('port'), -user => $self->o('user'), -pass => $self->o('password'), },
...
###############################################################################

    {
      ...
      -rc_name   => 'default',
      -input_ids => [ {} ],
      -flow_into => { 1 => ['download_assembly_info'], 2 => ['download_contigs'], 3 => ['create_ensembl_db'], }, },

    {
      # Create 1mb toplevel slices, each species flow into this independantly
      -logic_name => 'create_1mb_slice_ids',
      -module     => 'Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveSubmitAnalysis',
      -parameters => { coord_system_name => 'toplevel', slice => 1, slice_size => 1000000, include_non_reference => 0, top_level => 1, },
      -flow_into  => {
...

    {
      ...
      -module     => 'Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveSubmitAnalysis',
      -parameters =>
        { feature_type => 'prediction_transcript', slice_to_feature_ids => 1, prediction_transcript_logic_names => ['genscan'], },
      -flow_into => { 4 => [ 'run_uniprot_blast', 'run_vertrna_blast', 'run_unigene_blast' ], },
      -rc_name   => 'default', },
...
###############################################################################
...

    {
      ...
      -parameters => {},
      -rc_name    => 'default',
      -flow_into  => { 1 => ['create_1mb_slice_ids'], }, },
...
###############################################################################

    {
      ...
      -parameters => { logic_name         => 'repeatmasker_' . $self->o('repeatmasker_library'),
                       module             => 'HiveRepeatMasker',
                       repeatmasker_path  => $self->o('repeatmasker_path'),
                       commandline_params => '-nolow -s -species "' .
                         $self->o('repeatmasker_library') . '" -engine "' . $self->o('repeatmasker_engine') . '"', },
      -rc_name   => 'repeatmasker',
      -flow_into => { -1 => ['run_repeatmasker_himem'], -2 => ['run_repeatmasker_long'], 4 => [ 'run_genscan', 'run_firstef' ], },
    ...
    { -logic_name => 'run_repeatmasker_himem',
      ...
      -flow_into => { -2 => ['run_repeatmasker_long'], 4 => [ 'run_genscan', 'run_firstef' ], },
    ...
    { -logic_name => 'run_repeatmasker_long',
      ...
      -flow_into    => { 4 => [ 'run_genscan', 'run_firstef' ], },
      -can_be_empty => 1, },
...
###############################################################################

    {
      ...
      -parameters => { logic_name => 'dust', module => 'HiveDust', dust_path => $self->o('dust_path'), },
...

    {
      ...
      -parameters => { logic_name => 'trf', module => 'HiveTRF', trf_path => $self->o('trf_path'), },
      -rc_name    => 'simple_features', },
...

    {
      ...
      -parameters => { logic_name => 'cpg', module => 'HiveCPG', cpg_path => $self->o('cpg_path'), },
      -rc_name    => 'simple_features', },

    {
      ...
      -parameters => { logic_name => 'trnascan', module => 'HiveTRNAScan', trnascan_path => $self->o('trnascan_path'), },
      -rc_name    => 'simple_features', },
...
##############################################################################

    {
      ...
      -parameters => { logic_name                 => 'genscan',
                       module                     => 'HiveGenscan',
                       genscan_path               => $self->o('genscan_path'),
                       repeat_masking_logic_names => [ 'repeatmasker_' . $self->o('repeatmasker_library') ], },
      -rc_name   => 'genscan',
      -flow_into => { 4 => ['create_prediction_transcript_ids'], -1 => ['run_genscan_himem'], -2 => ['run_genscan_long'], }, },
    ...
    { -logic_name => 'run_genscan_himem',
      ...
      -flow_into    => { 4 => ['create_prediction_transcript_ids'], -2 => ['run_genscan_long'], },
      -can_be_empty => 1, },
    ...
    { -logic_name => 'run_genscan_long',
      ...
      -flow_into    => { 4 => ['create_prediction_transcript_ids'], },
      -can_be_empty => 1, },
...
##############################################################################

    {
      ...
      -parameters => { blast_db_path                     => $self->o('uniprot_blast_db_path'),
      ...
      -flow_into => { -1 => ['run_uniprot_blast_himem'], -2 => ['run_uniprot_blast_long'], },
      -rc_name   => 'blast', },
    ...
    { -logic_name => 'run_uniprot_blast_himem',
      ...
      -parameters => { blast_db_path                     => $self->o('uniprot_blast_db_path'),
                       ...
                       module                            => 'HiveBlastGenscanPep',
                       config_settings                   => $self->get_config_settings( 'HiveBlast', 'HiveBlastGenscanPep' ), },
      -flow_into    => { -2 => ['run_uniprot_blast_long'], },
      -rc_name      => 'blast_himem',
      -can_be_empty => 1, },
...

    {
      ...
      -parameters => { blast_db_path                     => $self->o('vertrna_blast_db_path'),
      ...
      -flow_into => { -1 => ['run_vertrna_blast_himem'], -2 => ['run_vertrna_blast_long'], },
      -rc_name   => 'blast', },
    ...
    { -logic_name => 'run_vertrna_blast_himem',
      ...
      -parameters => { blast_db_path                     => $self->o('vertrna_blast_db_path'),
                       ...
                       module                            => 'HiveBlastGenscanDNA',
                       config_settings                   => $self->get_config_settings( 'HiveBlast', 'HiveBlastGenscanVertRNA' ), },
      -flow_into    => { -2 => ['run_vertrna_blast_long'], },
      -rc_name      => 'blast_himem',
      -can_be_empty => 1, },
...

    {
      ...
      -module     => 'Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveAssemblyLoading::HiveBlastGenscanDNA',
      -parameters => { blast_db_path                     => $self->o('unigene_blast_db_path'),
      ...
      -flow_into => { -1 => ['run_unigene_blast_himem'], -2 => ['run_unigene_blast_long'], },
      -rc_name   => 'blast', },
    ...
    { -logic_name => 'run_unigene_blast_himem',
      ...
      -parameters => { blast_db_path                     => $self->o('unigene_blast_db_path'),
                       ...
                       module                            => 'HiveBlastGenscanDNA',
                       config_settings                   => $self->get_config_settings( 'HiveBlast', 'HiveBlastGenscanUnigene' ), },
      -flow_into    => { -2 => ['run_unigene_blast_long'], },
      -rc_name      => 'blast_himem',
      -can_be_empty => 1, },
...
sub resource_classes {
  ...
  return { 'default' => {
           ...
           'repeatmasker_long' => {
              LSF => '-q long -M5900 -R"select[mem>5900] rusage[mem=5900,myens_build' .
                $reference_db_server_number . 'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']"'
           },
           'simple_features' => {
              LSF => '-q normal -M2900 -R"select[mem>2900] rusage[mem=2900,myens_build' .
           ...
           'blast_long' => { LSF => '-q long -M5900 -n 3 -R "select[mem>5900] rusage[mem=5900,myens_build' . $reference_db_server_number .
                               'tok=' . $num_tokens . ',myens_build' . $pipe_db_server_number . 'tok=' . $num_tokens . ']" span[hosts=1]"'
           }, };
