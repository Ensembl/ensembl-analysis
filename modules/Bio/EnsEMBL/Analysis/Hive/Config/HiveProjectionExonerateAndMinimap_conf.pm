...
package HiveProjectionExonerateAndMinimap_conf;
...
######################################################
    ...
    'assembly_name'             => '', # target species assembly name, e.g. 'MUSP714'
    'assembly_accession'        => '', # target species assembly accession, e.g. 'GCA_003336285.1'
    'assembly_refseq_accession' => '', # target species refseq accession (it can be empty)
    'species_name'              => '', # scientific name of the target species to project genes to, e.g. 'mus_spicilegus'
    'production_name'           => '' || $self->o('species_name'), # e.g. 'mus_spicilegus'
    ...
    'projection_source_production_name' => '', # e.g. 'mus_musculus',

    # this database must be the pipeline db unless lastz was run for these species and you are reusing the pipeline db which was created at
    # the time and it contains the relevant lastz tables 
    'projection_compara_db_name'    => '' || $self->o('pipe_db_name'),
    'projection_compara_db_server'  => '' || $self->o('pipe_db_server'),
    'projection_compara_db_port'    => '' || $self->o('pipe_db_port'),
    ...
    'faidx_genome_file'             => catfile($self->o('output_path'),$self->o('species_name').'_toplevel.fa'), # This one is used in replacement of the dna table in the core db, so where analyses override slice->seq. Has simple headers with just the seq_region name. Also used by bwa in the RNA-seq analyses. Not masked. Required by minimap2
...
#######################################################
    ...
    'compara_db_name'       => '', # e.g. XXX_ensembl_compara_95
    'compara_db_server'     => '',
    'compara_db_port'       => ,
...
########################
   ...
   'projection_compara_db' => {
      -dbname => $self->o('projection_compara_db_name'),
      -host   => $self->o('projection_compara_db_server'),
      -port   => $self->o('projection_compara_db_port'),
      -user   => $self->o('user_r'),
      -pass   => $self->o('password_r'),
      -driver => $self->o('hive_driver'),
    },
...
########################################################################
...

      {
        ...
        -rc_name    => 'default',
        -flow_into  => {
          '1->A' => ['exonerate_create_projection_input_ids','minimap_create_projection_input_ids'],#['cesar_create_projection_input_ids','wga_create_projection_input_ids'],
...

      {
        ...
        -hive_capacity => 900,
        -flow_into => {
                        -3 => ['exonerate_failed_projection'],
                      },
      },

      {
        -logic_name => 'exonerate_failed_projection',
...

      {
        ...
        -hive_capacity => 900,
        -flow_into => {
                        -3 => ['minimap_failed_projection'],
                      },
      },

      {
        -logic_name => 'minimap_failed_projection',
        -module     => 'Bio::EnsEMBL::Hive::RunnableDB::Dummy',
        -parameters => {
                       },
        -rc_name          => 'default',
...
######################################################################################
...

      {
        ...
        -parameters => {
                        ...
                        registry_file => $self->o('registry_path'),
        },
        -flow_into => {
...
sub resource_classes {
  ...
  return {
    ...
    '2GB_lastz' => { LSF => [$self->lsf_resource_builder('production-rh74', 2000, [$self->default_options->{'pipe_db_server'}, $self->default_options->{'dna_db_server'}], [$self->default_options->{'num_tokens'}]), '-reg_conf '.$self->default_options->{registry_path}]},
    ...
    '4GB_lastz' => { LSF => [$self->lsf_resource_builder('production-rh74', 4000, [$self->default_options->{'pipe_db_server'}, $self->default_options->{'dna_db_server'}], [$self->default_options->{'num_tokens'}]), '-reg_conf '.$self->default_options->{registry_path}]}, 
    ...
    '6GB_registry' => { LSF => [$self->lsf_resource_builder('production-rh74', 6000, [$self->default_options->{'pipe_db_server'}, $self->default_options->{'dna_db_server'}], [$self->default_options->{'num_tokens'}]), '-reg_conf '.$self->default_options->{registry_path}]},
    ...
    '8GB_lastz' => { LSF => [$self->lsf_resource_builder('production-rh74', 8000, [$self->default_options->{'pipe_db_server'}, $self->default_options->{'dna_db_server'}], [$self->default_options->{'num_tokens'}]), '-reg_conf '.$self->default_options->{registry_path}]},
    ...
    '15GB_lastz' => { LSF => [$self->lsf_resource_builder('production-rh74', 15000, [$self->default_options->{'pipe_db_server'}, $self->default_options->{'dna_db_server'}], [$self->default_options->{'num_tokens'}]), '-reg_conf '.$self->default_options->{registry_path}]},
    ...
    'project_transcripts' => { LSF => $self->lsf_resource_builder('production-rh74', 7200, [$self->default_options->{'pipe_db_server'}, $self->default_options->{'projection_db_server'}, $self->default_options->{'projection_compara_db_server'}, $self->default_options->{'dna_db_server'}], [$self->default_options->{'num_tokens'}])},
