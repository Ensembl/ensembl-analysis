...
##########################################################################
        ...
        'dna_dbname'    => '',
        'blast_dbname'  => $self->o('dbowner').'_'.$self->o('pipeline_name').'_blast',
        'refine_dbname' => $self->o('dbowner').'_'.$self->o('pipeline_name').'_refine',
        'rough_dbname'  => $self->o('dbowner').'_'.$self->o('pipeline_name').'_rough',
	...
	'rnaseq_ftp_base' => 'ftp://ftp.sra.ebi.ac.uk/vol1/fastq/',
        ...
        'ensembl_code_base'    => '', #You may need to set this to the directory containing your Ensembl APIs
        ...
        'rnaseq_summary_file' => '', # You need to specify the full path to your csv summary file, must be in the format created by Bio::EnsEMBL::Analysis::Hive::RunnableDB::HiveDownloadCsvENA
        ...
        # 'picard_lib_jar' => 'picard.jar', # modified 03/01/2017, pipeline failed to find picard
        'picard_lib_jar' => '/nfs/software/ensembl/RHEL7/linuxbrew/Cellar/picard-tools/2.6.0/libexec/picard.jar',
        'short_read_aligner' => 'bwa', #You may need to specify the full path to the bwa binary
        'refine_ccode_exe' => '/nfs/production/panda/ensembl/genebuild/bin/RefineSolexaGenes ', #You may need to specify the full path to the RefineSolexaGenes binary

        # Blast database for comparing the final models to.
	uniprotdb => '/hps/nobackup/production/ensembl/genebuild/blastdb/uniprot/uniprot_2018_01/PE12_vertebrata',

        # Index for the blast database.
        uniprotindex => '/hps/nobackup/production/ensembl/genebuild/blastdb/uniprot/uniprot_2018_01/uniprot_index/',
        ...
        splicing_aligner => '/nfs/software/ensembl/RHEL7/linuxbrew/opt/exonerate09/bin/exonerate', # changed from exonerate-0.9.0 - osagie 01/2017
...
=cut
	return {
        %{ $self->SUPER::resource_classes() },  # inherit other stuff from the base class
      '1GB' => { LSF => $self->lsf_resource_builder('production-rh7', 1000, [$self->default_options->{'pipe_db_server'}])},
      ...
      '2GB' => { LSF => $self->lsf_resource_builder('production-rh7', 2000, [$self->default_options->{'pipe_db_server'}])},
      '4GB' => { LSF => $self->lsf_resource_builder('production-rh7', 4000, [$self->default_options->{'pipe_db_server'}, $self->default_options->{'dna_db_server'}])},
      '5GB' => { LSF => $self->lsf_resource_builder('production-rh7', 5000, [$self->default_options->{'pipe_db_server'}])},
      ...
      '5GB' => { LSF => $self->lsf_resource_builder('production-rh7', 5000, [$self->default_options->{'pipe_db_server'}])},
      '10GB' => { LSF => $self->lsf_resource_builder('production-rh7', 10000, [$self->default_options->{'pipe_db_server'}])},
