...
sub default_options {
  ...
  return {
    ...
    #BRAKER
    'augustus_config_path' => '/nfs/production/panda/ensembl/ftricomi/singularity_test/test_short_genome/config',
    'augustus_species_path' => '/nfs/production/panda/ensembl/ftricomi/singularity_test/test_short_genome/config/species/',
    'cores' => 30,
    'genome_file' => '',
    #working_dir???
    ...
    'num_threads' => 20,
    #'dbowner' => 'fergal',
    'base_output_dir' => '/hps/nobackup2/production/ensembl/ftricomi/braker_subpipe/',
    'protein_file'              => '',
    'busco_protein_file'        => '',
    'rfam_accessions_file'      => '',
    'registry_file'             => '/hps/nobackup2/production/ensembl/ftricomi/gbiab_plants/Databases.pm',
...
#      {
#        -logic_name => 'load_windowmasker_repeats',
#        -module => 'Bio::EnsEMBL::Hive::RunnableDB::SystemCmd',
#        -parameters => {
#                         cmd => 'perl '.catfile($self->o('enscode_root_dir'), 'ensembl-common', 'scripts', 'load_repeats_from_file.pl').
#                                      ' -dbname #core_dbname#'.
#                                      ' -host '.$self->o('dna_db_server').
#                                      ' -port '.$self->o('dna_db_port').
#                                      ' -user '.$self->o('user').
#                                      ' -pass '.$self->o('password').
#                                      ' -repeat_logic_name windowmasker'.
#                                      ' -genome_file_dir #output_path#'.
#                                      ' -assembly_accession #assembly_accession#',
#                       },
#        -rc_name => '10GB',
#        -flow_into => {
#          1 => ['dump_toplevel_file'],
#        },
#      },
...
######################BRAKER###########
      {
        ...
        -rc_name => 'default',
        -flow_into  => {
          1 => {'run_gbiab' => {'inputspecies' => '#species_name#'}},
          2 => {'fan_braker_ep_setup' => {'inputspecies' => '#species_name#'}},
        },
      },
      {
      ...
      },
      {
        ...
        -module => 'Bio::EnsEMBL::Hive::RunnableDB::SystemCmd',
        -parameters => {
                         cmd => 'if [ -d "'. $self->o('augustus_species_path').'#inputspecies#" ]; then exit 0; else exit 42;fi',
                         return_codes_2_branches => {'42' => 2},
                       },
        -rc_name => 'default',
        -flow_into  => {
          1 => {'run_braker_etp_mode' => {'output_path' => '#output_path#', 'reheadered_toplevel_genome_file' => '#reheadered_toplevel_genome_file#', 'species_name' => '#species_name#'}},
         '2->A' => {'run_braker_ab_initio' => {'output_path' => '#output_path#', 'reheadered_toplevel_genome_file' => '#reheadered_toplevel_genome_file#', 'species_name' => '#species_name#'}},
          'A->2' => {'run_braker_etp_mode' => {'output_path' => '#output_path#', 'reheadered_toplevel_genome_file' => '#reheadered_toplevel_genome_file#', 'species_name' => '#species_name#'}},
        },
      },
      {
        ...
        -module => 'Bio::EnsEMBL::Hive::RunnableDB::SystemCmd',
        -parameters => {
                         cmd => 'if [ -d "'. $self->o('augustus_species_path').'#inputspecies#_PIPE" ]; then exit 0; else exit 42;fi',
                         return_codes_2_branches => {'42' => 2},
                       },
        -rc_name => 'default',
        -flow_into  => {
...
#                             #                                    'mv braker braker_etp_mode',
...

      {
        ...
        -parameters => {
                        cmd => 'python process_braker_gtf.py /hps/nobackup2/production/ensembl/ftricomi/braker_lepidoptera/GCA_905147765.1/braker_etp_mode/braker.gtf new_gtf_GCA_905147765v1.gtf #output_path#;'.     
                               'singularity exec --bind #output_path#/:/data:rw  '.$self->o('braker_singularity_image').' braker.pl --genome=#reheadered_toplevel_genome_file# --softmasking --esmode --species=#species_name#_PIPE  --AUGUSTUS_CONFIG_PATH= '. $self->o('augustus_config_path').' --cores '.$self->o('cores').';'.
                               'mv braker braker_etp_mode;',
                             },
        -rc_name => 'default',
