...
sub run_analysis {
  ...
  throw( $program . " is not executable GenBlast::run_analysis " ) unless ( $program && -x $program );
  ...
  my $command = $program .
    ' -p genblastg ' . ' -q ' . $self->query . ' -t ' . $self->database . ' -o ' . $self->query . ' -cdna -pro   ' . $self->options;
...
sub parse_results {
  ...
  open( OUT, "<" . $results ) or throw( "FAILED to open " . $results . "GenBlast:parse_results" );
...
LINE: while (<OUT>) {
    ...
    if (/transcript|coding_exon/i) {
      my @elements = split;
      if ( @elements != 9 ) {
        throw( "Can't parse " . $_ . " splits into wrong number of elements " . "GenBlast:parse_results" );
      }
    ...
    } ## end if (/transcript|coding_exon/i)
  } ## end LINE: while (<OUT>)
