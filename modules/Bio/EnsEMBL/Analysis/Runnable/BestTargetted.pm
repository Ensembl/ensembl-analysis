...
sub new {
  ...
  my ( $biotypes, $seqfetcher, $verbose, $genes, $keep_single_analysis, $cluster_on_coding_exons ) = rearrange( [ qw(
        BIOTYPES
        SEQFETCHER
        VERBOSE
        GENES
...
OUTER: foreach my $type (@$biotypes) {
  ...
  if ( scalar( @match_pep == 1 ) ) {
    ...
    if ( scalar(@doubles) > 0 ) {
      ...
      print "Keeping first gene " . $match_pep[0]->dbID . " from array as we prefer its analysis (" . $match_pep[0]->biotype . ")\n";
      @best = ( $match_pep[0] );
    }
    ...
    if ( ( $num_genes > 1 ) && ( scalar( keys %tln_groups ) == 1 ) && ( scalar( @doubles < 1 ) ) ) {
      print "All transcripts have the same translated sequence. \nKeeping first gene " .
        $tln_groups{$tln_seq}->[0]->dbID . " from array as we prefer its analysis (" . $tln_groups{$tln_seq}->[0]->biotype . ")\n";
      print "*** Maybe we should exonerate or look at exon structure to decide what to choose?\n";
      @best = ( $tln_groups{$tln_seq}->[0] );
    }
    else {
      print "We have $num_genes genes, " .
        scalar( keys %tln_groups ) . " different sequences " . " and " .
        scalar(@doubles) . " doubles. Let's dig deeper.\n";
      $exonerate = 1;
    }
  }
  ...
  if ( defined $exonerate ) {
      ...
      if ( defined $hash{$protein}{$biotype} ) {
        foreach my $gene ( @{ $hash{$protein}{$biotype} } ) {
          ...
          foreach my $transcript ( @{ $gene->get_all_Transcripts } ) {
            ...
            $scores{ $gene->biotype }{ $gene->dbID }{'ratio'} =
              ( $transcript->translation->length )/( $transcript->end - $transcript->start );
    ...
    if ( scalar(@doubles) == 0 ) {
      ...
      if ( $scores{ $best->biotype }{ $best->dbID }{'ratio'} >= 0.8*( $scores{ $first_best->biotype }{ $first_best->dbID }{'ratio'} ) ) {
        @best = ($best);
        print "ratio OK. Keeping gene " .
          $best->dbID . " biotype " . $best->biotype . " , ratio " . $scores{ $best->biotype }{ $best->dbID }{'ratio'} .
          " vs " . $scores{ $first_best->biotype }{ $first_best->dbID }{'ratio'} . "\n";
      }
      else {
    ...
    } ## end elsif ( scalar(@doubles) ... [ if ( scalar(@doubles) ...)])
    foreach my $b (@best) {
      if ( $scores{ $b->biotype }{ $b->dbID }{'percent_id'} < 90 ) {
        print "FLAG: best_gene " . $b->dbID .
          " has percent_id " . $scores{ $b->biotype }{ $b->dbID }{'percent_id'} . "<90 (start " . $b->start . " end " . $b->end . ")\n";
      }
    }
  }    #if (defined $exonerate)
  return \@best;
} ## end sub get_best_gene
...
GENE: foreach my $gene ( @{ $cluster->get_Genes() } ) {
  ...
  } ## end GENE: foreach my $gene ( @{ $cluster...})
  return \%protein_hash;
} ## end sub cluster_by_protein
  ...
  INNER: foreach my $inner_gene ( @{ $cluster->get_Genes() } ) {
      ...
      if ( $exon_string eq $inner_exon_string ) {
        ...
        print "Genes " . $gene->dbID .
          " (exon string " . $exon_string . ") and " . $inner_gene->dbID . " (exon string $inner_exon_string) do not overlap.\n";
      }    #if ($exon_string eq $inner_exon_string) {
    }    #inner
  }    # genes
  return \%alternates;
} ## end sub get_alternate_transcripts
...
OUTER: foreach my $outer (@ordered) {
  INNER: foreach my $inner (@non_overlapping) {
    ...
    } ## end INNER: foreach my $inner (@non_overlapping)
    if ( $overlaps < 1 ) {
      print "ADDING A DOUBLE\n";
      push @non_overlapping, $outer;
    }
  } ## end OUTER: foreach my $outer (@ordered)
...
# print "index name ".$self->seqfetcher->index_name."\n"; # prints /lustre/work1/ensembl/ba1/cow4/Seq/BestTargetted_proteome_final/proteome.fa
      ...
      print STDERR "Transcript with dbID " . $transcript->dbID . " has " .
        scalar(@tsfs) . " tsfs. " . "Should only have one transcript_supporting_feature\n";
      $pass_checks = 0;
    }
...
sub run_exonerate {
  ...
  my $exonerate =
    Bio::EnsEMBL::Analysis::Runnable::BestTargettedExonerate->new( -program     => $self->program,
