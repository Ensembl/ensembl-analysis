...
sub run {
  ...
  eval {
    open( my $fh, "$command 2>&1 |" ) || $self->throw("Error counting reads");
    while (<$fh>) {
      chomp;
      if ( $_ =~ /(\d+) $fastq/ ) {
        if ($pair_file) {
  ...
  print "Remember to index your genome file something like:\n$indexcommand/gmap_build -d genome -D " .
    $self->genome . " -d " . $self->genome_name . "\n";
  ...
  if ($header) {
    ...
    $readgroup .= ' --read-group-id "' . $kvp{'ID'} . '" '       if $kvp{'ID'};
    $readgroup .= ' --read-group-name "' . $kvp{'SM'} . '" '     if $kvp{'SM'};
    $readgroup .= ' --read-group-library "' . $kvp{'LB'} . '" '  if $kvp{'LB'};
    $readgroup .= ' --read-group-platform "' . $kvp{'PL'} . '" ' if $kvp{'PL'};
  }

  $command = "$program  $options $readgroup -D " . $self->genome . " -d " . $self->genome_name . " " . $self->indir . "/$filename ";
  if ($pair_file) {
  ...
  # exit;
  eval {
    open( my $fh, "$command 2>&1 |" ) || $self->throw("Error running gsnap $@\n");
    while (<$fh>) {
      chomp;
  ...
  eval {
    open( my $fh, "$command 2>&1 |" ) || $self->throw("Error sorting bam $@\n");
    while (<$fh>) {
      chomp;
      print STDERR "SORT: $_\n";
    }
  ...
  eval {
    open( my $fh, "$command 2>&1 |" ) || $self->throw("Error indexing bam $@\n");
    while (<$fh>) {
      chomp;
  ...
  eval {
    open( my $fh, "$command 2>&1 |" ) || $self->throw("Error checking alignment $@\n");
    while (<$fh>) {
      print STDERR "$_";
