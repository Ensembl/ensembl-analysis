...
sub new {
  ...
  my ( $protein_file, $protein_length, $max_intron_length, $min_coverage ) =
    rearrange( [ 'PROTEIN_FILE', 'PROTEIN_LENGTHS', 'MAX_INTRON_LENGTH', 'MIN_COVERAGE' ], @args );
...
sub protein_lengths {
  my ( $self, $arg ) = @_;
  if ($arg) {
    throw( "Runnable::Pmatch::protein_lengths must be passed a hashref not a " . $arg ) unless ( ref($arg) eq 'HASH' );
...
sub make_protein_length_hash {
  my ($self) = @_;
  my %hash;
  my $seqio = Bio::SeqIO->new( -format => 'Fasta', -file => $self->protein_file );
...
sub run_analysis {
  ...
  throw( $program . " is not executable Pmatch::run_analysis " ) unless ( $program && -x $program );
  my $command =
    $self->program . " -D " . $self->options . " " . $self->protein_file . " " . $self->queryfile . " > " . $self->resultsfile;
...
sub parse_results {
  ...
  foreach my $list (@lists) {
    ...
    foreach my $id ( keys %hits ) {
      my $pmf = new Bio::EnsEMBL::Analysis::Tools::Pmatch::First_PMF( -plengths     => $self->protein_lengths,
  ...
  foreach my $hit (@all_merged_hits) {
    ...
    my $fp = $ff->create_feature_pair( $start,  $end,  $hit->strand, $hit->coverage,
                                       $hstart, $hend, 1,            $hit->target,
                                       100,     0,     $hit->query,  $self->query,
                                       $self->analysis );
