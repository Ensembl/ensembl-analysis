...
sub new {
  ...
  my ( $workdir, $multi_fasta_file, $tree_string, $taxon_species_tree, $semphy_exe, $treebest_exe, $parameters, $options ) =
    rearrange(
               [ 'WORKDIR',    'MULTI_FASTA_FILE', 'TREE_STRING', 'TAXON_SPECIES_TREE',
                 'SEMPHY_EXE', 'TREEBEST_EXE',     'PARAMETERS',  'OPTIONS' ],
               @args );
...
sub run_treebest_2x {
  ...
  #Run treebeset
  my $command =
    $self->treebest_exe . " phyml -Snf $species_tree_file " .
    $self->multi_fasta_file . " | " . $self->treebest_exe . " sdi -rs $species_tree_file - > $tree_file";
...
sub run_semphy_2x {
  ...
  #Run semphy directly
  my $command = $self->semphy_exe .
    " --treeoutputfile=" . $tree_file . " -a 4 --hky -J -H -S --ACGprob=0.300000,0.200000,0.200000 --sequence=" . $self->multi_fasta_file;
...
sub rearrange_multi_fasta_file {
  ...
  close MFA;
} ## end sub rearrange_multi_fasta_file
