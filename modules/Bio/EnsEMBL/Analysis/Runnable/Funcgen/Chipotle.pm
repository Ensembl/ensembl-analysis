...
sub run_analysis {
  ...
  throw( $program . " is not executable Chipotle::run_analysis " ) unless ( $program && -x $program );
  ...
  my $command = join( ' ',
                      $self->program,       'get_peaks',            '0',             '0',
                      $self->infile(),      $self->resultsfile(),   $param->{alpha}, $param->{stepSize},
...
sub write_infile {
  ...
  if ( $replicates > 1 ) {
    ...
    foreach my $rset ( sort keys %{ $self->result_features } ) {
      ...
      open( DAT, ">$tmpfile" ) or throw("Can't open file $tmpfile");

      map { printf DAT "%d\t%s\t%d\t%d\t%f\n", $i++, (@$_)[ 0 .. 3 ]; } @{ $self->result_features->{$rset} };

      close DAT or throw("Can't close file $tmpfile");

      push( @tmpfiles, $tmpfile );

    }

    my $command = join( ' ', $self->program, 'merge_files', '0', $replicates, @tmpfiles, $filename );
    ...
    open( F, ">" . $filename ) or throw("Can't open file $filename.");
