...
sub run_analysis {
  ...
  throw( $program . " is not executable Nessie::run_analysis " ) unless ( $program && -x $program );
  ...
  my $command =
    $self->program . ' --data="' . $self->infile() . '" ' . '--replicates=' . $replicates . $nessie_parameters . ' > ' . $outfile;

  warn( "Running analysis " . $command . "\n" );
  eval { system($command) };
  throw( "FAILED to run $command: ", $@ ) if ($@);

  my $parser = 'oligo_peaks.pl --sang --autonorm ' . $peak_parameters . ' -l ' . $outfile . ' ' . $self->infile . ' > ' . $resultsfile;
...
sub write_infile {
  ...
  foreach my $rset_name ( sort keys %{ $self->result_features } ) {
    ...
    unless ( -e $datafile ) {
      ...
      open( F, ">" . $datafile ) or throw("Can't open file $datafile.");
...
sub write_filelist {
  ...
  open( F, ">" . $self->infile() ) or throw( "Can't open file " . $self->infile );
