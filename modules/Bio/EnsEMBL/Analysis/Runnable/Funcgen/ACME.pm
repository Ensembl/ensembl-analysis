...
sub write_infile {
  ...
  foreach my $rset_name ( keys %{ $self->result_features } ) {
    ...
    warn( "fname: ", $fname );
    push( @fnames, $fname );
    open( F, ">$workdir/$fname" ) or throw("Can't open file $filename.");
  ...
  open( R, ">" . $Rfile ) or throw("Can't open file $Rfile.");
...
sub run_analysis {
  ...
  throw( $program . " is not executable ACME::run_analysis " ) unless ( $program && -x $program );
