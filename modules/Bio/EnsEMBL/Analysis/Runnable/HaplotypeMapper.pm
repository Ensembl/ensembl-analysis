...
sub new {
  ...
  my ( $hap_slice, $slice, $input_id ) = $self->_rearrange( [qw(HAP_SLICE SLICE INPUT_ID)], @args );

  $self->throw("Must input a reference slice and a haplotype slice to HaplotypeProjection")
    unless ( defined($slice) && defined($hap_slice) );
  $self->{_final_genes} = [];
  $self->{_gene_types}  = [];
...
sub create_alignment {
  ...
  my $target_top_slice =
    $self->ensembl_db->get_SliceAdaptor->fetch_by_region( $target_slice->coord_system->name,
                                                          $target_slice->seq_region_name,
                                                          undef, undef, 1, $target_slice->coord_system->version );

  my $query_top_slice =
    $self->ensembl_db->get_SliceAdaptor->fetch_by_region( $query_slice->coord_system->name,
                                                          $query_slice->seq_region_name,
                                                          undef, undef, 1, $query_slice->coord_system->version );
  ...
  my $run = Bio::EnsEMBL::Analysis::Runnable::Blastz->new(
                                ...
                                -query    => $query_slice,
                                -options  => "C=2 B=0",
                                -database => [ Bio::PrimarySeq->new( -id => $target_slice->seq_region_name, -seq => $target_slice->seq ) ]
  );
  ...
  foreach my $f ( @{ $run->output } ) {
    ...
    printf( "PRELIM: %d %d %d %d %d %d %d\n", $f->start, $f->end, $f->strand, $f->hstart, $f->hend, $f->hstrand, $f->score );
    ...
    printf( "HIT: %s %d %d %d %s %d %d %d (%d, %.2f)\n",
            $f->seqname, $f->start, $f->end, $f->strand, $f->hseqname, $f->hstart, $f->hend, $f->hstrand, $f->score, $f->percent_id );
...
sub trim_hit {
  ...
  foreach my $oh (@others) {
    if ( ( $oh->end >= $h->start and $oh->start <= $h->end ) or ( $oh->hend >= $h->hstart and $oh->hstart <= $h->hend ) ) {
      ...
      my $f1 = $h->restrict_between_positions( $ov_start,  $ov_end,  'SEQ' );
      my $f2 = $h->restrict_between_positions( $ov_hstart, $ov_hend, 'HSEQ' );
      if ( defined $f2 ) {
      ...
      push @ov_regs, { start => $ov_start, end => $ov_end, hstart => $ov_hstart, hend => $ov_hend, };
    } ## end if ( ( $oh->end >= $h->...))
  } ## end foreach my $oh (@others)
  ...
  if (@ov_regs) {
    if ( $ov_regs[0]->{start} > $h->{start} ) {
      my $new_f = $h->restrict_between_positions( $h->start, $ov_regs[0]->{start} - 1, 'SEQ' );
      if ( defined $new_f ) {
        push @cut_feats, $new_f;
      }
    }
    for ( my $i = 1; $i < @ov_regs; $i++ ) {
      my $new_f = $h->restrict_between_positions( $ov_regs[ $i - 1 ]->{end} + 1, $ov_regs[$i]->{start} - 1, 'SEQ' );
      if ( defined $new_f ) {
        push @cut_feats, $new_f;
      }
    }
    if ( $ov_regs[-1]->{end} < $h->{end} ) {
      my $new_f = $h->restrict_between_positions( $ov_regs[-1]->{end} + 1, $h->end, 'SEQ' );
      if ( defined $new_f ) {
        push @cut_feats, $new_f;
      }
    }
...
sub make_map_regions {
  ...
  my $tl_target_slice =
    $self->ensembl_db->get_SliceAdaptor->fetch_by_region( $target_slice->coord_system->name, $target_slice->seq_region_name );
  ...
  foreach my $g (@genes) {
    foreach my $t ( @{ $g->get_all_Transcripts } ) {
      foreach my $e ( @{ $t->get_all_Exons } ) {
        push @ex_regs, { start => $e->start, end => $e->end, };
      }
    }
  }
  ...
  foreach my $e (@ex_regs) {
    if ( not @nr_ex_regs or $nr_ex_regs[-1]->{end} < $e->{start} - 1 ) {
      push @nr_ex_regs, $e;
    }
    else {
  ...
  foreach my $bit ( @{ $query_slice->project('seqlevel') } ) {
    if ( not @seq_bits or $seq_bits[-1]->{end} < $bit->from_start - 1 ) {
      push @seq_bits, { start => $bit->from_start, end => $bit->from_end, };
    }
    else {
  ...
  foreach my $f (@ungapped) {
    if ( not @merged_ungapped or $f->start - $merged_ungapped[-1]->end != $f->hstart - $merged_ungapped[-1]->hend ) {
      ...
      foreach my $b (@seq_bits) {
        if ( $bridge_start >= $b->{start} and $bridge_end <= $b->{end} ) {
          $inside_seq = 0;
          last;
        }
      }
  ...
  } ## end foreach my $f (@ungapped)

  foreach my $f (@merged_ungapped) {
    push @assembly_writes, [ $query_dbid, $target_dbid, $f->start, $f->end, $f->hstart, $f->hend, 1 ];
  }

  my $mapping_path = $query_slice->coord_system->name .
    ":" . $query_slice->seq_region_name . "#" . $target_slice->coord_system->name . ":" . $target_slice->coord_system->version;
  ...
  my $contig_mapping_path = $query_slice->coord_system->name . ":" . $query_slice->seq_region_name . "#contig";
  ...
  my $cs = "INSERT into coord_system(name,version,rank,attrib) VALUES(\"chromosome\", \"" .
    $query_slice->seq_region_name . "\", " . $rank_val . ",\" \")";
  push( @mappings, $cs );
  ...
  foreach my $bit (@seq_bits) {
    printf( "# Missing: %d %d (%d)\n", $bit->{start}, $bit->{end}, $bit->{end} - $bit->{start} + 1 );
  }
  ...
  foreach my $el (@unique_assembly_writes) {
    ...
    my $stl =
      "INSERT into assembly values(" . $real_vals[0] . ", " . $real_vals[1] .
      ", " . $real_vals[2] . ", " . $real_vals[3] . ", " . $real_vals[4] . ", " . $real_vals[5] . ", " . $real_vals[6] . ")";
    push( @mappings, $stl );
  ...
  my $sr = "UPDATE seq_region sr, coord_system cs set sr.coord_system_id =cs.coord_system_id where sr.name = cs.version and sr.name =\"" .
...
sub remove_from_list {
  ...
  foreach my $bit (@$bits) {
    if ( $start <= $bit->{end} and $end >= $bit->{start} ) {
      $overlap = $bit;
    }
    else {
      push @keep, $bit;
  ...
  if ( defined $overlap ) {
    if ( $start > $overlap->{start} and $end < $overlap->{end} ) {
      my $new_b = { start => $end + 1, end => $overlap->{end}, };
      push @keep, $new_b;
      $overlap->{end} = $start - 1;
      push @keep, $overlap;
    }
...
sub check_transcript_in_discarded_db {
  ...
  my $discardedslice =
    $self->discarded_db->get_SliceAdaptor->fetch_by_region( 'toplevel', $tran->slice->seq_region_name,
...
DGENE:
  ...
  DTRANS: foreach my $dtran ( @{ $dgene->get_all_Transcripts } ) {
    ...
    } ## end DTRANS: foreach my $dtran ( @{ $dgene...})
  } ## end DGENE: foreach my $dgene ( @{ $discardedslice...})
