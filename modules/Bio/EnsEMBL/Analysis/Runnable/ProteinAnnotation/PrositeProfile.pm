...
sub run_analysis {
  ...
  throw("Failed during Profile run $!\n")
    unless ( system( $self->program . ' -f ' . $self->queryfile . ' ' . $self->database . ' > ' . $self->resultsfile ) == 0 );
...
sub parse_results {
  ...
  while (<$fh>) {
    if (/^\s*(\S+)\s+(\d+)\s*pos\.\s+(\d+)\s+\-\s+(\d+)\s+(\w+)\|/) {
      ...
      my $fp = $self->create_protein_feature( $st, $en, $sc, $seqid, 0, 0, $acc, $self->analysis, 0, 0 );
      push @pfs, $fp;
    }
  }
