...
sub run_analysis {
  ...
  my $cmd = $self->program . ' ' . $self->options . ' ' . '-P ' . '/software/worm/iprscan/bin/binaries/blast/ ' . '-p ' . 'blastp' .
    ' ' . '-d ' . $dbfiles[0] . ' ' . '-s ' . $self->queryfile . ' ' . '-t ' . '/tmp' . ' ' . '-h 0 -f ' . ' >' . $self->resultsfile;
...
#//
  ...
  while (<CPGOUT>) {
    ...
    if ( my ( $id, $start, $end, $hid, $hstart, $hend, $score, $evalue ) =
         /^(\S+)\s+(\d+)\s+(\d+)\s+\S+\s+pd_(\S+);\S+\s+(\d+)\s+(\d+)\s+\S+\s+S=(\S+)\s+E=(\S+)/ )
    {
      my $fp =
        $self->create_protein_feature( $start, $end, $score, $id, $hstart, $hend, $hid, $self->analysis, sprintf( "%.3e", $evalue ), 0 );
