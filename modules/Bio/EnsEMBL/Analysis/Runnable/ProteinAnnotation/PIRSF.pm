...
sub run_analysis {
  ...
  while ( my $seq = $seqio->next_seq ) {
    ...
    my $cmd = $self->program . ' ' . $self->analysis->parameters . ' ' . '-i ' . $filename . ' ' . '>> ' . $self->resultsfile;
    print STDERR "$cmd\n";
    $self->throw( "Error running PIRSF " . $self->program . " on " . $self->queryfile ) unless ( ( system($cmd) ) == 0 );
  }
}
...
sub parse_results {
  ...
  while (<CPGOUT>) {
    ...
    if ( my ( $hid, $start, $end, $hstart, $hend, $score, $evalue ) =
         /^(\S+)\s+\d+\/\d+\s+(\d+)\s+(\d+)\s+\S+\s+(\d+)\s+(\d+)\s+\S+\s+(\S+)\s+(\S+)/ )
    {
