...
sub run_analysis {
  ...
  my $cmd = $self->program . ' ' . $self->analysis->parameters . ' ' . '-i ' . $self->queryfile . ' ' . '> ' . $self->resultsfile;
  print STDERR "$cmd\n";
  $self->throw( "Error running PIRSF_wormbase " . $self->program . " on " . $self->queryfile ) unless ( ( system($cmd) ) == 0 );
