...
sub parse_results {
  ...
  while (<$fh>) {
    ...
    my ( $tag, $q_id, $q_start, $q_end, $q_strand, $t_id, $t_start, $t_end, $t_strand, $score, $perc_id, $q_length, $t_length,
         $gene_orientation, @vulgar_blocks, $total_match_length )
      = split;
    ...
    while (@vulgar_blocks) {
      ...
      #      print "$str\n";
      if ( $str =~ /^3,(\d+),(\d+),I,(\d+),(\d+),5,(\d+),(\d+)/ or $str =~ /^5,(\d+),(\d+),I,(\d+),(\d+),3,(\d+),(\d+)/ ) {
      ...
      $cigar_string .= $query_match_length . $match_type;
    } ## end while (@vulgar_blocks)
    my $hcoverage = $total_match_length/$q_length*100;

    my $feature = $self->make_feature( $q_id,  $q_length, $q_start,      $q_end,     $q_strand,
...
#
sub make_feature {
  my ( $self,    $q_id,  $q_len,    $q_start, $q_end,   $q_strand,     $t_id,      $t_len,
       $t_start, $t_end, $t_strand, $score,   $perc_id, $cigar_string, $hcoverage, $gene_orientation ) = @_;
  ...
  my $obj_name = $self->query_type =~ /dna/i ? "Bio::EnsEMBL::DnaDnaAlignFeature" : "Bio::EnsEMBL::DnaPepAlignFeature";

  my $feature = $obj_name->new( -seqname      => $t_id,
                                -start        => $t_start,
                                -end          => $t_end,
                                -strand       => $t_strand,
                                -hseqname     => $q_id,
