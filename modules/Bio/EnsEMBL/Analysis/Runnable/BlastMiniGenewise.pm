...
sub new {
  ...
  my ( $ids,          $seqfetcher,     $minigenewise_options, $check_repeated,   $full_seq,
       $exonerate,    $exonerate_path, $exonerate_options,    $genewise_options, $blast_parser,
       $blast_filter, $post_filter,    $score_cutoff,         $blastdb,          $mmg_options )
    = rearrange(
    [ qw(IDS SEQFETCHER MINIGENEWISE_OPTIONS
...
sub ids {
  my ( $self, $arg ) = @_;
  if ( defined $arg ) {
    throw("BlastMiniGenewise::ids must be given an arrayref not a $arg") unless ( ref($arg) eq "ARRAY" );
    $self->{'ids'} = $arg;
  }
  return $self->{'ids'};
}
...
sub blast_filter {
  my ( $self, $arg ) = @_;
  if ( defined $arg ) {
    throw( $arg . " must have method filter_results" ) unless ( $arg->can("filter_results") );
    $self->{blast_filter} = $arg;
  }
  if ( !$self->{blast_filter} ) {
    $self->{blast_filter} = Bio::EnsEMBL::Analysis::Tools::FeatureFilter->new( -max_pvalue => 100, -prune => 1, -coverage => 100, );
  }
...
sub minigenewise_options {
  my ( $self, $arg ) = @_;
  if ( defined $arg ) {
    throw( "BlastMiniGenewise::minigenewise_options must be given an hasref" . "not a $arg" ) unless ( ref($arg) eq "HASH" );
...
sub multiminigenewise_options {
  my ( $self, $arg ) = @_;
  if ( defined $arg ) {
    throw( "BlastMiniGenewise::multiminigenewise_options must be given an hasref" . "not a $arg" ) unless ( ref($arg) eq "HASH" );
...
sub genewise_options {
  my ( $self, $arg ) = @_;
  if ( defined $arg ) {
    throw( "BlastMiniGenewise::genewise_options must be given an hasref" . "not a $arg" ) unless ( ref($arg) eq "HASH" );
...
sub seqfetcher {
  my ( $self, $value ) = @_;
  if ($value) {
    throw( "BlastMiniGenewise, seqfetcher must be a " . "Bio::EnsEMBL::Pipeline::SeqFetcher" )
...
sub exonerate_path {
  ...
  throw( $self->{'exonerate_path'} . " is not executable" ) if ( $self->{'exonerate_path'} && !( -x $self->{'exonerate_path'} ) );
...
sub blast_parser {
  my ( $self, $value ) = @_;
  if ($value) {
    throw( $value . " must have a method called parse_file" ) unless ( $value->can("parse_file") );
  ...
  if ( !$self->{blast_parser} ) {
    ...
    $self->{blast_parser} =
      Bio::EnsEMBL::Analysis::Tools::FilterBPlite->new( -regex => $regex, -query_type => "pep", -database_type => "dna", );
  }
  return $self->{blast_parser};
} ## end sub blast_parser

sub blastdb {
  my ( $self, $arg ) = @_;
  if ($arg) {
    throw( $arg . " must be a blastdb object" ) unless ( $arg->isa("Bio::EnsEMBL::Analysis::Tools::BlastDB") );
    $self->{blastdb} = $arg;
  }
  if ( !$self->{blastdb} ) {
    my $blastdb =
      Bio::EnsEMBL::Analysis::Tools::BlastDB->new( -sequences => [ $self->query ], -output_dir => $self->workdir, -mol_type => "DNA", );
    $self->{blastdb} = $blastdb;
  }
  return $self->{blastdb};
}
...
sub run {
  ...
  unless (@$features) {
    print $self->query->name . " has no associated features. " . "Finishing run.\n";
    return;
  }

  my $mg_runnables;

  my @sorted_features = sort { $a->start <=> $b->start || $a->end <=> $b->end } @$features;
  my %hash;
...
sub run_alignment {
  ...
  if ( @$seqs != @{ $self->ids } ) {
    warning(
       "Managed to get only " . scalar(@$seqs) . " sequences from " . scalar( @{ $self->ids } ) . " for alignment run, check indices\n" );
  }

  my $valid_seqs = $self->validate_Sequences($seqs);
  if ( @$valid_seqs != @$seqs ) {
    warning( "Only " . scalar(@$valid_seqs) . " sequences were validated from " . @$seqs . " sequences" );
  }
  ...
  return \@features;
} ## end sub run_alignment

sub run_blast {
  my ( $self, $seq ) = @_;
  my $blastdb_file = $self->blastdb_file;
  my $run = Bio::EnsEMBL::Analysis::Runnable::Blast->new( -query    => $seq,
                                                          -program  => 'wutblastn',
...
sub run_exonerate {
  ...
  my @seqs = @{ $self->get_Sequences };
  if ( @seqs != @{ $self->ids } ) {
    warning( "Managed to get only " . scalar(@seqs) . "  of " . scalar( @{ $self->ids } ) . "for Exonerate run; check indices\n" );
  }
  my @valid_seqs = $self->validate_sequence(@seqs);
  my @sorted_seqs = sort { $a->id cmp $b->id } @valid_seqs;
  foreach my $seq (@sorted_seqs) {
    my $exonerate =
      new Bio::EnsEMBL::Analysis::Runnable::ExonerateTranscript( -program     => $self->exonerate_path,
                                                                 -analysis    => $self->analysis,
                                                                 -query_seqs  => ( [$seq] ),
                                                                 -query_type  => 'protein',
  ...
  } ## end foreach my $seq (@sorted_seqs)
  return \@features;
} ## end sub run_exonerate
...
sub make_object {
  ...
  my $mg = Bio::EnsEMBL::Analysis::Runnable::MultiMiniGenewise->new( -query                => $miniseq,
                                                                     -features             => $features,
  ...
  foreach my $seq (@$seqs) {
    ...
    if ( $sequence !~ /[^acgtn]/i ) {
      ...
      if ( $invalidCharCount/$len > 0.05 ) {
        warning( "Ignoring " . $seq->display_id() . " contains more than 5% ($invalidCharCount) " .
