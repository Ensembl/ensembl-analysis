...
sub new {
  ...
  my ( $workdir, $fasta_file, $tree_file, $constraints_file ) =
    rearrange( [ 'WORKDIR', 'FASTA_FILE', 'TREE_FILE', 'CONSTRAINTS_FILE' ], @args );
...
sub run_mavid {
  ...
  throw( $self->program . " is not executable Mavid::run_analysis " ) unless ( $self->program && -x $self->program );
