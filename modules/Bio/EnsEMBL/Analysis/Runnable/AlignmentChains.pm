...
sub new {
  ...
  my (
    $features, $query_slice, $query_nib_dir, $target_slices, $target_nib_dir, $min_chain_score, $linear_gap, $fa_to_nib, $lav_to_axt,
    $axt_chain,
  ...
  throw("You must supply a hash ref of target sequences with -target_slices")   if not defined $target_slices;
...
sub run {
  ...
  if ( $self->query_nib_dir ) {
    ...
    my $seqio = Bio::SeqIO->new( -format => 'fasta', -file => ">$query_nib_dir/$query_name.fa" );
    ...
    system( $self->faToNib, "$query_nib_dir/$query_name.fa", "$query_nib_dir/$query_name.nib" ) and
      throw("Could not convert fasta file $query_nib_dir/$query_name.fa to nib");
  ...
  if ( $self->target_nib_dir ) {
    ...
    foreach my $nm ( keys %{ $self->target_slices } ) {
      ...
      my $seqio = Bio::SeqIO->new( -format => 'fasta', -file => ">$target_nib_dir/$target_name.fa" );
      $seqio->write_seq($target);
      $seqio->close;

      system( $self->faToNib, "$target_nib_dir/$target_name.fa", "$target_nib_dir/$target_name.nib" ) and
        throw("Could not convert fasta file $target_nib_dir/$target_name.fa to nib");
  ...
  ##############################
  open $fh, ">$lav_file" or throw("could not open lav file '$lav_file' for writing\n");
  $self->write_lav($fh);
  close($fh);
  ...
  ##############################
  system( $self->lavToAxt, $lav_file, $query_nib_dir, $target_nib_dir, $axt_file ) and
    throw("Could not convert $lav_file to Axt format\n");
  unlink $lav_file;
  ...
  system( $self->axtChain, $min_parameter, $linearGap_parameter, $axt_file, $query_nib_dir, $target_nib_dir, $chain_file ) and
    throw("Something went wrong with axtChain\n");
    ...
    foreach my $qstrand ( keys %{ $features{$target} } ) {
      foreach my $tstrand ( keys %{ $features{$target}{$qstrand} } ) {
        ...
        foreach my $reg ( @{ $features{$target}{$qstrand}{$tstrand} } ) {
          ...
          print $fh "}\n";
        } ## end foreach my $reg ( @{ $features...})

        print $fh "x {\n   n 0\n}\n";
      } ## end foreach my $tstrand ( keys ...)
    } ## end foreach my $qstrand ( keys ...)
    print $fh "m {\n   n 0\n}\n#:eof\n";
  } ## end foreach my $target ( sort keys...)
} ## end sub write_lav
    ...
    /^chain\s+(\S.+)$/ and do {
      ...
      while (<$fh>) {
        if (/^(\d+)(\s+\d+\s+\d+)?$/) {
          ...
          my ( $current_q_end, $current_t_end ) = ( $current_q_start + $ungapped - 1, $current_t_start + $ungapped - 1 );

          push @{ $blocks[-1] },
            { q_start => $current_q_start, q_end => $current_q_end, t_start => $current_t_start, t_end => $current_t_end, };
