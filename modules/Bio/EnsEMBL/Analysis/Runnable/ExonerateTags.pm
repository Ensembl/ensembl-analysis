...
sub new {
  ...
  my ( $query_seqs,  $query_file,       $program,         $analysis,  $maxmismatch,    $options, $maxdistance,
       $mindistance, $delete_queryfile, $specificoptions, $splitseqs, $maxhitsallowed, $keep_order )
    = rearrange(
                 [ 'QUERYSEQS',   'QUERY_FILE',  'PROGRAM',         'ANALYSIS',    'MAXMISMATCH', 'OPTIONS',
                   'MAXDISTANCE', 'MINDISTANCE', 'DELETEQUERYFILE', 'SPECOPTIONS', 'SPLITSEQS',   'MAXHITS',
                   'KEEPORDER' ],
                 @args );
...
sub run {
  ...
  my $command =
    $self->program . " " . $self->options . " --querytype " . $self->query_type . " --targettype " .
    $self->target_type . " --query " . $self->query_file . " --target " . $self->target_file . " " . $self->specificoptions;
...
sub parse_results {
  ...
  while ( my $hits = <$fh> ) {
    ...
    chomp $hits;
    my ( $null, $q_id, $q_start, $q_end, $q_strand, $t_id, $t_start, $t_end, $t_strand, $score, $perc_id, $q_length, $t_length,
         $gene_orientation, @vulgar_blocks )
      = split( ' ', $hits );
...
ALLTAGS:
  foreach my $tagsides ( keys %$alltags ) {
    ...
    if ( ( scalar @taglist_L > 100 ) or ( scalar @taglist_R > 100 ) ) {
      print "Not storing tags: too many hits (" . ( scalar @taglist_L ) . "/" . ( scalar @taglist_R ) . ").\n" if ( $self->{'_verbose'} );
      $toomany++;
      next ALLTAGS;
    }
  ...
  OUTER:
    ...
    INNER:
      for ( my $j = 0; $j < ( scalar @taglist_R ); $j++ ) {
        ...
        #save this pair
        print "keep ($i/$j): " . $taglist_L[$i]->{'ditag_id'} . " / " . $taglist_R[$j]->{'ditag_id'} . "\n" if ( $self->{'_verbose'} );
        push( @tokeep_1, $taglist_L[$i] );
        push( @tokeep_2, $taglist_R[$j] );
      } ## end INNER: for ( my $j = 0; $j < (...))
    } ## end OUTER: for ( my $i = 0; $i < (...))
  ...
  } ## end ALLTAGS: foreach my $tagsides ( keys...)
...
sub make_features {
  ...
  foreach my $tempfeature (@$tempfeatures) {
    ...
    my $feature = Bio::EnsEMBL::Map::DitagFeature->new( -slice         => $tempfeature->{'slice_name'},
                                                        -start         => $real_tstart,
                                                        -end           => $real_tend,
                                                        -strand        => $q_strand,
  ...
  } ## end foreach my $tempfeature (@$tempfeatures)
  print "\nhave Ditags:" . ( scalar @ditagFeatures ) . "\n";

  return \@ditagFeatures;
} ## end sub make_features
