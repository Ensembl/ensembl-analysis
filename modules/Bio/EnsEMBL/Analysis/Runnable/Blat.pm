...
sub run {
  ...
  my $cmd = $self->program . " " . $self->database . " " . $self->query . " " . $self->options;
  ...
  open( $blat_output_pipe, "$cmd |" ) ||
    throw( "Error opening Blat cmd <$cmd>." .
           " Returned error $? BLAT EXIT: '" . ( $? >> 8 ) . "'," . " SIGNAL '" . ( $? & 127 ) . "', There was " .
           ( $? & 128 ? 'a' : 'no' ) . " core dump" );
  ...
  unless ( close $blat_output_pipe ) {
    ...
    throw( "Error running Blat cmd <$cmd>. Returned " .
           "error $? BLAT EXIT: '" . ( $? >> 8 ) . "', SIGNAL '" . ( $? & 127 ) . "', There was " .
           ( $? & 128 ? 'a' : 'no' ) . " core dump" );
  }
  $self->output($results);
} ## end sub run
...
sub parse_results {
  ...
  while (<$blat_output_pipe>) {
    ...
    my ( $matches, $mismatches,  $rep_matches, $n_count,  $q_num_insert, $q_base_insert, $t_num_insert, $t_base_insert,
         $strand,  $q_name,      $q_length,    $q_start,  $q_end,        $t_name,        $t_length,     $t_start,
         $t_end,   $block_count, $block_sizes, $q_starts, $t_starts,     $q_seqs,        $t_seqs ) = split;
    ...
    my @query_ends;
    my @target_ends;
    my @reversed_block_sizes;
    my @reversed_q_starts;
    my @reversed_t_starts;
    ...
    for ( my $i = 0; $i < $block_count; $i++ ) {
      ...
      my $this_q_bioseq = Bio::Seq->new( -seq => $q_sequences[$i], -moltype => "dna", -alphabet => 'dna', -id => "q_seq" );
      my $this_t_bioseq = Bio::Seq->new( -seq => $t_sequences[$i], -moltype => "dna", -alphabet => 'dna', -id => "t_seq" );
      if ( not $self->gapped ) {
        # calculate a local score and percent id for the block
        ( $score, $percent_id ) = get_best_score_in_all_frames( $this_q_bioseq, $this_t_bioseq );
      }
...
} ## end sub get_best_score_in_all_frames
...
sub database {
  ...
  if ( defined $val ) {
    if ( not ref($val) ) {
      ...
      if ( ref($val) eq 'ARRAY' ) {
        foreach my $el (@$val) {
          throw("All elements of given database array should be Bio::PrimarySeqs") if not ref($el) or not $el->isa("Bio::PrimarySeq");
        }
      }
