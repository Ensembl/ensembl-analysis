...
sub run {
  ...
  foreach my $query (@$queries) {
    ...
    $self->throw( "Can't run " . $self . " without a query sequence" ) unless ( $self->query );
    $self->checkdir();
    # write the seq file
...
sub run_analysis {
  ...
  open( my $fh, "$command |" ) ||
    $self->throw( "Error opening Infernal cmd <$command>." .
                  " Returned error $? Infernal EXIT: '" . ( $? >> 8 ) . "'," . " SIGNAL '" . ( $? & 127 ) . "', There was " .
                  ( $? & 128 ? 'a' : 'no' ) . " core dump" );
...
sub parse_results {
  ...
  if ( !-e $results ) {
    $self->throw( "Can't parse a  results file that dosen't exist " . $results . " Infernal:parse_results" );
  }
  my @results;
  my ( $hit, $start, $end, $hstart, $hend, $score, $evalue, $strand, $str ) = 0;

  open( INFERNAL, $results ) or $self->throw( "FAILED to open " . $results . " INFERNAL:parse_results" );
LINE: while (<INFERNAL>) {
    chomp;
    if ( $_ =~ /results:/ ) {
      if ( $score && $threshold && $score > $threshold && $align->{'name'} ) {
        $str = $self->parse_structure($align);
        print STDERR "positve result at " .
          $daf->seq_region_name . ":" . $daf->seq_region_start . "-" . $daf->seq_region_end . " strand " . $daf->strand . "\n"
          if $verbose;
        push @results,
          { str    => $str,
            score  => $score,
  ...
  } ## end LINE: while (<INFERNAL>)
  if ( $align->{'name'} && $score > $threshold ) {
    print STDERR "positve result at " .
      $daf->seq_region_name . ":" . $daf->seq_region_start . "-" . $daf->seq_region_end . " strand " . $daf->strand . "\n"
      if $verbose;
    $str = $self->parse_structure($align);
    push @results, { str => $str, score => $score, start => $start, end => $end, strand => $strand };
  }
  close(INFERNAL) or $self->throw( "FAILED to close " . $results . " INFERNAL:parse_results" );
...
sub parse_structure {
  ...
  for ( my $i = 0; $i < scalar( @{ $align->{'str'} } ); $i++ ) {
    if ( $align->{'str'}[$i] eq '(' or $align->{'str'}[$i] eq '<' or $align->{'str'}[$i] eq '[' or $align->{'str'}[$i] eq '{' ) {
      push @stack, $i;
    }
...
sub make_gene {
  ...
  foreach my $result (@$results) {
    ...
    $exon = Bio::EnsEMBL::Exon->new( -start     => $start,
                                     -end       => $end,
                                     -strand    => $daf->strand,
                                     -slice     => $slice,
                                     -phase     => -1,
  ...
  $gene->source("ensembl");
  # XREFS
  my $xref = Bio::EnsEMBL::DBEntry->new( -primary_id  => $domain,
                                         -display_id  => $name,
                                         -dbname      => 'RFAM',
                                         -version     => 0,
                                         -description => $description . " [Source: RFAM;Acc:$domain]", );
  # Use RNA fold to tidy up the structure parsed from cmsearch results
  my $seq = Bio::PrimarySeq->new( -display_id => $domain, -seq => $gene->seq, );
  my $RNAfold = Bio::EnsEMBL::Analysis::Runnable::RNAFold->new( -analysis => $self->analysis, -sequence => $seq, -structure => $str, );
  $RNAfold->run;
  ...
  foreach my $str (@final_str) {
    ...
    my $attribute = Bio::EnsEMBL::Attribute->new( -CODE        => 'ncRNA',
                                                  -NAME        => 'Structure',
...
sub get_sequence {
  ...
  my $seq = Bio::Seq->new( -seq => $daf->seq, -display_id => $domain, );
  # remove padding
  $daf->start($old_start);
  $daf->end($old_end);
...
sub get_descriptions {
  ...
  open( T, $self->analysis->db_file . "/Rfam.seed" ) or $self->throw( "can't file the " . $self->analysis->db_file . "/Rfam.seed file" );
  while (<T>) {
    chomp;
    if ( $_ =~ /^\#=GF AC   (.+)/ ) {
      $domain = $1;
...
sub output {
  ...
  if ($output) {
    throw( "Must pass Runnable:output an hashref not a " . $output ) unless ( ref($output) eq 'HASH' );
