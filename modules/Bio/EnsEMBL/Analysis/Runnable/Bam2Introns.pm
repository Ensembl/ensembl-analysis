...
sub new {
  ...
  my ( $model,   $missmatch, $mask,  $sam_dir,    $bam_file, $percent_id, $coverage,
       $fullseq, $max_tran,  $start, $start_exon, $offset,   $batch_size )
    = rearrange( [qw(MODEL MISSMATCH MASK SAM_DIR BAM_FILE  PERCENT_ID COVERAGE FULLSEQ MAX_TRAN START STARTEXON OFFSET BATCH_SIZE)],
                 @args );
  $self->model($model);
  $self->MASK($mask);
...
#    }
  ...
  if ( $self->count <= $self->BATCH_SIZE ) {
    ...
    my $bioseq = Bio::Seq->new( -seq => $read->query->dna, -display_id => $name, -desc => $rg );
    push( @$reads, $bioseq );
...
sub run {
  ...
  my $seqio = Bio::Seq->new( -seq => $seq, -display_id => $rough->stable_id );
