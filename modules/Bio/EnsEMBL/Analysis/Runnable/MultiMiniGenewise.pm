...
sub new {
  ...
  my ( $features, $seqfetcher, $cluster_start, $cluster_end, $full_seq, $genewise_options, $minigenewise_options, $min_length ) =
    rearrange(
    [ qw( FEATURES SEQFETCHER CLUSTER_START CLUSTER_END
  ...
  throw("MultiMiniGenewise needs features to run across") unless ( $self->features && scalar( @{ $self->features } ) );
...
sub minigenewise_options {
  ...
  if ( defined($arg) ) {
    throw("Must pass minigenewise options a hash ref not a $arg") unless ( ref($arg) eq "HASH" );
...
ID: foreach my $id (@$ids) {
    ...
    warning( $id . " has produced no peptide sequence from " . $self->seqfetcher ) if ( !$peptide_sequence );
    ...
    if ( $self->cluster_end ) {
      ...
      my $string_seq =
        ( 'N' x ( $self->cluster_start - 1 ) ) . $self->query->subseq( $self->cluster_start, $self->cluster_end ) .
        ( 'N' x ( $self->query->length - ( $self->cluster_end ) ) );
      $slice = Bio::EnsEMBL::Slice->new( -seq             => $string_seq,
    ...
    my $forward_output = $self->run_MiniGenewise( \@forward, $slice, $peptide_sequence ) if ( @forward >= 1 );
    $self->output($forward_output);
    my $reverse_output = $self->run_MiniGenewise( \@reverse, $slice, $peptide_sequence ) if ( @reverse >= 1 );
    $self->output($reverse_output);
    push( @output, @$forward_output ) if ($forward_output);
    push( @output, @$reverse_output ) if ($reverse_output);
  } ## end ID: foreach my $id (@$ids)
  if ( $self->cluster_end ) {
  ...
  #$self->output(\@output);
  if ( $failed_count == @$ids ) {
    throw( "Can't find any sequences for the " . @$ids . " ids which match " . $self->query->name );
  }
  return 1;
} ## end sub run
...
sub run_MiniGenewise {
  ...
  if ( $self->full_seq ) {
    my $rangefeat =
      new Bio::EnsEMBL::FeaturePair( -start => $features->[0]->start, -end => $features->[-1]->end, -strand => 1, -slice => $slice );
    $rangefeat->strand(-1) if ( $features->[0]->strand == -1 );
    $features = [$rangefeat];
  }
  my %params = %{ $self->minigenewise_options } if ( $self->minigenewise_options );
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::MiniGenewise->new( -query            => $slice,
                                                                      -protein          => $peptide,
                                                                      -features         => $features,
...
FEAT: foreach my $f ( @{ $self->features } ) {
    ...
    $scorehash{ $f->hseqname } = $f->score if ( $f->score > $scorehash{ $f->hseqname } );
  }
