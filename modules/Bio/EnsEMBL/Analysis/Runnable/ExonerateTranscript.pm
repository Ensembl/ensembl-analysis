...
sub new {
  ...
  my ($coverage_aligned) = rearrange( [ qw(
        COVERAGE_BY_ALIGNED
        ) ],
    @args );
...
TRANSCRIPT:
  while (<$fh>) {
    ...
    my ( $tag, $q_id, $q_start, $q_end, $q_strand, $t_id, $t_start, $t_end, $t_strand, $score, $perc_id, $q_length, $t_length,
         $gene_orientation, @align_components )
      = split;
    ...
    my ( $exons, %flag ) =
      $self->_parse_vulgar_block( $t_start, $t_end, $t_strand, $t_length, $q_start, $q_end, $q_strand, $q_length, \@align_components );
    ...
    my ( @tran_feature_pairs, $cds_start_exon, $cds_start, $cds_end_exon, $cds_end );

    foreach my $proto_exon (@$exons) {
      if ( $flag{$proto_exon} ) {
        my $link_attrib = Bio::EnsEMBL::Attribute->new( -CODE        => 'lost_frameshift',
      ...
      if (@exon_feature_pairs) {
        ...
        eval {
          if ( $self->query_type eq 'protein' ) {
            $supp_feature = Bio::EnsEMBL::DnaPepAlignFeature->new( -features => \@exon_feature_pairs );
          }
          else {
            $supp_feature = Bio::EnsEMBL::DnaDnaAlignFeature->new( -features => \@exon_feature_pairs );
          }
        };
        if ($@) {
          warning($@);
    ...
    } ## end foreach my $proto_exon (@$exons)

    # Create a single supporting feature for the whole transcript
    my $t_supp_feat;
    eval {
      if ( $self->query_type eq 'protein' ) {
        $t_supp_feat = Bio::EnsEMBL::DnaPepAlignFeature->new( -features => \@tran_feature_pairs );
      }
      else {
        $t_supp_feat = Bio::EnsEMBL::DnaDnaAlignFeature->new( -features => \@tran_feature_pairs );
      }
    };
    if ($@) {
...
sub _parse_vulgar_block {
  my ( $self,        $target_start, $target_end,   $target_strand, $target_length,
       $query_start, $query_end,    $query_strand, $query_length,  $vulgar_components ) = @_;
  ...
  if ( $target_start > $target_end ) {
    warn("For target, start and end are in thew wrong order for a reverse strand match") if $target_strand != -1;
  ...
  if ( $query_start > $query_end ) {
    warn("For query, start and end are in thew wrong order for a reverse strand match") if $query_strand != -1;
  ...
  while (@$vulgar_components) {
    throw( "Something funny has happened to the input vulgar string." . "  Expecting components in multiples of three, but only have [" .
    ...
    throw( "Vulgar string does not start with a match.  Was not " . "expecting this. (Have type $type)" )
    ...
    } ## end elsif ( $type eq "I" or $type... [ if ( $type eq 'M' or $type...)])
  ...
  for ( my $i = 0; $i < @exons; $i++ ) {
    ...
    if ( $self->query_type eq 'dna' ) {
      ...
      foreach my $sf (@ex_sf) {
        my $merged = 0;
        if ( @nr_sf and $sf->{type} eq 'C' and $nr_sf[-1]->{type} eq 'C' and $sf->{query_start} == $nr_sf[-1]->{query_end} + 1 ) {
          if ( $target_strand == -1 ) {
    ...
    $ex->{sf} = \@ex_sf;
  } ## end for ( my $i = 0; $i < @exons...)

  return \@exons, %flag;
} ## end sub _parse_vulgar_block
