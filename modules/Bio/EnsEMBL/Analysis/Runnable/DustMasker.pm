...
use parent qw(Bio::EnsEMBL::Analysis::Runnable::Dust);
...
sub run_analysis{
  ...
  $command .= " -level ".$self->level if($self->level);
  $command .= " -window ".$self->window_size if($self->window_size);
  $command .= ' '.$self->options if ($self->options);
  $command .= " -in ".$self->queryfile." > ".$self->resultsfile;
