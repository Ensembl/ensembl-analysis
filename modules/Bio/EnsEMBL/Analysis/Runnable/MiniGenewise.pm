...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::EvidenceUtils qw(create_feature_from_gapped_pieces);
...
sub new {
  ...
  my ( $protein, $features, $minimum_intron, $terminal_padding, $exon_padding, $max_iterate_dist, $genewise_options ) = rearrange(
    [ qw(PROTEIN FEATURES MINIMUM_INTRON TERMINAL_PADDING EXON_PADDING
        MAX_SPLIT_ITERATE_DIST GENEWISE_OPTIONS) ],
    @args );
  ...
  throw("MiniGenewise needs an array of features") unless ( $self->features && scalar( @{ $self->features } ) );
  return $self;
} ## end sub new
...
sub genewise_options {
  ...
  if ($arg) {
    throw("Must pass genewise options a hash ref not a $arg") unless ( ref($arg) eq "HASH" );
...
sub protein_sequence {
  ...
  if ($value) {
    throw( "Input isn't a Bio::PrimarySeq but " . $value ) unless ( $value->isa("Bio::PrimarySeqI") );
      ...
      foreach my $g (@$gw_output) {
        ...
        foreach my $e ( $g->get_all_Exons ) {
          ...
          if ( @pieces > 1 ) {
            ...
            if ($can_merge) {
              push @bridge_features, Bio::EnsEMBL::Feature->new( -start => $pieces[0]->start, -end => $pieces[-1]->end );
            }
          }
        }
      } ## end foreach my $g (@$gw_output)
      if (@bridge_features) {
        push( @$features, @bridge_features );
        $must_try_again = 1;
        warning(
              $self->protein_sequence->id . " has a predicted feature which splits " . " when mapped back to the genomic, trying again" );
      }
    } ## end if ( $self->max_split_iterate_distance)
  } ## end while ($must_try_again)

  my $output = [];
  foreach my $gene (@$gw_output) {
    my $new_gene = $self->convert_to_genomic($gene);
    warning( "Failed to convert " . $gene . " to genomic coordinates, skipping" ) unless ($new_gene);
...
sub run_Genewise {
  ...
  my $gw = new Bio::EnsEMBL::Analysis::Runnable::Genewise( -query    => $miniseq,
...
TEXON: for ( my $i = 0; $i < $nexon; $i++ ) {
    ...
    if ( !@genomics ) {
      warning(
            id($exon) . " " . coord_string($exon) . " didn't produce any features " . "when converted to genomic coordinates, skipping" );
      return undef;
    }
    if ( @genomics > 1 ) {
      ...
      warning( "Warning : feature converts into " . scalar(@genomics) . " features; " );
      if ( $i == $nexon - 1 ) {
      ...
      my $genomic_exon = create_Exon( $genomics[0]->start, $genomics[0]->end, $exon->phase, $exon->end_phase,
                                      $strand,             $self->analysis,   undef,        undef,
                                      $self->query );
      ...
      foreach my $sf ( @{ $exon->get_all_supporting_features } ) {
        ...
        foreach my $aln (@ungapped) {
          ...
          if ( $#alns > 0 ) {
            warning( "Warning : sub_align feature converts into > 1 features " . scalar(@alns) );
          }
          my $align = create_feature_from_gapped_pieces( "Bio::EnsEMBL::DnaPepAlignFeature",
                                                         \@alns, 100, undef, undef, $self->query, $self->protein_sequence->id,
                                                         $self->analysis );
          push @features, $align;
        }
        push @all_supp_features, @features;

        my $gapped = create_feature_from_gapped_pieces( "Bio::EnsEMBL::DnaPepAlignFeature",
                                                        \@features, 100, undef, undef, $self->query, $self->protein_sequence->id,
                                                        $self->analysis );
        $genomic_exon->add_supporting_features($gapped);
      }
      push( @converted_exons, $genomic_exon );
    } ## end else [ if ( @genomics > 1 ) ]
  } ## end TEXON: for ( my $i = 0; $i < $nexon...)
  ...
  if ( scalar(@all_supp_features) ) {
    my $daf = create_feature_from_gapped_pieces( "Bio::EnsEMBL::DnaPepAlignFeature",
                                                 \@all_supp_features, 100, undef, undef, $self->query, $self->protein_sequence->id,
                                                 $self->analysis );
    $converted_transcript->add_supporting_features($daf);
  }

  if ( $#converted_exons < 0 ) {
    warning( "Odd.  No exons found in MiniGenewise running on " . $self->query->seq_region_name . " " . $self->protein_sequence->id );
    return undef;
  }
  else {
    ...
    $converted_translation->end( $converted_exons[$#converted_exons]->end - $converted_exons[$#converted_exons]->start + 1 );
    foreach my $exon (@converted_exons) {
      $converted_transcript->add_Exon($exon);
    }
  } ## end else [ if ( $#converted_exons...)]

  $converted_transcript->slice( $self->query );

  return $converted_transcript;
} ## end sub convert_to_genomic
...
FEAT: foreach my $f (@features) {
    ...
    $count++;
  } ## end FEAT: foreach my $f (@features)
  ...
  foreach my $f (@genomic_features) {
    ...
    my $fp = $ff->create_feature_pair( $f->start, $f->end, $f->strand, undef, $cdna_start, $cdna_end, 1, $f->seqname . ".cdna",
                                       undef, undef, $self->query->seq_region_name,
                                       $self->query, $self->analysis );
    $pairaln->addFeaturePair($fp);
    $current_coord = $cdna_end + 1;
  }

  my $miniseq = new Bio::EnsEMBL::Analysis::Tools::MiniSeq( -id => 'test', -pairalign => $pairaln );

  $self->miniseq($miniseq);
  return $miniseq;

} ## end sub make_MiniSeq
...
sub is_reversed {
  ...
  if ( !( $self->{_reverse} ) ) {
    ...
    if ( $fcount && $rcount ) {
      warning( "Forward and reverse strand features see to have been " . "passed to MiniGenewise together" );
    }
  }
