...
sub new {
  ...
  my ( $linc_rna_genes, $ensembl_genes ) = rearrange( [qw(linc_rna_genes ensembl_genes )], @args );
...
sub run {
  ...
  print "  Found " .
    scalar(@no_prot_domain_genes) . " lincRNA candidate genes which do not contain protein domains " .
  ...
  print "  Found " .
    scalar(@genes_with_prot_domain) .
    " rejected lincRNA candidate genes which are somehow associated with protein domains.\n";    ##  cDNA DEBUG
  ...
  print "  Up to this stage, out of all lincRNA candidates, " .
    scalar(@$multi_exon_genes) . " is multi-exon and " .
    scalar(@$single_exon_genes) . " is single-exon.\n";
...
MULTI_EX_GENE: foreach my $gene (@$multi_exon_genes) {
  ...
  if ( $self->exclude_single_exon_lincRNAs && $self->exclude_artefact_two_exon_lincRNAs ) {
    ...
    print "     $artefactual_2ex_gene_cnt two-exon lincRNA candidates discarded.\n";
    print "     " .
      scalar( @{$single_exon_genes} ) . " single-exon and " .
      scalar(@multi_exon_genes_passed) . " multi-exon lincRNA candidates will proceed to stage 2 clustering.\n";
    @filtered_no_prot_domain_genes = ( @{$single_exon_genes}, @multi_exon_genes_passed );
  }
  elsif ( !$self->exclude_single_exon_lincRNAs && !$self->exclude_artefact_two_exon_lincRNAs ) {
    print "     " .
      scalar( @{$single_exon_genes} ) . " single-exon and " .
      scalar( @{$multi_exon_genes} ) . " multi-exon lincRNA candidates will proceed to stage 2 clustering.\n";
  ...
  my ( $step2_clusters, $step2_unclustered ) =
    cluster_Genes( [ @{ $self->ensembl_genes }, @filtered_no_prot_domain_genes ], \%types_hash2 );

  my @lincrna_unclustered = ( @{ get_oneway_clustering_genes_of_set( $step2_clusters,    "NO_PROT_DOM" ) },
                              @{ get_oneway_clustering_genes_of_set( $step2_unclustered, "NO_PROT_DOM" ) } );
  print "\n";

  print "  Found " .
    scalar(@lincrna_unclustered) . " lincRNAs which don't contain protein domains AND don't cluster with any Ensembl genes.\n";
...
CLUSTER: for my $cl ( @{ get_twoway_clusters($step2_clusters) } ) {
    ...
    print "  Looking at a two_way cluster which contains " .
      scalar(@ens_genes) . " Ensembl genes and " .
  ...
  foreach my $pt_gene (@proc_tran_genes_to_update) {
    print "    INFO : This processed_transcript gene clusters with lincRNA: " .
      $pt_gene->dbID . "\t" . $pt_gene->analysis->logic_name . " " . $pt_gene->seq_region_start . "\t" . $pt_gene->seq_region_end .
  ...
  foreach my $ol_gene (@old_lincRNA_genes_to_update) {
    print "    INFO : This existing lincRNA gene clusters with newly-identified lincRNA: " .
      $ol_gene->dbID . "\t" . $ol_gene->analysis->logic_name . " " . $ol_gene->seq_region_start . "\t" . $ol_gene->seq_region_end .
      "\n";    #  This will be printed out regardless of the config value of "PERFORM_UPDATES_ON_SOURCE_PROTEIN_CODING_DB"
  }
  ...
  print "  " .
    scalar( @{ $self->ncrna_clusters_with_single_ens_biotype } ) .
    " rejected as they cluster with Ensembl gene of single biotype (other than proc_trans and lincRNA).\n";
  print "  " .
    scalar( @{ $self->ncrna_clusters_with_multiple_ens_biotypes } ) .
    " rejected as they cluster with Ensembl genes of multiple biotypes (other than proc_trans and lincRNA).\n";
...
sub compare_lincrna_vs_ensembl {
  ...
  return $lincrna;
} ## end sub compare_lincrna_vs_ensembl
...
sub print_gene_info {
  ...
  print "\n\nprocessing potential ncRNA : " .
    $db_id . " [ " . $gene->biotype . " ] " . $sid . "\t" . @{ $gene->get_all_Transcripts } . "exons\n";
}
...
sub create_old_coord_string_for_simple_feature {
  ...
  my $old_coord =
    $sf_name . "_" . $gene->seq_region_name . "_" . $gene->seq_region_start . "_" . $gene->seq_region_end . "_" . $gene->dbID;
  return $old_coord;
}
...
sub get_reference_slice {
  ...
  my $reference_slice = $sa->fetch_by_region( $gene->slice->coord_system->name, $gene->slice->seq_region_name,
...
} ## end sub get_cdnas_on_underlying_slices
...
sub get_cdnas_on_slices_of_mapped_protein_coding_genes {
  ...
  for my $mg (@$all_mapped_prot_coding) {
    ...
    my $mouse_trimmed_slice =
      $cmp_core->get_SliceAdaptor->fetch_by_region( $mouse_gene->slice->coord_system_name(), $mouse_gene->slice->seq_region_name,
                                                    $mouse_gene->seq_region_start,           $mouse_gene->seq_region_end );
    ...
    print "\n";
  } ## end for my $mg (@$all_mapped_prot_coding)
  return [ values %unique_cdnas_overlapping_protein_coding ];
} ## end sub get_cdnas_on_slices_of_mapped_protein_coding_genes
