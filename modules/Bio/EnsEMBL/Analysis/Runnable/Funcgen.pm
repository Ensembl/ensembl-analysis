...
sub new {
  ...
  my ( $result_features, $options, $workdir ) = rearrange( [ 'RESULT_FEATURES', 'OPTIONS', 'WORKDIR' ], @args );
...
sub get_parameters {
  ...
  throw("Object needs to be a Bio::EnsEMBL::Analysis::Runnable::Funcgen") if ( !$self->isa("Bio::EnsEMBL::Analysis::Runnable::Funcgen") );

  my @parameters = split( /\s+/, $self->analysis->parameters() );
  map {
    throw("Parameter $_ has not the correct format.") if ( !m/^(.+)=(.+)/ );
    $parameters{$1} = $2;
  } @parameters;
...
sub infile {
  ...
  if ( !$self->{'infile'} ) {
    $self->{'infile'} = $self->create_filename( $self->analysis->module . '/' . $self->analysis->module, 'dat' );
...
sub output_fields {
  ...
  throw("No outfile field string set in Runnable.") if ( !$self->{'outfile_fields'} );
