  ...
  foreach my $seqname ( keys %partitioned_features ) {
    ...
    if ( (@$clustered_features) && ( $clusters_seem_real > 0 ) ) {
      ...
      print "Multi-gene code has turned itself on.\nProtein sequence is $seqname\nGenomic region is " .
        $self->genomic_sequence->id . "\tLength " . $self->genomic_sequence->length . "\n";
    ...
    GENE:
      foreach my $gene_cluster (@$gene_clusters) {
        ...
        my $runnable =
          $self->make_object( $self->genomic_sequence, $unfiltered_partitioned_features{$seqname}, $cluster_start, $cluster_end );

        push( @runnables, $runnable );

        $runnable_featids{$seqname}++;
      } ## end GENE: foreach my $gene_cluster (@$gene_clusters)

    } ## end if ( (@$clustered_features...))
    else {
      ...
      foreach my $other_cluster (@other_clusters) {
        ...
        if (@$other_cluster) {
          ...
          foreach my $candidate_exon (@$other_cluster) {
            ...
            my $distance = abs( $candidate_exon->start - $seed_exon->start );
            ...
            if ( ( $distance < $closest ) && ( $candidate_exon->hstart > $seed_exon->hstart ) && ( $distance < 20000 ) ) {
        ...
        } ## end if (@$other_cluster)
      } ## end foreach my $other_cluster (...)
      push( @final_gene_clusters, \@gene_cluster );
    } ## end while (@$this_cluster)
  } ## end while ( $remaining_unclustered_exons...)

  return \@final_gene_clusters;
} ## end sub form_gene_clusters
