...
sub new {
  ...
  my ( $tablenamefile, $intronpenalty, $exonpenalty ) = rearrange( [ 'TABLENAMEFILE', 'INTRONPENALTY', 'EXONPENALTY' ], @args );
...
sub run_analysis {
  ...
  throw( $program . " is not executable Genefinder::run_analysis " ) unless ( $program && -x $program );

  my $err_string = $self->filecheck;
  throw("Filecheck found missing files $err_string") if ($err_string);

  my $command =
    $program . ' -tablenamefile ' . $self->tablenamefile . ' -intronPenaltyFile ' .
    $self->intronpenalty . ' -exonPenaltyFile ' . $self->exonpenalty . ' ' . $self->queryfile . ' > ' . $self->resultsfile;
...
sub filecheck {
  ...
  if ( !-e $self->tablenamefile ) {
    $err_string = "Tablenamefile " . $self->tablenamefile . " does not " . "exist\n";
  }
  if ( !-e $self->intronpenalty ) {
    $err_string .= "IntronPenalty " . $self->intronpenalty . " does not" . " exists\n";
  }
  if ( !-e $self->exonpenalty ) {
    $err_string .= "ExonPenalty " . $self->exonpenalty . " does not " . "exist\n";
  }
...
sub parse_results {
  ...
  open( OUT, "<" . $results ) or throw( "FAILED to open " . $results . "Genefinder:parse_results" );
  ...
  my $gene_count = $self->parse_lines( \@forward_lines, $prefix, $ff, 0, 1 );

  $gene_count = $self->parse_lines( \@reverse_lines, $prefix, $ff, $gene_count, -1 );

  close(OUT) or throw( "FAILED to close " . $results . "Genefinder:parse_results" );
...
TRANS: foreach my $transcript (@transcripts) {
  PHASE: foreach my $phase (@phases) {
      my $temp_exons = $self->set_phases( $phase, $transcript->get_all_Exons, );
  ...
  if ( scalar(@checked_transcripts) != scalar(@transcripts) ) {
    throw( "One of the transcripts doesn't translate as there are " .
           @transcripts . " from genefinder but only " . @checked_transcripts . " transcripts which translate" );
  }
...
sub parse_lines {
  ...
  foreach my $line (@$lines) {
    ...
    my $score = $1;
    if ( $line =~ s/\*// && $strand != -1 ) {
      throw( "Transcript is reverse strand but it isn't " . "being set to that" );
    }
    $line =~ s/\*//;
    if ( $strand == -1 ) {
    ...
    foreach my $coord (@values) {
      ...
      my $exon = $ff->create_prediction_exon( $start, $end, $strand, $score, 0, $phase, $exonname, $self->query, $self->analysis );
