...
sub transcript_evidence {
  ...
  foreach my $exon (@exons) {
    ...
    if ( $self->seqlevel ) {
      ...
      foreach my $c ( @{ $self->seqlevel } ) {
        if ( $exon->start <= $c->end and $exon->end >= $c->start ) {
          $overlaps = 1;
          last;
        }
      }
    ...
    if ( defined($prev_exon) ) {
      my $intron = Bio::EnsEMBL::Feature->new( -START => $prev_exon->end + 1, -END => $exon->start - 1, -STRAND => $exon->strand );
      # ignore the intron if it lies completely in a sequence gap;
      my $intron_in_gap = 0;
      if ( $self->seqlevel ) {
        my $overlaps = 0;
        foreach my $c ( @{ $self->seqlevel } ) {
          if ( $intron->start <= $c->end and $intron->end >= $c->start ) {
            $overlaps = 1;
            last;
          }
        }
    ...
    } ## end if ( defined($prev_exon...))

    if ( not $exon_in_gap ) {
      my $seq_feature_exon = Bio::EnsEMBL::Feature->new( -START => $exon->start, -END => $exon->end, -STRAND => $exon->strand );
  ...
  $results = { 'num_introns'        => $num_introns,
...
sub _intersect_list {
  ...
  foreach my $f (@blocks) {
    if ( $f->end < $f1->start ) {
      ...
      push @ret, Bio::EnsEMBL::Feature->new( -start => $start, -end => $end, -strand => $f1->strand );
    }
  }
