...
sub run {
  ...
  my ( $step1_clusters, $step1_unclustered ) =
    cluster_Genes( [ @{$multi_exon_cdna_genes}, @{ $self->set_2_prot_genes } ], \%types_hash, 0, $self->ignore_strand );
  ...
  if ( $self->find_single_exon_candidates ) {
    ...
    ( $single_exon_clustered, $single_exon_unclustered ) =
      cluster_Genes( [ @$single_exon_cdna, @{ $self->set_2_prot_genes } ], \%types_hash, 0, 1 );    # 4th arg = ignore_strand
...
"\nStage 2) cluster all H3 features vs genes in cdna_update_protein_coding clusters to get H3 features which do NOT overlap with the cDNAs...\n";
  ...
  my ( $typref, $genes ) =
    @{ make_types_hash_with_genes( \@cdna_gene_clusters_with_pc, \@efg_sfg, 'cdna_update_protein_coding', 'efg' ) };
  ...
  my @unclustered_efg_genes = (
    ...
    ,    # "stacks" of H3 features in a region without cdna_update_protein_coding clusters
    @{ get_oneway_clustering_genes_of_set( $step2_unclustered, "efg" )
      }    # "standalone" H3 features in a region without cdna_update_protein_coding clusters
  );
...
"\nStage 3) cluster 'good' cDNAs from stage 1 (which did not overlap with protein_coding genes) vs filtered H3 features (from stage 2)...\n\n";
  ...
  my @unclustered_cdna_genes = ( @{ get_oneway_clustering_genes_of_set( $step1_clusters,    "SET_1_CDNA" ) },
  ...
  foreach my $lg ( keys %logicname_2_efgfeat ) {
    ...
    my @unclust_efg = ( @{ get_oneway_clustering_genes_of_set( $step3_clusters,    "unclust_efg" ) },
                        @{ get_oneway_clustering_genes_of_set( $step3_unclustered, "unclust_efg" ) } );

    print "  Got " . @unclust_efg .
      " $lg  features which don't cluster with any cDNAs or protein_coding genes, not even cluster with 'good' cDNAs from stage 1.\n";
...
"\n  There are more than one K36 feature overlapping with cDNA(s).  Now checking if cDNA(s) overlap with multiple K36 features distributed across a genomic region...\n";
  ...
  } ## end foreach my $cdna_dbID ( keys...)

  print "\nThere are altogether " .
    scalar( keys %non_redun_cdnas_clustering_with_efg_only ) . " cDNAs clustering with H3K4me3 and/or H3K36me3.\n";
  ...
  print "  " .
    scalar(@cdnas_overlapping_with_multiple_K36) . " cDNAs overlap with multiple H3K36me3 features but not H3K4me3 features.\n";
...
RG: for my $rg ( @{ $self->result_set } ) {
    ...
    if ( !defined $new_gene->get_all_Transcripts ) {
      print "  Could not compute translation for cDNA: gene dbID " .
        $rg->dbID . " " . $rg->seq_region_name . " " . $rg->seq_region_start . " " . $rg->seq_region_end . "\n";
      next RG;
    }
...
GENES: for my $g (@$genes_with_6f_translations) {
  ...
  } ## end GENES: for my $g (@$genes_with_6f_translations)
  return \@capped_longest_genes;
} ## end sub cap_number_of_translations_per_gene
...
GENES: for my $g (@$capped_longest_genes) {
    for my $t ( @{ $g->get_all_Transcripts } ) {
      ...
      if ( $ratio > $max_trans_length_ratio ) {
        ...
        print
"  LONG_TRANSLATION: cDNA has translation-length to transcript-length ratio ($ratio) higher than max. value allowed in config ($max_trans_length_ratio).\n";
        my $analysis = Bio::EnsEMBL::Analysis->new( -logic_name => 'long_translation_cDNA', );
        $g->analysis($analysis);
...
} ## end sub filter_genes_with_long_translations
...
sub score_cdna_overlap_with_multi_K36 {
  ...
  print "    Got a total of $K36_cluster_cnt clusters after local clustering, " .
    scalar @$clustered . " of which are K36 clusters and " .
    scalar @$unclustered . " are K36 singletons.\n";
    ...
    foreach my $cdna (@$cdna_genes) {
      ...
      foreach my $K36_region (@K36_regions) {
        ...
        if ( $cdna->seq_region_end >= $K36_clust_start && $cdna->seq_region_start <= $K36_clust_end ) {    # overlap
          print "    FOUND MATCH in this region for cDNA: " .
            $cdna->seq_region_start . " -  " . $cdna->seq_region_end . " (dbID " . $cdna->dbID . ")\n";
          $scoring{ $cdna->dbID }++;
        }
      }
    }
  ...
  return keys %scoring;
} ## end sub score_cdna_overlap_with_multi_K36
