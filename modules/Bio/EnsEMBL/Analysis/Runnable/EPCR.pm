...
sub new {
  ...
  my ( $sts_file, $sts_features, $margin, $word_size, $min_mismatch, $max_mismatch ) =
    rearrange( [ 'STS_FILE', 'STS_FEATURES', 'M', 'W', 'NMIN', 'NMAX' ], @args );
  ######################
  ...
  if ( $sts_file && $sts_features ) {
    throw( "Must pass either an STS_FILE $sts_file or an array of " . "STS_FEATURES $sts_features not both" );
  }
...
sub sts_features {
  my ( $self, $features ) = @_;
  if ($features) {
    throw( "Must pass EPCR sts_features an arrayref not " . $features ) unless ( ref($features) eq 'ARRAY' );
    my $test = $features->[0];
    if ( !$test || !( $test->isa("Bio::EnsEMBL::Map::Marker") ) ) {
      my $err = "arrayref " . $features . " must contain " . "Bio::EnsEMBL::Map::Marker";
...
sub run_epcr {
  ...
  my $command = $self->program . " " . $sts_file . " " . $query_file . " " . $options . " > " . $results;
...
MARKER: foreach my $m (@$sts_features) {
    ...
    my $string =
      $m->dbID . "\t" . $m->left_primer . "\t" . $m->right_primer . "\t" . $m->min_primer_dist . "-" . $m->max_primer_dist . "\n";
    print OUT $string;
  }
  close(OUT) or throw("FAILED to close $filename");
  return $filename;
} ## end sub dump_sts_features
