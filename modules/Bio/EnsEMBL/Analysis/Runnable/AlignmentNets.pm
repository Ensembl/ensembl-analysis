...
sub new {
  ...
  my ( $chains,       $chains_sorted, $query_lengths,       $target_lengths, $chain_net,
       $net_syntenic, $net_filter,    $filter_non_syntenic, $min_chain_score )
    = rearrange( [ qw(
        CHAINS
        CHAINS_SORTED
        QUERY_LENGTHS
  ...
  throw("You must supply a ref to array of alignment chains")           if not defined $chains;
  throw("You must supply a reference to an hash of query seq. lengths") if not defined $query_lengths;
  throw("You must supply a reference to an hash of query seq. lengths") if not defined $target_lengths;
  throw("You must supply the chainNet executable")                      if not defined $chain_net;

  if ( defined $filter_non_syntenic ) {
    throw("You must supply the netSyntenic executable when doing synteny filtering") if not defined $net_syntenic;
    throw("You must supply the netFilter executable when doing synteny filtering")   if not defined $net_filter;
  }
...
sub run {
  ...
  ##############################
  foreach my $el ( [ $query_length_file, $self->query_length_hash ], [ $target_length_file, $self->target_length_hash ] ) {
    my ( $file, $hash ) = @$el;

    open $fh, ">$file" or throw("Could not open seq length file '$file' for writing");
  ...
  if ( not $self->chains_sorted ) {
    for ( my $i = 0; $i < @{ $self->chains }; $i++ ) {
      if ( $self->chains->[$i]->[0]->isa("Bio::EnsEMBL::Compara::GenomicAlignBlock") ) {
        $self->chains->[$i] =
          [ sort { $a->reference_genomic_align->dnafrag_start <=> $a->reference_genomic_align->dnafrag_end } @{ $self->chains->[$i] } ];
      }
      else {
        $self->chains->[$i] = [ sort { $a->start <=> $b->start } @{ $self->chains->[$i] } ];
      }
    }
  }
  ...
  # write chains
  ##############################
  open $fh, ">$chain_file" or throw("could not open chain file '$chain_file' for writing\n");
  ...
  push @arg_list, ( $chain_file, $query_length_file, $target_length_file, $query_net_file, $target_net_file );

  system( $self->chainNet, @arg_list ) and throw("Something went wrong with chainNet");
  ...
  if ( $self->filter_non_syntenic ) {
    ...
    system( $self->netSyntenic, $query_net_file, $syntenic_net_file ) and throw("Something went wrong with netSyntenic");
    open( FILTER, $self->netFilter . " -syn $syntenic_net_file |" ) or throw("Could not run netFilter");
    open( FILTERED, ">$filtered_net_file" ) or throw("Could not open filtered net file for writing");
    while (<FILTER>) {
      print FILTERED $_;
    }
    close(FILTERED);
  ...
  open $fh, $query_net_file or throw("Could not open net file '$query_net_file' for reading\n");
...
sub write_chains {
  ...
  for ( my $chain_id = 1; $chain_id <= @{ $self->chains }; $chain_id++ ) {
    ...
    my ( @ungapped_features, $chain_score, $query_name, $query_strand, $target_name, $target_strand );
    ...
    foreach my $gf (@$chain) {
      if ( not defined $query_name ) {
        ...
        if ( $gf->isa("Bio::EnsEMBL::Compara::GenomicAlignBlock") ) {
          ...
          $query_name = $qga->dnafrag->name, $target_name = $tga->dnafrag->name, $query_strand = $qga->dnafrag_strand;
          ...
          $target_name   = $gf->hseqname, $query_strand = $gf->strand;
      ...
      if ( $gf->isa("Bio::EnsEMBL::Compara::GenomicAlignBlock") ) {
        foreach my $uf ( @{ $gf->get_all_ungapped_GenomicAlignBlocks } ) {
          ...
          my $sens_f = { q_start => $qga->dnafrag_start,
        ...
        foreach my $uf ( $gf->ungapped_features ) {
          ...
          my $sens_f =
            { q_start => $uf->start, q_end => $uf->end, t_start => $uf->hstart, t_end => $uf->hend, len => $uf->end - $uf->start + 1, };
    ...
    print $fh join( " ",
                    ( "chain",     $chain_score,
                      $query_name, $self->query_length_hash->{$query_name},
                      $query_strand == -1 ? "-" : "+", $ungapped_features[0]->{q_start} - 1,
                      $ungapped_features[-1]->{q_end}, $target_name,
                      $self->target_length_hash->{$target_name}, $target_strand == -1 ? "-" : "+",
                      $ungapped_features[0]->{t_start} - 1, $ungapped_features[-1]->{t_end},
                      $chain_id ) ),
      "\n";
    ...
    print $fh "\n";
  } ## end for ( my $chain_id = 1;...)
} ## end sub write_chains
...
sub parse_Net_file {
  ...
  while (<$fh>) {
    ...
    /(\s+)fill\s+(\d+)\s+(\d+)\s+\S+\s+\S+\s+\d+\s+\d+\s+(.+)$/ and do {
      ...
      my ($restricted_fps) = $self->restrict_between_positions( $self->chains->[ $chain_id - 1 ], $q_start, $q_end );
      ...
      if (@$restricted_fps) {
        ...
        if ( $indent > 0 ) {
          ...
          my ( $insert_start, $insert_end ) = ( $last_gap[ $indent - 1 ]->[0], $last_gap[ $indent - 1 ]->[1] );
          ...
          my ($chain1) = $self->restrict_between_positions( $parent_chain, $left_start,     $insert_start - 1 );
          my ($chain2) = $self->restrict_between_positions( $parent_chain, $insert_end + 1, $right_end );

          $new_chains{ $last_parent_chain[ $indent - 2 ] } = [ @$chain1, @$chain2 ];
        } ## end if ( $indent > 0 )
        $last_parent_chain[$indent] = $chain_id;
      } ## end if (@$restricted_fps)
    };
...
sub restrict_between_positions {
  ...
  if ( $first_idx < 0 ) {
    ...
    if (@new_chain) {
      ...
      my $b_start =
        $block->isa("Bio::EnsEMBL::Compara::GenomicAlignBlock") ? $block->reference_genomic_align->dnafrag_start : $block->start;
      my $b_end = $block->isa("Bio::EnsEMBL::Compara::GenomicAlignBlock") ? $block->reference_genomic_align->dnafrag_end : $block->end;
      ...
      $block   = pop @new_chain;
      $b_start = $block->isa("Bio::EnsEMBL::Compara::GenomicAlignBlock") ? $block->reference_genomic_align->dnafrag_start : $block->start;
      $b_end   = $block->isa("Bio::EnsEMBL::Compara::GenomicAlignBlock") ? $block->reference_genomic_align->dnafrag_end : $block->end;
  ...
  } ## end else [ if ( $first_idx < 0 )  [elsif ( $last_idx < 0 ) ]]
...
} ## end sub restrict_between_positions
