...
sub new {
  ...
  my ( $reads, $genomic ) = rearrange( [qw(READS GENOMIC)], @args );
  $self->reads($reads);
...
sub run {
  ...
  foreach my $analysis ( keys %reads_by_analysis ) {
    ...
    my $seqfetcher = Bio::EnsEMBL::Pipeline::SeqFetcher::OBDAIndexSeqFetcher->new( -db => ["$queryseqs/../index"], -format => 'fasta' );
    # sort by name?
    ...
    foreach my $read (@reads) {
      ...
      my $seqa   = Bio::Seq->new( -seq => $seq->seq, -display_id => $name, );
      my $seqb   = Bio::Seq->new( -seq => $seq->seq, -display_id => $name, );
