...
sub new {
  ...
  if ( !$self->query ) {
    throw( "need to have a query defined in order to run protein " . "annotation" );
  }
  if ( !$self->analysis ) {
    throw( "need to have a analysis defined in order to run protein " . "annotation" );
  }
  if ( not $self->program ) {
...
sub run {
  ...
  if ( -s $self->query and not $self->multiprotein ) {
    ...
    if ( -s $self->query ) {
      ...
      throw( "Can't run if " . $self->query . " isn't either a Bio::PrimarySeq  " . " or a file which has a size greater than 0" );
    }
...
sub get_individual_protein_files {
  ...
  while ( my $tmpseq = $in->next_seq() ) {
    ...
    my $filename = $self->create_filename( $self->analysis->logic_name, $stub . "seq" );
...
sub create_protein_feature {
  my ( $self, $start, $end, $score, $seqname, $hstart, $hend, $hseqname, $analysis, $p_value, $percent_id ) = @_;

  my $fp = Bio::EnsEMBL::ProteinFeature->new( -start      => $start,
                                              -end        => $end,
                                              -hstart     => $hstart,
                                              -hend       => $hend,
...
sub query {
  ...
  if ($seq) {
    if ( ref($seq) and $seq->isa("Bio::PrimarySeqI") ) {
      ...
      throw( "You must provide either a Bio::Seq or a file which " . " exists not $seq" );
    }
  }
...
sub queryfile {
  ...
  if ( not exists $self->{_queryfile} ) {
    $self->{_queryfile} = $self->create_filename( $self->analysis->logic_name . '.seq', 'fa' );
...
sub resultsfile {
  ...
  if ( not exists $self->{_resultsfile} ) {
    $self->{_resultsfile} = $self->create_filename( $self->analysis->logic_name . '.results', 'out' );
