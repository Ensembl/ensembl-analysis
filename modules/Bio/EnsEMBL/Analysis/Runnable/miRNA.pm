...
FAM: foreach my $family ( keys %queries ) {
  DAF: foreach my $daf ( @{ $queries{$family} } ) {
      ...
      my $seq = Bio::PrimarySeq->new( -display_id => $daf->hseqname, -seq => $daf->seq, );
      my $RNAfold = Bio::EnsEMBL::Analysis::Runnable::RNAFold->new( -analysis => $self->analysis, -sequence => $seq );
      $RNAfold->run;
...
sub get_miRNAs {
  my ($self) = @_;
  my %miRNA;
  my $seqIO = Bio::SeqIO->new( -file => $self->analysis->db_file, -format => "embl" );
...
sub run_analysis {
  ...
  foreach my $mature (@$all_mature) {
    ...
    my $slice = Bio::EnsEMBL::Slice->new( -seq_region_name => $daf->hseqname,
                                          -start           => 1,
                                          -end             => $miRNA->length,
                                          -seq             => $seq,
                                          -strand          => 1,
  ...
  } ## end foreach my $mature (@$all_mature)
...
sub get_mature {
  ...
  my $status = "PUTATIVE";
  unless ($miRNA) {
    print STDERR "Unable to locate miRNA fom embl file that corresponds to " . $query->hseqname . "$@ \n";
    return 0;
  }
  ...
  foreach my $sf (@feats) {
    ...
    foreach my $fth (@fth) {
      ...
      my $location = $fth->loc;
      $status = "KNOWN" if ( $fth->field->{"evidence"}[0] && $fth->field->{"evidence"}[0] eq "experimental" );
      if ( $location =~ /(\d+)\.+(\d+)/ ) {
        push @mature, { 'start' => $1, 'end' => $2 };
      }
      else {
        return 0;
      }
...
sub make_gene {
  ...
  # exons
  my $slice = $daf->slice;
  my $exon = Bio::EnsEMBL::Exon->new( -start     => $daf->start,
                                      -end       => $daf->end,
                                      -strand    => $daf->strand,
                                      -slice     => $slice,
                                      -phase     => -1,
  ...
  foreach my $code (@$structure) {
    my $str_attrib = Bio::EnsEMBL::Attribute->new( -CODE        => 'ncRNA',
                                                   -NAME        => 'Structure',
  ...
  foreach my $align (@$aligns) {
    my $miRNA_attrib = Bio::EnsEMBL::Attribute->new( -CODE        => 'miRNA',
                                                     -NAME        => 'Micro RNA',
...
sub display_stuff {
  ...
  print STDERR "Query start end " . $daf->hstart . ":" . $daf->hend . " strand " . $daf->hstrand . "\n";
  print STDERR "Start " . $daf->start . " end " . $daf->end . " Hit end " . $daf->hstart . " hit end " . $daf->hend . "\n";
