...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::EvidenceUtils qw(create_feature create_feature_from_gapped_pieces);
...
sub new {
  ...
  my ( $protein, $memory, $reverse, $endbias, $gap, $ext, $subs, $matrix, $verbose, $hmm, $splice_model ) = rearrange( [ qw(
        PROTEIN
        MEMORY
        REVERSE
        ENDBIAS
  ...
  throw( "Must not have both a hmm " . $self->hmm . " and a protein " . $self->protein ) if ( $self->protein  && $self->hmm );
  throw("Must have a query sequence defined")                                            if ( !$self->query );
...
sub run_analysis {
  ...
  throw( $program . " is not executable Runnable::Genewise::run_analysis " ) unless ( $program && -x $program );
  my $options =
    " -genesf -kbyte " . $self->memory .
    " -ext " . $self->extension . " -gap " . $self->gap . " -subs " . $self->subs . " -m " . $self->matrix . " " . $self->options;
  ...
  logger_info($command);
  print "MMG COMMAND for " . $self->protein->id . " " . $self->query->id . " " . $command . "\n";
...
sub parse_genewise_output {
  ...
  while (<$fh>) {
    ...
    if ( $l[0] eq 'Gene' and /^Gene\s+\d+$/ ) {
      ...
      push @{ $genes[-1]->{exons} }, { ex => $exon, sf => [], };

    } ## end elsif ( $l[0] eq 'Exon' ) [ if ( $l[0] eq 'Gene' and...)]
      ...
      my $fp =
        create_feature( "Bio::EnsEMBL::FeaturePair", $gstart, $gend, $strand, $pstart, $pend, 1, undef, undef, undef, $hit_name, undef,
                        $self->analysis );
    ...
    } ## end elsif ( $l[0] eq 'Supporting') [ if ( $l[0] eq 'Gene' and...)]
  } ## end while (<$fh>)
...
sub protein {
  ...
  if ( defined($arg) ) {
    throw("[$arg] is not a Bio::SeqI or Bio::Seq or Bio::PrimarySeqI")
