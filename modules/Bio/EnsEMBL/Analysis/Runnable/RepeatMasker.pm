...
sub write_seq_file {
  ...
  my $newseq = Bio::PrimarySeq->new( -id => 'QUERY_SEQ', -seq => $self->query->seq );
...
sub run_analysis {
  ...
  throw( $program . " is not executable RepeatMasker::run_analysis " ) unless ( $program && -x $program );
  my $cmd = $self->program . " ";
  ...
  system($cmd) == 0 or throw( "FAILED to run " . $cmd . " RepeatMasker:run_analysis" );
  foreach my $file ( glob $self->queryfile . "*" ) {
    $self->files_to_delete($file);
  }
  if ( !-e $self->resultsfile ) {
    throw( "FAILED to run repeatmasker on " .
           $self->queryfile . " " . $self->resultsfile . " has not been produced " . "RepeatMasker:run_analysis" );
  }
...
sub moved_home {
  ...
  if ( !-d $sethome ) {
    ...
    mkdir $sethome or throw("FAILED to mkdir(HOME=$sethome) for the cache: $!");
  }
...
sub parse_results {
  ...
  open( OUT, "<" . $results ) or throw( "FAILED to open " . $results . "RepeatMasker:parse_results" );
REPEAT: while (<OUT>) {
    ...
    if (/\d+/) {      #ignoring introductory lines
      ...
      my ( $score, $name, $start, $end, $strand, $repeat_name, $repeat_class, $repeatmunge, $repeatmunge2 );
      if ( @columns == 15 ) {
        ( $score, $name, $start, $end, $strand, $repeat_name, $repeat_class ) = @columns[ 0, 4, 5, 6, 8, 9, 10 ];
      }
      elsif ( @columns == 14 ) {
        ( $score, $name, $start, $end, $strand, $repeatmunge, $repeatmunge2 ) = @columns[ 0, 4, 5, 6, 8, 9, 10 ];
      ...
      } ## end elsif ( @columns == 14 )  [ if ( @columns == 15 ) ]
      else {
        throw( "Can't parse repeatmasker output unexpected number " .
               "of columns in the output " . @columns . " in " . $_ . " " . "RepeatMasker:parse_results" );
      }
      my $start_column;
      if ( $strand eq '+' ) {
        $start_column = 11;
      ...
      my $rc = $feature_factory->create_repeat_consensus( $repeat_name, $repeat_class );
      my $rf = $feature_factory->create_repeat_feature( $start, $end, $strand, $score, $repeat_start, $repeat_end, $rc, $self->query );

      $self->output( [$rf] );
    } ## end if (/\d+/)
  } ## end REPEAT: while (<OUT>)
  close(OUT) or throw( "FAILED to close " . $results . "RepeatMasker:parse_results" );
