...
sub run {
  ...
  $command =
    "$program $method $readgroup " . $self->genome . " $outdir/$filename.sai $fastq  \| $samtools  view - -b -S -o $outdir/$outfile.bam ";
  if ($fastqpair) {
    $command =
      "$program $method $readgroup " . $self->genome .
      " $outdir/$filename.sai $outdir/$pairfilename.sai $fastq $fastqpair \| $samtools  view - -b -S -o $outdir/$outfile.bam ";
  }

  print STDERR "Command: $command\n";
  open( my $fh, "$command 2>&1 |" ) || $self->throw("Error processing alignment $@\n");
  while (<$fh>) {
    chomp;
    print STDERR "VIEW: $_\n";
    if ( $_ =~ /truncated file/ ) {
