...
sub transcript {
  my ( $self, $pt ) = @_;
  if ($pt) {
    throw( "BlastGenscanPep:transcript must be a " . "Bio::EnsEMBL::Transcript " ) unless ( $pt->isa("Bio::EnsEMBL::Transcript") );
  ...
  for my $feature (@$features) {
    ...
    foreach my $ugFeature ( $feature->ungapped_features() ) {
      ...
      foreach my $gcoord (@split) {
        ...
        my $fp = $ff->create_feature_pair( $gstart,              $gend,             $gstrand, $feature->score,
                                           $hstart,              $hend,             1,        $feature->hseqname,
                                           $feature->percent_id, $feature->p_value, undef,    $self->query,
                                           $feature->analysis );
        push( @{ $exon_hash{$exon} }, $fp );
      } ## end foreach my $gcoord (@split)
    } ## end foreach my $ugFeature ( $feature...)

    # Take the pieces for each exon and make gapped feature
    foreach my $ex ( keys %exon_hash ) {
      my $dna_align_feature = Bio::EnsEMBL::DnaPepAlignFeature->new( -features => $exon_hash{$ex} );
