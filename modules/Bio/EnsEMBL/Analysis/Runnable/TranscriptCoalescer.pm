...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptUtils
  qw (convert_to_genes print_Transcript print_Transcript_and_Exons Transcript_info );
...
sub new {
  ...
  my ( $all_genes, $evidence_sets, $dnadb, $utils_verbosity ) = rearrange( [ qw(
        ALL_GENES
        EVIDENCE_SETS
        DNADB
        UTILS_VERBOSITY
...
GENE_CLUSTER: foreach my $gene_cluster (@$clusters) {
    if ( $self->{v} ) {
      ...
      print "\n";
      print_cluster_info( $self->query, $gene_cluster );
      print "=" x 80;
      print "\n";
    }
    ...
    if ($tr) {
      ...
      my @non_red_trans = @{ remove_redundant_transcripts( $self->edit_terminal_exons( remove_redundant_transcripts($tr) ) ) };

      print "Stage 2: Number Trans. after term exon edit / removing redundancy : " . scalar(@non_red_trans) . "\n" if $self->{v};
...
GENE_CLUSTERS: for my $gene_cluster (@$clusters2) {
    ...
    if ( exists $sets_in_gene_cluster{est_merged} ) {
      ...
      my @tr_merged = map { @{ $_->get_all_Transcripts } } @{ $gene_cluster->get_Genes_by_Set('est_merged') };
      ...
      if ( @{ $self->{merged_tr} } > 0 ) {
        ...
        @all_merged_est_transcripts = @{ remove_redundant_transcripts( \@all_merged_est_transcripts ) };

        ( $non_overlapping_trans, $removed_trans ) = remove_overlapping_transcripts( \@all_merged_est_transcripts );
        @all_merged_est_transcripts = @$non_overlapping_trans;    # this is ok

        push @tr_to_delete, @$removed_trans if $self->{write_filtered_transcripts};
      } ## end if ( @{ $self->{merged_tr...}})
      else {
        # recover transcripts cause they haven't been merged
        push @all_merged_est_transcripts, @tr_merged;
      }
    } ## end if ( exists $sets_in_gene_cluster...)
    else {
      #print "have no est_merged genes\n" ;
    }
  } ## end GENE_CLUSTERS: for my $gene_cluster (@$clusters2)
  ...
  if ( $self->{adjudicate_simgw_est} ) {
    ...
    #    }
    #  }
  } ## end if ( $self->{adjudicate_simgw_est...})
...
OLD_TR: for my $old_tr (@all_merged_est_transcripts) {
  ...
  OLD_EX: for my $oe ( @{ $old_tr->get_all_Exons } ) {
      ...
      my $ne = new Bio::EnsEMBL::Exon( -START    => $oe->start,
                                       -END      => $oe->end,
                                       -STRAND   => $oe->strand,
                                       -SLICE    => $oe->slice,
  ...
  my @trans_with_tl =
    @{ $self->add_translation_and_trans_supp_features_to_transcripts( \@all_merged_est_transcripts, $self->{min_translation_length} ) };
...
TRANSCRIPT: for my $tr (@$trans) {
  ...
  } ## end TRANSCRIPT: for my $tr (@$trans)
  return \@trans_with_tl;
} ## end sub add_translation_and_trans_supp_features_to_transcripts
...
} ## end sub remove_overlapping_transcripts
        ...
        for my $ce (@candidate_exons) {
          ...
          if ( $ce->prev_exon && $ce->next_exon ) {
            ...
            if ( $ce_start == $tr1_boundary && $ce_end == $tr2_boundary ) {
              ...
              } ## end if ( $tr1->end_Exon->prev_exon...)
            } ## end if ( $ce_start == $tr1_boundary...)
            else {
              print "Boundaries does not match\n" if $self->{v};
            }
          } ## end if ( $ce->prev_exon &&...)
        }    # for candidate exons
      } ## end if ( $tr2->start_Exon->...)
    } ## end unless ( $ex_3prim->next_exon)
    ####
  } ## end if ( @$transcripts_ref...)
...
} ## end sub merge_transcripts_recursion
...
sub merge_transcripts {
  ...
  my $cloned_ex =
    new Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::ExonExtended( -start     => $merge_exon->start,
                                                                     -end       => $merge_exon->end,
  ...
  my $tr_merged =
    new Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptExtended( -BIOTYPE => $new_biotype, -ANALYSIS => $base_trans->analysis,
    );
  ...
  my $dna_align_feat = Bio::EnsEMBL::DnaDnaAlignFeature->new( -features => [$feat_pair], -analysis => $merge_exon->analysis );
...
} ## end sub main_clustering_and_recursion
...
sub start_est_linking {
  ...
  for my $act_start_exon (@$start_exons) {
    ...
    my $t = $self->exon_recursion( $ex_cluster_real_ev,    # all ec with exon ranked > 0
...
sub exon_recursion {
  ...
  if ( $cnt == 0 ) {
    ...
    $init = 1;
    $transcript = new Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptExtended( -BIOTYPE => $tmp_biotype, );
...
ALL_EXONS_IN_EC: for my $te (@all_ex_in_ec) {
  ...
  return;
} ## end sub exon_recursion

sub new_tr {
  my ($old_tr) = @_;
  my $new_tr =
    new Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptExtended( -EXONS   => $old_tr->get_all_Exons(),
...
sub check_if_all_exons_are_overlapped {
  ...
  for my $lge ( @{ $lg->get_all_Exons } ) {
    for my $ste ( @{ $st->get_all_Exons } ) {
      ...
      } ## end else [ if ( $lge->seq_region_start...)]
    } ## end for my $ste ( @{ $st->get_all_Exons...})
  } ## end for my $lge ( @{ $lg->get_all_Exons...})
  #  print "\n\n" ;
  ...
  if ( $test_ex_ok == scalar( @{ $st->get_all_Exons } ) ) {
    ...
    for my $lger ( @{ $lg->get_all_Exons } ) {
      unless ( exists $mark_exon{ $lger->hashkey } ) {
        ...
        if ( $st->seq_region_start <= $lger->seq_region_end && $st->seq_region_end >= $lger->seq_region_end ) {
...
} ## end sub check_if_all_exons_are_overlapped
...
} ## end sub remove_redundant_transcripts

sub print_object {
  my ( $ex, $string ) = @_;
  print "$string :\n" if $string;
  print "srs -- sre:" .
    "\t" . $ex->seq_region_start . "---" . $ex->seq_region_end . "\t" . $ex->seq_region_strand . "\t" . $ex->biotype . "\n";
}
...
sub print_exon {
  ...
  if ( $self->{v} ) {
    ...
    print " " . $hseq_name . "\t" . $ex->seq_region_start . "---" . $ex->seq_region_end .
      "\t" . $ex->seq_region_end . "---" . $ex->seq_region_start . "\t" . $ex->seq_region_strand . "\t" . $ex->biotype . "$string" . "\n";
  }
}
...
sub get_longest_5prim_term_exon_in_exon_cluster_of_this_exon {
  ...
  } ## end for my $exon (@all_exons_in_clust)
  return $longest_term_exon_in_clust;
} ## end sub get_longest_5prim_term_exon_in_exon_cluster_of_this_exon
...
sub get_longest_3prim_term_exon_in_exon_cluster_of_this_exon {
  ...
  return $longest_term_exon_in_clust;
} ## end sub get_longest_3prim_term_exon_in_exon_cluster_of_this_exon
