...
sub project_genes {
  ...
  my $full_query_slice =
    $self->ensembl_db->get_SliceAdaptor->fetch_by_region( $query_slice->coord_system->name,
                                                          ...
                                                          undef, undef, undef, $query_slice->seq_region_name );
  ...
  foreach my $g (@genes) {
    ...
    foreach my $t ( @{ $g->get_all_Transcripts } ) {
      ...
      if ( defined $tt ) {
        ...
        } ## end else [ if ( $self->transcript_is_missed...)]
      } ## end else [ if ( defined $tt ) ]
    } ## end foreach my $t ( @{ $g->get_all_Transcripts...})
...
sub merge_genes {
  ...
  my $gene_query_slice =
    $self->ensembl_db->get_SliceAdaptor->fetch_by_region( "toplevel", $query_slice->seq_region_name,
                                                          $query_slice->start, $query_slice->end, );

  my $output_query_slice =
    $self->output_db->get_SliceAdaptor->fetch_by_region( "toplevel", $query_slice->seq_region_name,
                                                         $query_slice->start, $query_slice->end, );
...
P_GENE: foreach my $p_gene (@projected_genes) {
  ...
  H_GENE: foreach my $h_gene (@haplo_genes) {
      ...
      EXON: foreach my $p_exon (@p_exons) {
          foreach my $h_exon (@h_exons) {
            ...
            if ( $p_exon->seq_region_start < $h_exon->seq_region_end && $p_exon->seq_region_end > $h_exon->seq_region_start ) {
              #WE HAVE A MATCH
              $match = 1;
              last EXON;
            }
      ...
      } ## end if ( $p_gene->strand ==...)
    } ## end H_GENE: foreach my $h_gene (@haplo_genes)
    if ( $match == 1 ) {
    ...
    TRANS: foreach my $p_trans ( @{ $p_gene->get_all_Transcripts } ) {
        ...
        } ## end foreach my $h_trans ( @{ $matched_gene...})
        if ( $p_match == 0 ) {
      ...
      } ## end TRANS: foreach my $p_trans ( @{ $p_gene...})
      ...
      print "GENE WITH ADDED TRANSCRIPT: ", $matched_gene->dbID, " : ", $matched_gene->seq_region_start, " - ",
        $matched_gene->seq_region_end, "\n";
  ...
  } ## end P_GENE: foreach my $p_gene (@projected_genes)
...
sub gene_overlap_percent {
  ...
  } ## end foreach my $p_exon ( @{ $p_gene...})

  return $max_overlap;
} ## end sub gene_overlap_percent
...
sub clean_tables {
  ...
  my $meta_coord = $self->ensembl_db->dbc->prepare(
                      "DELETE mt.* from meta_coord mt, coord_system cs where mt.coord_system_id = cs.coord_system_id and cs.version = ?");
...
sub get_complete_transcript {
  ...
  my $attribute = Bio::EnsEMBL::Attribute->new( -CODE        => 'SourceTran',
  ...
  return $full_t;
} ## end sub get_complete_transcript
...
sub project_transcript_the_hard_way {
  ...
  foreach my $e (@exons) {
    ...
    } ## end else [ if ( defined $te ) ]
  } ## end foreach my $e (@exons)
...
} ## end sub project_transcript_the_hard_way
...
sub fix_coding_phases {
  ...
  } ## end if ( defined( $trans->...))
} ## end sub fix_coding_phases
...
sub log_summarise_projection_by_exon {
  ...
  foreach my $e ( @{ $t->get_all_Exons } ) {
    ...
    if ( defined $te ) {
      ...
      elsif ( $bits[0]->from_start != 1 or $bits[-1]->from_end != $e->length ) {
        $exons_part_missed++;
      }
      else {
        $exons_over_indel++;
  ...
  printf( " %d exons; %d transferred, %d missed, %d part missed, %d over indel\n",
          $exons_total, $exons_transferred, $exons_missed, $exons_part_missed, $exons_over_indel );
} ## end sub log_summarise_projection_by_exon
...
} ## end sub log_compare_transcripts
