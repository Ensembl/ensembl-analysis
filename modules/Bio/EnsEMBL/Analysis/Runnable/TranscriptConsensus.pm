...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranslationUtils qw( clone_Translation compute_translation);
...
sub run {
  ...
  foreach my $coding_cluster ( @{$coding_clusters} ) {
    ...
    if ($verbose) {
      print("\n***\n# add overlapping non_coding genes\n" . "CLUSTER " . $coding_cluster->start() . " " . $coding_cluster->end() . "\n" );
    }
...
SCORE: foreach my $t ( sort { $b->score <=> $a->score } @$transcripts ) {
    ...
    if ($add_utr) {
      ...
      print "Add UTR: Transcript " .
        $transcript->start . ":" . $transcript->end . ":" . $transcript->strand . " " . $transcript->biotype . " "
        if $verbose;
  ...
  while (@genes) {
    ...
    my ( $genes_to_store, $genes_to_recluster ) = $self->_find_genes_to_recluster( \@genes_to_consider );

    printf( "Iteration %d: Cluster of %d models " . "broken up into %d finished models " . "and %d models to reconsider\n",
            ++$iteration,
    ...
    if ( $gene->biotype() eq $good_biotype ) {
      foreach my $transcript ( @{ $gene->get_all_Transcripts() } ) {
        foreach my $exon ( @{ $transcript->get_all_Exons() } ) {
          ...
          push( @{ $exon_coords{$key} }, [ $slice->start(), $slice->end() ] );
        }
      }
    }
  }
        ...
        foreach my $exon ( @{ $transcript->get_all_Exons() } ) {
          ...
          if ( exists( $exon_coords{$key} ) ) {
            foreach my $range ( @{ $exon_coords{$key} } ) {
              if ( ( $slice->start() <= $range->[0] && $slice->end() >= $range->[0] ) ||
                   ( $slice->start() <= $range->[1] && $slice->end() >= $range->[1] ) ||
                   ( $slice->start() >= $range->[0] && $slice->end() <= $range->[1] ) )
              {
                $overlaps = 1;
...
} ## end sub weight_scores_by_cdna_length
...
sub make_transcripts {
  ...
  foreach my $similarity (@similarity_genes) {
    foreach my $trans ( @{ $similarity->get_all_Transcripts } ) {
      ...
      if ( scalar(@start_utr) > 0 ) {
        ...
        foreach my $est (@start_utr) {
          ...
          my $new_transcript = $self->transcript_from_exons( \@exon_array, undef, $collapsed_cluster ) if scalar( @exon_array > 0 );
          if ($new_transcript) {
      ...
      } ## end foreach my $transcript (@partial_transcripts)
    ...
    } ## end foreach my $trans ( @{ $similarity...})
  } ## end foreach my $similarity (@similarity_genes)
  return \@all_transcripts;
} ## end sub make_transcripts
  ...
  if ( scalar( keys %delete ) >= $gene_count - 2 ) {
    ...
    for ( my $g = 0; $g < scalar(@$genes); $g++ ) {
      if ( $filter_singletons && $singletons{$g} && $singletons{$g} >= $filter_singletons ) {
        print "Filtering out " . $genes->[$g]->dbID .
          " " . $genes->[$g]->biotype . " " . $genes->[$g]->start . ":" . $genes->[$g]->end . ":" . $genes->[$g]->strand . " "
          if $verbose;
...
} ## end sub clone_transcript_extended
