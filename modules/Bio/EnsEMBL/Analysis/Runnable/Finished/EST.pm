...
sub new {
  my ( $class, @args ) = @_;
  my $self = bless {}, $class;
  my ( $query, $unmasked, $analysis, $seqfetcher ) = rearrange( [ qw{
        QUERY
        UNMASKED
        ANALYSIS
        SEQFETCHER
...
sub run {
  ...
  my $blast = Bio::EnsEMBL::Analysis::Runnable::Finished::Blast->new( -query    => $self->query,
                                                                      -program  => 'wublastn',
...
DB_FILE: foreach my $db_file ( sort @$db_files ) {
    ...
    $self->run_est_genome_on_strand( -1, $features );
  } ## end DB_FILE: foreach my $db_file ( sort ...)
} ## end sub run
...
sub run_est_genome_on_strand {
  ...
  } ## end while ( my ( $hid, $flist...))
} ## end sub run_est_genome_on_strand
...
sub do_mini_est_genome {
  ...
  my $e2g =
    new Bio::EnsEMBL::Analysis::Runnable::Finished::MiniEst2Genome( '-genomic'  => $self->unmasked,
                                                                    '-features' => $linear,
...
sub _make_seqfetcher {
  ...
  if ( ref($indices) eq "ARRAY" ) {
    $seqfetcher = Bio::EnsEMBL::Pipeline::SeqFetcher::Finished_xdget->new( -db => $indices, -executable => '/software/farm/bin/xdget' );
  }
  else {
