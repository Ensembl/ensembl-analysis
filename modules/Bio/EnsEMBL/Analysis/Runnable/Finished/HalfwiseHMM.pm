...
sub new {
  ...
  my ( $genomic, $features, $hmmfetch, $hmmconvert, $hmmdb, $pfamdb, $memory, $options, $analysis, $program ) = rearrange(
    [ qw(QUERY
        FEATURES
        HMMFETCH
        HMMCONVERT
...
sub pfamDB {
  ...
  if ($dbobj) {
    # set
    $self->{'_pfamDB'} = $dbobj;
    throw("Not a Bio::EnsEMBL::DBSQL::DBAdaptor") unless $self->{'_pfamDB'}->isa("Bio::EnsEMBL::DBSQL::DBConnection");
    return $dbobj;
  }
  else {
...
sub pfam_ls_version {
  ...
  unless ( $self->{'_pfam_ls_version'} ) {
    my $ver = Bio::EnsEMBL::Analysis::Tools::BlastDBTracking::get_db_version_mixin( $self, '_pfam_ls_version', $self->hmmdb );
  }
...
sub create_genewisehmm_individually {
  ...
  } ## end while ( my ( $pfam_id, $strand...))
  throw("finished this bit");
  return undef;
} ## end sub create_genewisehmm_individually
