...
sub new {
  ...
  my ( $genomic, $features, $seqcache, $analysis ) = rearrange(
    [ qw(GENOMIC
        FEATURES
        SEQCACHE
        ANALYSIS) ],
  ...
  $self->throw("No seqcache provided") unless defined($seqcache);
...
FEAT: foreach my $f (@features) {
  ...
  foreach my $f (@genomic_features) {
    ...
    my $tmp = new Bio::EnsEMBL::SeqFeature( -seqname => $f->seqname . '.cDNA', -start => $cdna_start, -end => $cdna_end, -strand => 1 );

    my $fp = new Bio::EnsEMBL::FeaturePair( -feature1 => $f, -feature2 => $tmp );
  ...
  my $miniseq = new Bio::EnsEMBL::Analysis::Tools::MiniSeq( -id => $seqname, -pairalign => $pairaln );
...
sub print_FeaturePair {
  ...
  print( STDERR "FeaturePair is " . $nf->seqname . "\t" . $nf->start . "\t" .
         $nf->end . "\t(" . $nf->strand . ")\t" . $nf->hseqname . "\t" . $nf->hstart . "\t" . $nf->hend . "\t(" . $nf->hstrand . ")\n" );
}
...
sub run_blaste2g {
  ...
  my $eg = new Bio::EnsEMBL::Analysis::Runnable::Finished::Est2Genome( -genomic => $g, -est => $hseq, -analysis => $analysis, );
  ...
  } ## end foreach my $fp ( @{ $eg->output...})
} ## end sub run_blaste2g
