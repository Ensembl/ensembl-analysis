...
sub new {
  ...
  my ( $target, $query_db, $exo_options ) = rearrange( [ qw(
        TARGET
        QUERY_DB
        EXO_OPTIONS
        ) ],
...
sub run {
  ...
  foreach my $q (@query) {
    ...
    my $command = join( '  ',
                        ( "'$program'",      $self->options,   $self->exo_options, ' --querytype ',
                          $self->query_type, ' --targettype ', $self->target_type, ' --query ',
                          $q,                ' --target ',     $target_file ) );

    # Execute command and parse results
    print STDOUT "Running exonerate : $command\n";

    my $exo_fh;
    open( $exo_fh, "$command |" ) or throw("Error opening exonerate command <$command>\n $? $!");
...
sub _parse_vulgar_block {
  ...
  while (@$vulgar_components) {
    ...
    throw( "Vulgar string does not start with a match.  Was not " . "expecting this." )
