...
sub query {
  ...
  if ( defined($arg) ) {
    throw( "Genomic sequence input is not a Bio::SeqI or Bio::Seq or Bio::PrimarySeqI" )
...
sub _align_protein {
  ...
  #unlink $gwfile;
  #unlink $outputfile;
} ## end sub _align_protein

sub write_fasta {
  my ( $self, $genfile ) = @_;
  my $genio = Bio::SeqIO->new( '-file' => ">$genfile", '-format' => 'fasta' );
