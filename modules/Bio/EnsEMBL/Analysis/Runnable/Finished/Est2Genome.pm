...
sub run {
  ...
  {
    my $genOutput = Bio::SeqIO->new( -file => ">$genfile", '-format' => 'Fasta' ) or
      throw("Can't create new Bio::SeqIO from $genfile : $!");
    my $estOutput = Bio::SeqIO->new( -file => ">$estfile", '-format' => 'Fasta' ) or
      throw("Can't create new Bio::SeqIO from $estfile : $!");
  ...
  while (<ESTGENOME>) {
    ...
    if ( $_ =~ /^Segment|^Exon/ ) {
      ...
      my ( $primary, $score, $percent_id, $gen_start, $gen_end, $gen_id, $est_start, $est_end, $est_id ) =
        (split)[ 0, 1, 2, 3, 4, 5, 6, 7, 8 ];
      $self->$primary( -score      => $score,
  ...
  if ( !close(ESTGENOME) ) {
    my $err = ($!) ? "error $!" : sprintf( "%s %d", $? & 127 ? ( signal => $? & 127 ) : ( 'exit code', $? >> 8 ) );
    $self->_deletefiles( $genfile, $estfile );
...
sub Segment {    # named to match output from est2genome
  ...
  my $fp = $self->_create_FeaturePair(
    $p->{-score}, $p->{-percent_id}, $p->{-gen_start}, $p->{-gen_end}, $p->{-gen_id}, $p->{-est_start}, $p->{-est_end}, $p->{-est_id},
    $self->{'_source_tag'}, $self->gen_strand, $self->est_strand,    # est_strand IS stored in the db now!
    $p->{-primary} );
...
sub _add_segment_to_exon {
  ...
  } ## end for ( my $i = 0; $i < $exon_line_count...)
} ## end sub _add_segment_to_exon
...
sub convert_output {
  ...
  foreach my $ex (@exons) {
    ...
    foreach my $g (@genes) {
      if ( $ex->start >= $g->start && $ex->end <= $g->end && $ex->strand == $g->strand && !$added ) {
  ...
  foreach my $sf (@supp_feat) {
    my $added = 0;
    foreach my $e (@exons) {
      if ( $sf->start >= $e->start && $sf->end <= $e->end && $sf->strand == $e->strand && !$added ) {
...
sub _create_FeaturePair {
  ...
  my ( $self, $f1score, $f1percent_id, $f1start, $f1end, $f1id, $f2start, $f2end, $f2id, $f1source, $f1strand, $f2strand, $f1primary ) =
    @_;
  ...
  #create featurepair
  my $fp = new Bio::EnsEMBL::FeaturePair( -feature1 => $feat1, -feature2 => $feat2 );
  return $fp;
} ## end sub _create_FeaturePair

sub _createfeatures {
  my ( $self, $f1score,  $f1start,  $f1end,    $f1id,     $f2start,   $f2end,
       $f2id, $f1source, $f2source, $f1strand, $f2strand, $f1primary, $f2primary ) = @_;
...
sub _diskspace {
  ...
  open DF, "df -k $dir |" || throw( "FAILED to open 'df' pipe " . "Runnable::diskspace : $!\n" );
  my $count  = 0;
  my $status = 1;
  while (<DF>) {
    if ( $count && $count > 0 ) {
