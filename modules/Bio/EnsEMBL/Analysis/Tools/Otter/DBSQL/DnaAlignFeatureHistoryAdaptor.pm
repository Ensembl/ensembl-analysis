...
sub fetch_all_by_seq_region {
  ...
  my $sth =
    $self->prepare( "SELECT align_feature_history_id, " . "seq_region_id, analysis_id, align_feature_id_start, " .
            "align_feature_id_end, db_version, date " . "FROM dna_align_feature_history " .
            "WHERE seq_region_id = ? " . "AND analysis_id = ? " . "AND align_feature_id_start <= ? " . "AND align_feature_id_end >= ? " );
...
} ## end sub fetch_all_by_seq_region
...
sub _columns {
  my $self = shift;
  return ( 'dafh.align_feature_history_id', 'dafh.seq_region_id',        'dafh.analysis_id',
           'dafh.align_feature_id_start',   'dafh.align_feature_id_end', 'dafh.db_version',
           'dafh.date' );
}
...
sub fetch_by_DnaAlignFeature_info {
  ...
  return $dafh;
} ## end sub fetch_by_DnaAlignFeature_info
...
sub _objFromHashref {
  ...
  my $dna_align_feature_history =
    Bio::EnsEMBL::Analysis::Tools::Otter::DnaAlignFeatureHistory->new( -id                     => $rowHash->{align_feature_history_id},
...
sub _objs_from_sth {
  ...
  my ( $align_feature_history_id, $seq_region_id, $analysis_id, $align_feature_id_start, $align_feature_id_end, $db_version, $date );
  $sth->bind_columns( \$align_feature_history_id, \$seq_region_id, \$analysis_id, \$align_feature_id_start, \$align_feature_id_end,
                      \$db_version, \$date );
  ...
  while ( $sth->fetch() ) {
    ...
    push @out,
      Bio::EnsEMBL::Analysis::Tools::Otter::DnaAlignFeatureHistory->new( -id                     => $align_feature_history_id,
