...
sub new {
  ...
  my $self = bless {}, $class;
  my ( $sequences, $sequence_file, $molecule_type, $blast_type, $format_command, $output_dir ) = rearrange(
    [ qw(SEQUENCES
        SEQUENCE_FILE
        MOL_TYPE
        BLAST_TYPE
...
sub sequences {
  ...
  if ($arg) {
    throw( "BlastDB::sequences " . $arg . " must be an array ref " ) unless ( ref($arg) eq "ARRAY" );
...
sub molecule_type {
  ...
  if ($arg) {
    $arg = uc($arg);
    throw( $arg . " must be either DNA or PROTEIN" ) unless ( $arg eq "DNA" || $arg eq "PROTEIN" );
    $self->{mol_type} = $arg;
  }
...
sub create_blastdb {
  ...
  if ( !-e $seq_file ) {
    throw( "Your seqfile " . $seq_file . " does not exist you must have some " . "seq objects defined to dump in it" )
...
sub discover_command {
  ...
  throw( "Format command " . $self->format_command . " must both be defined" ) unless ( $self->format_command );
  return $self->format_command;
} ## end sub discover_command

sub list_dbfiles {
  my ($self) = @_;
  if ( !$self->{index_files} ) {
    opendir( BLASTDB_DIR, $self->temp_dir ) or throw( "BlastDB: Failed to " . "open " . $self->temp_dir );
