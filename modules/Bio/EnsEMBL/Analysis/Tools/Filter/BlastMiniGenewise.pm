...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptUtils qw(convert_to_genes are_strands_consistent are_phases_consistent
...
sub new {
  my ( $class, @args ) = @_;
  my $self = bless {}, $class;
  my ( $max_exon_length, $split_genes,        $slice,      $unwanted_evidence,        $max_intron_length,
       $min_coverage,    $max_low_complexity, $seqfetcher, $single_exon_min_coverage, $min_split_coverage )
    = rearrange(
    [ qw(MAX_EXON_LENGTH SPLIT_GENES SLICE UNWANTED_EVIDENCE
        MAX_INTRON_LENGTH MULTI_EXON_MIN_COVERAGE MAX_LOW_COMPLEXITY
        SEQFETCHER SINGLE_EXON_MIN_COVERAGE MIN_SPLIT_COVERAGE) ],
    @args );
  #Setting defaults
  ...
  throw("Filter::BlastMiniGenewise needs a seqfetcher object to run")  if ( !$self->seqfetcher );
  throw("Filter::BlastMiniGenewise needs a max exon length ")          if ( !defined( $self->max_exon_length ) );
  throw("Filter::BlastMiniGenewise needs a max intron length ")        if ( !defined( $self->max_intron_length ) );
  throw("Filter::BlastMiniGenewise needs a multi exon min coverage")   if ( !defined( $self->min_coverage ) );
  throw("Filter::BlastMiniGenewise needs a single exon min coverage")  if ( !defined( $self->single_exon_min_coverage ) );
  throw("Filter::BlastMiniGenewise needs a max low complexity value ") if ( !defined( $self->max_low_complexity ) );
  throw("Filter::BlastMiniGenewise needs a min split coverage")        if ( !defined( $self->min_split_coverage ) );
...
sub slice {
  my ( $self, $arg ) = @_;
  if ( defined $arg ) {
    throw( $arg . " should be a Bio::EnsEMBL::Slice" ) unless ( $arg->isa("Bio::EnsEMBL::Slice") );
    $self->{slice} = $arg;
  }
  return $self->{slice};
}
...
sub unwanted_evidence {
  my ( $self, $arg ) = @_;
  if ( defined $arg ) {
    throw( $arg . " should be a hashref" ) unless ( ref($arg) eq "HASH" );
...
sub seqfetcher {
  my ( $self, $arg ) = @_;
  if ($arg) {
    throw( "Filter::BlastMiniGenewise " . $arg . " must have a method get_Seq_by_acc" ) unless ( $arg->can("get_Seq_by_acc") );
    $self->{seqfetcher} = $arg;
  }
...
sub accepted_genes {
  ...
  if ($arg) {
    my $temp;
    if ( ref($arg) ne "ARRAY" ) {
      throw( $arg . " must be a Bio::EnsEMBL::Gene object" ) unless ( $arg->isa("Bio::EnsEMBL::Gene") );
      $temp = [$arg];
    }
    else {
      $temp = $arg;
...
sub split_genes {
  ...
  if ($arg) {
    ...
    if ( ref($arg) ne "ARRAY" ) {
      ...
      throw( $arg->[0] . " must be a Bio::EnsEMBL::Gene object" ) unless ( $arg->[0]->isa("Bio::EnsEMBL::Gene") );
      $temp = $arg;
    }
...
GENE: foreach my $gene ( @$genes, @{ $self->split_genes } ) {
    ...
    if ( @transcripts > 1 ) {
      ...
      warning( "Filter::BlastMiniGenewise only works on single transcript " .
               "structures. Will convert " . $gene . " into to one gene one " . "transcript structures" );
...
sub validate_Transcript {
  ...
  unless ( has_no_unwanted_evidence( $transcript, $self->unwanted_evidence ) ) {
    $is_valid++;
...
EXON: foreach my $exon ( @{ $transcript->get_all_Exons } ) {
  ...
  unless ( low_complexity_less_than_maximum( $transcript, $self->max_low_complexity ) ) {
    $is_valid++;
  ...
  if ( @{ $transcript->get_all_Exons } >= 2 ) {
    ...
    unless ( evidence_coverage_greater_than_minimum( $transcript, $evidence, ( $self->min_coverage - 1 ) ) ) {
      $is_valid++;
    ...
    unless ( evidence_coverage_greater_than_minimum( $transcript, $evidence, ( $self->single_exon_min_coverage - 1 ) ) ) {
      $is_valid++;
      my $attrib = $self->get_Attribute("evi_coverage");
      $transcript->add_Attributes($attrib);
    }
  ...
  warning( Transcript_info($transcript) . " $hit_name failed " . $is_valid . " tests out of 9 " . "returning 0" ) if ( $is_valid >= 1 );
  return 0 if ( $is_valid >= 1 );
  return 1;
} ## end sub validate_Transcript

sub get_Transcript_supporting_evidence {
  my ( $self, $transcript, $seqfetcher ) = @_;

  $seqfetcher = $self->seqfetcher if ( !$seqfetcher );
  throw( "Can't get " . $transcript . " transcrpts supporting features without " . "a seqfetcher object" ) if ( !$seqfetcher );
...
sub create_Attribute {
  ...
  my $attrib = Bio::EnsEMBL::Attribute->new( -code => $reason, -name => $reason, -description => $description, -value => $description, );
  return $attrib;
}
