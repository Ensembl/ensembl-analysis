...
sub new {
  ...
  &verbose('WARNING');
  my ( $regex, $query, $target, $analysis ) = rearrange( [ 'REGEX', 'QUERY_TYPE', 'DATABASE_TYPE', 'ANALYSIS' ], @args );
  ######################
...
sub filenames {
  ...
  if ($files) {
    throw( $files . " must be an arrayref BPliteWrapper:filenames " ) unless ( ref($files) eq 'ARRAY' );
...
sub query_type {
  ...
  if ($dtype) {
    $dtype = lc($dtype);
    throw( "Query type must be either dna or pep not " . $dtype ) unless ( $dtype eq 'pep' || $dtype eq 'dna' );
...
sub database_type {
  ...
  if ($dtype) {
    $dtype = lc($dtype);
    throw( "Database type must be either dna or pep not " . $dtype ) unless ( $dtype eq 'pep' || $dtype eq 'dna' );
...
sub output {
  ...
  if ($output) {
    throw( "Must pass and arrayref not a " . $output . " BPliteWrapper:output" ) unless ( ref($output) eq 'ARRAY' );
...
PARSER: foreach my $parser (@$parsers) {
  NAME: while ( my $sbjct = $parser->nextSbjct ) {
      ...
      throw( "Error parsing name from " . $sbjct->name . " check your " . "blast setup and blast headers" ) unless ($name);
...
sub split_hsp {
  ...
  while ( $count <= $#qchars ) {
    ...
    if ( $qchars[$count] ne '-' && $hchars[$count] ne '-' ) {
      ...
      if ( $found == 1 ) {
        my $fp = $self->convert_to_featurepair( $qstart,     $qend,         $qstrand, $qinc,          $hstart,
                                                $hend,       $hstrand,      $hinc,    $name,          $query_seqname,
                                                $hsp->score, $hsp->percent, $hsp->P,  $hsp->positive, $hsp->match );
        push( @tmpf, $fp );
      }
      ...
      $found = 0;
    } ## end else [ if ( $qchars[$count] ne...)]
    $count++;
  } ## end while ( $count <= $#qchars)
  ...
  if ( $fp->hstart > $fp->hend ) {
    throw( "Failed start " . $fp->hstart . " is greater than end " . $fp->hend . " " . "for " . $fp->hseqname . "\n" );
  }
  return $fp;
} ## end sub split_hsp
...
sub convert_to_featurepair {
  ...
  my $fp = $self->feature_factory->create_feature_pair( $tmpqstart, $tmpqend, $qstrand,        $score,    $tmphstart,
                                                        $tmphend,   $hstrand, $name,           $percent,  $pvalue,
                                                        $seqname,   undef,    $self->analysis, $positive, $matches );

  return $fp;

} ## end sub convert_to_featurepair
