...
sub get_slice {
  ...
  my $slice =
    Bio::EnsEMBL::Slice->new( - SEQ_REGION_NAME => "test_slice", - START => 1, - END => 100000, - COORD_SYSTEM => $coord_system );
  return $slice;
}
...
sub parse_gff_and_get_genes {
  ...
  while ( my $line = <GFF> ) {
    ...
    if ( $type =~ m/CDS/ ) {
      my $newexon = new Bio::EnsEMBL::Exon( - start     => $g_start,
                                            - end       => $g_end,
                                            - strand    => $strand,
                                            - slice     => $slice,
                                            - phase     => $g_phase,
  ...
  for my $tid ( sort keys %exon2trans ) {
    ...
    push @all_genes, $gene;
  } ## end for my $tid ( sort keys...)
  return \@all_genes;
} ## end sub parse_gff_and_get_genes
