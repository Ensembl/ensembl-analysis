...
sub new {
  ...
  ( $self->{'NAME'}, $self->{'LENGTH'}, $self->{'FH'}, $self->{'LASTLINE'}, $self->{'PARENT'} ) = rearrange(
    [ qw(NAME
        LENGTH
        FH
        LASTLINE
...
sub nextHSP {
  ...
  my ( $score, $bits );
  if ( $scoreline =~ /\d bits\)/ ) {
    ( $score, $bits ) = $scoreline =~ /Score = (\d+) \((\S+) bits\)/;    # WU-BLAST
  }
  else {
    ( $bits, $score ) = $scoreline =~ /Score =\s+(\S+) bits \((\d+)/;    # NCBI-BLAST
  }
  ...
  while (<$FH>) {
    if ( $_ =~ /^WARNING:|^NOTE:/ ) {
      ...
      my $l2 = <$FH>;
      push @hspline, $l2;    # grab/store the sbjct line
    }
  } ## end while (<$FH>)
