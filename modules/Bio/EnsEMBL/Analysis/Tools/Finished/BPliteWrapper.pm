...
sub new {
  ...
  my ( $regex, $skip_name, $query, $target, $analysis, $coverage, $threshold_type, $threshold, $discard_overlaps ) =
    rearrange(
               [ 'REGEX',          'SKIP_NAME', 'QUERY_TYPE', 'DATABASE_TYPE', 'ANALYSIS', 'COVERAGE',
                 'THRESHOLD_TYPE', 'THRESHOLD', 'DISCARD_OVERLAPS' ],
               @args );
  ######################
...
sub _apply_coverage_filter {
  ...
  foreach my $bin_n (@bin_numbers) {
    ...
    foreach my $top_score ( sort { $b <=> $a } keys %$score_hits ) {
      ...
      } ## end foreach my $hit (@$hits)
    } ## end foreach my $top_score ( sort...)
  } ## end foreach my $bin_n (@bin_numbers)
  print "*** FINISHED PARSING ***\n";

  $self->output( \@output );
} ## end sub _apply_coverage_filter
...
sub _discard_worst_overlapping {
  ...
  return @hsps;
} ## end sub _discard_worst_overlapping
