...
sub new {
  ...
  my ( $sort_func, $cut ) = rearrange( [ 'sort', 'cut', ], @args );
    ...
    foreach my $fam ( keys %{ $res_by_name{$seqname} } ) {
      ...
      foreach my $h (@hits) {
        ...
        foreach my $kh (@hits_so_far) {
          if ( $h->start <= $kh->end and $h->end >= $kh->start ) {
            $overlap = 1;
            last;
          }
        }
    ...
    if ( not $self->cut ) {
      ...
      foreach my $idx ( 1 .. $max ) {
        if ( defined( $hit_string[$idx] ) ) {
          if ( not @domains or $domains[-1]->{end} < $idx - 1 or $hit_string[$idx] != $domains[-1]->{domain} ) {
            push @domains, { start => $idx, end => $idx, domain => $hit_string[$idx], };
          }
