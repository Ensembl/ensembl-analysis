...
sub _initialize {
  ...
  $self->sequence_factory( new Bio::Seq::SeqFactory( -verbose => $self->verbose(), -type => 'Bio::Seq::RichSeqIMGT' ) );

}

sub next_seq {
  my ( $self, @args ) = @_;
  my ( $pseq, $c, $line, $name, $dataclass, $desc, $acc, $seqc, $mol, $div, $date, $comment, @date_arr );
  ...
  $self->throw("EMBL stream with no ID. Not embl in my book") unless $line =~ /^ID\s+\S+/;
...
BEFORE_FEATURE_TABLE: until ( !defined $buffer ) {
    ...
    if (/^DT\s+(.+)$/) {
      ...
      if ( $version =~ /\(Rel\. (\d+), Created\)/xms ) {
        my $release = Bio::Annotation::SimpleValue->new( -tagname => 'creation_release', -value => $1 );
        $annotation->add_Annotation($release);
      }
      elsif ( $version =~ /\(Rel\. (\d+), Last updated, Version (\d+)\)/xms ) {
        my $release = Bio::Annotation::SimpleValue->new( -tagname => 'update_release', -value => $1 );
        $annotation->add_Annotation($release);

        my $update = Bio::Annotation::SimpleValue->new( -tagname => 'update_version', -value => $2 );
  ...
  } ## end BEFORE_FEATURE_TABLE: until ( !defined $buffer )
  ...
  if ( defined($buffer) && $buffer =~ /^FT\s+\S+/ ) {
    until ( !defined($buffer) ) {
      ...
      # process ftunit
      my $feat = $ftunit->_generic_seqfeature( $self->location_factory(), $name );
    ...
    } ## end until ( !defined($buffer))
  } ## end if ( defined($buffer) ...)
  # skip comments
...
sub _read_FTHelper_EMBL {
  ...
  if ( $$buffer =~ /^FT\s{3}(\S+)\s*(\S*)$/ ) {
    ...
    } ## end while ( defined( $_ = $self...))
  } ## end if ( $$buffer =~ /^FT\s{3}(\S+)\s*(\S*)$/)
  else {
    # No feature key
    return;
  }
...
QUAL: for ( my $i = 0; $i < @qual; $i++ ) {
    ...
    if ( defined $value ) {
      ...
      QUOTES: while ( $value !~ /\"$/ or $value =~ tr/"/"/ % 2 ) {    #"
          ...
          my $next = $qual[$i];
          if ( !defined($next) ) {
            $self->warn( "Unbalanced quote in:\n" . join( "\n", @qual ) . "\nAdding quote to close..." . "Check sequence quality!" );
            $value .= '"';
            last QUOTES;
          }
  ...
  } ## end QUAL: for ( my $i = 0; $i < @qual...)

  return $out;
} ## end sub _read_FTHelper_EMBL
