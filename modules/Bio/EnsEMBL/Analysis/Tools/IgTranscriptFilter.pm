...
sub new {
  ...
  my ( $percent_range, $min_percent, $min_score ) = rearrange( [ 'PERCENTRANGE', 'MINPERCENT', 'MINSCORE' ], @args );
  ...
  foreach my $hid ( keys %by_hit ) {
    ...
    foreach my $hit (@entries) {
      if ( $hit->[1]->score >= $best_cov and
           ( $hit->[1]->percent_id >= $best_percent or $best_percent - $hit->[-1]->percent_id <= $self->percent_range ) )
      {
        push @keep, $hit->[0];
      }
    }
