...
sub new {
  ...
  my ( $name,                 $genomic_align_blocks, $transcripts, $from_slice, $to_slices,
       $max_readthrough_dist, $extend_into_gaps,     $add_gaps,    $direct_target_slice, )
    = rearrange( [ qw(
        NAME
  ...
  if ($direct_target_slice) {
    if ( $add_gaps or $extend_into_gaps ) {
      warning( "GeneScaffold: cannot use -direct_target_coords with either of " . "-add_gaps or -extend_into_gaps; unsetting both" );
      $add_gaps         = 0;
      $extend_into_gaps = 0;
    }

    # check that the genomic align blocks will allow this
    $direct_target_slice = _check_direct_target_coordinates( $genomic_align_blocks, $to_slices );
  }

  my $aln_map = _make_alignment_mapper($genomic_align_blocks);

  my ( $gs_seq, $from_mapper, $to_mapper ) =
    _construct_sequence( $genomic_align_blocks, $aln_map,   $coding_transcripts,
                         $from_slice,           $to_slices, $max_readthrough_dist,
                         $extend_into_gaps,     $add_gaps,  $direct_target_slice );

  return undef if not defined $from_mapper;

  #  print "1: ", $direct_target_slice,"\n2: ",$direct_target_slice->length,"\n3: ",length($gs_seq),"\n";

  my $gs_end = $direct_target_slice ? $direct_target_slice->length : length($gs_seq);

  my $self = $class->SUPER::new(
    -coord_system    => Bio::EnsEMBL::CoordSystem->new( -name => $GENE_SCAFFOLD_CS_NAME, -rank => 1 ),
    -seq_region_name => $name,
    -seq => ( $gs_seq eq "" ? undef : $gs_seq ),
    -start => 1,
    -end   => $gs_end, );

  $self->direct_target_slice($direct_target_slice)   if defined $direct_target_slice;
  $self->max_readthrough_dist($max_readthrough_dist) if defined $max_readthrough_dist;
...
sub place_transcript {
  my ( $self, $tran, $add_attributes, $external_db_id ) = @_;
  ...
  my $orig_exon_coords = transcripts_to_coords( $self->alignment_mapper, $FROM_CS_NAME, $tran );
...
# print "ORIGINAL EXON COORDS: First start: ",$orig_exon_coords->[0]->start, " last end: ",$orig_exon_coords->[-1]->end," length: ",$orig_exon_coords->[-1]->end-$orig_exon_coords->[0]->start+1 ,"\n";
    $restricted_range = Bio::EnsEMBL::Mapper::Coordinate->new( $orig_exon_coords->[0]->id,   $orig_exon_coords->[0]->start,
                                                               $orig_exon_coords->[-1]->end, $orig_exon_coords->[0]->strand );
  }
  else {
...
#print "ORIGINAL EXON COORDS: First start: ",$orig_exons[0]->start, " last end: ",$orig_exons[-1]->end," length ",$orig_exons[-1]->end-$orig_exons[0]->start+1,"\n";
    $restricted_range = Bio::EnsEMBL::Mapper::Coordinate->new( $orig_exons[0]->slice->seq_region_name, $orig_exons[0]->start,
                                                               $orig_exons[-1]->end,                   $orig_exons[0]->strand );
  }
...
#print "Single exon original coords:  START: ", $orig_exon->start," END: ", $orig_exon->end," length ",$orig_exon->end-$orig_exon->start+1,"\n";
    my @crds =
      $self->from_mapper->map_coordinates( $orig_exon->slice->seq_region_name, $orig_exon->start, $orig_exon->end, 1, $FROM_CS_NAME );

    foreach my $c (@crds) {
      if ( $c->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {

        #print "Exon Mapper from coordinates: START:", $c->start, " END: ",$c->end," length ",$c->end-$c->start+1,"\n";
        my ($tc) = $self->to_mapper->map_coordinates( $GENE_SCAFFOLD_CS_NAME, $c->start, $c->end, 1, $GENE_SCAFFOLD_CS_NAME );
        ...
        if ( $tc->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
          ...
          my @alncrds = $self->alignment_mapper->map_coordinates( $tc->id, $tc->start, $tc->end, $tc->strand, $TO_CS_NAME );
          ...
          foreach my $ac (@alncrds) {
            ...
            my $newc = Bio::EnsEMBL::Mapper::Coordinate->new( $c->id, $current_start, $current_end, $c->strand );
      ...
      } ## end if ( $c->isa("Bio::EnsEMBL::Mapper::Coordinate"...))
      else {
        push @all_coords, $c;
      }
    } ## end foreach my $c (@crds)
  } ## end foreach my $orig_exon (@orig_exons)

  my @kept_coords;
  foreach my $c (@all_coords) {
    if ( $c->isa("Bio::EnsEMBL::Mapper::Coordinate") and exists $filled_coords{$c} ) {
      ...
      my ($oc) = $self->from_mapper->map_coordinates( $GENE_SCAFFOLD_CS_NAME, $c->start, $c->end, 1, $GENE_SCAFFOLD_CS_NAME );
      if ( $oc->start >= $restricted_range->start and $oc->end <= $restricted_range->end ) {
        push @kept_coords, $c;
      }
      else {
  ...
  GAP: foreach my $idx (@gap_indices) {
      ...
      if ($frameshift) {
        ...
        if ($left_surplus) {
          ...
          if ( $left_coord->length > $bases_to_remove ) {
            ...
            $proc_coords[ $idx - 1 ] = Bio::EnsEMBL::Mapper::Gap->new( 1, $left_coord->length );
          }
        }
        if ($right_surplus) {
          ...
          if ( $right_coord->length > $bases_to_remove ) {
            ...
            # we need to eat away the whole of this coord
            $proc_coords[ $idx + 1 ] = Bio::EnsEMBL::Mapper::Gap->new( 1, $right_coord->length );
          }
        }
    ...
    } ## end GAP: foreach my $idx (@gap_indices)
  ...
  my $sl = $self->direct_target_slice ? $self->direct_target_slice : $self;

  foreach my $coord (@all_coords) {
    push @new_exons, Bio::EnsEMBL::Exon->new( -start => $coord->start, -end => $coord->end, -strand => $tran->strand, -slice => $sl );
  }
  ...
  foreach my $exon (@new_exons) {
    ...
    if ( $extent_end > $extent_start ) {
      ...
      my @gen_coords =
        $self->from_mapper->map_coordinates( $GENE_SCAFFOLD_CS_NAME, $extent_start, $extent_end, 1, $GENE_SCAFFOLD_CS_NAME );
      ...
      foreach my $g_coord (@gen_coords) {
        ...
        if ( $g_coord->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
          ...
          my ($p_coord) = $tran->genomic2pep( $g_coord->start, $g_coord->end, $tran->strand );
          ...
          my $fp = Bio::EnsEMBL::FeaturePair->new( -seqname        => $self->seq_region_name,
                                                   -start          => $cur_gs_start,
                                                   -end            => $cur_gs_end,
                                                   -strand         => $exon->strand,
                                                   -score          => 100.0,
                                                   ...
                                                   -slice          => $sl );
          push @fps, $fp;
        } ## end if ( $g_coord->isa("Bio::EnsEMBL::Mapper::Coordinate"...))

        $cur_gs_start += $g_coord->length;
      } ## end foreach my $g_coord (@gen_coords)
    ...
    } ## end if ( $extent_end > $extent_start)
    else {
  ...
  foreach my $exon (@new_exons) {
    if (@merged_exons) {
      ...
      if ( $tran->strand < 0 ) {
        ...
        if ( $intron_len % 3 == 0 and $intron_len <= $self->max_readthrough_dist ) {
      ...
      if ( defined $new_start and defined $new_end ) {
        ...
        if (@ug_feats) {
          ...
          $new_sup_feat->percent_id( ( ( $len_l*$pid_l ) + ( $len_r*$pid_r ) )/( $len_l + $len_r ) );
          $merged_exon->add_supporting_features($new_sup_feat);
        }

        push @merged_exons, $merged_exon;
        next;
      } ## end if ( defined $new_start...)
      else {
  ...
  my $perc_id_attr = Bio::EnsEMBL::Attribute->new( -code        => 'HitSimilarity',
                                                   -name        => 'hit similarity',
                                                   -description => 'percentage id to parent transcripts',
                                                   -value => sprintf( "%.1f", 100*( $transcript_identical_aas/$transcript_aligned_aas ) )
  );
  push @attributes, $perc_id_attr;

  my $cov_attr = Bio::EnsEMBL::Attribute->new( -code        => 'HitCoverage',
                                               -name        => 'hit coverage',
                                               -description => 'coverage of parent transcripts',
                                               -value       => sprintf( "%.1f", 100*( $transcript_aligned_aas/length($source_pep) ) ) );
  push @attributes, $cov_attr;

  my $gap_attr = Bio::EnsEMBL::Attribute->new( -code        => 'PropNonGap',
                                               -name        => 'proportion non gap',
                                               -description => 'proportion non gap',
                                               -value       => sprintf( "%.1f", $prop_non_gap ) );
  push @attributes, $gap_attr;

  my $prop_non_gap_of_coverage_attr = Bio::EnsEMBL::Attribute->new(
                                          -code        => 'NonGapHCov',
                                          -name        => 'proportion non gap of hit coverage',
                                          -description => 'proportion non gap of hit coverage',
                                          -value => sprintf( "%.1f", 100*( ( $transcript_aligned_aas - $num_gaps )/length($source_pep) ) )
  );
  push @attributes, $prop_non_gap_of_coverage_attr;

  if ( $start_not_found and $tran->strand > 0 or $end_not_found and $tran->strand < 0 ) {
    my $attr = Bio::EnsEMBL::Attribute->new( -code        => 'StartNotFound',
                                             -name        => 'start not found',
                                             -description => 'start not found',
                                             -value       => 1 );
    push @attributes, $attr;
  }
  if ( $end_not_found and $tran->strand > 0 or $start_not_found and $tran->strand < 0 ) {
    my $attr =
      Bio::EnsEMBL::Attribute->new( -code => 'EndNotFound', -name => 'end not found', -description => 'end not found', -value => 1 );
    push @attributes, $attr;
  }

  my $stop_attr = Bio::EnsEMBL::Attribute->new( -code  => 'NumStops',
  ...
  foreach my $e ( @{ $proj_tran->get_all_Exons } ) {
    my ($coord) = $self->to_mapper->map_coordinates( $GENE_SCAFFOLD_CS_NAME, $e->start, $e->end, 1, $GENE_SCAFFOLD_CS_NAME );
    if ( $coord->isa("Bio::EnsEMBL::Mapper::Gap") ) {
      $gap_exons++;
    }
  }
  my $gap_exon_attr = Bio::EnsEMBL::Attribute->new( -code        => 'GapExons',
                                                    -name        => 'gap exons',
                                                    -description => 'number of gap exons',
                                                    -value       => $gap_exons );
  push @attributes, $gap_exon_attr;

  my $tranid_attr = Bio::EnsEMBL::Attribute->new( -code        => 'SourceTran',
  ...
  if ( $self->direct_target_slice and $self->direct_target_slice->strand < 0 ) {
    $proj_tran = $proj_tran->transfer( $self->direct_target_slice->invert );
  }
  return $proj_tran;
} ## end sub place_transcript
...
sub project_up {
  ...
  my @comps = $self->from_mapper->map_coordinates( $GENE_SCAFFOLD_CS_NAME, 1, $self->length, 1, $GENE_SCAFFOLD_CS_NAME );
  ...
  foreach my $c (@comps) {
    ...
    if ( $c->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
      ...
      my $slice = $tlsl->adaptor->fetch_by_region( $tlsl->coord_system->name, $tlsl->seq_region_name, $c->start, $c->end, 1 );

      push @segments, bless( [ $start, $end, $slice ], "Bio::EnsEMBL::ProjectionSegment" );
    }
  }
...
sub project_down {
  ...
  my @comps = $self->to_mapper->map_coordinates( $GENE_SCAFFOLD_CS_NAME, 1, $self->length, 1, $GENE_SCAFFOLD_CS_NAME );
  ...
  foreach my $c (@comps) {
    ...
    if ( $c->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
      ...
      my $slice = $tlsl->adaptor->fetch_by_region( $tlsl->coord_system->name, $tlsl->seq_region_name, $c->start, $c->end, $c->strand );
...
} ## end sub _check_transcripts

#################################################

sub _construct_sequence {
  my ( $gen_al_blocks,        $map,              $transcripts, $from_slice, $to_slices,
       $max_readthrough_dist, $extend_into_gaps, $add_gaps,    $direct_target_slice ) = @_;
  ...
  foreach my $bl ( @{$gen_al_blocks} ) {
    ...
    my $from_coord = Bio::EnsEMBL::Mapper::Coordinate->new( $qy_al->dnafrag->name, $qy_al->dnafrag_start, $qy_al->dnafrag_end, 1 );
    my $to_coord =
      Bio::EnsEMBL::Mapper::Coordinate->new( $tg_al->dnafrag->name, $tg_al->dnafrag_start, $tg_al->dnafrag_end, $tg_al->dnafrag_strand );
    my $pair = Bio::EnsEMBL::Mapper::Pair->new( $from_coord, $to_coord );
  ...
  my $tran_coords = transcripts_to_coords( $map, $FROM_CS_NAME, @$transcripts );
  ...
  foreach my $tc ( sort { $a->start <=> $b->start } @$tran_coords ) {
    ...
    foreach my $c ( $map->map_coordinates( $tc->id, $tc->start, $tc->end, 1, $FROM_CS_NAME ) ) {
      my $from_coord = Bio::EnsEMBL::Mapper::Coordinate->new( $tc->id, $current_pos, $current_pos + $c->length - 1, 1 );
      ...
      if ( $c->isa("Bio::EnsEMBL::Mapper::Gap") ) {
        ...
        foreach my $bl (@block_coord_pairs) {
          if ( $bl->from->start <= $from_coord->end and $bl->from->end >= $from_coord->start ) {
            $overlaps_block = 1;
            last;
          }
        }
        if ( not $overlaps_block ) {
          push @fillable_gaps, Bio::EnsEMBL::Mapper::Pair->new( $from_coord, $c );
        }
      }
    }
  }
  ...
  if ($extend_into_gaps) {
    ...
    for ( my $i = 0; $i < @all_coord_pairs; $i++ ) {
      ...
      if ( $this_pair->to->isa("Bio::EnsEMBL::Mapper::Gap") ) {
        ...
        for ( my $j = $i - 1; $j >= 0; $j-- ) {
          if ( $all_coord_pairs[$j]->to->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
        ...
        my ( $ex_left,  $ex_left_up,  $ex_left_down )  = extend_coord( $left_non_gap->to,  $to_slices->{ $left_non_gap->to->id } );
        my ( $ex_right, $ex_right_up, $ex_right_down ) = extend_coord( $right_non_gap->to, $to_slices->{ $right_non_gap->to->id } );
        ...
        if ( not check_consistent_coords( $left_non_gap->to, $right_non_gap->to ) or
             $ex_left->start > $ex_right->end or
             $ex_left->end < $ex_right->start )
        {

          if ( $left_non_gap->from->end == $this_pair->from->start - 1 and $right_non_gap->from->start == $this_pair->from->end + 1 ) {

            my $remove_coord = 1;
            my (@replace_coord);

            if ( $left_non_gap->to->strand > 0 and $ex_left->end - $left_non_gap->to->end <= $NEAR_CONTIG_END ) {
              $remove_coord = 0;

              if ( defined $ex_left_down and $this_pair->to->length <= $ex_left_down->start - $ex_left->end - 1 ) {
                push @replace_coord,
                  Bio::EnsEMBL::Mapper::Coordinate->new( $ex_left->id,
                                                         $ex_left->end + 1,
                                                         $ex_left->end + $this_pair->to->length,
                                                         $ex_left->strand );
              }
            }
            elsif ( $left_non_gap->to->strand < 0 and $ex_left->start - $left_non_gap->to->start <= $NEAR_CONTIG_END ) {
              $remove_coord = 0;

              if ( defined $ex_left_up and $this_pair->to->length <= $ex_left->start - $ex_left_up->end - 1 ) {
                push @replace_coord,
                  Bio::EnsEMBL::Mapper::Coordinate->new( $ex_left->id,
            ...
            if ( $right_non_gap->to->strand > 0 and $right_non_gap->to->start - $ex_right->start <= $NEAR_CONTIG_END ) {
              $remove_coord = 0;

              if ( defined $ex_right_up and $this_pair->to->length <= $ex_right->start - $ex_right_up->end - 1 ) {
                push @replace_coord,
                  Bio::EnsEMBL::Mapper::Coordinate->new( $ex_right->id,
                                                         $ex_right->start - $this_pair->to->length,
                                                         $ex_right->start - 1,
                                                         $ex_right->strand );
              }
            }
            elsif ( $right_non_gap->to->strand < 0 and $ex_right->end - $right_non_gap->to->end <= $NEAR_CONTIG_END ) {
              $remove_coord = 0;

              if ( defined $ex_right_down and $this_pair->to->length <= $ex_right_down->start - $ex_right->end - 1 ) {
                push @replace_coord,
                  Bio::EnsEMBL::Mapper::Coordinate->new( $ex_right->id,
                                                         $ex_right->end + 1,
            ...
            if ($remove_coord) {
              ...
              push @replacements, [ $this_pair, $replace_coord[0] ];
            }

          } ## end if ( $left_non_gap->from...)
            ...
            if ( $left_non_gap->to->strand > 0 and $ex_left->end - $left_non_gap->to->end <= $NEAR_CONTIG_END ) {
              ...
              if ( defined $ex_left_down and $this_pair->to->length <= $ex_left_down->start - $ex_left->end - 1 ) {
                push @replacements,
                  [ $this_pair,
                    Bio::EnsEMBL::Mapper::Coordinate->new( $ex_left->id,                           $ex_left->end + 1,
                                                           $ex_left->end + $this_pair->to->length, $ex_left->strand ) ];
              }
            }
            elsif ( $left_non_gap->to->strand < 0 and $ex_left->start - $left_non_gap->to->start <= $NEAR_CONTIG_END ) {
              ...
              if ( defined $ex_left_up and $this_pair->to->length <= $ex_left->start - $ex_left_up->end - 1 ) {
                push @replacements,
                  [ $this_pair,
                    Bio::EnsEMBL::Mapper::Coordinate->new( $ex_left->id,        $ex_left->start - $this_pair->to->length,
                                                           $ex_left->start - 1, $ex_left->strand ) ];
              }
            }
            else {
              # gap does not align with the end of a contig; junk it
            ...
            if ( $right_non_gap->to->strand > 0 and $right_non_gap->to->start - $ex_right->start <= $NEAR_CONTIG_END ) {
              ...
              if ( defined $ex_right_up and $this_pair->to->length <= $ex_right->start - $ex_right_up->end - 1 ) {
                push @replacements,
                  [ $this_pair,
                    Bio::EnsEMBL::Mapper::Coordinate->new( $ex_right->id,        $ex_right->start - $this_pair->to->length,
                                                           $ex_right->start - 1, $ex_right->strand ) ];
              }
            }
            elsif ( $right_non_gap->to->strand < 0 and $ex_right->end - $right_non_gap->to->end <= $NEAR_CONTIG_END ) {
              ...
              if ( defined $ex_right_down and $this_pair->to->length <= $ex_right_down->start - $ex_right->end - 1 ) {
                push @replacements,
                  [ $this_pair,
                    Bio::EnsEMBL::Mapper::Coordinate->new( $ex_right->id,                           $ex_right->end + 1,
                                                           $ex_right->end + $this_pair->to->length, $ex_right->strand ) ];
              }
            }
            else {
              # gap does not align with the end of a contig; junk it
        ...
        } ## end if ( not check_consistent_coords...)
        else {
          $pairs_to_remove{$this_pair} = 1;
        }
      } ## end if ( $this_pair->to->isa...)
    } ## end for ( my $i = 0; $i < @all_coord_pairs...)

    @all_coord_pairs = grep { not exists $pairs_to_remove{$_} } @all_coord_pairs;

    @replacements = sort { $a->[1]->id cmp $b->[1]->id or $a->[1]->start <=> $b->[1]->start; } @replacements;
    ...
    while (@replacements) {
      ...
      if (@replacements) {
        ...
        if ( $rep->id eq $nrep->id and $rep->start <= $nrep->end and $rep->end >= $nrep->start ) {
  ...
  for ( my $i = 0; $i < @all_coord_pairs; $i++ ) {
    ...
    if ( $this_pair->to->isa("Bio::EnsEMBL::Mapper::Coordinate") and
         ...
         check_consistent_coords( $merged_pairs[-1]->to, $this_pair->to ) )
    {

      my $dist = distance_between_coords( $merged_pairs[-1]->to, $this_pair->to );

      if ( $dist <= $max_readthrough_dist ) {

        my $last_pair = pop @merged_pairs;

        my $new_from = merge_coords( $last_pair->from, $this_pair->from );
        my $new_to   = merge_coords( $last_pair->to,   $this_pair->to );
        ...
        if ($overlap) {
          ...
          push @merged_pairs, Bio::EnsEMBL::Mapper::Pair->new( $new_from, $new_to );
        }
      } ## end if ( $dist <= $max_readthrough_dist)
      else {
  ...
  my $t_map = Bio::EnsEMBL::Mapper->new( $TO_CS_NAME,   $GENE_SCAFFOLD_CS_NAME );
  my $q_map = Bio::EnsEMBL::Mapper->new( $FROM_CS_NAME, $GENE_SCAFFOLD_CS_NAME );
  ...
  for ( my $i = 0; $i < @merged_pairs; $i++ ) {
    ...
    if ($direct_target_slice) {
      if ( $pair->to->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
        ...
        $t_map->add_map_coordinates( $pair->to->id, $pair->to->start, $pair->to->end, $pair->to->strand, $GENE_SCAFFOLD_CS_NAME,
                                     $gs_start, $gs_end );
      ...
      if ( $pair->to->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
        ...
        $q_map->add_map_coordinates( $from_slice->seq_region_name,
                                     $pair->from->start, $pair->from->end, 1, $GENE_SCAFFOLD_CS_NAME,
                                     $last_end_pos + 1,
      ...
      if ( $i < @merged_pairs - 1 ) {
        $last_end_pos += $pair->to->length + $INTERPIECE_PADDING;
        $seq .= ( 'n' x $INTERPIECE_PADDING );
      }
    } ## end else [ if ($direct_target_slice)]
  } ## end for ( my $i = 0; $i < @merged_pairs...)

  #
  # add the gaps we have extended into to the query map
  #
  foreach my $pair (@extend_pairs) {
    my ($coord) = $t_map->map_coordinates( $pair->to->id, $pair->to->start, $pair->to->end, $pair->to->strand, $TO_CS_NAME );
    $q_map->add_map_coordinates( $from_slice->seq_region_name,
                                 $pair->from->start, $pair->from->end, 1, $GENE_SCAFFOLD_CS_NAME, $coord->start, $coord->end );
  }

  #
  # finally add of the original alignment pieces to the query map
  #
  my @aln_coords = $map->map_coordinates( $from_slice->seq_region_name, $from_slice->start, $from_slice->end, 1, $FROM_CS_NAME );
  ...
  foreach my $c (@aln_coords) {
    ...
    if ( $c->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
      ...
      my ($coord) = $t_map->map_coordinates( $c->id, $c->start, $c->end, $c->strand, $TO_CS_NAME );

      $q_map->add_map_coordinates( $from_slice->seq_region_name,
                                   $current_start, $current_end, 1, $GENE_SCAFFOLD_CS_NAME, $coord->start, $coord->end );
    }
    $current_start += $c->length;
  }

  return ( $seq, $q_map, $t_map );
} ## end sub _construct_sequence
...
sub _make_alignment_mapper {
  ...
  my $mapper = Bio::EnsEMBL::Mapper->new( $FROM_CS_NAME, $TO_CS_NAME );
  ...
  foreach my $bl (@$gen_al_blocks) {
    foreach my $ugbl ( @{ $bl->get_all_ungapped_GenomicAlignBlocks } ) {
      ...
      $mapper->add_map_coordinates( $from_bl->dnafrag->name,
                                    $from_bl->dnafrag_start, $from_bl->dnafrag_end, $from_bl->dnafrag_strand*$to_bl->dnafrag_strand,
                                    $to_bl->dnafrag->name, $to_bl->dnafrag_start, $to_bl->dnafrag_end );
    }
  }
...
sub _check_direct_target_coordinates {
  ...
  return $target_slice;
} ## end sub _check_direct_target_coordinates
