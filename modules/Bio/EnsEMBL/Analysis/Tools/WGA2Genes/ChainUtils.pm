...
sub filter_irrelevant_chains {
  ...
  return \@kept_chains;
} ## end sub filter_irrelevant_chains
...
} ## end sub filter_block_overlap_chains
...
sub filter_inconsistent_chains {
  ...
  for ( my $i = 0; $i < @sorted_chains; $i++ ) {
    ...
    foreach my $kc (@kept_chains) {
      if ( $c->{qstart} <= $kc->{qend} and $c->{qend} >= $kc->{qstart} ) {
    ...
    KCHAIN: foreach my $kc (@ov_chains) {
        foreach my $kb ( @{ $kc->{blocks} } ) {
          ...
          if ( $qga->dnafrag_start <= $k_qga->dnafrag_end and $qga->dnafrag_end >= $k_qga->dnafrag_start ) {

            my $ov_start = $k_qga->dnafrag_start;
            $ov_start = $qga->dnafrag_start if $qga->dnafrag_start > $ov_start;
            my $ov_end = $k_qga->dnafrag_end;
            $ov_end = $qga->dnafrag_end if $qga->dnafrag_end < $ov_end;

            push @{ $block_ov_regs[$i] }, { start => $ov_start, end => $ov_end, };
          }
        }
      }
    }
    ...
    if ( not grep { scalar( @{$_} ) > 0 } @block_ov_regs ) {
      ...
      for ( my $i = 0; $i < @these_blocks; $i++ ) {
        ...
        if (@$ovs) {
          if ( $ovs->[0]->{start} > $qga->dnafrag_start ) {
            if ($last_was_non_ov) {
              push @{ $contig_chain_parts[-1] }, { index => $i, start => $qga->dnafrag_start, end => $ovs->[0]->{start} - 1, };
            }
            else {
              push @contig_chain_parts, [ { index => $i, start => $qga->dnafrag_start, end => $ovs->[0]->{start} - 1, } ];
            }
          }

          for ( my $j = 1; $j < @$ovs; $j++ ) {
            push @contig_chain_parts, [ { index => $i, start => $ovs->[ $j - 1 ]->{end} + 1, end => $ovs->[$j]->{start} - 1, } ];
          }
          if ( $ovs->[-1]->{end} < $qga->dnafrag_end ) {
            push @contig_chain_parts, [ { index => $i, start => $ovs->[-1]->{end} + 1, end => $qga->dnafrag_end, } ];
            $last_was_non_ov = 1;
          }
          else {
            $last_was_non_ov = 0;
          ...
          if ($last_was_non_ov) {
            push @{ $contig_chain_parts[-1] }, { index => $i, start => $qga->dnafrag_start, end => $qga->dnafrag_end, };
          }
          else {
            push @contig_chain_parts, [ { index => $i, start => $qga->dnafrag_start, end => $qga->dnafrag_end, } ];
          }
          $last_was_non_ov = 1;
        }
      } ## end for ( my $i = 0; $i < @these_blocks...)
      ...
      if ( $longest_len/$total_chain_len > $overlap_chain_thresh ) {
        ...
        foreach my $el (@$longest) {
          ...
          if ( $st != $qga->dnafrag_start or $en != $qga->dnafrag_end ) {
        ...
        $c->{tstart} = ( $tga_l->dnafrag_strand > 0 ) ? $tga_l->dnafrag_start : $tga_r->dnafrag_start;
        $c->{tend}   = ( $tga_l->dnafrag_strand > 0 ) ? $tga_r->dnafrag_end : $tga_l->dnafrag_end;

        push @kept_chains, $c;
      } ## end if ( $longest_len/$total_chain_len...)
      else {
        $rejected = 1;
      }
    } ## end else [ if ( not grep { scalar...})]
    if ($rejected) {
      ...
      for ( my $j = $i + 1; $j < @sorted_chains; $j++ ) {
        ...
        my $consistent = 0;
        if ( $oc->{tid} eq $c->{tid} and $oc->{tstrand} eq $c->{tstrand} ) {
          if ( $oc->{qstart} > $c->{qend} ) {
            if ( $oc->{tstrand} > 0 and $oc->{tstart} > $c->{tend} or $oc->{tstrand} < 0 and $oc->{tend} < $c->{tstart} ) {
              $consistent = 1;
            }
          }
          elsif ( $oc->{qend} < $c->{qstart} ) {
            if ( $oc->{tstrand} > 0 and $c->{tstart} > $oc->{tend} or $oc->{tstrand} < 0 and $c->{tend} < $oc->{tstart} ) {
              $consistent = 1;
            }
          }
        }
        if ($consistent) {
          ...
          for ( my $k = 0; $k < @kept_chains; $k++ ) {
            if ( $oc->{qstart} <= $kept_chains[$k]->{qend} and $oc->{qend} >= $kept_chains[$k]->{qstart} ) {
              $consistent = 0;
              last;
            }
          }
    ...
    } ## end else [ if ($rejected) ]
  } ## end for ( my $i = 0; $i < @sorted_chains...)

  return [ map { $_->{blocks} } @kept_chains ];
} ## end sub filter_inconsistent_chains
...
sub flatten_chains {
  ...
  @blocks = sort { $a->reference_genomic_align->dnafrag_start <=> $b->reference_genomic_align->dnafrag_start; } @blocks;
  ...
  if ($check) {
    for ( my $i = 1; $i < @blocks; $i++ ) {
      ...
      if ( $this->reference_genomic_align->dnafrag_start < $prev->reference_genomic_align->dnafrag_end ) {
  ...
  foreach my $c (@$chains) {
    ...
    foreach my $b (@$c) {
      ...
      $str .= sprintf( " %s/%d-%d, %s/%d-%d %d (%d)\n",
                       $q->dnafrag->name, $q->dnafrag_start, $q->dnafrag_end,    $t->dnafrag->name,
                       $t->dnafrag_start, $t->dnafrag_end,   $t->dnafrag_strand, $t->level_id );
    }
  }
