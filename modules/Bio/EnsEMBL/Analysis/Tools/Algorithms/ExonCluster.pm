...
sub _add_new_exon {
  ...
  if ( exists $self->{_exonidhash}{ $exon->dbID . $exon->adaptor->db->dbc->dbname } ) {
    $self->throw(
      "Error : there seem to be exons with the same dbID and dbname in the databases " . $exon->dbID . " " . $exon->adaptor->db->dbname );
  }
  $self->{_exonidhash}{ $exon->dbID . $exon->adaptor->db->dbc->dbname } = $exon;
  $self->{_all_exons_in_cluster}{$exon} = $exon;
} ## end sub _add_new_exon
...
} ## end sub unique_exon_combinations
...
sub _determine_type {
  ...
  if ( $maxexoncount == 1 ) {
    ...
    foreach my $comb (@combs) {
      ...
      if ( $comb->{exons}[0]->start != $first_start || $comb->{exons}[0]->end != $first_end ) {
        $failed = 1;
        last;
      }
      else {
        if ( $comb->{exons}[0]->phase != $first_phase ) {
          ...
          foreach my $trans ( @{ $comb->{transcripts} } ) {
            if ( $trans->translation->start_Exon == $comb->{exons}[0] || $trans->translation->end_Exon == $comb->{exons}[0] ) {
              $countterm++;
            }
          }
    ...
    SINGLE:
      foreach my $comb (@allsingle) {
        foreach my $comp_comb (@notallsingle) {
          if ( $comb->{exons}[0]->start < $comp_comb->{exons}[0]->start || $comb->{exons}[0]->end > $comp_comb->{exons}[0]->end ) {
            $failed = 1;
            last SINGLE;
          }
          ...
          elsif ( $notsingle_start != $comp_comb->{exons}[0]->start || $notsingle_end != $comp_comb->{exons}[0]->end ) {
            $failed = 1;
            last SINGLE;
          }
        }
  ...
  COMB:
    foreach my $comb (@combs) {
      ...
      foreach my $trans ( @{ $comb->{transcripts} } ) {
        if ( scalar( @{ $trans->get_all_Exons } ) == 1 ) {
          ...
          if ( $comb->{exons}[0] == $trans->start_Exon && ( !defined($end) || ( $end == 0 ) ) ) {
            $end = 0;
          ...
          elsif ( $comb->{exons}[0] == $trans->end_Exon && ( !defined($end) || $end == 1 ) ) {
            $end = 1;
            if ( !defined($first_end) ) {
              $first_end = $end;
            }
          ...
          if ( $comb->{exons}[0] == $trans->start_Exon && ( !defined($end) || ( $end == 1 ) ) ) {
            $end = 1;
            if ( !defined($first_end) ) {
              $first_end = $end;
            }
          ...
          elsif ( $comb->{exons}[0] == $trans->end_Exon && ( !defined($end) || $end == 0 ) ) {
            $end = 0;
            if ( !defined($first_end) ) {
              $first_end = $end;
            }
    ...
    } ## end COMB: foreach my $comb (@combs)
  ...
  if ( $maxexoncount == 1 ) {
    ...
    $self->{'_type'} = 5;
    return 5;
  } ## end else [ if ( $maxexoncount == ...)]
} ## end sub _determine_type
