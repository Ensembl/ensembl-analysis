...
sub get_ExonCluster {
  ...
  foreach my $trans ( @{ $self->get_Transcripts } ) {
    ...
    CLUSTER: foreach my $cluster (@clusters) {
        ...
        if ( !( $exon->start >= $cluster->end || $exon->end <= $cluster->start ) ) {
...
sub get_ExonCluster_using_all_Exons {
  ...
  return \@clusters;
} ## end sub get_ExonCluster_using_all_Exons
...
sub get_coding_ExonCluster {
  ...
  return \@clusters;
} ## end sub get_coding_ExonCluster
...
sub get_IntronClusters {
  ...
  foreach my $trans ( @{ $self->get_Transcripts } ) {
    ...
    } ## end foreach my $intron ( @{ get_all_Introns_between_translateable_Exons...})
  } ## end foreach my $trans ( @{ $self...})

  return \@clusters;
} ## end sub get_IntronClusters
