...
sub new {
  ...
  my ( $min_coverage, $min_percent, $best_in_genome, $rpp, $verbosity ) =
    rearrange( [ 'COVERAGE', 'PERCENT_ID', 'BEST_IN_GENOME', 'REJECT_PROCESSED_PSEUDOS', 'VERBOSITY', ], @args );
...
TRAN:
  ...
  } ## end TRAN: foreach my $transcript (@$transcripts)
...
QUERY:
  foreach my $query_id ( keys(%matches) ) {
    @{ $matches_sorted_by_coverage{$query_id} } =
      sort { $b->{coverage} <=> $a->{coverage} or $b->{num_exons} <=> $a->{num_exons} or $b->{percent_id} <=> $a->{percent_id} }
      @{ $matches{$query_id} };
  ...
  TRANSCRIPT:
    ...
    foreach my $hit ( @{ $matches_sorted_by_coverage{$query_id} } ) {
      ...
      if (
        ( ( $coverage >= $self->min_coverage && $percent_id >= $self->min_percent )
    ...
    } ## end TRANSCRIPT: foreach my $hit ( @{ $matches_sorted_by_coverage...})
    ...
    foreach my $hit ( @{ $matches_sorted_by_coverage{$query_id} } ) {
      ...
      elsif ( $count > 1 && $splices_elsewhere && !$is_spliced ) {
      ...
      if ( $self->best_in_genome ) {
            ...
            ( $coverage >= ( 1 + 5/100 )*$self->min_coverage && $percent_id >= ( 1 - 3/100 )*$self->min_percent ) ) )
        {
          ...
          if ( $self->reject_processed_pseudos && $count > 1 && $splices_elsewhere && !$is_spliced ) {
            $accept = 'NO';
            if ($printing) {
              print "rpp $query_id\n";
            }
          ...
          } ## end elsif ( $max_intron > 250000) [ if ( $self->reject_processed_pseudos...)]
          else {
            $accept = 'YES';
            push( @good_matches, $transcript );

          }
        } ## end if ( $coverage == $max_coverage...)
        else {
          $accept = 'NO';
          if ($printing) {
      ...
      } ## end else [ if ( $self->best_in_genome)]
    } ## end foreach my $hit ( @{ $matches_sorted_by_coverage...})
  } ## end QUERY: foreach my $query_id ( keys...)
  return \@good_matches;
...
sub _get_transcript_coverage {
  my ( $self, $tran ) = @_;
  if ( @{ $tran->get_all_supporting_features } and defined $tran->get_all_supporting_features->[0]->hcoverage ) {
