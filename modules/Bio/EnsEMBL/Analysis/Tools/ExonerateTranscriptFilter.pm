...
sub new {
  ...
  &verbose('WARNING');
  my ( $min_coverage, $min_percent, $best_in_genome, $rpp ) =
    rearrange( [ 'COVERAGE', 'PERCENT_ID', 'BEST_IN_GENOME', 'REJECT_PROCESSED_PSEUDOS', ], @args );
    ...
    foreach my $hit ( @{ $matches_sorted_by_coverage{$query_id} } ) {
      ...
      if ( $self->best_in_genome ) {
            ...
            ( $coverage >= ( 1 + 5/100 )*$self->min_coverage && $percent_id >= ( 1 - 3/100 )*$self->min_percent ) ) )
        {
          ...
          elsif ( $best_slice eq $transcript_slice && $best_start > $transcript->start && $best_end < $transcript->end ) {
            $accept = 'NO';
          }
          else {
            $accept = 'YES';
