...
sub create_simple_feature {
  my ( $self, $start, $end, $strand, $score, $display_label, $seqname, $slice, $analysis ) = @_;
  my $simple_feature = Bio::EnsEMBL::SimpleFeature->new( -start         => $start,
                                                         -end           => $end,
                                                         -strand        => $strand,
                                                         -score         => $score,
...
sub create_repeat_consensus {
  ...
  my $repeat_consensus = Bio::EnsEMBL::RepeatConsensus->new( -name             => $name,
                                                             -length           => $length,
                                                             -repeat_class     => $class,
...
sub create_repeat_feature {
  my ( $self, $start, $end, $strand, $score, $repeat_start, $repeat_end, $repeat_consensus, $seqname, $slice, $analysis ) = @_;

  my $repeat_feature = Bio::EnsEMBL::RepeatFeature->new( -start            => $start,
                                                         -end              => $end,
                                                         -strand           => $strand,
                                                         -slice            => $slice,
                                                         -analysis         => $analysis,
...
sub create_feature_pair {
  my ( $self, $start, $end, $strand, $score, $hstart, $hend, $hstrand, $hseqname, $percent_id, $p_value, $seqname, $slice, $analysis,
       $positive_matches, $identical_matches )
    = @_;
...
sub create_misc_feature {
  ...
  my $mf = Bio::EnsEMBL::MiscFeature->new( -start => $start, -end => $end, -strand => $strand, -slice => $slice, );
  return $mf;
}
...
sub add_misc_feature_attribute {
  ...
  $mf->add_Attribute( Bio::EnsEMBL::Attribute->new( -CODE => $code, -NAME => $name, -DESCRIPTION => $description, -VALUE => $value, ) );
}
...
sub add_misc_set {
  ...
  $mf->add_MiscSet(
                Bio::EnsEMBL::MiscSet->new( -CODE => $code, -NAME => $name, -DESCRIPTION => $description, -VALUE => $longest_feature, ) );
}
...
sub create_prediction_exon {
  my ( $self, $start, $end, $strand, $score, $pvalue, $phase, $seqname, $slice, $analysis ) = @_;
  my $exon = Bio::EnsEMBL::PredictionExon->new( -start    => $start,
                                                -end      => $end,
                                                -strand   => $strand,
                                                -score    => $score,
                                                -p_value  => $pvalue,
...
sub create_prediction_transcript {
  ...
  my $transcript = Bio::EnsEMBL::PredictionTranscript->new( -exons => $exons, -slice => $slice, -analysis => $analysis, );
  return $transcript;
}
...
sub create_marker_feature {
  my ( $self, $start, $end, $strand, $marker, $seqname, $slice, $analysis ) = @_;
...
sub validate {
  ...
  my @error_messages;
  if ( !$feature ) {
    throw( "Can't validate a feature without a feature " . "FeatureFactory::validate" );
  }
  if ( !( $feature->isa('Bio::EnsEMBL::Feature') ) ) {
    throw( "Wrong type " . $feature . " must be a Bio::EnsEMBL::Feature " . "object FeatureFactory::validate" );
  }
  ...
  if ( $feature->start > $feature->end ) {
    my $string = "Start is greater than end " . $feature->start . " " . $feature->end;
    push( @error_messages, $string );
  }
  if ( @error_messages > 0 ) {
    print STDERR join( "\n", @error_messages );
...
sub validate_prediction_transcript {
  ...
  if ( !$pt ) {
    throw(
       "Can't validate a prediction transcript without a " . "prediction transcript " . "FeatureFactory:validate_prediction_transcript" );
  }
  if ( !( $pt->isa('Bio::EnsEMBL::PredictionTranscript') ) ) {
    throw( "Wrong type " . $pt . " must be a Bio::EnsEMBL::PredictionTranscript" . "FeatureFactory:validate_prediction_transcript" );
  }
  ...
  return 1;
} ## end sub validate_prediction_transcript
