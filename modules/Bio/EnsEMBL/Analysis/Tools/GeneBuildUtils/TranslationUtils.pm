...
sub print_Translation {
  ...
  warning( "Transcript is less than 3bp can't print " . "any more info" ) if ( $transcript->length < 3 );
  ...
  print $indent. "peptide contains " . $num_stops . " internal stop codons\n" if ($num_stops);
}
...
sub print_peptide {
  my ($transcript) = @_;
  warning( "Transcript is less than 3bp can't print " . "Its peptide" ) if ( $transcript->length < 3 );
  return if ( $transcript->length < 3 );
  print ">";
...
sub Translation_info {
  ...
  return $indent . "TRANSLATION: " .
    $id . " " . $start_id . " " . $translation->start . " " . $end_id . " " . $translation->end . " " . $cdna_length . " ";
}
...
sub print_Translation_genomic_coords {
  my ( $transcript, $indent ) = @_;
  print $indent. "genomic coords " . $transcript->coding_region_start . " " . $transcript->coding_region_end . "\n";
}
...
sub contains_internal_stops {
  ...
  my $num          = ( $pep =~ s/\*/\*/g );
  $num = 0 unless $num > 0;
  logger_info( Translation_info($transcript) . " contains internal stops " . $pep ) if ($num);
  return $num;
}
...
sub clone_Translation {
  ...
  foreach my $exon ( @{ $newtranscript->get_all_Exons } ) {
    my $id_string = $exon->start . "-" . $exon->end . "-" . $exon->strand;
    #print "Adding ".$id_string." exon to hash\n";
    $new_exons{$id_string} = $exon;
  }
  my $start_exon          = $translation->start_Exon;
  my $old_start_id_string = $start_exon->start . "-" . $start_exon->end . "-" . $start_exon->strand;

  my $new_start_Exon = $new_exons{$old_start_id_string};
  throw( "Failed to find exon " . $old_start_id_string . " for " . id($transcript) ) if ( !$new_start_Exon );
  $newtranslation->start_Exon($new_start_Exon);
  $newtranslation->start( $translation->start );
  my $end_exon          = $translation->end_Exon;
  my $old_end_id_string = $end_exon->start . "-" . $end_exon->end . "-" . $end_exon->strand;
  my $new_end_Exon      = $new_exons{$old_end_id_string};
  throw( $old_end_id_string . " failed to get exon" ) if ( !$new_end_Exon );
...
sub add_ORF_to_transcript {
  ...
  foreach my $exon ( @{ $transcript->get_all_Exons } ) {
    $exon_count++;
    logger_info( "exon:$exon_count exon_length:" . $exon->length . " pos:$pos orf_start:$orf_start orf_end:$orf_end pos+:" .
                 ( $pos + $exon->length - 1 ) );
  ...
  if ( !$translation_start || !$translation_end || !$translation_start_Exon || !$translation_end_Exon ) {
    warning( "problems making the translation " . "for " . id($transcript) );
    return $transcript;
  }
  else {
  ...
  foreach my $exon ( @{ $transcript->get_all_Exons } ) {
    ...
    if ( $exon == $translation->start_Exon ) {
      $end_phase = ( $exon->end - ( $exon->start + $translation->start - 1 ) + 1 ) % 3;
...
ORF:
  ...
  my @sorted_predictions = map { $_->[1] } sort { $b->[0] <=> $a->[0] } map { [ $_->[0], $_ ] } @orf_predictions;
  unlink $file;
  return \@sorted_predictions;
} ## end sub run_translate
...
sub validate_Translation_coords {
  ...
  if ( !$allow_negative_start && ( $translation->start < 1 ) ) {
    warning( Translation_info($transcript) . " has a start  " . $translation->start . " less than 1" );
    return 0;
  }
  if ( $translation->end < 0 || $translation->end > $translation->end_Exon->length ) {
    warning( Translation_info($transcript) .
             " has an end " . $translation->end . " less than 1 or greater than " . $translation->end_Exon->length );
    return 0;
  }
  return 1;
}
