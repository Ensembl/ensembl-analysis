...
sub Exon_info {
  ...
  my $id           = id($exon);
  return $indent . "EXON: " . $id . " " . $coord_string . "  " . $exon->phase . " " . $exon->end_phase . " " . $exon->length;
}
...
sub clone_Exon {
  ...
  my $newexon = create_Exon( $exon->start, $exon->end,  $exon->phase, $exon->end_phase, $exon->strand, $exon->analysis,
                             \@sfs,        $exon->dbID, $exon->slice, $exon->stable_id, $exon->version );
  return $newexon;
}
...
sub exon_length_less_than_maximum {
  ...
  if ( $exon->length >= $max_length ) {
    warning( id($exon) . " is longer than max length " . $max_length );
    return 0;
  }
  return 1;
}
...
sub transfer_supporting_evidence {
  ...
  my %target_evidence;
  my %source_evidence;
  foreach my $sf ( @{ $target_exon->get_all_supporting_features } ) {
    my $unique_id = $sf->start . "-" . $sf->end . "-" . $sf->strand . "-" . $sf->hseqname . "-" . $sf->cigar_string;
