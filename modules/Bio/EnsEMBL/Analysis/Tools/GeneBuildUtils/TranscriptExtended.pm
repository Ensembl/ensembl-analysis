...
sub remove_Exon {
  ...
  unless ($ex_to_remove) {
    throw( "you have to supply an Bio::EnsEMBL::Exon object " . "which will be removed from the transcript\n" );
  }
  my $clone     = [];
...
sub different_est_support {
  ...
  if ($new_tr) {
    for my $key ( keys %{ $self->{different_est_support} } ) {
      ...
      if ( $stored_tr->seq_region_start <= $new_tr->seq_region_end && $new_tr->seq_region_start <= $stored_tr->seq_region_end ) {
