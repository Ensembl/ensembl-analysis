...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptUtils
  qw(print_Transcript clone_Transcript get_evidence_ids attach_Slice_to_Transcript fully_load_Transcript empty_Transcript attach_Analysis_to_Transcript attach_Analysis_to_Transcript_no_support print_Transcript_and_Exons);
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranslationUtils
  qw(run_translate add_ORF_to_transcript compute_6frame_translations_for_transcript);
...
} ## end sub filter_Genes_by_Exon_count
...
sub prune_Exons {
  ...
  } ## end foreach my $tran ( @{ $cloned_gene...})
...
} ## end sub get_transcript_with_longest_CDS
...
sub get_one2one_orth_for_gene_in_other_species {
  ...
  foreach my $homolog_to_check ( @{ $gene->get_all_homologous_Genes() } ) {
    ...
    if ( $check_species eq $other_species ) {
      ...
      if ( $check_homology->description =~ m/ortholog_one2one/ ) {
        ...
        if ($one2one_homologue_in_other_species) {
          my $err_str =
            "We have more than one ortholog_one2one-relationship found for : " . $gene->stable_id . "\t" . $check_species . "\n";
    ...
    } ## end if ( $check_species eq...)
  } ## end foreach my $homolog_to_check...
...
} ## end sub get_one2one_orth_for_gene_in_other_species
...
sub get_one2one_homology_for_gene_in_other_species {
  ...
  foreach my $homolog_to_check ( @{ $gene->get_all_homologous_Genes() } ) {
    ...
    if ( $check_species eq $other_species ) {
      ...
      print "homology_key\t$other_species\t" .
        $check_homology->description . "\t" . $gene->stable_id . "\t" . $check_homologue_gene->stable_id . "\n";

      if ( $check_homology->description =~ m/ortholog_one2one/ || $check_homology->description =~ m/apparent_ortholog_one2one/ ) {
  ...
  return $homology;
} ## end sub get_one2one_homology_for_gene_in_other_species
