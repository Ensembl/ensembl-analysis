...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::ExonUtils
  qw(print_Exon clone_Exon Exon_info exon_length_less_than_maximum Exon_info get_upstream_Intron get_downstream_Intron get_upstream_splice_sites get_downstream_splice_sites validate_Exon_coords);
...
sub clone_Transcript {
  ...
  if ( $transcript->translation ) {
    $newtranslation = clone_Translation( $transcript, $newtranscript, $clone_xrefs );
  }
...
sub are_strands_consistent {
  ...
  if ( $exons->[0]->strand != $transcript->strand ) {
    warn( "Strands are inconsistent between the " . "first exon and the transcript for " . $transcript->display_id );
    return 0;
  }
  if ( @$exons >= 2 ) {
    for ( my $i = 1; $i < @$exons; $i++ ) {
      if ( $exons->[$i]->strand != $exons->[ $i - 1 ]->strand ) {
        warn( "Strands are inconsistent between " . "exon $i exon and exon " . ( $i - 1 ) . " for " . $transcript->display_id );
        return 0;
      }
    }
  }
...
sub exon_lengths_all_less_than_maximum {
  ...
  foreach my $exon ( @{ $transcript->get_all_Exons } ) {
    if ( !exon_length_less_than_maximum( $exon, $max_length ) ) {
      warn( "Transcript " . $transcript->display_id . " has " . "exon longer than " . $max_length );
      return 0;
    }
  }
  return 1;
...
sub intron_lengths_all_less_than_maximum {
  ...
  foreach my $intron ( @{ $transcript->get_all_Introns } ) {
    if ( !intron_length_less_than_maximum( $intron, $max_length ) ) {
  ...
  if ( @{ $transcript->get_all_Introns } == 0 ) {
    my $warn = "intron_lengths_all_less_than_maximum is an " . "inappropriate test for a single exon gene";
    logger_info($warn);
  }
  return 1;
}
...
sub are_phases_consistent {
  ...
  if ( not $transcript->translation ) {
    ...
    for ( my $i = 1; $i < @$exons; $i++ ) {
      ...
      if ( $prev_end_phase != $this_phase ) {
        ...
        if ( $prev_end_phase == -1 and $this_phase == 0 and $tr->start_Exon == $exons->[$i] ) {
          # okay
          next;
        }
        elsif ( $prev_end_phase == 0 and $this_phase == -1 and $tr->end_Exon == $exons->[ $i - 1 ] ) {
          # okay
          next;
        }
        else {
  ...
  } ## end else [ if ( not $transcript->...)]

  return 1;
} ## end sub are_phases_consistent
...
sub is_not_folded {
  ...
  my $exons = $transcript->get_all_Exons;
  if ( @$exons == 1 ) {
    my $warn = "is_not_folded " . "is an inappropriate test for a single " . "exon gene";
    logger_info($warn);
    return 1;
  }
  for ( my $i = 1; $i < @$exons; $i++ ) {
    ...
    if ( $exons->[$i]->strand == 1 ) {
      if ( $exons->[$i]->start < $exons->[ $i - 1 ]->end ) {
        ...
        warn( $i . " " . id( $exons->[$i] ) . " has a start which " . "is less than " .
              ( $i - 1 ) . " " . id( $exons->[ $i - 1 ] ) . " end" );
        return 0;
      }
    }
    else {
      if ( $exons->[$i]->end > $exons->[ $i - 1 ]->start ) {
        warning( $transcript->display_id . " is folded" );
        warn( $i . " " . id( $exons->[$i] ) . " has a end which " . "is greater than " .
              ( $i - 1 ) . " " . id( $exons->[ $i - 1 ] ) . " start" );
        return 0;
      }
    }
  }
  return 1;
} ## end sub is_not_folded
...
sub has_no_unwanted_evidence {
  ...
  foreach my $evi ( keys(%$evidence) ) {
    foreach my $unwanted ( keys(%$ids) ) {
      if ( $evi =~ /$unwanted/ ) {
        warn( $transcript->display_id . " has " . $evi . " unwanted evidence" );
        return 0;
      }
    }
  }
...
sub is_spliced {
  ...
  warning( $transcript->display_id . " has no introns " . "longer than $intron_size bps" ) if ( !$count );
  return 0 if ( !$count );
  return 1;
}
...
sub count_real_introns {
  ...
  my $warn       = $transcript->display_id . " has no introns. count_real" . "_introns makes no sense in those terms";
  logger_info($warn) if ( @introns == 0 );
  foreach my $intron (@introns) {
    $real_count++ if ( $intron->length > $intron_size );
  }
  logger_info(
     $transcript->display_id . " has " . $real_count . " introns " . "longer than " . $intron_size . " out of " . @introns . " introns" );
  return $real_count;
}
...
sub are_splice_sites_canonical {
  my ($transcript)        = @_;
  my $introns             = $transcript->get_all_Introns;
  my $non_canonical_count = count_non_canonical_splice_sites($transcript);
  if ($non_canonical_count) {
    warn( Transcript_info($transcript) .
          " contains " . $non_canonical_count . " non canonical " . "splice sites out of " . @$introns . " introns" );
    return 0;
  }
  if ( @$introns == 0 ) {
    my $warn = "are_splice_sites_canonical is an " . "inappropriate test for a single exon gene";
    logger_info($warn);
  }
  return 1;
}
...
sub count_non_canonical_splice_sites {
  ...
  my $none         = 0;
  foreach my $intron ( @{ $transcript->get_all_Introns } ) {
    my ( $upstream_site, $downstream_site ) = get_splice_sites($intron);
    $none++
      unless ( ( $upstream_site eq 'GT' && $downstream_site eq 'AG' ) ||
...
sub exonic_proportion {
  my ($transcript)   = @_;
  my $genomic_extent = ( $transcript->end - $transcript->start ) + 1;
...
sub coding_coverage {
  my ($transcript)      = @_;
  my $cdna_length       = $transcript->length;
  my $coding_seq_length = length( $transcript->translateable_seq );
...
sub split_Transcript {
  ...
  throw("TranscriptUtils:split_Transcript will not work on single exon transcripts") if ( @{ $transcript->get_all_Exons } == 0 );
  ...
  if ( $transcript->translation ) {
    my ($c) = $transcript->cdna2genomic( $transcript->cdna_coding_start, $transcript->cdna_coding_start );
    $cds_start = $c->start;

    ($c) = $transcript->cdna2genomic( $transcript->cdna_coding_end, $transcript->cdna_coding_end );
    $cds_end = $c->start;
...
INTRON:
  foreach my $intron ( @{ $cloned_transcript->get_all_Introns } ) {
    ...
    #translation
    if ( ( not defined $max_intron_length or intron_length_less_than_maximum( $intron, $max_intron_length ) ) and
  ...
  } ## end INTRON: foreach my $intron ( @{ $cloned_transcript...})
  ...
  foreach my $stran (@split_transcripts) {
    if ( $transcript->translation ) {
      if ( $stran->end >= $cds_start and $stran->start <= $cds_end ) {
    ...
    foreach my $sf ( @{ $transcript->get_all_supporting_features } ) {
      ...
      foreach my $ug ( $sf->ungapped_features ) {
        if ( $ug->start >= $stran->start and $ug->end <= $stran->end ) {
          push @ugs, $ug;
        }
      }
      if (@ugs) {
...
sub tidy_split_transcripts {
  ...
  foreach my $stran ( @{$split_trans} ) {
    ...
    if ( $stran->translate->seq =~ /\*/ ) {
      my $warn = Transcript_info($stran) . " does not translate\n";
      warning($warn);
      next;
    }
    ...
    if ( !$initial_peptide =~ /$stran->translate->seq/ ) {
      my $warn = Transcript_info($stran) . " translation " . "does not appear to be a subset of the original " . "translation";
      warning($warn);
      next;
    }
    ...
    if ( !are_strands_consistent($stran) ) {
      my $warn = Transcript_info($stran) . " has " . "inconsistent strands";
      warning($warn);
      next;
    }

    if ( !are_phases_consistent($stran) ) {
      my $warn = Transcript_info($stran) . " has " . "inconsistent phases";
      warning($warn);
      next;
    }

    push @keep, $stran;
  } ## end foreach my $stran ( @{$split_trans...})

  return ( \@keep );
} ## end sub tidy_split_transcripts
...
sub trim_cds_to_whole_codons {
  ...
  if ( $transcript->translation ) {
    ...
    if ( $tr->start_Exon == $exons[0] and $tr->start_Exon->phase > 0 and $tr->start == 1 ) {
      $remove5 = ( 3 - $tr->start_Exon->phase ) % 3;
    }
    if ( $tr->end_Exon == $exons[-1] and $tr->end_Exon->end_phase > 0 and $tr->end == $tr->end_Exon->length ) {
    ...
    } ## end if ( $remove5 or $remove3)
  } ## end if ( $transcript->translation)
  return $transcript;
} ## end sub trim_cds_to_whole_codons
...
#}
  ...
  if ( defined($max_stops) && $max_stops <= 0 ) {
    throw( "You have passed a value to max_stops but this value is <= 0. The value passed to max_stops should be " . ">= 1" );
  }
  ...
  while ( $pep =~ /\*/g ) {
    ...
    foreach my $stop (@coords) {
      ...
      foreach my $exon (@exons) {
        ...
        if ( $stop->start > $exon->start and $stop->end < $exon->end ) {
          ...
          foreach my $f (@sfs) {
            ...
            foreach my $ug ( $f->ungapped_features ) {
              ...
              if ( $ug->start >= $exon_left->start && $ug->end <= $exon_left->end + 3 ) {
                # completely within the left-side of the split
                # (including the stop length +3)
                push @ug_left, $ug;

              }
              elsif ( $ug->end >= $exon_left->start && $ug->end <= $exon_left->end + 3 ) {
              ...
              elsif ( $ug->start >= $exon_right->start - 3 && $ug->end <= $exon_right->end ) {
                # completely within the right-side of the split
                # (including the stop length -3)
                push @ug_right, $ug;

              }
              elsif ( $ug->start >= $exon_right->start - 3 && $ug->start <= $exon_right->end ) {
                # This case might crop up if the feature went over the edge of the end of the
                ...
                if ( $exon->strand > 0 ) {
                  ...
                  $fp_left->hend( $ug->hstart + ceil( $fp_left->length/$feature_unit_length ) - 1 );
                  $fp_right->hend( $ug->hend );
                  $fp_right->hstart( $ug->hend - ceil( $fp_right->length/$feature_unit_length ) + 1 );
                }
                else {
                  $fp_right->hstart( $ug->hstart );
                  $fp_right->hend( $ug->hstart + ceil( $fp_right->length/$feature_unit_length ) - 1 );
                  $fp_left->hend( $ug->hend );
                  $fp_left->hstart( $ug->hend - ceil( $fp_left->length/$feature_unit_length ) + 1 );
                }
              ...
              } ## end elsif ( $ug->start >= ( $exon_left... [ if ( $ug->start >= $exon_left...)]))
          ...
          if ( $transcript->translation->end_Exon->start == $orig_exon_start and $transcript->strand == 1 ) {
        ...
        } ## end elsif ( $stop->start == $exon... [ if ( $stop->start > $exon...)])
          ...
          if ( $transcript->translation->end_Exon->start == $exon->start and $transcript->strand == -1 ) {
        ...
        } ## end elsif ( $stop->end == $exon... [ if ( $stop->start > $exon...)])
        else {
...
#}
  ...
  if ( $new_transl_len > $old_transl_len ) {
    throw( "The edited transcript has a longer translation than the original, something has gone wrong." .
           " Original translation has length " . $old_transl_len . ", edited translation has length " .
  ...
  # Hopefully at this point the removal of the stops is okay.
    ...
    print "Original translation has length " . $old_transl_len . " but edited translation has length " .
      $new_transl_len . "\n>original\n" . $transcript->translate->seq . "\n>edited\n" . $newtranscript->translate->seq . "\n";
  }
  ...
  return $newtranscript;
} ## end sub replace_stops_with_introns
...
sub truncate_exon_features {
  ...
  my $new_exon = Bio::EnsEMBL::Exon->new( -slice     => $exon->slice,
                                          -start     => $exon->start,
                                          -end       => $exon->end,
                                          -strand    => $exon->strand,
                                          -phase     => $exon->phase,
  ...
  foreach my $f (@sfs) {
    ...
    foreach my $ug ( $f->ungapped_features ) {
      ...
      $trunc_feature->hstart( $ug->hend - ceil( $trunc_feature->length/$feature_unit_length ) + 1 );
      if ( $trunc_feature->end >= $trunc_feature->start ) {
        push @ug, $trunc_feature;
      }
    } ## end foreach my $ug ( $f->ungapped_features)
  } ## end foreach my $f (@sfs)

  $new_exon = add_dna_align_features_by_hitname_and_analysis( \@ug, $new_exon, $feature_isa );

  return $new_exon;
} ## end sub truncate_exon_features
...
sub remove_initial_or_terminal_short_exons {
  ...
  throw( "TranscriptUtils::remove_initial_or_terminal_short_exons will not work " .
         " if " . Transcript_info($transcript) . " has no translation" )
    if ( !$transcript->translation );
  my %translateable = %{ _identify_translateable_exons($transcript) };

  my $cloned_transcript = clone_Transcript($transcript);
  my $start_exon        = $cloned_transcript->translation->start_Exon;
  my $start_exon_id     = $start_exon->start . ":" . $start_exon->end . ":" . $start_exon->strand . ":" . $start_exon->phase;
  ...
  FIVE_PRIME_END: while ( scalar(@exons) ) {
      ...
      if ( $exon->length > $min_length ) {
        my $unique_string = $exon->start . ":" . $exon->end . ":" . $exon->strand . ":" . $exon->phase;
        if ( $translateable{$unique_string} && $exon ne $start_exon ) {
          $cloned_transcript->translation->start_Exon($exon);
    ...
    } ## end FIVE_PRIME_END: while ( scalar(@exons) )
  } ## end if ( $cloned_transcript...)
  ...
  THREE_PRIME_END: while (@exons) {
      ...
      if ( $exon->length > $min_length ) {
        my $unique_string = $exon->start . ":" . $exon->end . ":" . $exon->strand . ":" . $exon->phase;
        if ( $translateable{$unique_string} && $exon != $cloned_transcript->translation->end_Exon ) {
  ...
  return $cloned_transcript;
} ## end sub remove_initial_or_terminal_short_exons
...
EXON: foreach my $ex ( @{ $transcript->get_all_Exons } ) {
    my $unique_string = $ex->start . ":" . $ex->end . ":" . $ex->strand . ":" . $ex->phase;
...
sub dump_cDNA_file {
  ...
  logger_info( "You are going to dump the cdna of " . $transcript->display_id . " into " . $filename . " format " . $format );
...
sub evidence_coverage {
  ...
  throw( "Can't work out evidence coverage for " . $transcript->display_id . " without any evidence" ) if ( !$evidence );
  my $matches       = 0;
  my $pstart        = 0;
  my $pend          = 0;
  ...
  if ( !defined($plength) || $plength == 0 ) {
    warning( "TranscriptUtils: no sensible length for " . $evidence_name . " assuming 0% " . "coverage" );
    return 0;
  }
...
sub empty_Transcript {
  ...
  empty_Object( $transcript->translation, $remove_stable_id ) if ( $transcript->translation );
EXON: foreach my $e ( @{ $transcript->get_all_Exons } ) {
  ...
  return $transcript;
} ## end sub empty_Transcript

sub all_exons_are_valid {
  my ( $transcript, $max_length, $allow_negative_start ) = @_;

  foreach my $exon ( @{ $transcript->get_all_Exons } ) {
    throw( Transcript_info($transcript) . " seems to contain an undefined exon" ) if ( !$exon );
    if ( !exon_length_less_than_maximum( $exon, $max_length ) ) {
      warn( "Transcript " . Transcript_info($transcript) . " has " . "exon longer than " . $max_length );
      return 0;
    }
    if ( !validate_Exon_coords( $exon, $allow_negative_start ) ) {
      warn( "Transcript " . Transcript_info($transcript) . " has " . "invalid exon coords " . Exon_info($exon) );
      return 0;
    }
  }
  return 1;
...
sub evidence_coverage_greater_than_minimum {
  ...
  warning( "There has been no evidence passed in for " . Transcript_info($transcript) . "Assumung coverage is fine" ) if ( !$evidence );
  return 1 if ( !$evidence );
  my $coverage = evidence_coverage( $transcript, $evidence );
  warning( Transcript_info($transcript) . " based on " . $evidence->id . " has too " . "low coverage " . $coverage . " of the evidence" )
...
sub set_start_codon {
  ...
  if ( $cdna_coding_start > 3 ) {
    ...
    if ( $first_codon ne 'ATG' ) {
      ...
      unless ( $coords[0]->isa('Bio::EnsEMBL::Mapper::Coordinate') && $coords[$#coords]->isa('Bio::EnsEMBL::Mapper::Coordinate') ) {
      ...
      if ( scalar(@coords) > 2 ) {
        ...
        eval { $cloned_transcript->translate; };
        if ($@) {
        ...
        return $cloned_transcript;
      } ## end elsif ( scalar(@coords) ==... [ if ( scalar(@coords) >...)])
      else {
        logger_info("New start codon doesn't split across introns - but which exon is it in");
        $new_start = $coords[0]->start;
        $new_end   = $coords[0]->end;
        if ( ( $strand == 1 && $new_end == $pepgenstart - 1 ) || ( $strand == -1 && $new_start == $pepgenend + 1 ) ) {
        ...
        $cloned_transcript->recalculate_coordinates;
        return $cloned_transcript;
      } ## end else [ if ( scalar(@coords) >... [elsif ( scalar(@coords) ==...)])]
    } ## end else [ if ( $first_codon ne 'ATG')]
  } ## end if ( $cdna_coding_start...)
  ...
  # more complex cases: the previous codon falls off the cdna
    ...
    my $codonseq = uc( ${ $seq_adaptor->fetch_by_Slice_start_end_strand( $start_exon->slice, $codon_start, $codon_end, $strand ) } );
    ...
    if ( $codonseq ne "ATG" ) {
      ...
      $cloned_transcript->recalculate_coordinates;
      return $cloned_transcript;
    } ## end else [ if ( $codonseq ne "ATG")]
  } ## end else [ if ( $cdna_coding_start...)]
} ## end sub set_start_codon
...
sub get_previous_Exon {
  ...
  for ( my $i = 0; $i <= $#exons; $i++ ) {
    if ( $exons[$i]->start == $exon->start && $exons[$i]->end == $exon->end && $exons[$i]->strand == $exon->strand && $i > 0 ) {
      return $exons[ $i - 1 ];
    }
  }
  return undef;
...
sub set_stop_codon {
  ...
  if ( $end > 2 ) {
    ...
    if ($previous_exon) {
      my $subseq_start = $previous_exon->end - $previous_exon->start + 1 - $donor + 1;
      my $subseq_end   = $previous_exon->end - $previous_exon->start + 1;
      my $donor_seq    = $previous_exon->seq->subseq( $subseq_start, $subseq_end );
  ...
  if ( uc($last_codon) eq 'TAA' || uc($last_codon) eq 'TAG' || uc($last_codon) eq 'TGA' ) {
    logger_info( "transcript already has a stop at the end - no need " . "to modify" );
    return $cloned_transcript;
  }
  ############################################################
  ...
  if ( $end + 3 <= ( $end_exon->end - $end_exon->start + 1 ) ) {
    ...
    if ( uc($next_codon) eq 'TAA' || uc($next_codon) eq 'TAG' || uc($next_codon) eq 'TGA' ) {
      logger_info( "simple-case: next codon is a stop - " . "extending translation\n" );
  ...
  if ($next_exon) {
    ...
    if ( uc($next_codon) eq 'TAA' || uc($next_codon) eq 'TAG' || uc($next_codon) eq 'TGA' ) {
      logger_info( "shared-codon: next codon is a stop - " . "extending translation" );
    ...
    if ($adaptor) {
      ...
      if ( $end_exon->strand == 1 ) {
        ...
        my $codon_slice =
          $adaptor->fetch_by_region( $end_exon->slice->coord_system->name, $end_exon->slice->seq_region_name, $codon_start, $codon_end );
        my $codon = $codon_slice->seq;

        ############################################################
        if ( uc($codon) eq 'TAA' || uc($codon) eq 'TAG' || uc($codon) eq 'TGA' ) {
          logger_info( "forward-strand:next codon (falling off the exon) " . "is a stop - extending translation" );
        ...
        if ( $codon_start <= 0 ) {
          logger_info( "Can't extend the transcript off the end of a " . $end_exon->slice->coord_system->name );
          return $cloned_transcript;
        }
        my $codon_slice =
          $adaptor->fetch_by_region( $end_exon->slice->coord_system->name, $end_exon->slice->seq_region_name, $codon_start, $codon_end );
