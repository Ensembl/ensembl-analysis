...
sub new {
  ...
  my ( $plengths, $maxintronlen, $min_coverage ) = rearrange( [ 'PLENGTHS', 'MAXINTRONLEN', 'MIN_COVERAGE' ], @args );
...
sub make_coord_pair {
  ...
  my $cp = new Bio::EnsEMBL::Analysis::Tools::Pmatch::CoordPair( -query   => $cols[1],
                                                                 -target  => $protid,
                                                                 -qstart  => $cols[2],
                                                                 -qend    => $cols[3],
                                                                 -tstart  => $cols[6],
...
sub make_mergelist {
  ...
  CP:
    foreach my $cp (@cps) {
      ...
      if (
        $strand == $prev->strand &&
        ( ( $cp->tstart >= $prev_cp->tend ) || ( ( $prev_cp->tend - $cp->tstart ) <= 3 ) ) &&
    ...
    } ## end CP: foreach my $cp (@cps)
  } ## end foreach my $ch ( $ph->each_ContigHit...)
