...
sub check_InputId {
  ...
  if ( !defined $e ) {
    ...
    my $exp = Bio::EnsEMBL::Funcgen::Experiment->new( -NAME                => $ename,
                                                      -GROUP               => "$ENV{EFG_GROUP}",
                                                      -DATE                => join( '-', @date ),
...
sub check_Sets {
  ...
  my $feature_type = $self->efgdb->get_FeatureTypeAdaptor()->fetch_by_name($ft_name) or throw("Feature type '$ft_name' does not exist");
  $self->feature_type($feature_type);

  my $cell_type = $self->efgdb->get_CellTypeAdaptor()->fetch_by_name($ct_name) or throw("Cell type '$ct_name' does not exist");
  ...
  if ( !defined $iset ) {
    ...
    elsif ( ( scalar(@issets) == 1 ) && ( $issets[0]->name ne $file_name ) ) {
  ...
  if ( !defined $dset ) {
    ...
    $dset = Bio::EnsEMBL::Funcgen::DataSet->new( -SUPPORTING_SETS     => [$iset],
                                                 -FEATURE_SET         => $fset,
                                                 -DISPLAYABLE         => 1,
                                                 -NAME                => $set_name,
...
sub fetch_input {
  ...
  unless ( -e $cachefile ) {                 # dat file already dumped sorted; skipping dump
    ...
    if ( $file_format eq 'bed' ) {
      $sort = "gzip -dc " . $self->query . ' | grep -vE "^MT" | sort -k1,1 -k2,2n -k3,3n | gzip -c > ' . $cachefile . ' |';
    }
      ...
      warn(
         "This does not sort sam by end position! Hence will not work for paired ends reads. Need to implement samtools sort on bam files"
      );
      ...
      $sort =
        "gzip -dc " . $self->query .
' | grep -vE \'^@\' | grep -vE "^[^[:space:]]+[[:blank:]]4[[:blank:]].*$" | grep -vE "^[^[:space:]]+[[:blank:]][^[:space:]]+[[:blank:]][^[:space:]]+\:[^[:space:]]+\:MT\:" |sort -k3,3 -k4,4n | gzip -c > '
        . $cachefile . ' |';
    }
    else {
...
sub query {
  ...
  throw( "file " . $self->{'query'} . " doesn't exist" ) unless ( -e $self->{'query'} );
...
sub write_output {
  ...
  if ( scalar( @{ $self->output } ) == 0 ) {
    ...
    warn( "No features called from " . $self->query . " for experiment " . $self->experiment->name() . "!" );
    return 1;
  }
  ...
  if (@$af) {
    #Surely this should be throw?
    warn( "NOT IMPORTING " .
          scalar( @{ $self->output } ) . " annotated features! File " . $self->query . " already has been processed; contains " .
          scalar(@$af) . " annotated features of feature set " . $fset->dbID . "." );
    return 1;
    ...
    foreach my $ft ( @{ $self->output } ) {
      ...
      if ( !( ( $start =~ /^-?\d+$/ ) && ( $end =~ /^\d+$/ ) ) ) {
        warn "Feature being ignored due to incorect positions: Region:" .
      ...
      if ( !$slice{"$seqid"} ) {
        warn "Feature being ignored due to inexistent slice: Region:" .
          $seqid . " Start:" . $start . " End:" . $end . " Score:" . $score . " Summit:" . $summit . "\n";
        next;
      }

      my $af = Bio::EnsEMBL::Funcgen::AnnotatedFeature->new( -slice       => $slice{"$seqid"},
                                                             -start       => $start,
                                                             -end         => $end,
                                                             -strand      => 0,
                                                             -score       => $score,
