...
sub fetch_input {
  ...
  my $runnable = $runname->new( '-genes' => \@genes, '-analysis' => $self->analysis, '-PS_MULTI_EXON_DIR' => $self->PS_MULTI_EXON_DIR, );
  ...
  TRANS: foreach my $id ( keys %result_hash ) {
    ...
    DAF: foreach my $daf (@dafs) {
        ...
        foreach my $e ( @{ $retro_trans->get_all_Exons } ) {
          ...
          } ## end foreach my $block (@align_blocks)
        } ## end foreach my $e ( @{ $retro_trans...})
        ...
        if ( $real_span/$retro_span > $self->PS_SPAN_RATIO ) {
          print STDERR "---------------------------------------------------------------------------------------------------\n";
          print STDERR "DAF: " . $daf->start . " " . $daf->end . " " . $daf->strand .
            " " . $daf->hstart . " " . $daf->hend . " " . $daf->hstrand . " " . $daf->hseqname . " " . $daf->cigar_string . "\n";
          print STDERR "transcript id " . $retro_trans->dbID . " matches transcriptid " . $daf->hseqname . " at " . $daf->percent_id .
" %ID and $coverage % coverage with $aligned_genomic flanking genomic bases aligned pseudogene span of $retro_span vs real gene span of $real_span\n";
      ...
      } ## end DAF: foreach my $daf (@dafs)
      # transcript is real
      push @{ $trans_type{'real'} }, $retro_trans;
    } ## end TRANS: foreach my $id ( keys %result_hash)
    ...
    unless ( defined( $trans_type{'real'} ) ) {
      ...
      if ( defined $self->KEEP_TRANS_BIOTYPE && $self->KEEP_TRANS_BIOTYPE == 1 ) {
        warning( "keeping original transcript biotype " .
                 $only_transcript_to_keep->biotype() . " instead of setting it to " . $self->RETRO_TYPE . "\n" );
      }
      else {
