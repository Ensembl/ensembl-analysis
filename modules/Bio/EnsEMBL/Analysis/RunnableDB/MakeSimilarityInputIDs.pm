...
sub new {
  ...
  my ( $output_logicname, $protein_count, $bmg_logicname, $optimal_length ) =
    rearrange( [ 'OUTPUT_LOGICNAME', 'PROTEIN_COUNT', 'BMG_LOGICNAME', 'OPTIMAL_LENGTH' ], @args );
  ...
  throw( "Need an output logicname " . $self->output_logicname . " and a bmg logicname " . $self->bmg_logicname . " defined" )
    if ( !$self->output_logicname || !$self->bmg_logicname );
...
sub fetch_input {
  ...
  my $dba_pip = Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor->new( -host   => $self->db->host,
    ...
    for ( my $index = $optimal_length; ( $index - $optimal_length ) <= $self->query->length; $index += $optimal_length ) {
      while ( my $gene = shift @a_sorted_genes ) {
        if ( $gene->end <= $index ) {
          ...
          last;
        } ## end else [ if ( $gene->end <= $index)]
      } ## end while ( my $gene = shift ...)
    } ## end for ( my $index = $optimal_length...)
    print STDERR "empty\n" unless @a_input_genes;
    my ( $query_name, $range_end, $strand ) = $self->query->name =~ /(.*):\d+:(\d+):(.+)$/;
    $range_end = $a_input_genes[$#a_input_genes]->end + $max_padding
      unless ( $range_end < $a_input_genes[$#a_input_genes]->end + $max_padding );
    $query_name .= ':' . $range_start . ':' . $range_end . ':' . $strand;
    push( @a_iids, @{ $self->create_input_ids( \@a_input_genes, $num_seeds, $query_name ) } );
  } ## end else [ if ( $self->optimal_length...)]
  $self->output( \@a_iids );
} ## end sub run
...
sub get_good_features {
  ...
  SEQID: foreach my $sid ( keys %h_features ) {
    ...
    } ## end SEQID: foreach my $sid ( keys %h_features)

  } ## end if ($ra_mask_regions)
...
sub create_gene_from_feature {
  ...
  foreach my $feature (@a_sorted_features) {
    my $exon = Bio::EnsEMBL::Exon->new( -START     => $feature->start,
                                        -END       => $feature->end,
  ...
  foreach my $ra_exons (@a_exons) {
    my $transcript = Bio::EnsEMBL::Transcript->new( -START  => $start,
                                                    -END    => $end,
                                                    -STRAND => $a_sorted_features[0]->strand,
                                                    -SLICE  => $self->paf_slice,
                                                    -EXONS  => $ra_exons, );
    my $gene =
      Bio::EnsEMBL::Gene->new( -START => $start, -END => $end, -SLICE => $self->paf_slice, -DBID => $a_sorted_features[0]->hseqname, );
    $gene->add_Transcript($transcript);
    push( @a_genes, $gene );
  }

  return \@a_genes;
} ## end sub create_gene_from_feature
...
sub write_output {
  ...
  foreach my $iid ( @{ $self->output } ) {
    ...
    eval { $sic->store_input_id_analysis( $iid, $self->output_analysis, '' ); };
