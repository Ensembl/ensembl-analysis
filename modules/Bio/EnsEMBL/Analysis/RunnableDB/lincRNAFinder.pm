...
sub fetch_input {
  ...
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::lincRNAFinder->new( -query => $self->query, -analysis => $self->analysis, );
...
GENE: foreach my $gene ( @{ $self->output } ) {
    ...
    if ($@) {
      ...
      print "STORED LINCRNA GENE " .
        $gene->dbID . " " . $gene->biotype . " " . $self->output_db->dbname . " @ " . $self->output_db->host . " " .
        scalar(@tr) . " transcripts\n";
    }
...
sub convert_simple_features {
  ...
  for my $f (@simple_features) {
    ...
    my $ex = new Bio::EnsEMBL::Exon( -START     => $f_start,
                                     -END       => $f_end,
                                     -STRAND    => $f->strand,
                                     -SLICE     => $f->slice,
                                     -PHASE     => '0',
  ...
  } ## end for my $f (@simple_features)
  return \@converted_sf;
} ## end sub convert_simple_features
...
sub read_and_check_config {
  ...
  foreach my $var (qw(NEW_SET_1_CDNA NEW_SET_2_PROT OUTPUT_DB OUTPUT_BIOTYPE EFG_FEATURE_NAMES EXTEND_EFG_FEATURES )) {
    throw("RunnableDB::lincRNAFinder $var config variable is not defined") unless ( $self->$var );
  }
