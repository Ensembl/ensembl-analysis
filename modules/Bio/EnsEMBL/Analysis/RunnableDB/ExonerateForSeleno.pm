...
sub process_query_file {
  ...
  my $dnafile = Bio::SeqIO->new( '-file' => "< $query_file", '-format' => 'embl', );
  ...
  while ( ( my $dna = $dnafile->next_seq() ) ) {
    ...
    foreach my $feat ( $dna->get_SeqFeatures ) {
      ...
      } ## end foreach my $tag ( $feat->all_tags...)
    } ## end foreach my $feat ( $dna->get_SeqFeatures)
    if ( $contains_seleno == 1 ) {
      my $seqobj = Bio::Seq->new( -display_id => $dna->display_name(), -seq => $seq );
      ...
      my $seqout = Bio::SeqIO->new( -file => ">" . $sel_output_file, -format => 'fasta', );

      $seqout->write_seq($seqobj);

      my $annot_output_file = $self->QUERYANNOTATION;

      open( ANNOT, ">>$annot_output_file" ) or die "can open output file: $annot_output_file for writting\n";
    ...
    } ## end if ( $contains_seleno ...)
  } ## end while ( ( my $dna = $dnafile...))
  return @sequences;
} ## end sub process_query_file
