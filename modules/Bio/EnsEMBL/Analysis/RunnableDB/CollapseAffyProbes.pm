  ...
  while (<PROBES>) {
    ...
    if (/^>probe:(\S+):(\S+):(\S+:\S+;).*$/) {
      ...
      if ($current_sequence) {
        ...
        if ( !$existing_probe ) {
          $existing_probe = $self->create_new_probe( $current_affy_array, $probeset, $probe_name, $current_sequence );

          $probes_by_sequence{ $probeset . ":---:" . $current_sequence } = $existing_probe;
        }
        else {
          $self->add_array_to_existing_probe( $existing_probe, $current_affy_array, $probeset, $probe_name );
        }
...
sub add_array_to_existing_probe {
  ...
  if ( !( $probe->probeset eq $probeset ) ) {
    throw( "Inconsistency: have found a probe " . $array->name .
           ":$probeset:$probename with " . "identical sequence but different probeset to another one: " . $probe->probeset . "\n" );
  }
...
sub create_new_probe {
  ...
  my $affy_probe = new Bio::EnsEMBL::AffyProbe( -probeset => $probeset, -name => $probe_name, -array => $affy_array );
...
sub create_new_array {
  ...
  my $affy_array = new Bio::EnsEMBL::AffyArray( -name => $array_name, -setsize => 0, );
}
############################################################
...
sub populate_affy_arrays_and_probes {
  ...
  while (<PROBES>) {
    ...
    if ( !$affy_array ) {
      $affy_array = new Bio::EnsEMBL::AffyArray( -name => $array_name );
    }

    $self->affy_arrays->{$array_name} = $affy_array;

    my $affy_probe = $affy_probe_adaptor->fetch_by_array_probeset_probe( $array_name, $probeset, $probe_name );

    if ( !$affy_probe ) {
      $affy_probe = new Bio::EnsEMBL::AffyProbe( -probeset => $probeset, -name => $probe_name, -array => $affy_array );
    }

    $self->affy_probes->{ $array_name . ":" . $probeset . ":" . $probe_name } = $affy_probe;
  } ## end while (<PROBES>)

} ## end sub populate_affy_arrays_and_probes
