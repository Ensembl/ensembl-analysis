...
my %ana2extdbid = ( Uniprot_raw => { "."  => 2250 },
...
sub write_output {
  ...
  eval {
    ...
    foreach my $output (@$outputs) {
      ...
      my $all = 0;
      if ($is_align) {
        $all = 1 if ( $self->analysis->module =~ /DepthFilter/ || $self->analysis->logic_name =~ /refseq/ );
...
sub remove_stored_AlignFeatures {
  ...
  my $db_features = $adaptor->fetch_all_by_Slice( $self->query, $self->analysis->logic_name );
  ...
  if ( $self->analysis->logic_name eq 'Uniprot_raw' ) {
    foreach my $new_f (@$output) {
      ...
      if ( $stored_f && $stored_f->display_id !~ /\.\d+/ ) {
        print STDOUT "Finished: Remove old Uniprot feature " . $stored_f->display_id . ", replaced by " . $new_f->display_id . "\n";
...
} ## end sub remove_stored_AlignFeatures
...
sub get_feature_key {
  my ( $self, $feat, $uni ) = @_;
  throw( "Must pass Bio::EnsEMBL::Analysis::RunnableDB::Finished::get_feature_key a Bio::EnsEMBL::BaseAlignFeature" . "not a " . $feat )
    unless ( $feat->isa('Bio::EnsEMBL::BaseAlignFeature') );
  my $name = $feat->display_id;
  $name =~ s/\.\d// if ($uni);
  return join( ':', $name, $feat->seq_region_name, $feat->cigar_string, $feat->start, $feat->end, $feat->hstart, $feat->hend );
}
