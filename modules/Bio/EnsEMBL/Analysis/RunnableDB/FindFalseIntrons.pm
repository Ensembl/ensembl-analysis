...
GENES: for my $g (@genes) {
    ...
    for my $intron ( values %unique_introns ) {
      ...
      if ( $intron->length < 150 ) {
        ...
        } ## end if ( scalar(@$aligned_homolog_trans...))
        else {
          print "no hit -  intron did not align\n";
        }
        print "\n\n";
      } ## end if ( $intron->length <...)
      else {
        # // length < xxx
    ...
    for my $f_tsi ( keys %false_transcripts ) {
      ...
      for my $false_intron ( @{ $false_transcripts{$f_tsi} } ) {
        print "\tFI coding in HOMOLOGUES: " .
          join( ",", map { $_->stable_id } @{ $intron_is_coding_in_homologues{$false_intron} } ) . "\n";
      ...
      } ## end for my $false_intron ( ...)

    } ## end for my $f_tsi ( keys %false_transcripts)
  ...
  for my $set ( keys %transcripts_marked_for_deletion ) {
    ...
    for my $tsi ( keys %{ $transcripts_marked_for_deletion{$set} } ) {
      ...
      my $feature = Bio::EnsEMBL::SimpleFeature->new( -seq_region_start => $rm_transcript->seq_region_start,
  ...
  my $analysis_obj = new Bio::EnsEMBL::Pipeline::Analysis( -logic_name    => "RecoverFalseIntrons",
                                                           -program       => "exonerate",
                                                           -module        => "RecoverFalseIntrons",
                                                           -input_id_type => 'prot_slice_form', );
  my $submit_ana = new Bio::EnsEMBL::Pipeline::Analysis( -logic_name    => "Submit_RecoverFalseIntrons",
                                                         -module        => "Dummy",
                                                         -input_id_type => 'prot_slice_form', );
...
sub reformat_input_ids {
  ...
  } ## end for my $region_key ( keys...)
  return \@new_ids;
} ## end sub reformat_input_ids
...
HOMOLOG: for my $homolog_gene (@homo_genes) {
  ...
  TRANS: for my $homology_trans ( @{ $homolog_gene->get_all_Transcripts } ) {
      ...
      my $intron_id =
        $transcript_of_intron->stable_id . "_" . $intron_to_check->prev_Exon->stable_id . "_" . $intron_to_check->next_Exon->stable_id;
      ...
      TRANSCRIPTS: for my $tr ( @{ $gene->get_all_Transcripts } ) {
        ...
        my %hom_coding_alinging_exons =
          %{ exonerate_intron_vs_exons( $intron_to_check, $homology_trans, $intron_id, $transcript_of_intron ) };
          ...
          for my $hom_trans_intron (@introns_in_homolog_gene) {
            if ( $hom_trans_intron->overlaps($exon) ) {
              ...
              print $hom_trans_intron->prev_Exon->stable_id .
                "   " . $hom_trans_intron->next_Exon->stable_id . "   " . $exon->stable_id . "\n";
              print "\nXXXXXXXXXXXXXX\n";
            }
          }
        }
      } ## end if ( $query_length > 0)
    }    # // foreach transcript
  }    # // foreach homolog
  return ( \@aligned_homolog_transcripts );
} ## end sub exonerate_intron_vs_homologs
  ...
  for my $version (@versions) {
    for my $extra_opt (@options) {
      ...
      if ($hit) {
        ...
        print "\nIntron_exonerate_alignment_OK :  " . $transcript_of_intron->stable_id .
          " [ " . $intron_to_check->prev_Exon->stable_id . " <intron>  " . $intron_to_check->next_Exon->stable_id .
          "]  ALIGNS " . $target->stable_id . " ]  QLEN $query_length iLEN " . $intron_to_check->length . "\n\n";
  ...
  for my $hom_exon (@homology_exons) {
    ...
    if ($hit) {
      print "\ntest-intron aligns vs coding exon : " . $transcript_of_intron->stable_id .
        " [ " . $intron_to_check->prev_Exon->stable_id . " <intron>  " . $intron_to_check->next_Exon->stable_id .
        "]  ALIGNS " . $hom_exon->stable_id . " ]  QLEN $query_length iLEN " . $intron_to_check->length . "\n\n";
    ...
    system("rm $f2");
    system("rm $f3");
  } ## end for my $hom_exon (@homology_exons)
  system("rm $f1");
  return \%aligning_hom_exons;
} ## end sub exonerate_intron_vs_exons
  ...
  for my $tr ( @{ $g->get_all_Transcripts } ) {
    ...
    foreach ( my $i = 0; $i < scalar(@tr_exons); $i++ ) {
      ...
      if ( $i + 1 < scalar(@tr_exons) ) {
        ...
        my $intron_hkey =
          $intron->slice->name . "-" . $intron->seq_region_start . "-" . $intron->seq_region_end . "-" . $intron->seq_region_strand;
...
} ## end sub get_unique_introns_of_transcripts
...
sub fetch_input {
  my ($self) = @_;
  print "\n\nChecking introns for the species configured in " . " Config/GeneBuild/OrthologEvaluator.pm : $$MAIN_CONFIG{QUERY_SPECIES}\n";
  ...
  unless ($ga) {
    print " I can't get a gene-adaptor for " .
      $self->cmp_species . "\n" . " Maybe you forgot to add the alias for this to your compara registry file\n";
    exit(0);
  }

  print "==> Fetching gene from db : " . $ga->db->dbname . " \@ " . $ga->db->host . " : " . $ga->db->port . "\n";
