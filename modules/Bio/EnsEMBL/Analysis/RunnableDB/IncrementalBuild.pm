...
sub fetch_input {
  my ($self) = @_;
  my $secondary_slice = $self->fetch_sequence( $self->input_id, $self->secondary_database );
  my $secondary_genes = $self->get_Genes( $secondary_slice, $self->SECONDARY_BIOTYPES );
  my $primary_slice = $self->fetch_sequence( $self->input_id, $self->primary_database );
  my $primary_genes = $self->get_Genes( $primary_slice, $self->PRIMARY_BIOTYPES );
...
sub read_and_check_config {
                           ...
                           OUTPUT_DB) )
  {
    ...
    throw( "You must define $config_var in config for logic '" . $self->analysis->logicname . "'" ) if not defined $self->$config_var;
  }
  ...
  if ( $self->SECONDARY_FILTER ) {
    if ( $self->SECONDARY_FILTER->{OBJECT} ) {
      ...
      throw( "Couldn't require " . $class . " IncrementalBuild:read_and_check_config $@" ) if ($@);
...
sub mask_gene_regions {
  ...
  foreach my $gene (@$genes) {
    push @mask_gene_regions, { start => $gene->start, end => $gene->end };
  }
  @mask_gene_regions = sort { $a->{'start'} <=> $b->{'start'} } @mask_gene_regions;
