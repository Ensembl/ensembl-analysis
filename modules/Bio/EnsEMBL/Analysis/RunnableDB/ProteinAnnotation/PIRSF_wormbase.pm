...
=head1 NAME
  ...
  Bio::EnsEMBL::Pipeline::RunnableDB::ProteinAnnotation::PIRSF_wormbase

=head1 SYNOPSIS

  my $seg = Bio::EnsEMBL::Pipeline::RunnableDB::ProteinAnnotation::PIRSF_wormbase->
    new ( -db      => $db,
    -input_id   => $input_id,
    -analysis   => $analysis,
                                                                      );
...
use Bio::EnsEMBL::Analysis::Runnable::ProteinAnnotation::PIRSF_wormbase;
...
sub fetch_input {
  ...
  # PIRSF_wormbase needs sequence lengths, so we must create one Runnable
  ...
  foreach my $seq (@query_seqs) {
    my $run =
      Bio::EnsEMBL::Analysis::Runnable::ProteinAnnotation::PIRSF_wormbase->new( -query    => $seq,
