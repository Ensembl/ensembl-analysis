...
} ## end sub extract_range_out_of_fasta
...
sub fetch_input {
  ...
  my $path_to_chunk_file = get_file_from_s3(
           $self->chunk_bucket_name, $self->chunk, $analysis_work_dir, 1, undef,    # config file set as environment var or in S3Config.pm
  ...
  my $path_to_genomic_file = get_file_from_s3(
    $self->genomic_bucket_name, $self->genomic_file_name, $analysis_work_dir, 1,
