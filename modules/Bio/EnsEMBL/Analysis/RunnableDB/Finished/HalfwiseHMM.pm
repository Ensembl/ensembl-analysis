...
sub fetch_input {
  ...
  foreach my $database ( @{$GB_SIMILARITY_DATABASES} ) {
    ...
    my $fps         = [];
    my $features    = $alignAdaptor->fetch_all_by_Slice_and_pid( $slice, $threshold, $analysis_ln );
    ...
    my $runnable =
      Bio::EnsEMBL::Analysis::Runnable::Finished::HalfwiseHMM->new( '-query'    => $maskedslice,
                                                                    '-features' => $fps,
  ...
  } ## end foreach my $database ( @{$GB_SIMILARITY_DATABASES...})
...
sub get_db_version {
  my ( $self, $db ) = @_;
  my $ver = Bio::EnsEMBL::Analysis::Tools::BlastDBTracking::get_db_version_mixin( $self, '_pfam_db_version', $db, );
  return $ver;
}
...
sub run {
  ...
  foreach my $runnable ( $self->runnable ) {
    ...
    if ( my $err = $@ ) {
      chomp $err;
      $self->failing_job_status($1) if $err =~ /^\"([A-Z_]{1,40})\"$/i;    # only match '"ABC_DEFGH"'
      throw("$@");
    }
  }
...
sub _make_genes {
  ...
  foreach my $tmp_gene (@$results) {
    ...
    push( @genes, $gene );
  } ## end foreach my $tmp_gene (@$results)
  return @genes;
} ## end sub _make_genes
...
sub _check_that_external_db_table_populated {
  ...
  # insert the row
  my $insert_sql = q`INSERT INTO external_db (external_db_id, db_name, db_release, status, priority) VALUES(?, ?, ?, ?, 5)`;
  ...
  $sth->execute();
  return $max_db_id;
} ## end sub _check_that_external_db_table_populated
