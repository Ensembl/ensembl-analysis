...
sub read_and_check_config {
  ...
  throw( "BLAST_PARSER must be defined either in the DEFAULT entry or in " .
         "the hash keyed on " . $self->analysis->logic_name . " Blast::read_and_check_config" )
  ...
  #refs
  throw( "PARSER_PARAMS must be a hash ref not " . $self->PARSER_PARAMS . " Blast::read_and_check_config" )
    if ( $self->PARSER_PARAMS && ref( $self->PARSER_PARAMS ) ne 'HASH' );
  throw( "FILTER_PARAMS must be a hash ref not " . $self->FILTER_PARAMS . " Blast::read_and_check_config" )
    if ( $self->FILTER_PARAMS && ref( $self->FILTER_PARAMS ) ne 'HASH' );
  throw( "BLAST_PARAMS must be a hash ref not " . $self->BLAST_PARAMS . " Blast::read_and_check_config" )
...
sub fetch_input {
  ...
  $slice->{'seq'} = $slice->seq();
  $self->query($slice);
  my $hard_masked_slice = $slice->get_repeatmasked_seq( $ANALYSIS_REPEAT_MASKING, 0 ) or throw("Unable to fetch contig");
  my $soft_masked_slice = $slice->get_repeatmasked_seq( $ANALYSIS_REPEAT_MASKING, 1 ) or throw("Unable to fetch contig");
  ...
  my $runnable =
    Bio::EnsEMBL::Analysis::Runnable::Finished::BlastExonerate->new( '-hard_masked' => $hard_masked_slice,
...
sub check_with_seg {
  ...
  my $file = Bio::SeqIO->new( -file => ">$filename", -format => 'Fasta' ) or throw("Can't create Bio::SeqIO $filename $!");
  $file->write_seq($seqObj_to_test);

  my $seg_cmd = "nseg $filename -x";
  my $seg = Bio::SeqIO->new( -file => "$seg_cmd |", -format => 'Fasta' ) or throw("Can't create Bio::SeqIO $seg_cmd $!");
  my $seq;
  eval { $seq = $seg->next_seq->seq; };
  unlink($filename);

  if ($@) {
    throw("There was a problem with SEG masking.\nI tried to '$seg_cmd'");
  }
  if ( $seq =~ /[CATG]{3}/i ) {
    $self->input_is_void(0);
  }
...
sub run {
  ...
  foreach my $runnable (@$runnables) {
    ...
    if ( my $err = $@ ) {
      chomp $err;
      $self->failing_job_status($1) if $err =~ /^\"([A-Z_]{1,40})\"$/i;    # only match '"ABC_DEFGH"' and not all possible throws
      throw("$@");
    }
