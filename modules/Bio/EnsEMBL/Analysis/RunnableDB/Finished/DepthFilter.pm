...
sub fetch_input {
  my ($self) = @_;
  my $slice = $self->fetch_sequence( $self->input_id, $self->db, $ANALYSIS_REPEAT_MASKING, $SOFT_MASKING );
  $self->query($slice);
...
sub run {
  ...
  my ( $filtered_features, $saturated_zones ) =
    $self->depth_filter( $orig_features, $slice, $max_coverage, $percentid_cutoff, $hits_to_keep, $no_filter, $max_hits_per_meta_cluster,
                         $hit_db );
...
sub get_original_features {
  ...
  } ## end if ( $hit_db || $taxon_id...)
  else {
    return $orig_features;
  }

  return $hit_db_features;
} ## end sub get_original_features

sub depth_filter {

  my ( $self, $orig_features, $slice, $max_coverage, $percentid_cutoff, $hits_to_keep, $no_filter, $max_hits_per_meta_cluster, $hit_db )
    = @_;
  ...
  my @bisorted =
    sort { ( $b->{max_score} <=> $a->{max_score} ) || ( $b->{max_percentid} <=> $a->{max_percentid} ) } values %grouped_byname;
  ...
  for my $node (@bisorted) {
    ...
    $keep_node = 1 if $no_filter && grep( /^$node->{taxon_id}$/, @{ $self->get_taxon_id_child($no_filter) }, $no_filter );
