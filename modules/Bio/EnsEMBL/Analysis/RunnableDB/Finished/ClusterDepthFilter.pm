...
sub group_features_by_name {
  ...
  for my $af (@$orig_features) {
    ...
    if ( $no_filter && $self->get_hit_description( $af->hseqname() ) ) {
      $hit->{taxon_id} = $self->get_hit_description( $af->hseqname() )->taxon_id;
    }
  }
...
sub break_discontinuities {
  ...
  for my $hit ( values %$hits_by_name ) {
    ...
    my @remaining_features =
      sort { $a->hstrand <=> $b->hstrand || $a->start <=> $b->start || $a->hstart <=> $b->hstart } @{ $hit->{features} };
    ...
    while (@remaining_features) {
      ...
      print STDERR "ClusterDepthFilter: starting new hit: hstrand: ", $last->hstrand, " af: ", $last->hstart, "-", $last->hend, " (",
        $last->start, "-", $last->end, ")\n"
        if $DEBUG;
    ...
    FEAT: for my $af (@features) {
             ...
             $last->end == $af->end )
        {
          ...
          print STDERR "ClusterDepthFilter: Found duplicate align " . "feature for hit " . $af->hseqname .
            " [coords: " . $af->hstart . "-" . $af->hend . " (" . $af->start . "-" . $af->end . ")" . ", slice: " . $slice->name . "]\n";

          next FEAT;
        }

        my $delta = $af->hstrand == $af->strand ? ( $af->hstart - $last->hend ) : ( $last->hstart - $af->hend );

        if ( $af->hstrand == $last->hstrand && $af->start <= $last->end ) {

          # found an overlap - bump for reprocessing
          print STDERR "ClusterDepthFilter: found overlap: hstrand: ", $af->hstrand, " last: ", $last->hstart, "-", $last->hend, " (",
            $last->start, "-", $last->end, ") af: ", $af->hstart, "-", $af->hend, " (", $af->start, "-", $af->end, ")\n"
            if $DEBUG;
          ...
          print STDERR "ClusterDepthFilter: discontinuity: hstrand: ", $af->hstrand, " last: ", $last->hstart, "-", $last->hend, " (",
            $last->start, "-", $last->end, ") af: ", $af->hstart, "-", $af->hend, " (", $af->start, "-", $af->end, ")\n"
            if $DEBUG;
          ...
          print STDERR "ClusterDepthFilter: continuing hit: hstrand: ", $af->hstrand, " last: ", $last->hstart, "-", $last->hend, " (",
            $last->start, "-", $last->end, ") af: ", $af->hstart, "-", $af->hend, " (", $af->start, "-", $af->end, ")\n"
            if $DEBUG;
  ...
  } ## end for my $hit ( values %$hits_by_name)

  return \@hits;
} ## end sub break_discontinuities
...
sub cluster_hits {
  ...
  my @hits =
    sort { ( $a->{features}->[0]->start <=> $b->{features}->[0]->start ) || ( $b->{features}->[-1]->end <=> $a->{features}->[-1]->end ) }
    @$hits;
  ...
  for my $hit (@hits) {
    ...
    for my $cluster (@clusters) {
           ...
           is_consistent( $hit, $cluster ) )
      {
        ...
        print STDERR "ClusterDepthFilter: can't add hit to cluster: ", cluster_str($cluster), "\n" if $DEBUG;
      }
    }
    ...
    if ( !$added ) {
      ...
      my $cluster =
        { start => $hit->{start}, end => $hit->{end}, introns => [ map { [@$_] } @introns ], hits => [$hit], strand => $hit->{strand} };
      push @clusters, $cluster;

      print STDERR "ClusterDepthFilter: creating new cluster: ", cluster_str($cluster), "\n" if $DEBUG;
    }
  } ## end for my $hit (@hits)

  return \@clusters;
} ## end sub cluster_hits
...
} ## end sub compute_vulgar_strings
...
} ## end sub is_consistent_by_vulgar_string
...
sub cluster_hits_by_strings {
  ...
  for my $hit (@hits) {
    ...
    if ( !$added ) {
      ...
      my $cluster =
        { start => $hit->{start}, end => $hit->{end}, vulgar_string => $hit->{vulgar_string}, hits => [$hit], strand => $hit->{strand} };
      push @clusters, $cluster;
  ...
  return @clusters;
} ## end sub cluster_hits_by_strings
...
sub cluster_clusters {
  ...
  for my $cluster (@$clusters) {
    ...
    for my $meta (@meta_clusters) {
      if ( overlap( $cluster->{start}, $cluster->{end}, $meta->{start}, $meta->{end} ) && $cluster->{strand} == $meta->{strand} ) {
    ...
    if ( !$added ) {
      my $new_meta =
        { start => $cluster->{start}, end => $cluster->{end}, strand => $cluster->{strand}, clusters => [$cluster], hits_added => 0 };
    ...
    for my $cluster ( @{ $meta_cluster->{clusters} } ) {
      ...
      for my $hit ( @{ $cluster->{hits} } ) {
        ...
        if ( defined $start ) {
          ...
          my $new_introns = find_new_introns( $introns, $hit->{introns} );
      ...
      if ( defined $start ) {
        ...
        for my $intron (@$introns) {
          my $summary = Bio::EnsEMBL::SimpleFeature->new(-start         => $start,
                                                         -end           => $intron->[0],
                                                         -strand        => $cluster->{strand},
                                                         -score         => $score,
                                                         -display_label => sprintf( "summary feature from %d redundant hits", $filtered ),
        ...
        my $summary = Bio::EnsEMBL::SimpleFeature->new( -start         => $start,
                                                        ...
                                                        -analysis      => $self->analysis() );
        push @summary_features, $summary;
      } ## end if ( defined $start )

      $tot_filtered += $filtered;
    } ## end for my $cluster ( @{ $meta_cluster...})
  } ## end for my $meta_cluster (@$meta_clusters)

  print STDERR "ClusterDepthFilter: built summary features from $tot_filtered filtered hits\n";

  return \@summary_features;
} ## end sub build_summary_features

sub filter_features {

  my ( $self, $meta_clusters, $hits_to_keep, $no_filter, $matching_taxa, $max_hits_per_meta_cluster ) = @_;
  ...
  for my $meta_cluster (@$meta_clusters) {
    ...
    for my $cluster (@clusters) {
      ...
      $cluster->{hits} =
        [ sort { ( $b->{tot_score} <=> $a->{tot_score} ) || ( $b->{avg_percentid} <=> $a->{avg_percentid} ) } @{ $cluster->{hits} } ];
      ...
      print STDERR "ClusterDepthFilter: adding features of cluster: ", $cluster->{start}, "-", $cluster->{end}, "\n" if $DEBUG;
      ...
      while ( $added < $hits_to_keep && @{ $cluster->{hits} } ) {
        ...
        print STDERR "ClusterDepthFilter: adding features of hit: ", $hit_to_keep->{start}, "-", $hit_to_keep->{end}, "\n" if $DEBUG;

        for my $af ( @{ $hit_to_keep->{features} } ) {
          print STDERR "ClusterDepthFilter: adding feature: ", $af->start, "-", $af->end, "\n" if $DEBUG;
    ...
    } ## end for my $cluster (@clusters)

    print STDERR "ClusterDepthFilter: meta-cluster hits added: " . $meta_cluster->{hits_added} . "\n" if $DEBUG;

    if ( $no_filter && $max_hits_per_meta_cluster && ( $meta_cluster->{hits_added} < $max_hits_per_meta_cluster ) ) {
      ...
      @candidates = sort { ( $b->{tot_score} <=> $a->{tot_score} ) || ( $b->{avg_percentid} <=> $a->{avg_percentid} ) } @candidates;

      # and add as many as possible

      while ( $meta_cluster->{hits_added} < $max_hits_per_meta_cluster && @candidates ) {
        ...
        for my $af ( @{ $hit_to_keep->{features} } ) {
          print STDERR "ClusterDepthFilter: adding extra feature: ", $af->start, "-", $af->end, "\n" if $DEBUG;
    ...
    } ## end if ( $no_filter && $max_hits_per_meta_cluster...)
  } ## end for my $meta_cluster (@$meta_clusters)
...
sub depth_filter {
  ...
  my ( $orig_features, $slice, $max_coverage, $percentid_cutoff, $hits_to_keep, $no_filter, $max_hits_per_meta_cluster, $hit_db ) = @_;
  ...
  print STDERR "ClusterDepthFilter: MaxHitsPerMetaCluster=$max_hits_per_meta_cluster\n";
  print STDERR "ClusterDepthFilter: " . scalar(@$orig_features) . " features before filtering\n";
  ...
  my $grouped_by_name = $self->group_features_by_name( $orig_features, $percentid_cutoff, $no_filter );

  print STDERR "ClusterDepthFilter: " . scalar( keys %$grouped_by_name ) . " unique hitnames\n";

  # first break apart any hits that contain discontinuous hits

  my $hits = $self->break_discontinuities( $grouped_by_name, $no_filter, $slice );
  ...
  print STDERR "ClusterDepthFilter: grouped " . scalar(@$hits) . " hits into " . scalar(@$clusters) . " clusters";
  ...
  print STDERR sprintf( " (average of %.2f hits per cluster)", ( scalar(@$hits)/scalar(@$clusters) ) ) if @$clusters > 0;
  ...
  print STDERR "ClusterDepthFilter: grouped " . scalar(@$clusters) . " clusters into " . scalar(@$meta_clusters) . " meta clusters\n";
  ...
  my ( $filtered_features, $summary_features ) =
    $self->filter_features( $meta_clusters, $hits_to_keep, $no_filter, \@matching_taxa, $max_hits_per_meta_cluster );

  print STDERR "ClusterDepthFilter: " . scalar(@$summary_features) . " summary features\n";
