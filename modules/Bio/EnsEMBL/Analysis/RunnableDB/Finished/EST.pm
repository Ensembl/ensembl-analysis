...
sub fetch_input {
  ...
  my $unmasked = Bio::PrimarySeq->new( -display_id => $slice->display_id(), -id => $slice->id(), -seq => $slice->seq() );
  my $seq = $slice->seq;
  ...
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::Finished::EST->new( '-query'    => $maskedslice,
                                                                       '-unmasked' => $unmasked,
