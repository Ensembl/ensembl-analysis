...
sub fetch_input {
  ...
  $self->get_initial_geneset( $$MAIN_CONFIG{QUERY_SPECIES}, $$FIND_PARTIAL_GENES{DEFAULT_GENE_BIOTYPES} );
...
QUERY_GENES: for my $query_gene ( @{ $self->genes } ) {
    ...
    if ($tg1_homol) {
      ...
      if ( $tg2_homol || $tg3_homol ) {
        ...
        " " . $tg2_species . " " . ( defined($tg2_homol) ? $len_longest_tg2 : "N/A" ) . " " . $tg3_species . " " .
          ( defined($tg3_homol) ? $len_longest_tg3 : "N/A" ) . "\n";
             ...
             $ratio_tg1_tg3 > $ratio_tg1_tg3_low && $ratio_tg1_tg3 < $ratio_tg1_tg3_high )
        {
          if ( $ratio_tg1_qy < $ratio_tg1_qy_75 ) {
            print "Significant length difference for :" . $qy_species . "\t" . $longest_qy->stable_id .
              "\t" . $query_gene->stable_id . " (" . ( $query_gene->external_name ? $query_gene->external_name : '' ) . ")" . " with " .
              $tg1_species . "\t" . $longest_tg1->stable_id . "\t" . $tg1_homol->stable_id . " (" . $tg1_homol->external_name . ")\n";
      ...
      } ## end if ( $tg2_homol || $tg3_homol)
    } ## end if ($tg1_homol)
  } ## end QUERY_GENES: for my $query_gene ( @{...})
} ## end sub run
...
sub write_output {
  ...
  foreach my $id ( keys %identified_partials ) {
    ...
    foreach my $match_gene ( @{ $identified_partials{$id}{matches} } ) {
      printf " %-9s %18s %3s %9d %9d     %2d   0 %18s %3s %8d %8d     %2d   0\n", "NA", $gene->stable_id, $gene->slice->seq_region_name,
        $gene->start, $gene->end, $gene->strand, $match_gene->stable_id, $match_gene->slice->seq_region_name, $match_gene->start,
...
sub read_and_check_config {
  ...
  foreach my $logic_name ( keys %analysis_sets ) {
    ...
    unless ( exists $$EXONERATE_CONFIG_BY_LOGIC{$logic_name} ) {
      throw( "You have defined a logic_name " . "\"$logic_name\"\n in our OrthologueEvaluator.pm" .
             ...
             " I suggest to add a configuration for $logic_name to your " . "Exoneate2Genes.pm config\n" );
    }
    ...
    unless ($analysis) {
      throw( "There's no analysis $logic_name in your database - this analysis is normally added by" . " the setup script\n" );
    }
