...
sub fetch_input {
  ...
  my @features;
  $chr_slice = $self->repeat_slice_adaptor->fetch_by_region( 'toplevel', $slice->seq_region_name, );

  $self->chr_slice($chr_slice);
  my $repeat_slice = $self->repeat_slice_adaptor->fetch_by_region( 'toplevel', $slice->seq_region_name, $slice->start, $slice->end, 1 );
  my @repeats =
    sort { $a->start <=> $b->start } @{ $self->repeat_feature_adaptor->fetch_all_by_Slice( $repeat_slice, $self->REPEAT_LN ) };
  ...
  my $feature_slice = $feature_slice_adaptor->fetch_by_region( 'toplevel', $slice->seq_region_name, $slice->start, $slice->end, );
  $self->slice($feature_slice);
  $chr_slice = $feature_slice_adaptor->fetch_by_region( 'toplevel', $slice->seq_region_name, );
...
sub batch_fetch {
  ...
  unless ($start) {
    ...
    $end = $self->sql(
      "  select max( dna_align_feature_id) from dna_align_feature where seq_region_id = $seq_id and seq_region_start >= " .
        $slice->start . " 
and seq_region_start < " . $slice->end, $self->get_dbadaptor( $self->ALIGNMENT_DB ) )->[0];
    $start = $self->sql(
      "  select min( dna_align_feature_id) from dna_align_feature where seq_region_id = $seq_id and seq_region_start >= " .
        $slice->start . " 
and seq_region_start < " . $slice->end, $self->get_dbadaptor( $self->ALIGNMENT_DB ) )->[0];
  ...
  @dafs = @{
    $daf_adaptor->fetch_all_by_Slice_constraint( $slice,
                                                 " dna_align_feature_id >= $batch_start AND dna_align_feature_id <= $batch_end " ) };
  ...
  if ( $self->BATCH_FETCH ) {
    ...
    while ( $self->read_count > 0 ) {
      ...
      foreach my $gene_cluster (@$gene_clusters) {
        ...
        print $gene_cluster->{'start'} . " " . $gene_cluster->{'end'} . " ";
      ...
      for ( my $i = 1; $i < scalar(@tmp_clusters); $i++ ) {
        ...
        if ( $left_cluster->end >= $right_cluster->start && $right_cluster->end >= $left_cluster->start ) {

          # merge them
          $left_cluster->start( $right_cluster->start ) if $right_cluster->start < $left_cluster->start;
          $left_cluster->end( $right_cluster->end ) if $right_cluster->end > $left_cluster->end;
          # merge scores
      ...
      } ## end for ( my $i = 1; $i < scalar...)
      $self->batch_fetch;
    } ## end while ( $self->read_count...)
  } ## end if ( $self->BATCH_FETCH)
  else {
    my @reads = @{ $self->reads };
    # do it all in one go
...
sub process_exon_clusters {
  ...
  foreach my $read ( keys %{$cluster_data} ) {
    ...
    for ( my $i = 1; $i < @clusters; $i++ ) {
      ...
      $cluster_hash->{$left_cluster}->{$right_cluster}++ unless $left_cluster eq $right_cluster;
    }
  }
  ...
  foreach my $key ( sort keys %{$clean_cluster_hash} ) {
    ...
    if ( $clean_cluster_hash->{$key}->{'left'} && $clean_cluster_hash->{$key}->{'left'} >= $self->END_EXON_COVERAGE ) {
      $clean_exon_clusters->{$key} = 1;
      next;
    }
    # is it a well supported right end exon?
    if ( $clean_cluster_hash->{$key}->{'right'} && $clean_cluster_hash->{$key}->{'right'} >= $self->END_EXON_COVERAGE ) {
      $clean_exon_clusters->{$key} = 1;
      next;
    }
    # othwise ignore it
...
LOOP: while ( scalar(@clean_clusters) > 0 ) {
  ...
  } ## end LOOP: while ( scalar(@clean_clusters...))
...
READ: foreach my $ugf (@ugfs) {
    ...
    unless ($clustered) {
      ...
      my $feat = Bio::EnsEMBL::FeaturePair->new( -start      => $ugf->start,
                                                 -end        => $ugf->end,
                                                 -strand     => $ugf->strand,
                                                 -slice      => $ugf->slice,
                                                 -hstart     => $ugf->hstart,
...
sub pad_exons {
  ...
  foreach my $exon (@exon_clusters) {
    my $padded_exon = create_Exon( $exon->start - 20, $exon->end + 20, -1, -1, -1, $exon->analysis, undef, undef, $self->chr_slice, );
    ...
    my $feat = new Bio::EnsEMBL::DnaDnaAlignFeature( -slice        => $exon->slice,
  ...
  foreach my $f (@features) {
    ...
    foreach my $cluster (@clusters) {
      ...
      if ( $f->start <= $cluster->{'end'} && $f->end >= $cluster->{'start'} ) {
        # Expand the cluster
        $cluster->{'start'} = $f->start if $f->start < $cluster->{'start'};
        $cluster->{'end'}   = $f->end   if $f->end > $cluster->{'end'};
