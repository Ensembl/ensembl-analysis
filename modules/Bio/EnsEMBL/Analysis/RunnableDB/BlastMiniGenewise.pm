...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptUtils
  qw(convert_to_genes Transcript_info set_start_codon set_stop_codon list_evidence attach_Slice_to_Transcript);
...
#print "****HAVE ".@$features." features with ".$logic_name." and min score ".$self->PAF_MIN_SCORE_THRESHOLD."  with ".keys(%unique)." unique hit names from ".$self->paf_slice->adaptor->dbc->dbname."*****\n";
  ...
  FEATURE: foreach my $feature (@$features) {
      ...
      logger_info( "SKIPPING " . $feature->hseqname . " on kill list" ) if ( $kill_list{$temp_id} );
      next FEATURE                                                      if ( $kill_list{$temp_id} );
      logger_info( "SKIPPING " . $feature->hseqname . " as greater than upper threshold" )
        if ( $self->PAF_UPPER_SCORE_THRESHOLD and $feature->score < $self->PAF_UPPER_SCORE_THRESHOLD );
      next FEATURE if ( $self->PAF_UPPER_SCORE_THRESHOLD and $feature->score < $self->PAF_UPPER_SCORE_THRESHOLD );
      logger_info( "SKIPPING " . $feature->hseqname . " on the mask list" )
        if ( $self->PRE_GENEWISE_MASK && exists $ids_to_ignore->{ $feature->hseqname } );
      next FEATURE if ( $self->PRE_GENEWISE_MASK && exists $ids_to_ignore->{ $feature->hseqname } );
  ...
  if ( $self->use_id && !( keys(%hit_list) ) ) {
    my $feature = $self->feature_factory->create_feature_pair( $self->query->start, $self->query->end, 1, 1, 1, 1, 1, $self->use_id );
  ...
  if ( $self->id_pool_bin and $self->id_pool_index ) {
    ...
    my (@restricted_list);
    for ( my $i = $self->id_pool_index - 1; $i < @local_ids; $i += $self->id_pool_bin ) {
  ...
  if ( $killed_count && $feature_count && $killed_count == $feature_count ) {
    warning( "RunnableDB:BlastMiniGenewise Killed all the potential " . "ids in this run there are NO IDS TO RUN" );
    return;
  }
  if ( scalar( keys(%hit_list) ) == 0 && $feature_count >= 1 ) {
    warning( "RunnableDB:BlastMiniGenewise not all ids were killed but you " . "still have NO IDS TO RUN with" );
    return;
  }
...
} ## end sub generate_ids_to_ignore
...
sub parse_input_id {
  ...
  my ( $slice_name, $first, $second ) = ( $input_id =~ /^([^:]*:[^:]*:[^:]*:[^:]*:[^:]*:[^:]*)(?::([^:]+):([^:]+))?$/ );

  throw( "RunnableDB::BlastMiniGenewise Your input id " . $input_id . " doesn't " .
         "match the regex and seems to be missing a slice name in the form " . "coordsystem:version:seq region name:start:end:strand" )
    if ( !$slice_name );
  ...
  if ( $first && $second ) {
    if ( $first =~ /^\d+$/ and $second =~ /^\d+$/ ) {
      ...
      if ( $self->id_pool_index > $self->id_pool_bin or $self->id_pool_index < 1 ) {

        warning( "RunnableDB::BlastMiniGenewise could not get sensible values for " .
                "id pool bin " . $self->id_pool_bin . " or id pool index " . $self->id_pool_index . " will us complete set of proteins" );
        $self->id_pool_bin(0);
...
sub create_bmg_runnables {
  ...
  throw( "RunnableDB::BlastMiniGenewise Can't create BlastMiniGenewise " . "runnables without a hash of hits" ) unless ($hits_hash);
  ...
  if ( $self->LIMIT_TO_FEATURE_RANGE ) {
    foreach my $id ( keys(%$hits_hash) ) {
      ...
      foreach my $feature (@$features) {
        ...
        my $name = ( $self->query->coord_system->name . ":" . $self->query->coord_system->version .
                     ":" . $self->query->seq_region_name . ":" . $start . ":" . $end . ":" . $self->query->strand );
        my $query = $self->fetch_sequence( $name, $self->db, $self->REPEATMASKING, $self->SOFTMASKING );
        logger_info(
          "Creating BlastMiniGenewise Runnable over a limited range with " . $id . " and " . $seqfetcher . " to run on " . $query->name );
        my $runnable =
          Bio::EnsEMBL::Analysis::Runnable::BlastMiniGenewise->new( -query                     => $query,
                                                                    -analysis                  => $analysis,
                                                                    ...
                                                                    -ids                       => [$id],
                                                                    %bmg_params, %params_hash, );
        $self->runnable($runnable);
      }
    } ## end foreach my $id ( keys(%$hits_hash...))
  } ## end if ( $self->LIMIT_TO_FEATURE_RANGE)
  else {
    ...
    my $runnable =
      Bio::EnsEMBL::Analysis::Runnable::BlastMiniGenewise->new( -query                     => $self->query,
                                                                -analysis                  => $analysis,
                                                                -seqfetcher                => $seqfetcher,
                                                                -minigenewise_options      => $self->MINIGENEWISE_PARAMETERS,
                                                                -genewise_options          => $self->GENEWISE_PARAMETERS,
...
sub create_mask_list {
  ...
  GENE: foreach my $gene ( @{ $slice->get_all_Genes_by_type($biotype) } ) {
      my @mask_exons = grep { $_->seqname eq $slice->id } ( sort { $a->start <=> $b->start } @{ $gene->get_all_Exons } );
      next GENE unless ( @mask_exons && scalar(@mask_exons) > 0 );
      push @mask_gene_regions, { start => $mask_exons[0]->start, end => $mask_exons[-1]->end };

      foreach my $mask_exon (@mask_exons) {
        push @mask_exon_regions, { start => $mask_exon->start, end => $mask_exon->end };
      }
    }
  }
  ...
  throw( "RunnableDB:BlastMiniGenewise can't specify masking both Gene and Exon " . "level masking " )
    if ( $self->GENE_BASED_MASKING && $self->EXON_BASED_MASKING );
  $self->gene_mask_list( \@nr_mask_gene_regions );
  $self->exon_mask_list( \@nr_mask_exon_regions );
  return \@nr_mask_exon_regions;
} ## end sub create_mask_list
...
sub run {
  ...
  my @genes = @{ convert_to_genes( $complete_transcripts, $self->analysis, $self->OUTPUT_BIOTYPE ) };
...
sub filter_genes {
  ...
  if ( $self->filter_object ) {
    my ( $filtered_set, $rejected_set ) = $self->filter_object->filter_genes($genes);
...
sub process_transcripts {
  ...
  foreach my $transcript (@$transcripts) {
    my $unmasked_slice = $self->fetch_sequence( $transcript->slice->name, $self->output_db );
...
sub filter_object {
  my ( $self, $arg ) = @_;
  if ($arg) {
    throw( "RunnableDB::BlastMiniGenewise " . $arg . " must have a method filter_genes" ) unless ( $arg->can("filter_genes") );
  ...
  if ( !$self->{filter_object} && $self->FILTER_OBJECT ) {
    ...
    $arg = $self->FILTER_OBJECT->new( -slice => $self->query, -seqfetcher => $self->seqfetcher, %params, );
    $self->{filter_object} = $arg;
  }
...
sub seqfetcher {
  ...
  if ($arg) {
    throw( "RunnableDB::BlastMiniGenewise " . $arg . " must have a method get_Seq_by_acc" ) unless ( $arg->can("get_Seq_by_acc") );
    $self->{seqfetcher} = $arg;
  }
  if ( !$self->{seqfetcher} ) {
    ...
    print $params{-db}->[0], "\n";
    $arg = $self->SEQFETCHER_OBJECT->new( %params, );
    $self->{seqfetcher} = $arg;
  }
...
sub read_and_check_config {
  ...
  foreach my $var ( qw(PAF_LOGICNAMES
                    ...
                    SEQFETCHER_OBJECT) )
  {
    throw("RunnableDB::BlastMiniGenewise $var config variable is not defined") unless ( $self->$var );
  }
  $self->OUTPUT_BIOTYPE( $self->analysis->logic_name ) if ( !$self->OUTPUT_BIOTYPE );
  throw("PAF_LOGICNAMES must contain at least one logic name") if ( @{ $self->PAF_LOGICNAMES } == 0 );
}
