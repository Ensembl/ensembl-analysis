...
sub new {
  ...
  #and I want the config precedence to be default, param hash, constructor and finally logic_name config over-riding others
...
sub fetch_input{
  ...
  #Check submit analysis exists in pipe db, where input ids of that type will be written
  ...
  my @genes = @{$gene_dba->get_SliceAdaptor->fetch_by_name($self->input_id)->get_all_Genes()};
...
sub get_pipe_dba{
  ...
  if ($self->PIPE_DB) {
    if ( ref($self->PIPE_DB)=~m/HASH/) {
      $pipedba = new Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor(%{$self->PIPE_DB},
                                                -dnadb => $self->DNA_DB);
    }
    else{
