...
sub new {
  ...
  my ( $submit_logic_name, $gene_db,           $pipe_db,       $expansion, $annotation,
       $seqfetcher_object, $seqfetcher_params, $gene_biotypes, $gene_logic_names )
    = rearrange(
                 [ 'SUBMIT_LOGIC_NAME', 'GENE_DB',           'PIPE_DB',           'EXPANSION',
                   'ANNOTATION',        'SEQFETCHER_OBJECT', 'SEQFETCHER_PARAMS', 'GENE_BIOTYPES',
                   'GENE_LOGIC_NAMES', ],
                 @args );
...
sub fetch_input {
  ...
  my $submit_analysis = $dba_pip->get_AnalysisAdaptor->fetch_by_logic_name( $self->SUBMIT_LOGIC_NAME );

  if ( !$submit_analysis ) {
    throw( "Failed to find an analysis with the logic_name " . $self->SUBMIT_LOGIC_NAME . " in the pipeline db. Cannot continue." );
  }
  ...
  if ( $biotypes[0] and $logic_names[0] ) {
    foreach my $logic_name (@logic_names) {
      foreach my $biotype (@biotypes) {
        push( @genes,
              @{$gene_dba->get_SliceAdaptor->fetch_by_name( $self->input_id )->get_all_Genes( $logic_name, undef, undef, undef, $biotype )
              } );
      }
    }
  }    #only biotype
    ...
    foreach my $biotype (@biotypes) {
      push( @genes,
            @{ $gene_dba->get_SliceAdaptor->fetch_by_name( $self->input_id )->get_all_Genes( undef, undef, undef, undef, $biotype ) } );
    }
  }    #only logic name
  elsif ( $logic_names[0] ) {
    foreach my $logic_name (@logic_names) {
      push( @genes, @{ $gene_dba->get_SliceAdaptor->fetch_by_name( $self->input_id )->get_all_Genes($logic_name) } );
    }
...
GENE: foreach my $gene ( @{ $self->genes } ) {
    ...
    if ( ( $genomic_slice->start < 1 ) or ( $genomic_slice->end > $genomic_slice->seq_region_length ) ) {
      ...
      if ( ( $genomic_slice->start < 1 ) and ( $genomic_slice->end > $genomic_slice->seq_region_length ) ) {
        $genomic_slice =
          $self->get_gene_dba->get_SliceAdaptor->fetch_by_region( $genomic_slice->coord_system_name,
                                                                  $genomic_slice->seq_region_name,
                                                                  1, $genomic_slice->seq_region_length,
                                                                  $genomic_slice->strand );
      }
      elsif ( $genomic_slice->start < 1 ) {
        $genomic_slice =
          $self->get_gene_dba->get_SliceAdaptor->fetch_by_region( $genomic_slice->coord_system_name,
                                                                  $genomic_slice->seq_region_name,
                                                                  1, $genomic_slice->end, $genomic_slice->strand );
      }
      else {
        $genomic_slice =
          $self->get_gene_dba->get_SliceAdaptor->fetch_by_region( $genomic_slice->coord_system_name, $genomic_slice->seq_region_name,
                                                                  $genomic_slice->start,             $genomic_slice->seq_region_length,
                                                                  $genomic_slice->strand );
      }
...
sub write_output {
  ...
  foreach my $iid ( @{ $self->output } ) {
    ...
    eval { $sic->store_input_id_analysis( $iid, $self->submit_analysis, '' ); };
...
sub get_gene_dba {
  ...
  if ( $self->GENE_DB ) {
    if ( ref( $self->GENE_DB ) =~ m/HASH/ ) {
      $genedba = new Bio::EnsEMBL::DBSQL::DBAdaptor( %{ $self->GENE_DB }, -dnadb => $self->DNA_DB );
    }
    else {
...
sub get_pipe_dba {
  ...
  if ( $self->PIPE_DB ) {
    if ( ref( $self->PIPE_DB ) =~ m/HASH/ ) {
      $pipedba = new Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor( %{ $self->PIPE_DB }, -dnadb => $self->DNA_DB );
    }
    else {
...
sub seqfetcher {
  ...
  if ($arg) {
    throw( "RunnableDB::Exonerate2GenesRegion " . $arg . " must have a method get_Seq_by_acc" ) unless ( $arg->can("get_Seq_by_acc") );
    $self->{seqfetcher} = $arg;
  }
  if ( !$self->{seqfetcher} ) {
...
sub submit_analysis {
  my ( $self, $value ) = @_;
  if ( $value && $value->isa('Bio::EnsEMBL::Pipeline::Analysis') ) {
...
sub pipeline_adaptor {
  my ( $self, $value ) = @_;
  if ( $value && $value->isa('Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor') ) {
