...
sub fetch_input {
  ...
  print "Found model " .
    $model->stable_id . " " . $model->seq_region_name . " " . $model->start . " " . $model->end . " " . $model->strand . "\n";
  ...
  foreach my $daf_slice (@daf_slices) {
    ...
    push @all_dafs,
      @{ $daf_adaptor->fetch_all_by_Slice_constraint( $daf_slice, "score < ( $score *2  ) AND right(hit_name,1) not in ('a','b')" ) };
  }
  if ( $self->LOGIC_NAMES ) {
  ...
  my $runnable =
    Bio::EnsEMBL::Analysis::Runnable::ExonerateSolexaLocalAlignment->new( -analysis   => $self->analysis,
                                                                          -program    => $program,
    ...
    if ( $f->{'_intron'} ) {
      if ( scalar(@features) == 2 ) {
        ...
        if ( $left_splice->seq eq 'NN' && $right_splice->seq eq 'NN' ) {
          warn( "Cannot find dna sequence for " . $f->display_id . " this is used in detetcting non canonical splices\n" );
        }
        else {
