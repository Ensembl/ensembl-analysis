...
sub new {
  ...
  $self->read_and_check_config_value( $TRANSCRIPT_CONSENSUS_CONFIG_BY_LOGIC,
                                      undef,
                                      [ 'MIN_CONSENSUS',      'END_EXON_PENALTY', 'EST_OVERLAP_PENALTY', 'SHORT_INTRON_PENALTY',
                                        'SHORT_EXON_PENALTY', 'UTR_PENALTY',      'VERBOSE' ] );
...
sub fetch_input {
  ...
  foreach my $db (@databases) {
    ...
    foreach my $input_genetype ( @{ $self->INPUT_GENETYPES } ) {
      ...
      print STDERR "got " .
        scalar( @{$input_genes} ) . " $input_genetype genes [ " . $dba->dbc->dbname() . "@" . $dba->dbc->host . " ]\n";
  ...
  if ( $self->BLESSED_DB and scalar( @{ $self->BLESSED_GENETYPES } ) ) {
    ...
    foreach my $bgt ( @{ $self->BLESSED_GENETYPES } ) {
      ...
      print STDERR "got " .
        scalar( @{$blessed_genes} ) . " $bgt genes [ " . $self->BLESSED_DB->dbname() . "@" . $self->BLESSED_DB->host . " ]\n";
  ...
  $self->EST_DB->dbc->disconnect_when_inactive(1)     if ( $self->EST_DB );
  $self->BLESSED_DB->dbc->disconnect_when_inactive(1) if ( $self->BLESSED_DB );
  $self->DITAG_DB->dbc->disconnect_when_inactive(1)   if ( $self->DITAG_DB );
  ...
  $self->{evidence_sets} = { 'est' => \@est_biotypes, 'simgw' => \@simgw_biotypes, 'cdna' => \@cdna_logicnames, };
...
GENES:
      ...
      if ($blessed) {
        if ( $test_transcript->start < 1 && $test_transcript->end > 1 ) {
          print STDERR "ignoring blessed gene at seq_region/chr " . $test_transcript->seq_region_name . " (" .
            $test_transcript->seq_region_start . "-" . $test_transcript->seq_region_end . ") as it falls off the slice by its lower end\n"
            if $self->VERBOSE;
        }
  ...
  } ## end GENES: foreach my $test_gene (@$genesref)
...
sub run_matching {
  ...
  print STDERR "\n### Got " . scalar(@merged_genes) . " blessed genes which have been filtered and frameshift-exon-merged. "
    if ($blessed);
  print STDERR "\n### Got " . scalar(@merged_genes) . " unblessed genes which have been filtered and frameshift-exon-merged. "
    if ( !$blessed );
  ...
  @merged_genes = sort {
    ...
    unless ($result) {
      return ( ( $b->get_all_Transcripts->[0]->length ) <=> ( $a->get_all_Transcripts->[0]->length ) );
    }
    return $result;
  } @merged_genes;
...
CDS:
  foreach my $cds (@merged_genes) {
    ...
    if ( $self->LOOK_FOR_KNOWN ) {
      ...
      if ( defined $predef_match ) {
        ...
        if ( defined $combined_transcript ) {
          ...
          if ($blessed) {
                 ...
                 has_no_unwanted_evidence($combined_transcript) )
            {
              ...
              if ( $self->find_cluster_joiners($combined_transcript) ) {
                print STDERR
                  "Found a cluster_joiner! Can't use this blessed combined transcript although it's made from predefined cDNA: "
                  if $self->VERBOSE;
              ...
              print STDERR "Blessed combined transcript at seq_region/chr " . $combined_transcript->seq_region_name .
                " " . $combined_transcript->start . "-" . $combined_transcript->end . " did not pass sanity check.\n\n"
                if $self->VERBOSE;
                 ...
                 has_no_unwanted_evidence($combined_transcript) )
            {
              ...
              if ( $self->find_cluster_joiners($combined_transcript) ) {
                print STDERR
                  "Found a cluster_joiner! Can't use this unblessed combined transcript although it's made from predefined cDNA: "
                  if $self->VERBOSE;
                print STDERR " start " . $combined_transcript->seq_region_start . " end " . $combined_transcript->seq_region_end . "\n\n"
                  if $self->VERBOSE;
                $combined_transcript = undef;
          ...
          } ## end elsif ( !$blessed )  [ if ($blessed) ]
        } ## end if ( defined $combined_transcript)
        else {
      ...
      } ## end if ( defined $predef_match)
    } ## end if ( $self->LOOK_FOR_KNOWN)
    ...
    if ( $predef_match && $combined_transcript ) {
      ...
      print STDERR "\nGot " .
        scalar @possible_transcripts . " possible transcripts as UTR evidence after clustering and TranscriptConsensus collapsing.\n";
    ...
    POS:
      ...
      } ## end POS: while ( my $chosen_transcript...)
    }    # end  POS find match
   ...
   # my $combined_genes = $self->make_gene($genetype, $combined_transcript, $blessed);  # this line of code messed up passing of $blessed!
      my $combined_genes =
        $self->make_gene( $genetype, [$combined_transcript], $blessed );    # make_gene method is expecting an array of transcript refs!
      ...
      print STDERR "RESULT-CombinedGene: " .
        $combined_gene->seq_region_name . " " . $combined_gene->seq_region_start . "-" . $combined_gene->seq_region_end . "\n";
    } ## end if ( defined $combined_transcript)
    else {
      #retrieving unmerged if no UTR could be defined
      my $unmerged_cds = $self->retrieve_unmerged_gene($cds);
      $self->unmatched_genes($unmerged_cds);
      print STDERR "RESULT-UnmergedGene: " .
        $unmerged_cds->seq_region_name . " " . $unmerged_cds->seq_region_start . "-" . $unmerged_cds->seq_region_end . "\n";
      next CDS;
    }

  } ## end CDS: foreach my $cds (@merged_genes)
  ...
  if ( !$blessed && ( $self->{'remove_redundant'} ) ) {
    ...
    print STDERR "without redundant models, we have " . ( scalar @{ $self->combined_genes } ) .
      " combined_genes genes" . " and " . ( scalar @{ $self->unmatched_genes } ) . " unmatched_genes\n"
      if $self->VERBOSE;
  }

} ## end sub run_matching
...
EXON:
  ...
  if ($cdna_gene) {
    ...
    print STDERR
      "Unable to fetch cDNA gene object for $cdna_id. Maybe the cDNA alignment is not present in one of the input databases.\n";
    return undef;
  }

  return $cdna_gene;
} ## end sub check_for_predefined_pairing
...
} ## end sub create_predefined_pairing
...
sub find_cluster_joiners {
  ...
  print STDERR "Checking transcript for cluster joining: seq_region/chr " .
    $transcript->seq_region_name . ", " . $transcript->seq_region_start . "-" . $transcript->seq_region_end . ".\n"
    if $self->VERBOSE;
...
cDNA_GENE:
  ...
  cDNA_TRANSCRIPT:
    foreach my $tran ( @{ $cdna->get_all_Transcripts } ) {
      ...
      next cDNA_TRANSCRIPT
        unless ( is_Transcript_sane($tran) &&
  ...
  } ## end cDNA_GENE: foreach my $cdna ( @{$cdna_arrayref...})
...
GENE:
  foreach my $gene ( @{ $self->output } ) {
    ...
    if ( !$gene->analysis || $gene->analysis->logic_name ne $self->analysis->logic_name ) {
...
sub convert_to_extended_genes {
  ...
  for my $g (@$genes) {
    ...
    for my $t ( @{ $g->get_all_Transcripts } ) {
      throw("gene has more than one trancript - only processing 1-gene-1-transcript-genes") if ( @{ $g->get_all_Transcripts } > 1 );

      # conversion
      my $gene_from_pt = Bio::EnsEMBL::Gene->new( -start    => $t->start,
                                                  -end      => $t->end,
                                                  -strand   => $t->strand,
                                                  -slice    => $t->slice,
                                                  -biotype  => $g->biotype,
      ...
      my $new_tr =
        Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptExtended->new( -BIOTYPE => $g->biotype, -ANALYSIS => $t->analysis, );
      $new_tr->ev_set($set);
    ...
    } ## end for my $t ( @{ $g->get_all_Transcripts...})
  } ## end for my $g (@$genes)

  return \@new_genes;
} ## end sub convert_to_extended_genes
...
sub make_gene {
  ...
  foreach my $trans (@$transcripts) {
    ...
    if ( !$blessed ) {
      ...
      unless ( is_Transcript_sane($trans) && has_no_unwanted_evidence($trans) ) {
  ...
  } ## end foreach my $trans (@$transcripts)
  ...
  foreach my $e2g (@genes) {
    ...
    if ( $#gw_exons == 0 ) {
      foreach my $current_exon (@eg_exons) {
        ...
        if ( $gw_exons[0]->end <= $current_exon->end && $gw_exons[0]->start >= $current_exon->start ) {
          $fiveprime_match  = 1;
          $threeprime_match = 1;
        ...
        if ( $gw_exons[0]->strand == 1 ) {
          ...
          # 3prime
            ...
            ( $current_exon->end >= $gw_exons[$#gw_exons]->end ||
              ...
              ( abs( $current_exon->end - $gw_exons[$#gw_exons]->end ) <= $exon_slop && $current_exon != $eg_exons[$#eg_exons] ) ) )
          {
            $threeprime_match = 1;
            $right_exon       = $current_exon;
            $right_diff       = $current_exon->end - $gw_exons[0]->end;
          ...
          if (
            ...
            ( $current_exon->end >= $gw_exons[0]->end ||
              # or there are UTR exons to be added
              ( abs( $current_exon->end - $gw_exons[0]->end ) <= $exon_slop && $current_exon != $eg_exons[0] ) ) )
          {
            $fiveprime_match = 1;
            $right_exon      = $current_exon;
            $right_diff      = $current_exon->end - $gw_exons[0]->end;
          ...
          #3prime
            ...
            ( $current_exon->start <= $gw_exons[$#gw_exons]->start ||
              # or there are UTR exons to be added
              ( abs( $current_exon->start - $gw_exons[$#gw_exons]->start ) <= $exon_slop && $current_exon != $eg_exons[$#eg_exons] ) ) )
          {
            $threeprime_match = 1;
            $left_exon        = $current_exon;
            $left_diff        = $gw_exons[0]->start - $current_exon->start;
          }
        } ## end elsif ( $gw_exons[0]->strand... [ if ( $gw_exons[0]->strand...)])
      ...
      if ( ( $cds_length*10 ) > $UTR_diff ) {
        ...
        if ( $self->VERBOSE ) {
          print STDERR "didnt pass UTR length check [cds_len*10" .
            ( $cds_length*10 ) . " vs UTR_diff " . $UTR_diff . "]: " . $e2g->seq_region_start . "-" . $e2g->seq_region_end . "\n";
        }
      }
...
UNMERGED_GENE:
  foreach my $unmerged ( @{$genesref} ) {
    ...
    if ( scalar(@trans) != 1 ) {
      throw( "Gene with dbID " .
             $unmerged->dbID . " has NO or more than one related transcript, where a 1-gene-to-1-transcript-relation is assumed." .
...
#      $cloned_exon->seq_region_start . " ". $cloned_exon->seq_region_end. "\n";
    ...
    } ## end EXON: foreach my $exon (@unmerged_exons)
    ...
    if ( scalar(@seqeds) ) {
      ...
      foreach my $se (@seqeds) {
        my $new_se = Bio::EnsEMBL::SeqEdit->new( -CODE    => $se->code,
                                                 -NAME    => $se->name,
                                                 -DESC    => $se->description,
                                                 -START   => $se->start,
                                                 -END     => $se->end,
...
EACH_E2G_EXON:
  foreach my $ee (@e2g_exons) {
    ...
    if ( scalar(@gw_exons) == 1 ) {
      ( $newtranscript, $modified_peptide_flag ) =
        $self->transcript_from_single_exon_genewise( $ee, $gw_exons[0], $newtranscript, $eecount, @e2g_exons );
    }

    else {
      ( $newtranscript, $modified_peptide_flag ) =
        $self->transcript_from_multi_exon_genewise( $ee, $newtranscript, $translation, $eecount, $gw, $e2g );
    }
  ...
  print STDERR "Combined transcript just created in combine_genes method has " .
    scalar @{ $newtranscript->get_all_Exons() } . " exon(s), translation length " . $newtranscript->translation->length() . "\n";
  ...
  if ( defined($newtranscript) ) {
    ...
    foreach my $ex ( @{ $newtranscript->get_all_Exons } ) {
      ...
      if ( $ex->sub_SeqFeature && scalar( $ex->sub_SeqFeature ) > 1 ) {
        ...
        print STDERR "Unpacking a merged exon of length " .
          $ex->length . " at seq_region/chr " . $ex->seq_region_name . " (" . $ex->seq_region_start . "-" . $ex->seq_region_end . ")\n"
          if $self->VERBOSE;
    ...
    if ( !$blessed ) {
      ...
      unless ( is_Transcript_sane($newtranscript) && has_no_unwanted_evidence($newtranscript) ) {
    ...
    unless ( validate_Translation_coords( $newtranscript, 1 ) && !contains_internal_stops($newtranscript) && $newtranscript->translate ) {
    ...
    if ($modified_peptide) {
      ...
      unless ( validate_Translation_coords( $newtrans, 1 ) &&
               is_Transcript_sane($newtrans) &&
...
#    $new_exon->length . " (" . $new_exon->seq_region_start . "-" . $new_exon->seq_region_end. ") in transcript_from_single_exon_genewise.\n";
  ...
  if ( $gw_exon->start >= $eg_exon->start && $gw_exon->end <= $eg_exon->end ) {
    ...
    if ( $gw_exon->strand == 1 ) {
      ...
      if ( $translation->end > $translation->end_Exon->length ) {
        warn( "Forward strand: setting dodgy translation end: " .
              $translation->end . " exon_length: " . $translation->end_Exon->length . "\n" );
        return ( undef, 1 );
      }
    }
    ...
    if ( defined( $ex->sub_SeqFeature ) && ( scalar( $ex->sub_SeqFeature ) > 1 ) ) {
      ...
      $transcript->translation->end($orig_tend)
        ;    # Putting translation end_Exon isn't enough. we need to say where the translation should end.
      ...
      if ( defined( $ex->sub_SeqFeature ) ) {
        ...
        foreach my $sf_after (@sf_after) {
          print STDERR "BAD!! subseqfeature remained with combined transcript after flush: " .
            $sf_after->seq_region_start . " " . $sf_after->seq_region_end . "\n"
            if $self->VERBOSE;
        }
      }
    } ## end if ( defined( $ex->sub_SeqFeature...))
  ...
  } ## end if ( $gw_exon->start >=...)
  return ( $transcript, 0 );
} ## end sub transcript_from_single_exon_genewise
...
} ## end sub transcript_from_multi_exon_genewise
...
sub transcript_from_multi_exon_genewise_forward {
  ...
  if (    #they have a coincident end
    ...
    ( $current_exon->start <= $gwexons[0]->start ||
      ...
      ( abs( $current_exon->start - $gwexons[0]->start ) <= $exon_slop && $current_exon != $egexons[0] ) ) )
  {
    ...
    } ## end elsif ( $gwstart < $current_start... [ if ( $gwstart >= $current_start)])
  ...
  # 3_PRIME:
    ...
    ( $current_exon->end >= $gwexons[$#gwexons]->end ||
      ...
      ( abs( $current_exon->end - $gwexons[$#gwexons]->end ) <= $exon_slop && $current_exon != $egexons[$#egexons] ) ) )
  {
    ...
    } ## end elsif ( $current_exon->end... [ if ( $current_exon->end...)])
...
} ## end sub transcript_from_multi_exon_genewise_forward
...
sub transcript_from_multi_exon_genewise_reverse {
  ...
  # 5_PRIME:
  if (
    $gwexons[0]->start == $current_exon->start &&
    # either e2g exon ends after gw exon
    ( $current_exon->end >= $gwexons[0]->end ||
      # or there are UTR exons to be added
      ( abs( $current_exon->end - $gwexons[0]->end ) <= $exon_slop && $current_exon != $egexons[0] ) ) )
  {
  ...
  } ## end if ( $gwexons[0]->start...)
  # end 5' exon

  # 3_PRIME:
  elsif (
    $gwexons[$#gwexons]->end == $current_exon->end &&
    # either e2g exon starts before gw exon
    ( $current_exon->start <= $gwexons[$#gwexons]->start ||
      # or there are UTR exons to be added
      ( abs( $current_exon->start - $gwexons[$#gwexons]->start ) <= $exon_slop && $current_exon != $egexons[$#egexons] ) ) )
  {
    my $end_translation_shift = 0;
    ...
    } ## end elsif ( $current_exon->start... [ if ( $current_exon->start...)])
    # strand = -1
    my $expanded = $self->expand_3prime_exon( $ex, $transcript, -1 );
  ...
  return ( $transcript, $modified_peptide );
} ## end sub transcript_from_multi_exon_genewise_reverse
...
sub add_5prime_exons {
  ...
  while ( $c < $exoncount ) {
    ...
    $modified = 1;
    $c++;
  } ## end while ( $c < $exoncount )

  if ( $modified == 1 ) {
    $transcript->translation->start_Exon->phase(-1);
  }
} ## end sub add_5prime_exons
...
GENE:
  foreach my $gene (@$genes) {
    ...
    unless ( validate_Translation_coords( $tran, 1 ) && !contains_internal_stops($tran) && $tran->translate ) {
      print STDERR "\nERROR: Gene at " . $gene->seq_region_name .
        " " . $gene->seq_region_start . "-" . $gene->seq_region_end . " " . $gene->seq_region_strand . " doesn't translate!\n\n";
...
sub _recalculate_translation {
  ...
  unless ( validate_Translation_coords( $mytranscript, 1 ) && contains_internal_stops($mytranscript) && $mytranscript->translate ) {
    ...
    if ( $trans->translation ) {
      ...
      if ( $startseq ne "ATG" ) {
        if ( $coding_start > 3 ) {
            ...
            if ( $testseq eq "ATG" ) {
              ...
              if ( $new_end - $new_start == 2 ) {
                ...
                if ( $newstartexon == $tln->start_Exon ) {
                  ...
                  my $copyexon =
                    new Bio::EnsEMBL::Exon( -start => $tln->start_Exon->start, -end => $tln->start_Exon->end, -strand => $gene->strand, );
                  my $copynewstartexon =
                    new Bio::EnsEMBL::Exon( -start => $newstartexon->start, -end => $newstartexon->end, -strand => $gene->strand, );
                  ...
                  foreach my $exon ( @{ $trans->get_all_Exons } ) {
                    ...
                    if ( $exon == $tln->start_Exon ) {
                      ...
                      print STDERR "Setting end_phase on new start exon to " .
                        $copynewstartexon->end_phase . " l = " . $exon->length . " ts = " . $tln->start . "\n"
                        if $self->VERBOSE;
                      $inrange = 1;
                    }
                    else {
          ...
          } ## end while ( $coding_start > 3...)
        } ## end if ( $coding_start > 3)
        else {
              ...
              if ( $testseq eq "TGA" or $testseq eq "TAA" or $testseq eq "TAG" ) {
                ...
                if ( $new_end - $new_start == 2 ) {
                  ...
                  if ( $newendexon == $tln->end_Exon ) {
                    ...
                    my $copyexon =
                      new Bio::EnsEMBL::Exon( -start => $tln->end_Exon->start, -end => $tln->end_Exon->end, -strand => $gene->strand, );
                    my $copynewendexon =
                      new Bio::EnsEMBL::Exon( -start => $newendexon->start, -end => $newendexon->end, -strand => $gene->strand, );
                    ...
                    if ( $copynewendexon->strand == 1 ) {
                      ...
                      my $tercodon =
                        $copynewendexon->seq->subseq( $copynewendexon->end - $new_start - 1, $copynewendexon->end - $new_start + 1 );
                      #reverse($tercodon);
                ...
                last;
              } ## end if ( $testseq eq "TGA"...)
              $coding_end += 3;
            } ## end while ( ( $cdnalen - $coding_end...))
          } ## end if ( ( $cdnalen - $coding_end...))
          else {
            print STDERR
              "Coding region ends between the 3rd last to the last base of the transcript.  Stop codon isn't TGG, TGA or TAG " .
...
sub blessed_genes {
  ...
  OLDGENE:
    ...
    } ## end OLDGENE: foreach my $gene ( @{$genes...})
  } ## end if ( defined $slice &&...)

  return $self->{'_blessed_genes'};
} ## end sub blessed_genes
