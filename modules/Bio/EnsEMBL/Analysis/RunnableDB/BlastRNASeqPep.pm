    ...
    foreach my $tran ( @{ $gene->get_all_Transcripts } ) {
      ...
      if ( $tran->translation ) {
        foreach my $db ( split ',', ( $self->analysis->db_file ) ) {
          $self->runnable( Bio::EnsEMBL::Analysis::Runnable::BlastTranscriptPep->new( -transcript => $tran,
                                                                                      -query      => $self->query,
  ...
  ALIGN: while ( scalar(@sorted_alignments) > 0 ) {
    ...
    HSP: foreach my $hsp ( sort { $b->score <=> $a->score } @hsps ) {
        ...
        foreach my $contig (@contigs) {
          if ( $hsp->start <= $contig->end && $hsp->end >= $contig->start ) {
            # reject it
            print "REJECT\n";
            next HSP;
          }
          if ( $hsp->hstart <= $contig->hend && $hsp->hend >= $contig->hstart ) {
            # reject it
            print "REJECT\n";
            next HSP;
          }
    ...
    } ## end ALIGN: while ( scalar(@sorted_alignments...))
    ...
    if ( scalar( @best > 0 ) ) {
      ...
      foreach my $gf (@best) {
        ...
        foreach my $f ( $gf->ungapped_features ) {
          print $f->seq_region_name . " " . $f->start . " " . $f->end . " " . $f->strand .
            " " . $f->hstart . " " . $f->hend . " " . $f->score . " " . $f->p_value . " " . $f->percent_id . " " . $f->hseqname . "\n";
      ...
      push my @tsf, Bio::EnsEMBL::DnaPepAlignFeature->new( -features => \@filtered_best );
      ...
      my $seqfetcher = Bio::EnsEMBL::Pipeline::SeqFetcher::OBDAIndexSeqFetcher->new( -db => [ $self->INDEX ], -format => 'fasta' );
    ...
    } ## end if ( scalar( @best > 0...))
  } ## end foreach my $tran ( @{ $gene...})

  push @output, $gene;
  $self->output( \@output );
} ## end sub add_supporting_features
