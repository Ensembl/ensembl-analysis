...
sub new {
  ...
  my ( $program, $options, $genedb, $outgenedb ) = rearrange( [ 'PROGRAM', 'OPTIONS', 'GENEDB', 'OUTGENEDB' ], @args );
    ...
    if ($parent_exon_key) {
      ...
      if ( exists( $ref_trans_hash{$parent_exon_key} ) ) {
        ...
        if ( !( $ref_stable_id eq $parent_sid ) ) {
          print( "Parent stable ID $ref_stable_id for projected transcript " . $out_transcript->stable_id .
                 " is different from the parent stable ID set in the old way $parent_sid. This is only for statistical purposes.\n" );
        }
        ...
        if ($ref_transcript) {
          ...
          my $runnable =
            Bio::EnsEMBL::Analysis::Runnable::ExonerateAlignFeature->new( -analysis           => $self->analysis,
        ...
        } ## end if ($ref_transcript)
        else {
          warning( "Parent transcript $ref_stable_id could not be fetched for projected transcript " . $out_transcript->stable_id );
        }
      } ## end if ( exists( $ref_trans_hash...))
      else {
        warning( "Projected transcript " . $out_transcript->stable_id . " does not have a parent." );
      }
    } ## end if ($parent_exon_key)
    else {
      throw( "Projected transcript " . $out_transcript->stable_id .
             " does not have a parent_exon_key attribute. It should have been added during the projection stage!" );
    }
  } ## end foreach my $out_transcript ...
} ## end sub fetch_input
...
FEATURE: foreach my $f (@$flist) {
    ...
    foreach my $ugf ( $f->ungapped_features ) {
      ...
      foreach my $obj ( $trans->cdna2genomic( $ugf->start, $ugf->end ) ) {
        if ( $obj->isa("Bio::EnsEMBL::Mapper::Coordinate") ) {
          ...
          my $fp;
          $fp = Bio::EnsEMBL::FeaturePair->new( -start      => $obj->start,
                                                -end        => $obj->end,
                                                -strand     => $strand,
                                                -slice      => $trans->slice,
                                                -hstart     => 1,
        ...
        } ## end if ( $obj->isa("Bio::EnsEMBL::Mapper::Coordinate"...))
      } ## end foreach my $obj ( $trans->cdna2genomic...)
    } ## end foreach my $ugf ( $f->ungapped_features)
    ...
    push @dafs, $feat;
  } ## end FEATURE: foreach my $f (@$flist)
  return \@dafs;
} ## end sub process_features
...
sub get_transcript_exon_key {
  my $transcript = shift;
  my $string     = $transcript->slice->seq_region_name . ":" . $transcript->biotype .
    ":" . $transcript->seq_region_start . ":" . $transcript->seq_region_end . ":" . $transcript->seq_region_strand . ":";
