...
sub fetch_input {
  ...
  my $feature_slice = $self->feature_slice_adaptor->fetch_by_region( 'toplevel', $slice->seq_region_name, $slice->start, $slice->end, );
  ...
  foreach my $DB (@alignment_dbs) {
    ...
    my $handle = $self->sql_array(
"SELECT dna_align_feature_id, seq_region_id, seq_region_start, seq_region_end, cigar_line from dna_align_feature where seq_region_id = $seq_id and seq_region_end >= "
        . $slice->start . " and seq_region_start <= " . $slice->end . " ;",
      $self->get_dbadaptor($DB) );
...
# $ugf->[3]  ungapped feature end   ?
    ...
    unless ($clustered) {
      ...
      # make a daf maybe?
      my $feat = Bio::EnsEMBL::FeaturePair->new( -SLICE      => $self->chr_slice,
                                                 -START      => $ugf->[2],
                                                 -END        => $ugf->[3],
                                                 -STRAND     => 1,
  ...
  } ## end foreach my $ugf (@ungapped_features)
  ...
  foreach my $read ( keys %{$cluster_data} ) {
    ...
    for ( my $i = 0; $i < @clusters; $i++ ) {
      ...
      $cluster_hash->{ $clusters[0] }->{$cluster}++ unless $clusters[0] == $cluster;
    }
  }
  ...
  foreach my $key ( sort { $a <=> $b } keys %{$cluster_hash} ) {
    ...
    foreach my $key2 ( keys %{ $cluster_hash->{$key} } ) {
      ...
      #     }
    } ## end foreach my $key2 ( keys %{ ...})
  } ## end foreach my $key ( sort { $a...})
...
sub pad_exons {
  ...
  foreach ( my $i = 0; $i <= $#exon_clusters; $i++ ) {
    ...
    my $padded_exon = create_Exon( $exon->start - $start_padding,
                                   $exon->end + $end_padding,
                                   -1, -1, -1, $exon->analysis, undef, undef, $self->chr_slice, );
...
sub gene_cluster {
  ...
  foreach my $f (@features) {
    ...
    foreach my $cluster (@clusters) {
      # do they overlap?
      if ( $f->[2] <= $cluster->{'end'} && $f->[3] >= $cluster->{'start'} ) {
        # Expand the cluster
        $cluster->{'start'} = $f->[2] if $f->[2] < $cluster->{'start'};
        $cluster->{'end'}   = $f->[3] if $f->[3] > $cluster->{'end'};
...
sub merge_genes {
  ...
  for ( my $i = 1; $i <= $#genes; $i++ ) {
    ...
    if ( $genes[ $i - 1 ]->start < $genes[$i]->start && $genes[ $i - 1 ]->end > $genes[$i]->end ) {
...
GENE: for ( my $i = 1; $i <= $#genes; $i++ ) {
    ...
    if ( $gap <= $self->MERGE_GENES && $left_gene->end < $right_gene->start ) {
      ...
      } ## end if ( $repeat_coverage ...)
    } ## end if ( $gap <= $self->MERGE_GENES...)
  } ## end GENE: for ( my $i = 1; $i <= ...)
...
sub get_dna_align_features_from_databases {
  ...
  foreach my $db_hash_key ( keys %dbnames_2_logicnames ) {
    ...
    my $sql =
      "SELECT dna_align_feature_id, seq_region_id, seq_region_start, seq_region_end, cigar_line from dna_align_feature " .
  ...
  } ## end foreach my $db_hash_key ( keys...)
...
} ## end sub get_dna_align_features_from_databases
