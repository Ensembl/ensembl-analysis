...
sub read_and_check_config {
  ...
  $self->efgdb( Bio::EnsEMBL::Funcgen::DBSQL::DBAdaptor->new( %{ $self->EFGDB }, -dnadb => $self->dnadb, ) );
  ...
  unless ( $self->analysis->input_id_type eq "file" ) {
    ...
    $self->norm_method($m) or throw( "Can't fetch analysis object for norm method " . $self->NORM_METHOD );
...
sub check_InputId {
  ...
  $self->experiment($e) or throw( "Can't fetch experiment with name " . $ename );
  ...
  if ( $input_id_type eq 'slice' ) {
    ...
    throw( "Can't fetch slice with name " . join( ':', @input ) ) unless (@slices);

  }
  elsif ( $input_id_type eq 'array' ) {

    @slices = @{ $sa->fetch_all('toplevel') };
    throw("Can't fetch toplevel slices") unless (@slices);
...
sub check_Analysis {
  ...
  if ( !defined $analysis ) {    # NEW
    ...
    warn( 'Analysis with logic name \'' . $logic_name . '\' already ' . 'exists, but has different options! Updating analysis ...' );
    ...
    warn( 'Analysis with logic name \'' . $logic_name . '\' already ' . 'exists.' );

  }

  $self->efg_analysis( $aa->fetch_by_logic_name($logic_name) );

} ## end sub check_Analysis
...
sub check_Sets {
  ...
  if ( !defined $fset ) {
    ...
    $fset = Bio::EnsEMBL::Funcgen::FeatureSet->new( -analysis      => $self->efg_analysis,
  ...
  if ( !defined $dset ) {
    ...
    $dset = Bio::EnsEMBL::Funcgen::DataSet->new( -SUPPORTING_SETS     => $self->ResultSets,
                                                 -FEATURE_SET         => $fset,
                                                 -DISPLAYABLE         => 1,
                                                 -NAME                => $set_name,
    ...
    map { $dset->add_supporting_sets( [$_] ) if ( !exists $ssets_dbIDs{ $_->dbID } ); } @{ $self->ResultSets }
...
sub fetch_ResultSets {
  ...
  my $rsets = $rsa->fetch_all_by_Experiment_Analysis( $self->experiment, $self->norm_method );
  ...
  foreach my $rset ( @{$rsets} ) {
    ...
    if ( !defined $self->feature_type ) {
      ...
      throw("replicates differ in feature types") if ( $self->feature_type->dbID != $rset->feature_type->dbID );
    }
    ...
    if ( !defined $self->cell_type() ) {
      ...
      throw("replicates differ in cell types") if ( $self->cell_type->dbID != $rset->cell_type->dbID );
    }
  ...
  print "Selected result sets: ", join( ', ', map { $_->name } @rsets ), ' (in total ', scalar(@rsets), ")\n";

  return \@rsets;

} ## end sub fetch_ResultSets
...
sub fetch_input {
  ...
  foreach my $rset ( @{ $self->ResultSets } ) {
    ...
    if ( $input_id_type eq 'slice' ) {
      ...
      warn( "WARNING: Result set '" .
            $rset->name . "' comprises more than one experimental chip, " . "which will result in very large datafiles!" )
        if ( scalar @echips > 1 );
    ...
    unless ( -e $datfile ) {    # dat file already dumped; skipping sql
      ...
      foreach my $slice ( @{ $self->query } ) {
        ...
        foreach my $rft ( sort { $a->start <=> $b->start } @{$result_features} ) {
          ...
          push(
            @result_features,
            [ $slice->seq_region_name, $rft->start, $rft->end, $rft->score*$self->SCORE_FACTOR, $ft_cnt++,
              #$rft->probe->get_probename,
            ] );
        }
      } ## end foreach my $slice ( @{ $self...})

      open( RF, "> $datfile" ) or throw("Can't open file $datfile");

      map { print RF join( "\t", @$_ ), "\n"; } @result_features;

      close RF or throw("Can't close file $datfile");

    } ## end unless ( -e $datfile )
    else {

      print "Using cached ResultFeatures:\t", $datfile, "\n";

      open( CACHE, $datfile ) or throw("Can't open file cache $datfile");
      while (<CACHE>) {
        chomp;
        #print;

        my @values = split(/\t/);
        push( @result_features, [ @values ] );
      }
      close CACHE;
    ...
    throw("No of result_features doesn't match") if ( @result_features && $norf && $norf != scalar(@result_features) );

    $result_features{ $rset->name } = \@result_features;

    $norf = scalar(@result_features);

  } ## end foreach my $rset ( @{ $self...})
  ...
  $runnable = $runnable->new( -query    => $self->query,
...
sub write_output {
  ...
  if ( scalar( @{ $self->output } ) == 0 ) {
    warn( "No features to annotate on slice " . $self->query->name . " for experiment " . $self->experiment->name() . "!" );
    return 1;
  }
  ...
  foreach my $s ( @{ $self->query } ) {
    ...
    push @stored_af, @{ $self->efgdb->get_AnnotatedFeatureAdaptor->fetch_all_by_Slice_FeatureSets( $s, [$fset] ) };
  ...
  if (@stored_af) {
    ...
    warn( "NOT IMPORTING " .
          scalar( @{ $self->output } ) . " annotated features! Slice(s) " .
          join( ', ', map { $_->name } @{ $self->query } ) . " already contains " .
          scalar(@stored_af) . " annotated features of feature set '" . $fset->name . "' (dbId " . $fset->dbID . ').' );
    return 1;
    ...
    if ( $input_id_type eq 'slice' && ( $self->query->[0]->start != 1 || $self->query->[0]->strand != 1 ) ) {
      my $sa = $self->efgdb->get_SliceAdaptor;
      $tl_slice = $sa->fetch_by_region(
        $self->query->[0]->coord_system->name(), $self->query->[0]->seq_region_name(), undef,    #start
        undef,                                                                                   #end
        undef,                                                                                   #strand
    ...
    foreach my $ft ( @{ $self->output } ) {
      ...
      my $af = Bio::EnsEMBL::Funcgen::AnnotatedFeature->new( -slice         => $slice,
                                                             -start         => $start,
                                                             -end           => $end,
                                                             -strand        => 0,
...
sub efgdb {
  my ( $self, $db ) = @_;
  if ($db) {
    throw( "Must pass RunnableDB:db a Bio::EnsEMBL::Funcgen::DBSQL::DBAdaptor " . "not a " . $db )
...
sub efg_analysis {
  my ( $self, $a ) = @_;
  if ($a) {
    throw( "Must pass a Bio::EnsEMBL::Analysis not a " . $a ) unless ( $a->isa('Bio::EnsEMBL::Analysis') );
    $self->{'efg_analysis'} = $a;
  }
...
sub query {
  ...
  if ($query) {
    ...
    throw( "Must pass RunnableDB:Funcgen:query a array ref not a " . ref($query) ) unless ( ref($query) eq 'ARRAY' );

    map { throw( $_->name . " is not a Bio::EnsEMBL::Slice" ) unless ( $_->isa("Bio::EnsEMBL::Slice") ); } @$query;
...
sub experiment {
  my ( $self, $e ) = @_;
  if ($e) {
    throw( "Must pass a Bio::EnsEMBL::Funcgen::Experiment not a " . $e ) unless ( $e->isa('Bio::EnsEMBL::Funcgen::Experiment') );
    $self->{'experiment'} = $e;
  }
...
sub norm_method {
  my ( $self, $m ) = @_;
  if ($m) {
    throw( "Must pass a Bio::EnsEMBL::Ananysis not a " . $m ) unless ( $m->isa('Bio::EnsEMBL::Analysis') );
    $self->{'norm_method'} = $m;
  }
...
sub feature_type {
  my ( $self, $ft ) = @_;
  if ($ft) {
    throw( "Must pass a Bio::EnsEMBL::Funcgen::FeatureType not a " . $ft ) unless ( $ft->isa('Bio::EnsEMBL::Funcgen::FeatureType') );
...
sub cell_type {
  my ( $self, $ct ) = @_;
  if ($ct) {
    throw( "Must pass a Bio::EnsEMBL::Funcgen::CellType not a " . $ct ) unless ( $ct->isa('Bio::EnsEMBL::Funcgen::CellType') );
    $self->{'cell_type'} = $ct;
  }
...
sub feature_set {
  my ( $self, $fs ) = @_;
  if ($fs) {
    throw( "Must pass a Bio::EnsEMBL::Funcgen::FeatureSet not a " . $fs ) unless ( $fs->isa('Bio::EnsEMBL::Funcgen::FeatureSet') );
    $self->{'feature_set'} = $fs;
  }
...
sub data_set {
  my ( $self, $ds ) = @_;
  if ($ds) {
    throw( "Must pass a Bio::EnsEMBL::Funcgen::DataSet not a " . $ds ) unless ( $ds->isa('Bio::EnsEMBL::Funcgen::DataSet') );
    $self->{'data_set'} = $ds;
  }
...
sub ResultSets {
  ...
  throw("No ResultSets in RunnableDB.") if ( !$self->{'ResultSets'} );
