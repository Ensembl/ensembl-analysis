...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::HomologyUtils qw(
...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::EvidenceUtils qw(create_feature_from_gapped_pieces create_feature);
...
#
  ...
  my ( $csys, $asm, $name, $start, $end, $strand, $transcript_sid, $homolog_transcript_sid, $simple_feature_index,
       $simple_feature_string ) = split /\:/, $self->input_id;
...
sub fetch_input {
  ...
  if ( $self->use_artifical_input_ids ) {
    print "\n" x 30;
    print "\nWarning - using artificial input_ids" x 30;
    print "\n" x 30;
    my %tmp;
    @tmp{@my_input_ids} = 1;
  ...
  if ( $self->limit_input_ids ) {
    print "\n" x 30;
    print "\nWarning - input-id range limited " x 30;
    print "\n" x 30;
...
################
    ...
    my $seqfetcher =
      Bio::EnsEMBL::Pipeline::SeqFetcher::OBDAIndexSeqFetcher->new( -db => [ ("/lustre/blastdb/Ensembl/uniprot_index/") ],
                                                                    -format => 'fasta', );

    my $seqx;
    eval { $seqx = $seqfetcher->get_Seq_by_acc($protein_id); };
    ...
    print STDERR "running ExonerateTranscript on Protein :  " .
      $protein_id . " and " . $self->slice->name . " length " . $self->slice->length . "\n";
    print "\nUsing hardcoded program vwerion exonerate 1.4.0 !\n";

    my $r =
      Bio::EnsEMBL::Analysis::Runnable::ExonerateTranscript->new( -analysis    => $self->analysis,
                                                                  -query       => $self->slice,
  ...
  } ## end PROTEIN_ID: for my $targ_xrate_input...
  ...
  if ( $self->only_run_exonerate ) {
    ...
    for my $gw_run_options (@gw_options) {
      for my $protein_file (@protein_files) {
        ...
        my $rangefeat = new Bio::EnsEMBL::FeaturePair( -start  => 1,
        ...
        my $gw = Bio::EnsEMBL::Analysis::Runnable::MiniGenewise->new( -query            => $self->slice,
                                                                      -protein          => $prot_seq,
        ...
        for my $t (@transcripts) {
          my $g = Bio::EnsEMBL::Gene->new( -start => $t->start, -end => $t->end, -strand => $t->strand, -slice => $self->slice, );

          $g->add_Transcript($t);
          $t->analysis( $self->analysis );
          $self->gw_genes($g);
        }
      } ## end for my $protein_file (@protein_files)
    } ## end for my $gw_run_options ...

  } ## end else [ if ( $self->only_run_exonerate)]
...
sub run {
  ...
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::BestTargetted->new( -biotypes   => [ keys %unique_biotypes ],
  ...
  } ## end for my $homolog_transcript_id...
  ...
  } ## end for my $g (@most_sim_non_redundant)
...
sub check_if_transcripts_are_different {
  ...
  return 0;
} ## end sub check_if_transcripts_are_different
...
sub check_if_genes_are_redundant {
  ...
  return [ values %tmp ];
} ## end sub check_if_genes_are_redundant
...
sub add_utr {
  ...
  for my $cdna (@cdna_seqs) {
    ...
    # CDNA !!!!
    my $options =
      "--model est2genome --forwardcoordinates FALSE --softmasktarget TRUE --exhaustive FALSE  --score 500 " .
    ...
    # filter results
    my $et_filter =
      new Bio::EnsEMBL::Analysis::Tools::ExonerateTranscriptFilter->new( -percent_id     => $percent_ident_filter,
                                                                         -best_in_genome => 1,
                                                                         -coverage       => 90, );
  ...
  if (@cdna_genes) {
    my $utr_builder =
      Bio::EnsEMBL::Analysis::RunnableDB::UTR_Builder->new( -db              => $self->db,
                                                            -input_id        => 'fake',
                                                            -analysis        => $self->analysis,
                                                            -dont_use_config => 1, );
    ...
    $utr_builder->{evidence_sets} = { 'est'   => [],                              #\@est_biotypes,
...
sub attach_stuff_to_new_gene {
  ...
  unless ($ta) {
    throw(
      "could not get gene-adaptor for $$MAIN_CONFIG{QUERY_SPECIES} - check your " . "registry file / your OrthologueEvaluator-config\n" );
  }
  my $source_gene = $ta->fetch_by_transcript_stable_id($transcript_sid);
  ...
  for my $st (@source_transcripts) {
    ...
    if ( $st->stable_id =~ m/$transcript_sid/ ) {
      ...
      $new_version++;
      my $new_transcript = new Bio::EnsEMBL::Transcript( -exons           => \@new_exons,
  ...
  } ## end for my $st (@source_transcripts)
  return $new_gene;
} ## end sub attach_stuff_to_new_gene
  ...
  for my $g (@test_genes) {
    ...
    for my $sf ( @{ $test_trans->get_all_supporting_features } ) {
      ...
      for my $homolog_trans (@homologues_transcripts_which_are_supported_by_sf) {
        print "\nCalculating homolog_score for " .
          $g . " [ " . $sf->hseqname . " ]  against_homolog: " . $homolog_trans->stable_id . "\n";
    ...
    } ## end for my $sf ( @{ $test_trans...})
    $g->biotype("lower_homolog_score");
  } ## end for my $g (@test_genes)
  ...
  for my $g (@test_genes) {
    if ( exists $scores{$g}{score} ) {
      if ( $scores{$g}{score} == $max_score ) {
        my $sg = $scores{$g}{gene};
        print "\nmax_scoring transcript : " .
          $sg->seq_region_start . "\t" . $sg->seq_region_end . " score : " . $scores{$g}{score} . "\n";
        $g->biotype("MAX");
      }
    }
  }
  return \@test_genes;
} ## end sub find_gene_which_has_exon_length_distr_most_similar_to_homolog
...
sub check_test_transcript_against_homolog {
  ...
  if ( $self->debug ) {
    print "\n\nmax_score for aligning test_transcript to homolog is : " .
      $homolog_trans->stable_id . " : $f_matrix[$length_A-1][$length_B-1]\n\n\n";
...
HOMO_EXON: for my $he (@homolog_exons) {
    ...
    for my $dog_exon ( @{ $exon_pairs{$he} } ) {
      ...
      if ( $self->debug ) {
        print " " .
          $dog_exon->seq_region_start . " " . $dog_exon->seq_region_end . " " . $dog_exon->length . " " . $he->length . " diff: ";
        print $diff . "\n";
      }
    }
    print "\n" if ( scalar( @{ $exon_pairs{$he} } ) > 1 );
  } ## end HOMO_EXON: for my $he (@homolog_exons)
...
#print "exon_hits : $exon_hits    \n\n\n" ;
  print "f_matrix max_score for aligning test_transcript to homolog is : " .
    $homolog_trans->stable_id . " : $f_matrix[$length_A-1][$length_B-1]\n"
    if $self->debug;
...
} ## end sub check_test_transcript_against_homolog
...
sub exonerate_he_exon_against_test_exons {
  ...
  my $cmd =
    "/software/ensembl/bin/exonerate-1.4.0 -q $homolog_exon_seq_file -t $test_exon_filename -m affine:local --bestn 1 > $result_file";
  system("$cmd");
  ...
  unless ( defined $at_least_one_hit ) {
    my $cmd =
"/software/ensembl/bin/exonerate-1.4.0 -q $homolog_exon_seq_file -t $test_exon_filename -m affine:local --bestn 1 --exhaustive  > $result_file";
    system("$cmd");
...
} ## end sub exonerate_he_exon_against_test_exons
...
} ## end sub print_best_targetted_genes
...
#print "overlap_ratio                      : " . scalar(@{$scoring{$ng}{overlapped_exon}}) / scalar(@{$src_trans[0]->get_all_translateable_Exons} ) . "\n"  ;
    $scoring{$ng}{overlap_ratio} =
      scalar( @{ $scoring{$ng}{overlapped_exon} } )/scalar( @{ $src_trans[0]->get_all_translateable_Exons } );
...
#  ??? nr of fixed exons ?
  ...
  if ( scalar(@best_genes) > 0 ) {
    ...
    for my $bg (@best_genes) {
      ...
      print "comparing translation length ... " .
        length( $bt->translateable_seq ) . " vs max. length : " . $max_translation_length . "\n";
...
} ## end sub check_exon_overlap_between_new_gene_and_src_gene
...
SIMPLE_FEATURE: for my $sf (@simple_features_to_be_overlapped_by_transcript) {
    ...
    for my $e ( @{ $trans_to_check->get_all_Exons } ) {
      if ( $self->debug ) {
        print "checking .......\n";
        print "EXON   : " . $e->seq_region_start . "\t" . $e->seq_region_end . "\t" . $e->seq_region_name . "\t" .
          ( $e->seq_region_end - $e->seq_region_start ) . "\n";
        print "GAP    : " . $sf->seq_region_start . "\t" . $sf->seq_region_end . "\t" . $sf->seq_region_name . "\t" .
          ( $sf->seq_region_end - $sf->seq_region_start ) . "\n";
        print $e->seq_region_end .
          " >= " . $sf->seq_region_start . " && " . $e->seq_region_start . " <= " . $sf->seq_region_end . " ???\n";
      }
        ...
        if ( $e->seq_region_start <= $sf->seq_region_start && $e->seq_region_end >= $sf->seq_region_end ) {
          ...
          if ( $self->debug ) {
            ...
            print "Exon  : " . $e->seq_region_start .
              "--eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee--" . $e->seq_region_end . " (end)\n";
          ...
          if ( $self->debug ) {
            ...
            print "Exon  : " .
              $e->seq_region_start . "--eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee--" . $e->seq_region_end . " (end)\n";
            print "  GAP :                       " .
              $sf->seq_region_start . "--IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII--" . $sf->seq_region_end . " (end)\n";
          }
        }
      } ## end if ( $e->seq_region_end...)
      else {
  ...
  } ## end SIMPLE_FEATURE: for my $sf (@simple_features_to_be_overlapped_by_transcript)
  ...
  return $new_gene;
} ## end sub check_if_false_intron_has_been_removed_in_new_trans
...
sub make_new_gene_from_source_transcript {
  ...
  return $new_gene;
} ## end sub make_new_gene_from_source_transcript
...
sub write_genomic_sequence_to_file {
  ...
  if ( @array != 6 ) {
    throw( "Malformed slice name [$slice_name].  Format is " . "coord_system:version:seq_region:start:end:strand" );
  }
  ...
  if ( $slice->end() > $slice->seq_region_length ) {
    ...
    $slice = $sliceadp->fetch_by_region( $cs_name, $seq_region, $new_start, $new_end, $strand, $cs_version );
  }
...
} ## end sub write_genomic_sequence_to_file
...
} ## end sub build_hash_of_recoverd_introns_per_transcript
