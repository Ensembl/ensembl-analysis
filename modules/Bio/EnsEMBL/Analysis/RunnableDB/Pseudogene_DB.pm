...
GENE: foreach my $gene ( @{$genes} ) {
    ...
    my $chromosome_slice = $rsa->fetch_by_region( 'toplevel', $genes_slice->chr_name, );
    ...
    my $rep_gene_slice = $rsa->fetch_by_region( 'toplevel', $genes_slice->chr_name, $transferred_gene->start, $transferred_gene->end, );
    # get repeat blocks
...
sub make_runnable {
  ...
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::Pseudogene->new( -analysis                    => $self->analysis,
                                                                    -genes                       => $self->genes,
...
REPLOOP: foreach my $repeat (@repeats) {
    ...
    if ( defined($curblock) && $curblock->end >= $repeat->start ) {
      ...
      $curblock = Bio::EnsEMBL::Feature->new( -START  => $repeat->start,
                                              -END    => $repeat->end,
                                              -STRAND => $repeat->strand,
                                              -SLICE  => $repeat->slice );
      push( @repeat_blocks, $curblock );
    }
  } ## end REPLOOP: foreach my $repeat (@repeats)
...
sub store_ids {
  ...
  foreach my $id ( @{$id_list} ) {
    my $flag = Bio::EnsEMBL::Pipeline::Flag->new( '-type' => 'gene', '-ensembl_id' => $id, '-goalAnalysis' => $analysis, );
