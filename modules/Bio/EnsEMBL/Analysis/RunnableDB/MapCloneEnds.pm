...
sub run {
  ...
  } ## end while ( my $line = <INFILE>)

  close INFILE;

  # Run ExonerateClones to get a first idea on the aligned/location of the clone Ends
  my $exonerate =
    Bio::EnsEMBL::Analysis::RunnableDB::ExonerateCloneEnds->new( -DB       => $self->db,
  ...
  foreach my $selected_alignment (@selected_alignments) {
    ...
    if ( $selected_alignment ne '' ) {
      ...
      my $input_id =
        $chr_name[0] . ":" . $chr_name[1] . ":" . $chr_name[2] . ":" . $start . ":" . $end . ":" . $chr_name[5] . ":" . $clone_id;
  ...
  } ## end foreach my $selected_alignment...
} ## end sub run
...
sub filter_alignments {
    ...
    if ( ( scalar @{ $clones{$pair} } )/4 > 1 ) {
      ...
      foreach my $clean_cloneEnd1 ( keys %clean_cloneEnds ) {
        ...
        foreach my $clean_cloneEnd2 ( keys %clean_cloneEnds ) {
          ...
          if ( $clean_cloneEnd1 != $clean_cloneEnd2 &&
               $clean_cloneEnd1 < $clean_cloneEnd2 &&
               $clone_dir{$clean_cloneEnd1} ne $clone_dir{$clean_cloneEnd2} )
          {
            ...
            foreach my $fa ( @{ $clean_cloneEnds{$clean_cloneEnd1} } ) {
                   ...
                   $fa->seqname() ne $first_chr_hit )
              {
                ...
                } ## end foreach my $sa ( @{ $clean_cloneEnds...})
              } ## end if ( ( $fa->seqname() ...))
            } ## end foreach my $fa ( @{ $clean_cloneEnds...})
          } ## end if ( $clean_cloneEnd1 ...)
        } ## end foreach my $clean_cloneEnd2...
      } ## end foreach my $clean_cloneEnd1...
  ...
  } ## end foreach my $pair ( keys %aligned_clones)
  return \@selected_alignments;

} ## end sub filter_alignments
...
sub single_filter_alignments {
  ...
  } ## end foreach my $chr_cluster_key...
  return \$selected_alignment;
} ## end sub single_filter_alignments
