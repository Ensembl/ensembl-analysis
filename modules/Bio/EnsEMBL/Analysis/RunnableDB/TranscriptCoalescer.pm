...
sub new {
  ...
  #
  $self->{evidence_sets} = { 'est' => \@est_biotypes, 'simgw' => \@simgw_biotypes, 'abinitio' => \@abinitio_logicnames, };
...
sub fetch_input {
  ...
  for my $database_class ( keys %databases ) {
    ...
    for my $biotype ( @{ $databases{$database_class}{BIOTYPES} } ) {
      ...
      my $db_used =
        "${$databases{$database_class}}{db}{-dbname}\@" .
        "${$databases{$database_class}}{db}{-host}::" . "${$databases{$database_class}}{db}{-port}";
      info("$db_used :");
...
sub _remove_redundant_genes {
  ...
  return \%result;
} ## end sub _remove_redundant_genes
...
sub _check_config {
  ...
  for my $ln ( keys %ev_set_definitions ) {
    throw( "\n\tError in config-file Analysis/Config/GeneBuild/TranscriptCoalescer.pm\n " .
           "\tType $ln has an evidence set but there's NO database for this type defined in Config" . " TranscriptCoalescer.pm : $ln\n" )
