...
sub new {
  ...
  my ($genomicseqs, $querytype, $queryseqs, $queryannotation, $iidregexp, $outdb, $filter, 
  $coverage_by_aligned, $options, $nonref_regions, $program, $seqfetcher_object, 
  $seqfetcher_params, $soft_masked_repeats) = 
  rearrange(['GENOMICSEQS','QUERYTYPE','QUERYSEQS','QUERYANNOTATION','IIDREGEXP','OUTDB',
  'FILTER','COVERAGE_BY_ALIGNED','OPTIONS','NONREF_REGIONS','PROGRAM','SEQFETCHER_OBJECT',
  'SEQFETCHER_PARAMS','SOFT_MASKED_REPEATS'], @args);
...
sub seqfetcher{
  ...
  if($arg){
    throw("RunnableDB::Exonerate2GenesRegion ".
  ...
  if(!$self->{seqfetcher}){
    ...
    print $params{-db}->[0], "\n";
    $self->{seqfetcher} = $self->SEQFETCHER_OBJECT->new(
                                    %params,
                                   );
  }
