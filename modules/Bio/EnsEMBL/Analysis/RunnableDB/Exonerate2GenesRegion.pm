...
sub new {
  ...
  my ( $genomicseqs,       $querytype,           $queryseqs,     $queryannotation, $iidregexp, $outdb,
       $filter,            $coverage_by_aligned, $options,       $nonref_regions,  $program,   $seqfetcher_object,
       $seqfetcher_params, $soft_masked_repeats, $use_kill_list, $kill_type )
    =

    rearrange(
               [ 'GENOMICSEQS',       'QUERYTYPE',           'QUERYSEQS',     'QUERYANNOTATION',
                 'IIDREGEXP',         'OUTDB',               'FILTER',        'COVERAGE_BY_ALIGNED',
                 'OPTIONS',           'NONREF_REGIONS',      'PROGRAM',       'SEQFETCHER_OBJECT',
                 'SEQFETCHER_PARAMS', 'SOFT_MASKED_REPEATS', 'USE_KILL_LIST', 'KILL_TYPE' ],
               @args );
...
sub fetch_input {
  ...
  foreach my $repeat_logic_name ( @{ $self->SOFT_MASKED_REPEATS } ) {
    my $repeat_analysis = $self->db->get_AnalysisAdaptor->fetch_by_logic_name($repeat_logic_name);
    if ( !$repeat_analysis ) {
      throw( "Failed to find an analysis with the logic_name " . $repeat_logic_name . ". Cannot continue." );
    }
  }

  my $genomic_seq =
    $self->db->get_SliceAdaptor->fetch_by_name( $self->genomic_slice_name )->get_repeatmasked_seq( $self->SOFT_MASKED_REPEATS, 1 );
  ...
  if ( defined $self->PROGRAM && defined $self->analysis->program_file ) {
    if ( $self->PROGRAM ne $self->analysis->program_file ) {
      throw( "CONFLICT: You have defined -program in your config file and " . "-program_file in your analysis table." );
    }
  }
...
sub read_and_check_config {
                           ...
                           GENOMICSEQS) )
  {
    ...
    throw("You must define $config_var in config for logic '$logic'") if not defined $self->$config_var;
  }

  my %index_params = %{ $self->SEQFETCHER_PARAMS };

  if ( ( not -d $index_params{-db}->[0] ) and ( not -d $self->QUERYSEQS ) ) {
    throw( "Need to define SEQFETCHER_PARAMS -db dir or QUERYSEQS dir " . "(with query seq file name or acc in input id after ::).\n" );
  }
  ...
  LINE: while (<F>) {
    ...
    close(F);
    throw(
      "No entry for " . $self->query_acc . " in supplied query annotation 
