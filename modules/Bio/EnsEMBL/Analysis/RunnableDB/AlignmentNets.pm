...
sub fetch_input {
  ...
  if ( $self->QUERY_CORE_DB ) {
    ...
    my $sl    = $q_dbh->get_SliceAdaptor->fetch_by_region( 'toplevel', $seq_name );
  ...
  my $out_mlss = $compara_dbh->get_MethodLinkSpeciesSetAdaptor->fetch_by_method_link_type_GenomeDBs( $self->OUTPUT_METHOD_LINK_TYPE,
                                                                                                     [ $q_gdb, $t_gdb ] );
  ...
  my $ref_dnafrag = $compara_dbh->get_DnaFragAdaptor->fetch_by_GenomeDB_and_name( $q_gdb, $seq_name );

  my $gen_al_blocks =
    $compara_dbh->get_GenomicAlignBlockAdaptor->fetch_all_by_MethodLinkSpeciesSet_DnaFrag( $mlss, $ref_dnafrag, $seq_start, $seq_end );
  ...
  foreach my $block (@$gen_al_blocks) {
    ...
    my $daf_cigar = $self->daf_cigar_from_compara_cigars( $qy_al->cigar_line, $tg_al->cigar_line );

    my $daf = Bio::EnsEMBL::DnaDnaAlignFeature->new( -seqname      => $qy_al->dnafrag->name,
  ...
  } ## end foreach my $block (@$gen_al_blocks)
...
sub calculate_simple_high_net {
    ...
    if ($keep_chain) {
      ...
      foreach my $bl (@b) {
        ...
        foreach my $seg ( @{ $self->query_seq_level_projection } ) {
          if ( $bl->start >= $seg->from_start and $bl->end <= $seg->from_end ) {
            $inside_seg = $seg;
            last;
          }
          elsif ( $seg->from_start <= $bl->end and $seg->from_end >= $bl->start ) {
        ...
        if ( defined $inside_seg ) {
          ...
          foreach my $seg (@overlap_segs) {
            ...
            my $cut_block = $bl->restrict_between_positions( $reg_start, $reg_end, "SEQ" );
    ...
    NEWBLOCK:
      for ( my $i = 0; $i < @new_list; $i++ ) {
        ...
        if ( exists( $new_block{$bl} ) ) {
          ...
          if ( $i > 0 and not exists( $new_block{ $new_list[ $i - 1 ] } ) ) {
            $flank_left = $new_list[ $i - 1 ];
          }
          if ( $i < scalar(@new_list) - 1 and not exists( $new_block{ $new_list[ $i + 1 ] } ) ) {
            $flank_right = $new_list[ $i + 1 ];
          }

          if ( defined $flank_left and
               $contigs_of_blocks{$bl}->to_Slice->seq_region_name eq $contigs_of_kept_blocks{$flank_left}->to_Slice->seq_region_name )
          {
            $keep_chain = 0;
            last NEWBLOCK;
          }
          ...
          if ( defined $flank_right and
               $contigs_of_blocks{$bl}->to_Slice->seq_region_name eq $contigs_of_kept_blocks{$flank_right}->to_Slice->seq_region_name )
          {
            $keep_chain = 0;
            last NEWBLOCK;
          }
        } ## end if ( exists( $new_block...))
      } ## end NEWBLOCK: for ( my $i = 0; $i < @new_list...)
  ...
  return \@net_chains;
} ## end sub calculate_simple_high_net
...
sub calculate_simple_low_net {
  ...
  return \@net_chains;
} ## end sub calculate_simple_low_net
...
sub make_runnable {
  ...
  $parameters{-chainNet}    = $self->CHAIN_NET    ? $self->CHAIN_NET    : $BIN_DIR . "/" . "chainNet";
  $parameters{-netSyntenic} = $self->NET_SYNTENIC ? $self->NET_SYNTENIC : $BIN_DIR . "/" . "netSyntenic";
  $parameters{-netFilter}   = $self->NET_FILTER   ? $self->NET_FILTER   : $BIN_DIR . "/" . "netFilter";
...
sub read_and_check_config {
  ...
  # check for sanity
  my %allowable_methods = ( STANDARD => 1, SYNTENIC => 1, SIMPLE_HIGH => 1, SIMPLE_MEDIUM => 1, SIMPLE_LOW => 1, );

  if ( $self->METHOD and not $allowable_methods{ $self->METHOD } ) {
