...
sub pair_features {
  ...
  foreach my $pair ( keys %$paired_features ) {
    ...
    READ: foreach my $bfeat (@bs) {
        if ( $afeat->seqname eq $bfeat->seqname ) {
          ...
          if (
            ( $afeat->end > $bfeat->start && $afeat->start - $bfeat->end <= $self->PAIREDGAP ) or
            ( $bfeat->end > $afeat->start && $bfeat->start - $afeat->end <= $self->PAIREDGAP ) or
            # they overlap
            ( $afeat->start <= $bfeat->end && $afeat->end >= $bfeat->start ) )
          {
            # they should be oriented pointing towards each other on
            # opposite strands...
            if ( $afeat->strand == 1 && $bfeat->strand == 1 && $afeat->hstrand != $bfeat->hstrand ) {
              # are they in the right order
              # on the forward strand a should come before b
              # and vice versa
              next READ
                unless ( ( $afeat->hstrand == 1 && $afeat->start < $bfeat->start ) or
          ...
          } ## end if ( ( $afeat->end > $bfeat...))
        } ## end if ( $afeat->seqname eq...)
      } ## end READ: foreach my $bfeat (@bs)
    } ## end foreach my $afeat (@as)
  ...
  } ## end foreach my $pair ( keys %$paired_features)
  return \@selected_reads;
} ## end sub pair_features
...
# I am  going to flip a and b over
    ...
    if ( scalar(@ugfs) == 2 ) {
      if ( $ugfs[0]->end >= $ugfs[1]->start && $ugfs[0]->start <= $ugfs[1]->end ) {
        $ugfs[0]->start( $ugfs[1]->start ) if $ugfs[1]->start < $ugfs[0]->start;
        $ugfs[0]->end( $ugfs[1]->end ) if $ugfs[1]->end > $ugfs[0]->end;
        $ugfs[0]->hstart(1);
