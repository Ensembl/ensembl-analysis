...
sub toggle_indexation_multkeys {
  ...
  foreach my $row ( @{ $sth->fetchall_arrayref } ) {
    next unless ( $row->[1] == 1 );
    if ( ( $self->MODIFIER eq 'disable' and $row->[11] ne 'disabled' ) or ( $self->MODIFIER eq 'enable' and $row->[11] ne '' ) ) {
...
sub dump_table {
  ...
  my $dump_file_name =
    $temp_dir . "/" . $self->DB_HKEY . "_" . $dbname . "_" . $self->TABLE_NAME . "_" . $host . "_" . $port . "_" . $$ . ".dump";
...
sub check_new_table {
  ...
  $sth->finish();
  warning( "The new table " . $self->TABLE_NAME . "has not the same number of rows as its original! $rows_count => $sorted_count" )
    if ( $sorted_count != $rows_count );
}
