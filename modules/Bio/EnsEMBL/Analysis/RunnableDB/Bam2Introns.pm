...
sub fetch_input {
  ...
  print "Found model " .
    $rough->stable_id . " " . $rough->seq_region_name . " " . $rough->start . " " . $rough->end . " " . $rough->strand . "\n";
  ...
  my $options =
    ...
    "--model est2genome --forwardcoordinates false " . "--softmasktarget $mask --exhaustive false --percent 80 " .
  ...
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::Bam2Introns->new( -analysis      => $self->analysis,
                                                                     -program       => $program,
...
sub run {
  ...
  if ($iids) {
    ...
    my $pipeline_analysis = $pipelinea->get_AnalysisAdaptor->fetch_by_logic_name( $self->analysis->logic_name );
    ...
    unless ($check) {
      foreach my $condition ( @{ $rule->list_conditions } ) {
        ...
        my $inputIDFactory =
          new Bio::EnsEMBL::Pipeline::Utils::InputIDFactory( -db => $pipelinea, -file => 1, -logic_name => $condition, );
