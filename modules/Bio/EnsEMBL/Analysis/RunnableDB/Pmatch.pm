...
sub fetch_input {
  my ($self) = @_;
  my $slice = $self->fetch_sequence( $self->input_id, $self->db, $self->REPEAT_MASKING );
  $self->query($slice);
  my %parameters = %{ $self->parameters_hash };
  my $program    = $self->analysis->program_file;
  $program = $self->BINARY_LOCATION if ( !$program );
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::Pmatch->new( -query             => $self->query,
                                                                -program           => $program,
...
sub REPEAT_MASKING {
  my ( $self, $arg ) = @_;
  if ($arg) {
    throw( "Runnable::Pmatch " . $arg . " must be an array ref of logic names not ." . $arg ) unless ( ref($arg) eq 'ARRAY' );
...
sub read_and_check_config {
  ...
  foreach my $config_var ( qw(PROTEIN_FILE
                           OUTPUT_DB) )
  {
    throw( "You must define $config_var in config for logic '" . $self->analysis->logic_name . "'" ) if not defined $self->$config_var;
  }
}
...
FEATURE: foreach my $feature ( @{ $self->output } ) {
    ...
    if ($@) {
      throw( "RunnableDB:store failed, failed to write " . $feature . " to " . "the database " . $adaptor->dbc->dbname . " $@" );
    }
  }
  return 1;
}
