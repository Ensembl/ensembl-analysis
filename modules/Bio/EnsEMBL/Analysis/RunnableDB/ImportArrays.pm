...
sub run_FASTA {
  ...
  my %valid_fields = ( -probe_set => undef, -name => undef, -array => undef, -array_chip => undef, -description => undef, );
    ...
    if (/$header_regex/) {    #This places header values into @match_refs
      ...
      if ($current_sequence) {
        ...
        if ( !$existing_probe ) {
          ...
          $existing_probe = $self->create_new_probe( $current_array_chip, \%probe_attrs, length($current_sequence), );
          ...
          if ( ( defined $existing_name ) && ( $existing_name eq $probe_attrs{'-name'} ) ) {
            ...
            if ( defined($existing_name) ) {
              warn "Found replicate probe for ($existing_name) replicate:\t" . $probe_attrs{'-name'} . "\n";
              $redundant_seq++;
            }

            $existing_probe->add_array_chip_probename( $current_array_chip->dbID, $probe_attrs{'-name'}, $current_array_chip->get_Array );
          }
        } ## end else [ if ( !$existing_probe )]

        $current_sequence = undef;
      } ## end if ($current_sequence)
  ...
  if ( !$existing_probe ) {
    ...
    if ( $existing_name && ( $existing_name eq $probe_attrs{'-name'} ) ) {
...
sub create_new_array_chip {
  ...
  if ( $array_chip = $achip_adaptor->fetch_by_array_design_ids( $array->dbID, $design_id ) ) {
    ...
    if ( $array_chip->has_status('IMPORTED') ) {
      throw( "$array_name ArrayChip has already been IMPORTED. Please rollback_ArrayChip or recreate your arrays_nr." .
             $self->ARRAY_FORMAT . '.fasta file for alignment' );
    }
    else {    #Rollback
...
sub outdb {
  ...
  if ( !defined $self->{'_outdb'} ) {
    ...
    $self->{'_outdb'} = Bio::EnsEMBL::Funcgen::DBSQL::DBAdaptor->new( %{ $self->OUTDB }, -dnadb => $dnadb, );
