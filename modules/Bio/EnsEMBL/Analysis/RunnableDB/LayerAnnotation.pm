...
sub fetch_input {
  ...
  foreach my $dbname ( @{ $self->SOURCEDB_REFS } ) {
    ...
    my $tlslice = $dbh->get_SliceAdaptor->fetch_by_region( $slice->coord_system->name, $slice->seq_region_name );
...
sub run {
  ...
  for ( my $i = 0; $i < @layers; $i++ ) {
    ...
    if ( $layer->genes ) {
      ...
      for ( my $j = $i - 1; $j >= 0; $j-- ) {
        if ( exists $filter_against{ $layers[$j]->id } and @{ $layers[$j]->genes } ) {
...
sub read_and_check_config {
  ...
  foreach my $i ( qw(LAYERS
                  ...
                  FILTER) )
  {
    throw("You must define $i in config") if not $self->$i;
  }

  # check types
  throw("Config var LAYERS must be an array reference")        if ref( $self->LAYERS ) ne "ARRAY";
  throw("Config var SOURCEDB_REFS must be an array reference") if ref( $self->SOURCEDB_REFS ) ne "ARRAY";
  throw("Config var TARGETDB_REF must be an scalar")           if ref( $self->TARGETDB_REF );
  throw("Config var FILTER must be an scalar")                 if ref( $self->FILTER );
  ...
  foreach my $el ( @{ $self->LAYERS } ) {
    throw("Elements of LAYERS must be hash references") if ref($el) ne "HASH";

    throw("Each element of LAYERS must contain a layer ID") if not exists $el->{ID};

    throw( "LAYER " . $el->{ID} . " should a list of BIOTYPES" ) if not exists $el->{BIOTYPES} or ref( $el->{BIOTYPES} ) ne "ARRAY";
    ...
    if ( exists $el->{FILTER_AGAINST} ) {
      throw("In layer $layer_id FILTER_AGAINST must contain a list of layer ids") if ref( $el->{FILTER_AGAINST} ) ne "ARRAY";
      @filter_against = @{ $el->{FILTER_AGAINST} };

      foreach my $id (@filter_against) {
        throw( "In FILTER_AGAINST in layer $layer_id, '$id' is not the name " . "of a higher level layer" ) if not exists $layer_ids{$id};
      }
    }

    push @layers,
      Bio::EnsEMBL::Analysis::RunnableDB::LayerAnnotation::Layer->new( -id             => $layer_id,
                                                                       -discard        => $discard,
                                                                       -biotypes       => \@biotypes,
...
sub new {
  ...
  my ( $id, $discard, $genes, $filter_object, $filter_against, $biotypes ) =
    rearrange( [ 'ID', 'DISCARD', 'GENES', 'FILTER_OBJECT', 'FILTER_AGAINST', 'BIOTYPES' ], @args );
