...
sub fetch_input {
  ...
  my $ref_slice = $q_dbh->get_SliceAdaptor->fetch_by_region( 'toplevel', $seq_name, undef, undef, undef, $q_assembly_version );
  ...
  my %blocks_by_target;
  foreach my $block ( @{ $compara_dbh->get_GenomicAlignBlockAdaptor->fetch_all_by_MethodLinkSpeciesSet_DnaFrag( $mlss, $ref_dnafrag ) } )
  {
  ...
  for ( my $i = $target_chunk_id - 1; $i < @all_target_names; $i += $target_chunk_total ) {

    my $target_name = $all_target_names[$i];

    $target_slices{$target_name} =
      $t_dbh->get_SliceAdaptor->fetch_by_region( 'toplevel', $target_name, undef, undef, undef, $t_assembly_version );
    ...
    } ## end foreach my $target_block ( ...)
  } ## end for ( my $i = $target_chunk_id...)
  ...
  } ## end foreach my $targets (@$target_batches)
