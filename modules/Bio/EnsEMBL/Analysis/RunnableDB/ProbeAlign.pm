...
sub new {
  ...
  if ( !$extdb_id ) {
    ...
    my $insert_sql =
      'INSERT into external_db(db_name, db_release, status, dbprimary_acc_linkable, priority, db_display_name, type)' .
...
sub filter_features {
  ...
  foreach my $probe_id ( keys %hits_by_probe ) {
    ...
    if ( $promiscuous || ( $num_hits > $max_hits ) ) {
...
sub get_display_name_by_stable_id {
  ...
  if ( !exists $self->{'display_name_cache'}->{$stable_id} ) {
    ( $self->{'display_name_cache'}->{$stable_id} ) = $self->outdb->dnadb->dbc->db_handle->selectrow_array(
...
sub set_probe_and_slice {
  ...
  foreach my $feature (@$features) {
    ...
    if ( $mapping_type eq 'transcript' ) {
      ...
      my %gap_lengths = ( 5 => undef, 3 => undef, );
      #warn "\n\n\ntrans($seq_id) start end strand $transcript_start $transcript_end $transcript_strand";
      ...
      } ## end foreach my $block (@genomic_blocks)
      ...
      foreach my $end ( keys %gap_lengths ) {
        ...
        } ## end if ( $gap_lengths{$end...})
      } ## end foreach my $end ( keys %gap_lengths)
      ...
      foreach my $block (@stranded_cigar_line) {
        ...
        } ## end if ( $block_end >= $gap_start)

        $cigar_line .= $block;
      } ## end foreach my $block (@stranded_cigar_line)
...
sub outdb {
  ...
  if ( !defined $self->{'efg_db'} ) {
    ...
    $self->{'efg_db'} = Bio::EnsEMBL::Funcgen::DBSQL::DBAdaptor->new( %{ $self->OUTDB }, -dnadb => $dnadb, );
