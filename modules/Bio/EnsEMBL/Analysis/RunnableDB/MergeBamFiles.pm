...
sub fetch_input {
  ...
  throw( "You're missing some bam files, you have $num_bam_files bam files instead of " . $self->analysis->db_version )
    unless ( $self->analysis->db_version == $num_bam_files );
  if ( $num_bam_files == 1 ) {
...
sub run {
  ...
  my $picard_cmd = $self->analysis->program .
    ' -Xmx2g -jar ' . $self->analysis->program_file . $options . ' OUTPUT=' . $self->outfile . $picard_cmd_files;
