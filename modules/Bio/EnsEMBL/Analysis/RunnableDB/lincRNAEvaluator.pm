...
sub fetch_input {
  ...
  print "Made " .
    scalar(@single_transcript_lincrna_genes) . " single_transcript genes, broken down from " .
    scalar(@$lincrna_genes) . " multi_transcript lincRNA candidates from lincRNAFinder stage.\n";
  ...
  my $runnable =
    Bio::EnsEMBL::Analysis::Runnable::lincRNAEvaluator->new( -query          => $self->query,
...
sub run {
  ...
  if ( $self->WRITE_LINCRNAS_WHICH_CLUSTER_WITH_PROC_TRANS == 1 ) {
    print "\nRunning GeneBuilder for " .
      scalar( @{ $self->single_runnable->ncrna_clusters_with_processed_transcript } ) .
      " lincRNAs which cluster with processed transcript...\n";

    $gb = Bio::EnsEMBL::Analysis::Runnable::GeneBuilder->new( -query    => $self->query,
                                                              -analysis => $self->analysis,
  ...
  if ( $self->WRITE_LINCRNAS_WHICH_CLUSTER_WITH_EXISTING_LINCRNAS == 1 ) {
    print "\nRunning GeneBuilder for " .
      scalar( @{ $self->single_runnable->ncrna_clusters_with_existing_lincRNAs } ) .
  ...
  print "\nGENEBUILDER RETURNED " . @output_clustered .
" lincRNA GENES FOR WRITING (THIS DOES NOT INCLUDE REJECTED lincRNAs). THE TYPES OF lincRNA GENES WRITTEN DEPEND ON THE lincRNAEvaluator CONFIG SETTINGS.\n";
...
sub write_output {
  ...
  if ( $self->MARK_OVERLAPPED_PROC_TRANS_IN_VALIDATION_DB == 1 ) {
    ...
    unless ( defined $update_analysis && ref($update_analysis) =~ m/Bio::EnsEMBL::Analysis/ ) {
      throw( " Analysis with logic_name " .
             $self->OVERLAPPED_GENES_NEW_MERGED_LOGIC_NAME . " can't be found in " . $db->dbname . "\@" . $db->host . "\n" );
    }
    ...
    for my $ug (@proc_tran_genes_to_update) {
      ...
      if ( $ug->analysis->logic_name =~ m/$hav_logic_name_to_match/ ) {
        ...
        warning( "not updating gene " .
                 $ug->biotype . " with dbID " . $ug->dbID . " as it has the wrong analysis logic_name: " . $ug->analysis->logic_name .
                 " ( to update, the logic_name should contain the string " . $self->PROC_TRANS_HAVANA_LOGIC_NAME_STRING . ")" );
      }
    }
  } ## end if ( $self->MARK_OVERLAPPED_PROC_TRANS_IN_VALIDATION_DB...)
  ...
  if ( $self->WRITE_REJECTED_NCRNAS == 1 ) {
    print
"Writing rejected genes. They have not been collapsed by GeneBuilder so are still in one-gene-one-transcript format, as generated by lincRNAFinder.\n";
  ...
  print $sucessful_count . " genes written to " . $self->output_db->dbname . " @ " . $self->output_db->host . "\n";
...
sub get_gene_sets {
  ...
  unless ( exists $sets_to_cluster{"SET_1_CDNA"} && exists $sets_to_cluster{"SET_2_PROT"} ) {
    throw(
" configuration error - I expect to get 2 sets of genes with names \"SET_1_CDNA\" and \"SET_2_PROT\" - check your config - you can't change these names!!!! \n"
    );
  }
...
sub read_and_check_config {
  ...
  #######
  foreach my $var (
          qw(LINCRNA_DB FINAL_OUTPUT_DB FINAL_OUTPUT_BIOTYPE VALIDATION_DBS WRITE_REJECTED_NCRNAS MARK_EXISTING_LINCRNA_IN_VALIDATION_DB))
  {
