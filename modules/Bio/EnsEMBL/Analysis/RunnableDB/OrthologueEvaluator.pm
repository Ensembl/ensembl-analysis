...
sub upload_input_ids {
  ...
  my $if = Bio::EnsEMBL::Pipeline::Utils::InputIDFactory->new( -db            => $self->pipeline_adaptor,
  ...
  my $a     = $self->pipeline_adaptor->get_AnalysisAdaptor->fetch_by_logic_name($submit_analysis);
...
sub chunk_and_write_fasta_sequences {
  ...
  unless ($base_dir) {
    ...
    $conf ? $base_dir = $conf :
      throw(
           "There's no output-dir configured in Exonerate2Genes.pm " . "for analysis " . $self->post_logic_name . " check the config\n" );
  }
  ...
  $seq_file->close();
  return \@filenames;
} ## end sub chunk_and_write_fasta_sequences
...
sub pipeline_adaptor {
  ...
  if ($db) {
    my $pa = Bio::EnsEMBL::Pipeline::DBSQL::DBAdaptor->new( -host   => $db->host,
                                                            -dbname => $db->dbname,
                                                            -user   => $db->username,
                                                            -pass   => $db->password,
                                                            -port   => $db->port, );
