  ...
  if ( !$hash->{$name} ) {    # if we don't already have an entry for this ...
    if ( exists $DATABASES->{$name} ) {
      ...
      if ( scalar( keys( %{$constructor_args} ) ) == 0 ) {
        ...
        # Return undef.
        warning( sprintf( "Empty entry for database '%s' in Databases.pm\n", $name ) );
        return undef;
      }
      ...
      if ( defined $non_standard_db_adaptor ) {    # value of
        if ( $non_standard_db_adaptor =~ m/1/ || $non_standard_db_adaptor eq "pipeline" ) {
          ...
          throw( "No matching non-standard adaptor could be found. If you want " . "a standard adaptor you should pass in undef." );
        }

      } ## end if ( defined $non_standard_db_adaptor)
      else {
      ...
      if ( $name ne $DNA_DBNAME && defined($DNA_DBNAME) && $try_to_attach_dna_db ) {
        if ($not_use_dna_database) {
          ...
          my $core_db_species = $db->get_MetaContainer->get_common_name();
          ...
          if ($dna_db_and_core_db_are_compatible) {
            $db->dnadb($dnadb);
            print "\nAttaching DNA_DB " . $dnadb->dbc->dbname . " to " . $db->dbc->dbname . "\n";
          }
        } ## end else [ if ($not_use_dna_database)]
      } ## end if ( $name ne $DNA_DBNAME...)
      else {
...
} ## end sub get_genes_of_biotypes_by_db_hash_ref
...
sub create_new_database {
  ...
  if ( !db_exists($new_db_key) ) {
    ...
    my $create_cmd = 'mysql -h ' . $h_new_db{-host} . ' -u ' .
      $h_new_db{-user} . ' -p' . $h_new_db{-pass} . ' -P' . $h_new_db{-port} . ' -e" create database  ' . $h_new_db{-dbname} . '"';
    system($create_cmd);
    $create_cmd = 'mysql -h ' . $h_new_db{-host} . ' -u ' .
      $h_new_db{-user} . ' -p' . $h_new_db{-pass} . ' -P' . $h_new_db{-port} . ' -D ' . $h_new_db{-dbname} . ' < ' . $template_dump;
    system($create_cmd);
