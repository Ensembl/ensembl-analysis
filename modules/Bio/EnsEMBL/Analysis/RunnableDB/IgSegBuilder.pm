...
sub fetch_input {
  ...
  my $tlslice = $trandb->get_SliceAdaptor->fetch_by_region( 'toplevel', $slice->seq_region_name );
  $self->query($tlslice);

  my ( @lv, @d, @j, @c );

  my @groups = ( { logics => $self->LV_LOGICS, result => \@lv, },
      ...
      foreach my $logic (@$logics) {
        foreach my $t ( @{ $slice->get_all_Transcripts( 1, $logic ) } ) {
          ...
          if ( $t->coding_region_start > $t->start or $t->coding_region_end < $t->end ) {
            ...
            my $tr  = Bio::EnsEMBL::Translation->new( -start_Exon => $e[0], -end_Exon => $e[-1], -start => 1, -end => $e[-1]->length );
            my @sfs = @{ $t->get_all_supporting_features };
            $t = Bio::EnsEMBL::Transcript->new( -analysis => $t->analysis, -exons => $t->get_all_translateable_Exons );
...
sub write_output {
  ...
  if ( $self->SUPPORTING_FEATURE_OUTPUT_LOGIC ) {
    $sf_ana = $out_db->get_AnalysisAdaptor->fetch_by_logic_name( $self->SUPPORTING_FEATURE_OUTPUT_LOGIC );
...
sub cluster_by_genomic_overlap {
  ...
  foreach my $grp ( values %tran_groups ) {
    ...
    foreach my $t ( sort { $a->start <=> $b->start } @$grp ) {
      if ( not @c or $t->start > $c[-1]->{end} + 1 ) {
        push @c, { start => $t->start, end => $t->end, trans => [$t], };
      }
      else {
        push @{ $c[-1]->{trans} }, $t;
        if ( $c[-1]->{end} < $t->end ) {
  ...
  } ## end foreach my $grp ( values %tran_groups)

  return \@genes;
} ## end sub cluster_by_genomic_overlap
...
sub transfer_to_local_slices {
  ...
  foreach my $g ( sort { $a->start <=> $b->start } @$genes ) {
    if ( not @groups or $groups[-1]->{strand} != $g->strand or $g->start > $groups[-1]->{end} + 100000 ) {
      push @groups, { start => $g->start, end => $g->end, strand => $g->strand, genes => [], };
    }
  ...
  foreach my $grp (@groups) {
    ...
    my $local_slice =
      $self->get_dbadaptor( $self->TRANDB_DATABASES_NAME )->get_SliceAdaptor->fetch_by_region( $self->query->coord_system->name,
                                                                                               $self->query->seq_region_name,
                                                                                               $gstart, $gend, $grp->{strand} );
    push @retgenes, map { $_->transfer($local_slice) } @{ $grp->{genes} };
  }

  return \@retgenes;
} ## end sub transfer_to_local_slices
...
sub prune_Exons {
  ...
  foreach my $tran ( @{ $gene->get_all_Transcripts } ) {
    ...
    foreach my $e ( @{ $tran->get_all_Exons } ) {
      my $k = join( ":", $e->start, $e->end, $e->strand, $e->phase, $e->end_phase );
    ...
    } ## end foreach my $e ( @{ $tran->get_all_Exons...})
...
TRAN: foreach my $t (@tran) {
  ...
  foreach my $t (@tran) {
    ...
    foreach my $kt (@ftran) {
      ...
      foreach my $i (@intr) {
        foreach my $ki (@kintr) {
          if ( $i->start == $ki->start and $i->end == $ki->end ) {
            $matches++;
            last;
          }
        }
    ...
    foreach my $ft (@ftran) {
      ...
      foreach my $i ( @{ $t->get_all_Introns } ) {
        ...
        foreach my $fi ( @{ $ft->get_all_Introns } ) {
          if ( $i->overlaps($fi) and abs( $i->start - $fi->start <= 9 ) and abs( $i->end - $fi->end <= 9 ) ) {
            $found_matching = 1;
            last;
          }
        }
  ...
  foreach my $tran (@tran) {
    ...
    foreach my $il (@intron_lists) {
      ...
      foreach my $ti (@introns) {
        my $found_in_other = 0;
        foreach my $oi (@$il) {
          if ( $ti->start == $oi->start and $ti->end == $oi->end ) {
            $found_in_other = 1;
            last;
          }
        }
    ...
    if ( not $is_redundant ) {
      ...
      foreach my $tex ( $exons[0], $exons[-1] ) {
        ...
        foreach my $eid ( keys %all_exons ) {
          if ( $all_exons{$eid}->start <= $tex->start and $all_exons{$eid}->end >= $tex->end ) {
...
sub prune_D_J_transcripts {
  ...
  if ( scalar( @{ $best->get_all_Exons } ) > 1 ) {
    ...
    foreach my $e ( @{ $best->get_all_Exons } ) {
      if ( not defined $exon ) {
        $exon = new Bio::EnsEMBL::Exon( -start => $e->start, -end => $e->end, -strand => $e->strand, -slice => $e->slice, );
        $min  = $e->start;
        $max  = $e->end;
      }
      else {
    ...
    my $exon = new Bio::EnsEMBL::Exon( -start  => $best->get_all_Exons->[0]->start,
...
} ## end sub trim_gene_translation5
...
} ## end sub trim_gene_translation3
...
} ## end sub transfer_exon_supporting_features
