...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptUtils
  qw(Transcript_info set_stop_codon set_start_codon attach_Slice_to_Transcript evidence_coverage list_evidence);
...
sub create_bmg_runnables {
  ...
  if ( $self->LIMIT_TO_FEATURE_RANGE == 0 ) {
    my $genomic_file = write_seqfile( $self->query, create_file_name( "genomic", "fa", "/tmp/" ) );
    $self->genomic_file($genomic_file);
    my @ids          = sort keys(%$hits_hash);
    my $seqs         = $self->get_protein_sequences( \@ids );
    my $peptide_file = write_seqfile( $seqs, create_file_name( "peptide", "fa", "/tmp/" ) );
    ...
    my $r =
      Bio::EnsEMBL::Analysis::Runnable::ExonerateTranscript->new( -analysis    => $self->analysis,
                                                                  ...
                                                                  %exonerate_parameters, %params_hash, );
    $self->runnable($r);
  }
  else {
    foreach my $id ( keys(%$hits_hash) ) {
      ...
      foreach my $feature (@$features) {
        ...
        my $genomic_file = write_seqfile( $query, create_file_name( "genomic", "fa", "/tmp/" ) );
        $self->genomic_file($genomic_file);
  ...
  } ## end else [ if ( $self->LIMIT_TO_FEATURE_RANGE...)]
...
sub process_transcripts {
  ...
  foreach my $transcript (@$transcripts) {
    ...
    my $unmasked_slice = $self->fetch_sequence( $query_name, $self->output_db );
    ...
    if ( contains_internal_stops($transcript) ) {
      warning( Transcript_info($transcript) . " contains internal stops will be thrown " . "This will be thrown away" );
    }
...
sub filter_genes {
  ...
  if ( $self->filter_object ) {
    ( $filtered_set, $rejected_set ) = $self->filter_object->filter_genes($genes);
...
sub extra_sanity_check {
  my ($self) = @_;
  throw("ExonerateAlignFeatures needs an exonerate parameters hash defined") if ( !$self->EXONERATE_PARAMETERS );
}
