...
sub sort_chains_by_max_block_score {
  ...
  foreach my $chain (@$chains) {
    ...
    foreach my $block (@$chain) {
      ...
      if ( not exists $chain_hash->{score} or $block->score > $chain_hash->{score} ) {
  ...
  my @sorted =
    map { $_->{chain} } sort { $b->{score} <=> $a->{score} or $a->{qname} cmp $b->{qname} or $a->{tname} cmp $b->{tname} } @chain_hashes;
...
sub compara_cigars_from_daf_cigar {
  ...
  $q_cigar_line .= ( ( $q_counter == 1 ) ? "" : $q_counter ) . "M" if $q_counter;
  $t_cigar_line .= ( ( $t_counter == 1 ) ? "" : $t_counter ) . "M" if $t_counter;

  return ( $q_cigar_line, $t_cigar_line, $align_length );
} ## end sub compara_cigars_from_daf_cigar
...
sub daf_cigar_from_compara_cigars {
  ...
  foreach my $piece ( split( /(\d*[MDGI])/, $q_cigar ) ) {
    ...
    if ( $num > 0 ) {
      push @q_pieces, { num => $num, type => $type, };
    }
  }
  foreach my $piece ( split( /(\d*[MDGI])/, $t_cigar ) ) {
    ...
    if ( $num > 0 ) {
      push @t_pieces, { num => $num, type => $type, };
    }
  }
  ...
  while ( @q_pieces and @t_pieces ) {
    ...
    if ( $q->{num} == $t->{num} ) {
      ...
      unshift @q_pieces, { type => 'M', num => $q->{num} - $t->{num}, };
      ...
      unshift @t_pieces, { type => 'M', num => $t->{num} - $q->{num}, };
    }
  } ## end while ( @q_pieces and @t_pieces)
  ...
  foreach my $piece ( split( /(\d*[MDI])/, $daf_cigar ) ) {
    ...
    if ( defined $last_type and ( ( $last_type eq 'I' and $type eq 'D' ) or ( $last_type eq 'D' and $type eq 'I' ) ) ) {
  ...
  return $daf_cigar;
} ## end sub daf_cigar_from_compara_cigars
  ...
  foreach my $chain_of_dafs (@$chains_of_dafs) {
    ...
    foreach my $raw_daf ( sort { $a->start <=> $b->start } @$chain_of_dafs ) {
      ...
      if ( defined $self->INPUT_GROUP_TYPE ) {
        $group = Bio::EnsEMBL::Compara::GenomicAlignGroup->new( -type => $self->INPUT_GROUP_TYPE );
      }

      foreach my $daf (@split_dafs) {
        my ( $q_cigar, $t_cigar, $al_len ) = $self->compara_cigars_from_daf_cigar( $daf->cigar_string );
        ...
        my $q_genomic_align =
          Bio::EnsEMBL::Compara::GenomicAlign->new( -dnafrag                 => $q_dnafrag,
        ...
        my $t_genomic_align =
          Bio::EnsEMBL::Compara::GenomicAlign->new( -dnafrag                 => $t_dnafrag,
        ...
        my $gen_al_block =
          Bio::EnsEMBL::Compara::GenomicAlignBlock->new( -genomic_align_array     => [ $q_genomic_align, $t_genomic_align ],
                                                         -score                   => $daf->score,
                                                         -length                  => $al_len,
                                                         -method_link_species_set => $out_mlss );

        push @chain_of_blocks, $gen_al_block;
      } ## end foreach my $daf (@split_dafs)
      if ( defined $group ) {
        $group->genomic_align_array( \@gas );
      }
    } ## end foreach my $raw_daf ( sort ...)

    push @chains_of_blocks, \@chain_of_blocks;
  } ## end foreach my $chain_of_dafs (...)

  return \@chains_of_blocks;
} ## end sub convert_output
