...
sub fetch_input {
  ...
  eval {
    $q_assembly_version = $q_dbh->get_CoordSystemAdaptor->fetch_by_name( 'toplevel', $q_gdb->assembly );

    $t_assembly_version = $t_dbh->get_CoordSystemAdaptor->fetch_by_name( 'toplevel', $t_gdb->assembly );
  };
  $@ and do {
  ...
  my $kill_list = {};
  if ( $self->KILL_LIST ) {
    open( KILL, $self->KILL_LIST ) or throw( "Could not open kill list " . $self->KILL_LIST . " for reading" );
    while (<KILL>) {
      /^\#/ and next;
      /^(\S+)/ and $kill_list->{$1} = 1;
    }
  ...
  if ( $input_id =~ /:/ ) {
    ...
    $self->query_slice( $sa->fetch_by_region( 'toplevel', $slice->seq_region_name ) );
    ...
    foreach my $g ( @{ $slice->get_all_Genes } ) {
      ...
      if (@good_trans) {
        if ( $self->LONGEST_SOURCE_TRANSCRIPT ) {
          ...
          @good_trans = sort { CORE::length( $b->translateable_seq ) <=> CORE::length( $a->translateable_seq ); } @good_trans;
          @good_trans = ( $good_trans[0] );
        }
        push @gene_records,
          Bio::EnsEMBL::Analysis::RunnableDB::WGA2Genes::GeneRecord->new( -gene => $g, -source_transcripts => \@good_trans );
      }
    } ## end foreach my $g ( @{ $slice->...})

  } ## end if ( $input_id =~ /:/ )
  else {
    # assume iid is a gene stable id; ignore the kill-list
    my ( $gene, $tran );
    eval { $gene = $q_dbh->get_GeneAdaptor->fetch_by_stable_id($input_id); };
    eval { $tran = $q_dbh->get_TranscriptAdaptor->fetch_by_stable_id($input_id); };
    ...
    $self->query_slice( $sa->fetch_by_region( 'toplevel', $gene->seq_region_name ) );
    ...
    if (@good_trans) {
      ...
      push @gene_records,
        Bio::EnsEMBL::Analysis::RunnableDB::WGA2Genes::GeneRecord->new( -gene => $gene, -source_transcripts => \@good_trans );
    }
  } ## end else [ if ( $input_id =~ /:/ )]
  ...
  #########
  my $mlss = $compara_dbh->get_MethodLinkSpeciesSetAdaptor->fetch_by_method_link_type_GenomeDBs( $self->INPUT_METHOD_LINK_TYPE,
                                                                                                 [ $q_gdb, $t_gdb ] );
  throw( "No MethodLinkSpeciesSet for :\n" . $self->INPUT_METHOD_LINK_TYPE . "\n" . $query_species . "\n" . $target_species )
    if not $mlss;

  my $dnafrag = $compara_dbh->get_DnaFragAdaptor->fetch_by_GenomeDB_and_name( $q_gdb, $self->query_slice->seq_region_name );

  my $gaba = $compara_dbh->get_GenomicAlignBlockAdaptor;

  my $gen_al_blocks = $gaba->fetch_all_by_MethodLinkSpeciesSet_DnaFrag( $mlss, $dnafrag, $reg_start, $reg_end );
  ...
  foreach my $block (@$gen_al_blocks) {
    ...
    if ( not exists $self->target_slices->{ $tga->dnafrag->name } ) {
      ...
      $self->target_slices->{ $tga->dnafrag->name } = $t_dbh->get_SliceAdaptor->fetch_by_region( 'toplevel', $tga->dnafrag->name );
    }
  ...
  foreach my $chain_id ( keys %chains ) {
    push @chains,
      [ sort { $a->reference_genomic_align->dnafrag_start <=> $b->reference_genomic_align->dnafrag_start; } @{ $chains{$chain_id} } ];
  }
...
sub run {
  ...
  logger_info( "INITIAL GENES: " . join( " ", map { $_->gene->stable_id } @{ $self->gene_records } ) . "\n" );

  logger_info( "ORIGINAL_CHAINS\n" . stringify_chains( $self->genomic_align_block_chains ) );

  @chains_generecs_pairs =
    $self->segregate_chains_and_generecords( $self->genomic_align_block_chains, $self->gene_records,
  ...
  foreach my $chains_generecs (@chains_generecs_pairs) {
    ...
    my $pair_id = join( ":", map { $_->gene->stable_id } @$generecs );
    ...
    for ( ;; ) {
      ...
      my $filtered_chains = filter_irrelevant_chains( $alignment_chains, \@cds_feats );

      logger_info( "RELEVANT_CHAINS\n" . stringify_chains($filtered_chains) );

      $filtered_chains = filter_inconsistent_chains( $filtered_chains, $self->OVERLAP_CHAIN_FILTER );
      ...
      $filtered_chains = $self->trim_exon_split_chains( $filtered_chains, \@cds_feats );
      logger_info( "TRIMMED_CHAINS\n" . stringify_chains($filtered_chains) );

      my @these_pairs =
        $self->segregate_chains_and_generecords( $filtered_chains, \@these_generecs, $self->MIN_CHAIN_ANNOTATION_OVERLAP );
      ...
      foreach my $pair (@these_pairs) {
        ...
        my $gene_scaffold =
          $self->make_gene_scaffold_and_project_genes( $net_blocks,            $subset_generecs,
                                                       $gs_name,               $self->MAX_EDITABLE_STOPS_PRIMARY,
                                                       $self->MIN_COVERAGE,    $self->MIN_NON_GAP,
                                                       $self->FULLY_GAP_EXONS, $self->PARTIAL_GAP_EXONS,
                                                       $external_db_id );
      ...
      foreach my $res (@these_results) {
        ...
        foreach my $res_gene (@res_genes) {
          if ( scalar( @{ $res_gene->source_transcripts } ) == scalar( @{ $res_gene->projected_transcripts } ) ) {
    ...
    my $filtered_chains = filter_block_overlap_chains( $alignment_chains, [ values %blocks_from_used_chains ] );

    logger_info("ITERATING over remaining single chains\n");

    foreach my $chain (@$filtered_chains) {
      my ($single_pair) = $self->segregate_chains_and_generecords( [$chain], $generecs, $self->MIN_CHAIN_ANNOTATION_OVERLAP );
      ...
      if ( defined $single_pair ) {
        ...
        my $gene_scaffold =
          $self->make_gene_scaffold_and_project_genes( $chain,                 \@grs,
                                                       $gs_name,               $self->MAX_EDITABLE_STOPS_NON_PRIMARY,
                                                       $self->MIN_COVERAGE,    $self->MIN_NON_GAP,
                                                       $self->FULLY_GAP_EXONS, $self->PARTIAL_GAP_EXONS,
                                                       $external_db_id );
        my @kept;
      ...
      } ## end if ( defined $single_pair)
    } ## end foreach my $chain (@$filtered_chains)
  } ## end foreach my $chains_generecs...
...
sub write_output {
  ...
  foreach my $obj ( @{ $self->output } ) {
    ...
    foreach my $g (@res_genes) {
      $line_numbers += $self->write_gene( $gs, $g, $fh );
    }
  }
  ...
  if ( $wc[0] !~ $line_numbers ) {
    throw(
      "The number of lines in the output files differ to what should have
...
sub make_gene_scaffold_and_project_genes {
  my ( $self, $blocks, $res_genes, $name, $max_stops, $min_coverage, $min_non_gap, $add_gaps, $extend_into_gaps, $external_db_id ) = @_;

  my $gene_scaffold =
    Bio::EnsEMBL::Analysis::Tools::WGA2Genes::GeneScaffold->new( -name                 => $name,
  ...
  foreach my $res_gene (@$res_genes) {
    ...
    foreach my $tran ( @{ $res_gene->source_transcripts } ) {
      ...
      $proj_trans = $self->process_transcript( $proj_trans, $max_stops, $min_coverage, $min_non_gap, $tran->stable_id );
...
} ## end sub make_gene_scaffold_and_project_genes
...
sub process_transcript {
  my ( $self, $tran, $max_stops, $min_coverage, $min_non_gap, $source_id ) = @_;
  ...
  if ( $tsf->hcoverage < $min_coverage ) {
    logger_info( "Rejecting proj of $source_id because coverage (" . $tsf->hcoverage . ") is below threshold" );
    return 0;
  }
...
sub remove_contig_split_chains {
  ...
  foreach my $c (@$input_chains) {
    ...
    foreach my $bl (@$blocks) {
      ...
      my $tgac =
        Bio::EnsEMBL::Mapper::Coordinate->new( $tga->dnafrag->name, $tga->dnafrag_start, $tga->dnafrag_end, $tga->dnafrag_strand );
    ...
    foreach my $tgac (@all_coords) {
      ...
      if ( @merged_coords and $merged_coords[-1]->id eq $tgac->id and $merged_coords[-1]->strand eq $tgac->strand ) {

        if ( $tgac->strand > 0 ) {
          if ( $tgac->start > $merged_coords[-1]->end ) {
            my $can_merge = 1;
            foreach my $o_c ( @{ $coords_by_tid{ $tgac->id } } ) {
              if ( $o_c->start > $merged_coords[-1]->end and $o_c->end < $tgac->start ) {
                $can_merge = 0;
                last;
              }
            }
          ...
          if ( $tgac->end < $merged_coords[-1]->start ) {
            my $can_merge = 1;
            foreach my $o_c ( @{ $coords_by_tid{ $tgac->id } } ) {
              if ( $o_c->start > $tgac->end and $o_c->end < $merged_coords[-1]->start ) {
                $can_merge = 0;
                last;
              }
            }
      ...
      } ## end if ( @merged_coords and...)

      if ( not $merged ) {
        push @merged_coords, $tgac;
      }
    } ## end foreach my $tgac (@all_coords)
  ...
  TID: foreach my $id ( keys %coords_by_tid ) {
      ...
      for ( my $i = 1; $i < @chain; $i++ ) {
        ...
        my ($ex_prev) = extend_coord( $prev, $self->target_slices->{$id} );

        my ($ex_this) = extend_coord( $this, $self->target_slices->{$id} );
  ...
  } ## end foreach my $c (@$input_chains)

  return \@kept_chains;
} ## end sub remove_contig_split_chains
...
sub trim_exon_split_chains {
  ...
  CH: foreach my $ch (@$chains) {
      ...
      foreach my $bl (@$ch) {
        ...
        if ( $ga->dnafrag_start <= $cds->end and $ga->dnafrag_end >= $cds->start ) {
      ...
      if ($ov_bps) {
        push @ov_chains, { chain => $ch, ov_bps => $ov_bps, };
      }
      else {
    ...
    if ( scalar(@ov_chains) > 1 ) {
        ...
        foreach my $bl ( @{ $ov_chains[$i]->{chain} } ) {
          ...
          if ( $ga->dnafrag_end < $cds->start or $ga->dnafrag_start > $cds->end ) {
            ...
            if ( $ga->dnafrag_start < $cds->start ) {
              my $cut_bl = $bl->restrict_between_reference_positions( $ga->dnafrag_start, $cds->start - 1 );
              $cut_bl->score( $bl->score );
              push @retained_blocks, $cut_bl;

            }
            if ( $ga->dnafrag_end > $cds->end ) {
              my $cut_bl = $bl->restrict_between_reference_positions( $cds->end + 1, $ga->dnafrag_end );
              $cut_bl->score( $bl->score );
              push @retained_blocks, $cut_bl;
            }
          }
      ...
      } ## end for ( my $i = 1; $i < @ov_chains...)
      $chains = \@new_chains;
    } ## end if ( scalar(@ov_chains...))
  } ## end foreach my $cds (@nr_cds)

  return $chains;
} ## end sub trim_exon_split_chains
...
sub segregate_chains_and_generecords {
  ...
  return @res_clusters;
} ## end sub segregate_chains_and_generecords
...
sub write_agp {
  ...
  for ( my $i = 0; $i < @tsegs; $i++ ) {
    ...
    printf( $fh "$prefix %s\t%d\t%d\t%s\t%s\t%s\t%d\t%d\t%s\n",
            $gscaf->seq_region_name, $seg->from_start, $seg->from_end, $piece_count++, "W", $seg->to_Slice->seq_region_name,
            $seg->to_Slice->start, $seg->to_Slice->end, $seg->to_Slice->strand < 0 ? "-" : "+" );
    $line_count++;
    ...
    if ( $i < @tsegs - 1 ) {
      printf( $fh "$prefix %s\t%d\t%d\t%s\t%s\t%d\n",
              ...
              $piece_count++, "N", $tsegs[ $i + 1 ]->from_start - $seg->from_end - 1, );
      $line_count++;
    }
  }
  return $line_count;
  ...
  foreach my $tran ( @{ $g->projected_transcripts } ) {
    ...
    for ( my $i = 0; $i < @exons; $i++ ) {
      ...
      printf( $fh "%s %s\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%s=%s; %s=%s; %s=%s\n",
              $prefix,    $seq_id,       "WGA2Genes",  "Exon",           $exon->start,
              $exon->end, $exon->strand, $exon->phase, $exon->end_phase, "exon",
              $exon_id,   "transcript",  $tran_id,     "gene",           $gene_id );
      $line_count++;
      ...
      foreach my $sup_feat ( @{ $exon->get_all_supporting_features } ) {
        foreach my $ug ( $sup_feat->ungapped_features ) {
          printf( $fh "%s %s\t%s\t%s\t%d\t%d\t%d\t%s=%s; %s=%s; %s=%s; %s=%s; %s=%s; %s=%s\n",
                  $prefix,          $seq_id,             "WGA2Genes", "Supporting", $ug->start,
                  $ug->end,         $ug->strand,         "exon",      $exon_id,     "hname",
                  $ug->hseqname,    "hstart",            $ug->hstart, "hend",       $ug->hend,
                  "external_db_id", $ug->external_db_id, "hcoverage", $ug->hcoverage );
          $line_count++;
        }
      }
    }
...
sub create_output_dir {
  ...
  if ( !-e $output_dir ) {
    ...
    eval { system( "mkdir -p " . $output_dir ); };
    if ($@) {
...
sub read_and_check_config {
                    ...
                    TARGET_SPECIES_EXTERNAL_DBNAME) )
  {
    ...
    throw( "You must define $var in config for logic '$logic'" . " or in the DEFAULT entry" ) if not $self->$var;
  }
}
...
sub get_external_db_id {
  ...
  my $sth = $query_db->prepare( "SELECT external_db_id " . "FROM external_db " . "WHERE db_name = ?" );
...
sub new {
  ...
  my ( $name, $gene, $source_trans ) = rearrange( [ 'NAME', 'GENE', 'SOURCE_TRANSCRIPTS' ], @args );
...
sub clone {
  ...
  my $new_one =
    Bio::EnsEMBL::Analysis::RunnableDB::WGA2Genes::GeneRecord->new( -source_transcripts => $self->source_transcripts,
                                                                    -gene               => $self->gene );
...
sub _get_all_transcript_cds_features {
  ...
  foreach my $tran ( @{ $self->source_transcripts } ) {
    foreach my $exon ( @{ $tran->get_all_translateable_Exons } ) {
      push @orig_cds_feats, Bio::EnsEMBL::Feature->new( -start => $exon->start, -end => $exon->end, -slice => $exon->slice );
    }
  }
