...
sub fetch_input {
  ...
  my $count = 0;
  my @genes;
  my $subjectdb = new Bio::EnsEMBL::DBSQL::DBAdaptor( '-host'   => $self->PSILC_SUBJECT_DBHOST,
                                                      '-user'   => 'ensro',
                                                      '-dbname' => $self->PSILC_SUBJECT_DBNAME,
                                                      '-port'   => $self->PSILC_SUBJECT_DBPORT, );
  my $orth1db = new Bio::EnsEMBL::DBSQL::DBAdaptor( '-host'   => $self->PSILC_ORTH1_DBHOST,
                                                    '-user'   => 'ensro',
                                                    '-dbname' => $self->PSILC_ORTH1_DBNAME,
                                                    '-port'   => $self->PSILC_ORTH1_DBPORT, );
  my $orth2db = new Bio::EnsEMBL::DBSQL::DBAdaptor( '-host'   => $self->PSILC_ORTH2_DBHOST,
                                                    '-user'   => 'ensro',
...
PFAM: foreach my $gene (@genes) {
  ...
  TRANSCRIPT: foreach my $transcript ( @{ $gene->get_all_Transcripts } ) {
    ...
    } ## end TRANSCRIPT: foreach my $transcript ( @{...})
  ...
  } ## end PFAM: foreach my $gene (@genes)
...
sub run_blast {
  ...
  my $blast = Bio::EnsEMBL::Analysis::Runnable::PSILC_BlastP->new( '-trans' => $pfam_transcript, '-analysis' => $self->analysis, );
  $blast->run;
  return $blast->output;
}
...
sub run_PSILC {
  ...
  my %domains = %{ $self->domains };
  # Run PSILC analysis
  my $PSILC = Bio::EnsEMBL::Analysis::Runnable::PSILC->new( '-trans'          => $pfam_transcript,
                                                            '-homologs'       => $homologs,
