...
sub run {
  ...
  my ( $clustered, $unclustered ) =
    cluster_Genes( [ @{ $self->merged_genes }, @{ $self->tissues_genes } ], \%types_hash, $self->CODING_ONLY, $self->IGNORE_STRAND );
  ...
  if ( $self->CHECK_SINGLE ) {
    ...
    if ( scalar @$single_set_clusters ) {
      ...
      foreach my $single_set_clust (@$single_set_clusters) {
        ...
        for my $set (@sets) {
         ...
         #$self->report_single_set_clusters($single_set_clust, $set1_name, $self->input_id, $oneway_clust_sr_start, $oneway_clust_sr_end);
            ...
            $self->report_single_set_clusters( $single_set_clust, $set2_name, $self->input_id, $oneway_clust_sr_start,
                                               $oneway_clust_sr_end );
      ...
      } ## end foreach my $single_set_clust...
    } ## end if ( scalar @$single_set_clusters)
    else {
  ...
  if ( $self->CHECK_TWOWAY ) {
    ...
    if ( scalar(@$twoway_clusters) ) {
      my $bam = Bio::DB::Sam->new( -bam => $self->INTRON_BAM_FILE, -autoindex => 1, );
      ...
      foreach my $twoway_clust (@$twoway_clusters) {
        ...
        logger_info(
             "\t\tLooking at Cluster number $cluster_counter: " . $self->input_id . " $twoway_clust_sr_start - $twoway_clust_sr_end.\n" );
        ...
        if ( $set1_start != $set2_start or $set1_end != $set2_end ) {
          ...
          foreach my $set1_gene (@set1_genes_overlapped_with_set2_genes) {
            ...
            if ( $is_set2_better > 0 ) {
              print STDERR 'You might want to delete this model: ', $set1_gene->display_id, 'Start: ', $set1_gene->seq_region_start,
                ' End: ', $set1_gene->seq_region_end, ' Strand: ', $set1_gene->strand, "\n";
              if (
                 !( $bestgene->seq_region_end < $set1_gene->seq_region_start or $set1_gene->seq_region_end < $bestgene->seq_region_start )
                )
              {
...
#                    print STDERR 'DEBUG DELETED ', $bestgene->seq_region_end, ' < ', $set1_gene->seq_region_start, ' OR ', $set1_gene->seq_region_end , ' < ', $bestgene->seq_region_start, "\n";
        ...
        } ## end if ( $set1_start != $set2_start...)
        $self->output( \@set1_genes_overlapped_with_set2_genes );
        $self->output( \@set2_genes_overlapped_with_set1_genes );
      } ## end foreach my $twoway_clust (@$twoway_clusters)
    } ## end if ( scalar(@$twoway_clusters...))
...
sub check_intron_support {
  ...
  for ( my $i = 0; $i < ( scalar(@set1_exons) - 1 ); $i++ ) {
    my $count_support =
      count_support( $bam, $set1_gene->seq_region_name, $set1_exons[$i]->seq_region_end, $set1_exons[ $i + 1 ]->seq_region_start );
    push( @count_support_set1, $count_support );
    $set1_count_support += $count_support;
  }
  for ( my $i = 0; $i < ( scalar(@set2_exons) - 1 ); $i++ ) {
    my $count_support =
      count_support( $bam, $set2_gene->seq_region_name, $set2_exons[$i]->seq_region_end, $set2_exons[ $i + 1 ]->seq_region_start );
...
sub print_gene_info {
  ...
  print STDOUT $gene->slice->seq_region_name . " " .
    $gene->seq_region_start . " " . $gene->seq_region_end . " " . $gene->biotype . " max_cod_ex_cnt: " . max_coding_exon_count($gene);
...
#  print STDERR 'DEBUG: ', $longest_translation_gene->display_id, '/', $longestID, '@', $max_translation_length , ' -- ', $max_exon_gene->display_id, '/', $maxexonID, '@', $max_exon_cnt , ' -- ', $longest_exons_gene->display_id, '/', $longexonID, '@', $max_exon_length, "\n";
 ...
 #              print STDERR 'DEBUG: hqfragmented: ', $most_abundant{$ids[0]}{genes}[0]->dbID, "\t", $most_abundant{$ids[0]}{count}, "\n";
      ...
      if ( $num_genes < $num_groups/10 ) {
        ...
        print STDERR 'INFO: ', $most_abundant{ $ids[0] }{count}, ' => ', $most_abundant{ $ids[0] }{genes}[0]->dbID, ' & ',
          $most_abundant{ $ids[1] }{genes}[0]->dbID, "\n";
      }
    }
  } ## end if ( @ids > 1 )
  if ( $longestID == $maxexonID and
       $longestID == $longexonID
       and
       ( grep { $_->dbID == $longexonID } @{ $most_abundant{ $ids[0] }{genes} } or
         ( $most_abundant{ $ids[1] }{count} > $most_abundant{ $ids[0] }{count}/2 and
           grep { $_->dbID == $longexonID } @{ $most_abundant{ $ids[1] }{genes} } ) ) )
  {
...
#              print STDERR 'DEBUG: using gene with maximum number of copies ', $selected_gene->dbID, ' => ', $most_abundant{$ids[0]}{genes}[0]->dbID, "\n";
    ...
    } ## end else [ if ( grep { $_->dbID ==...})]
  } ## end else [ if ( $longestID == $maxexonID... [elsif ( grep { $_->dbID ==...})])]
  ...
  print STDERR 'CHOSEN ', $bestgene->display_id, ' / ', $bestgene->dbID, ' @ ', $bestgene->seq_region_start, ' - ',
    $bestgene->seq_region_end, "\n";

  return $bestgene;
} ## end sub get_longest_translation_gene
...
} ## end sub check_frameshift_intron
