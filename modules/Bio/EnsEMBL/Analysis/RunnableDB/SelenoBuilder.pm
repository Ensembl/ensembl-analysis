...
sub fetch_input {
  ...
  my $targetseqobj = Bio::Seq->new( -display_id => $slice->name, -seq => $slice->seq );

  #write_seqfile($seqobj,
  #             create_file_name($dna->display_name(), "fa",
  #             "/tmp/"));

  my $seqout = Bio::SeqIO->new( -file => ">" . $target_file, -format => 'fasta', );
  ...
  foreach my $transcript (@all_transcripts) {
    ...
    NEW: foreach my $new_trans (@transcripts) {
        ...
        for ( my $i = 0; $i < scalar(@c_exons); $i++ ) {
          next NEW
            unless ( $c_exons[$i]->start == $new_c_exons[$i]->start &&
        ...
        if ( $exons[0]->strand == 1 ) {
          ...
          next NEW
            unless (    #$exons[0]->start == $new_exons[0]->start &&
          ...
          if ( scalar(@exons) > 2 ) {
            for ( my $i = 1; $i < scalar(@exons) - 1; $i++ ) {
              next NEW
                unless ( $exons[$i]->start == $new_exons[$i]->start &&
          ...
          next NEW
            unless (
            $exons[0]->start == $new_exons[0]->start &&
            #$exons[0]->end == $new_exons[0]->end &&
            $exons[0]->strand == $new_exons[0]->strand &&
            #$exons[-1]->start == $new_exons[-1]->start &&
            $exons[-1]->end == $new_exons[-1]->end && $exons[-1]->strand == $new_exons[-1]->strand );
        ...
        SOURCE_FEAT:
          ...
          } ## end SOURCE_FEAT: foreach my $feat ( @{ $exons...})
      ...
      T_SOURCE_FEAT:
        ...
        } ## end T_SOURCE_FEAT: foreach my $t_feat ( @{ $transcript...})
      } ## end NEW: foreach my $new_trans (@transcripts)
      if ( $transcript_exists == 0 ) {
        push( @transcripts, $transcript );
      }

    } ## end if ( @transcripts && scalar...)
    else {
      push( @transcripts, $transcript );
    }

  } ## end foreach my $transcript (@all_transcripts)
...
} ## end sub filter_redundant_transcript
...
sub read_and_check_config {
  ...
  throw( "QUERYANNOTATION '" . $self->QUERYANNOTATION . "' in config must be readable" )
    if $self->QUERYANNOTATION and
    not -e $self->QUERYANNOTATION;
