...
sub fetch_input {
  ...
  foreach my $bit ( @{ $genes_slice->project('seqlevel') } ) {
    push @seq_level_features,
      Bio::EnsEMBL::Feature->new( -start  => $bit->from_start + $genes_slice->start - 1,
...
REPLOOP: foreach my $repeat (@repeats) {
    ...
    if ( $rc->repeat_class !~ /LINE/ && $rc->repeat_class !~ /LTR/ && $rc->repeat_class !~ /SINE/ && $rc->repeat_class !~ /Unknown/ ) {
      next REPLOOP;
    }
    if ( $repeat->start <= 0 ) {
      $repeat->start(1);
    ...
    if ( defined($curblock) && $curblock->end >= $repeat->start ) {
      ...
      $curblock = Bio::EnsEMBL::Feature->new( -START => $repeat->start, -END => $repeat->end, -STRAND => $repeat->strand );
