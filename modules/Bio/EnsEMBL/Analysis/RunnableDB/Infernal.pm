...
sub fetch_input {
  ...
  # Make  the runnable
  my $runnable = $runname->new( -queries => \@dafs, -analysis => $self->analysis, -program => $self->analysis->program_file );
...
sub write_output {
  ...
  foreach my $gene_hash ( @{ $self->output } ) {
    ...
    if ($@) {
      $self->throw( "Infernal:store failed, failed to write " . $gene . " to " . "the database $@" );
    }
    foreach my $trans ( @{ $gene->get_all_Transcripts } ) {
      eval {
      ...
      if ($@) {
        $self->throw( "Infernal:store failed, failed to write xrefs or attributes  on transcript " . $trans . " in the database $@" );
      }
    }
