...
sub fetch_input {
  ...
  if ( defined $self->OUT_SAM_DIR ) {
    ...
    my $query_seqs = Bio::SeqIO->new( -file => $self->QUERYSEQS . "/" . $self->input_id, -format => 'fasta' );
    $self->throw( "Cannot open " . $self->QUERYSEQS . "/" . $self->input_id . " for reading querys into hash \n" ) unless $query_seqs;
...
sub run {
  ...
  $self->get_dbadaptor( $self->main_reference_db )->dbc->connect unless $self->get_dbadaptor( $self->main_reference_db )->dbc->connected;
...
FEATURE: foreach my $f (@$flist) {
    ...
    if ( $self->INTRON_OVERLAP && !$self->INTRON_MODELS ) {
      ...
      $accept = 1 if ( $es && $ee && $ee >= $self->INTRON_OVERLAP && $es <= $f->length - $self->INTRON_OVERLAP );
    } ## end if ( $self->INTRON_OVERLAP...)
    ...
    if ( $self->PROJECT ) {
      ...
      if ( $f->{'_intron'} and not $self->OUT_SAM_DIR ) {
        ...
        if ( scalar(@features) == 2 ) {
          ...
          if ( $left_splice->seq eq 'NN' && $right_splice->seq eq 'NN' ) {
            warn( "Cannot find dna sequence for " . $f->display_id . " this is used in detetcting non cannonical splices\n" );
          }
          else {
            # is it cannonical
        ...
        } ## end if ( scalar(@features)...)
      } ## end if ( $f->{'_intron'} and...)
      ...
      my $unique_id = $feat->seq_region_name . ":" . $feat->start . ":" . $feat->end . ":" . $feat->strand . ":" . $feat->hseqname;
