...
use Bio::EnsEMBL::Analysis::Config::GeneBuild::GeneBuilder qw(GENEBUILDER_CONFIG_BY_LOGIC);
...
use Bio::EnsEMBL::Analysis::Tools::GeneBuildUtils::TranscriptUtils qw(are_strands_consistent are_phases_consistent
...
sub fetch_input {
  ...
  my $runnable = Bio::EnsEMBL::Analysis::Runnable::GeneBuilder->new( -query                       => $self->query,
...
sub input_genes {
  my ( $self, $arg ) = @_;
  if ($arg) {
    throw( "Need to pass input genes an arrayref not " . $arg ) if ( !ref($arg) || ref($arg) ne 'ARRAY' );
...
sub read_and_check_config {
  ...
  foreach my $var ( qw(INPUT_GENES
                    OUTPUT_DB) )
  {
    throw("RunnableDB::GeneBuilder $var config variable is not defined") unless ( $self->$var );
  }
    ...
    foreach my $biotype (@$biotypes) {
      if ( !$unique{$biotype} ) {
        ...
        if ( $self->BLESSED_BIOTYPES->{$biotype} ) {
          throw( $biotype . " is defined for both " . $key . " and " . $unique{$biotype} . " and is found in the blessed biotype hash\n" .
                 "This is likely to cause problems for the filtering done in " . "the genebuilder code" );
        }
        else {
          warning(
             $biotype . " appears twice in your listing, make sure this " . "isn't for the same database otherwise it will cause issue" );
        }
      }
    }
  }
