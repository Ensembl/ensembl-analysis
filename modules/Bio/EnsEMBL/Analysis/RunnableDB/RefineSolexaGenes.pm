...
sub fetch_input {
  ...
  $self->intron_slice_adaptor( $self->get_dbadaptor( $self->INTRON_DB )->get_SliceAdaptor ) if $self->INTRON_DB;
  ...
  my $gene_slice = $self->gene_slice_adaptor->fetch_by_region( 'toplevel', $slice->seq_region_name, $slice->start, $slice->end, 1 );

  my $chr_slice = $self->db->get_SliceAdaptor->fetch_by_region( 'toplevel', $slice->seq_region_name );
  ...
  if ( $self->INTRON_BAM_FILES ) {
    foreach my $intron_files ( @{ $self->INTRON_BAM_FILES } ) {
      my $sam = Bio::DB::Sam->new( -bam => $intron_files->{FILE}, -autoindex => 1, -expand_flags => 1, );
  ...
  STRAND: for ( my $strand = -1; $strand <= 1; $strand += 2 ) {
    ...
    EXON: for ( my $i = 0; $i <= $exon_count; $i++ ) {
      ...
      INTRON: foreach my $intron (@$introns) {
        ...
        } ## end INTRON: foreach my $intron (@$introns)
        ...
        if (
          $self->STRICT_INTERNAL_SPLICE_SITES &&
        ...
        } ## end if ( $self->STRICT_INTERNAL_SPLICE_SITES...)
        else {
...
# I guess we also want to keep the original exon too?
          ...
          } ## end RETAINED: foreach my $intron (@retained_introns)
        ...
        } ## end if ( scalar(@retained_introns...))
      } ## end EXON: for ( my $i = 0; $i <= ...)
    ...
    if ( $self->SINGLE_EXON_MODEL && $single_exon == 2 ) {
      ...
      my $fwd_t_len;
      $fwd_t_len = $fwd_tran->translation->genomic_end - $fwd_tran->translation->genomic_start if $fwd_tran->translateable_seq;
      ...
      my $rev_t_len;
      $rev_t_len = $rev_tran->translation->genomic_end - $rev_tran->translation->genomic_start if $rev_tran->translateable_seq;
      #print STDERR "REV t length $rev_t_len\n";
      if ( $fwd_tran->translateable_seq && ( $fwd_t_len/$fwd_tran->length )*100 >= $self->SINGLE_EXON_CDS && $fwd_t_len > $rev_t_len ) {
        # keep this one
        $single_exon_model = $fwd_tran;
      }
      if ( $rev_tran->translateable_seq && ( $rev_t_len/$rev_tran->length )*100 >= $self->SINGLE_EXON_CDS && $rev_t_len > $fwd_t_len ) {
        # keep this one
    ...
    } ## end if ( $self->SINGLE_EXON_MODEL...)
  } ## end GENE: foreach my $gene ( @{ $self...})
} ## end sub refine_genes
    ...
    OTHERGENE: foreach my $gene ( @{ $stranded_cluster->{'final_models'} } ) {
      ...
      BESTEXON: for ( my $i = 0; $i <= $#best_exons; $i++ ) {
        ...
        OTHEREXON: for ( my $j = 0; $j <= $#other_exons; $j++ ) {
            ...
            if ( $be->start <= $oe->end && $be->end >= $oe->start ) {
      ...
      } ## end OTHERGENE: foreach my $gene ( @{ $stranded_cluster...})
...
sub filter_models {
  ...
  foreach my $fc (@fwd) {
    foreach my $rc (@rev) {
      ...
      if ( ( $fc->{'start'} >= $rc->{'start'} && $fc->{'end'} <= $rc->{'end'} ) or
      ...
      FG: foreach my $fg (@fg) {
          ...
          RG: foreach my $rg (@rg) {
              ...
              foreach my $re ( @{ $rt->get_all_translateable_Exons } ) {
                if ( $fe->{'start'} <= $re->{'end'} && $fe->{'end'} >= $re->{'start'} ) {
                  # coding overlap
        ...
        } ## end FG: foreach my $fg (@fg)
      } ## end if ( ( $fc->{'start'} ...))
    } ## end foreach my $rc (@rev)
  } ## end foreach my $fc (@fwd)
  ...
  foreach my $cluster (@models) {
    ...
    foreach my $gene ( @{ $cluster->{'final_models'} } ) {
      ...
      if ( $ee == scalar( @{ $gene->get_all_Exons } ) && $es == scalar( @{ $gene->get_all_Exons } ) ) {
    ...
    for ( my $g = 0; $g < scalar(@final_models); $g++ ) {
      ...
      if ( $transcript->translateable_seq ) {
        ...
        if ( $gene->biotype eq 'bad' && $transcript->cdna_coding_end - $transcript->cdna_coding_start > $best_cds ) {
          $gene->biotype( $self->OTHER_ISOFORMS );
        }
      }
      if ( $gene->biotype eq 'bad' ) {
    ...
    } ## end for ( my $g = 0; $g < scalar...)
  } ## end foreach my $cluster (@models)
} ## end sub filter_models
  ...
  MODEL: foreach my $model (@models_by_score) {
      ...
      for ( my $i = 0; $i < scalar(@$model); $i++ ) {
        if ( $model->[$i]->isa("Bio::EnsEMBL::DnaDnaAlignFeature") ) {
          ...
          my $ise = Bio::EnsEMBL::IntronSupportingEvidence->new( -ANALYSIS   => $intron->analysis,
                                                                 -INTRON     => $if,
    ...
    } ## end MODEL: foreach my $model (@models_by_score)
    ...
    foreach my $tran (@trans) {
      ...
      $new_gene->stable_id( $gene->stable_id . "-v$cluster_count.$version-" .
                      int( $tran->{'_score'} ) . "-" .
                      int( $tran->{'_depth'} ) . "-" . $tran->{'_intron_count'} . "-NC-" . $tran->{'_NC_introns'} . "-" . $tran->strand );
      push @{ $cluster->{'final_models'} }, $new_gene;
    }
  } ## end foreach my $cluster (@model_clusters)
...
sub prune_UTR {
  ...
  print "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
Trimming UTR
Transcript " . $transcript->seq_region_name . " " . $transcript->start . " " . $transcript->end . " " . $transcript->strand . " " .
    scalar( @{ $transcript->get_all_Exons } ) . "

";
  throw( "Something is wrong we are missing " .
         scalar( @{$introns} ) . " introns " .
         scalar(@exons) . "  exons " .
         scalar(@features) . " exons and introns\n" )
...
sub modify_transcript {
  ...
  my $translation = new Bio::EnsEMBL::Translation->new( -START_EXON => $se, -END_EXON => $ee, -SEQ_START => $ts, -SEQ_END => $te, );
...
GENE: foreach my $gene (@output) {
    ...
    $total++;
  } ## end GENE: foreach my $gene (@output)
  if ( $fails > 0 ) {
    throw( "Not all genes could be written successfully " . "($fails fails out of $total)" );
  }
  ...
  foreach my $intron ( @{ $self->intron_features } ) {
    ...
    eval { $intron_adaptor->store($intron); };
    if ($@) {
  ...
  if ( $fails > 0 ) {
    throw( "Not all introns could be written successfully " . "($fails fails out of $total)" );
  }
  ...
  for ( my $i = 0; $i < scalar( @{$exons} ); $i++ ) {
    if ( $exon_intron->[$i] ) {
      ...
      if ( $strict > 1 ) {
        ...
        print STDERR "EXON $i:" . $exons->[$i]->start . " - " . $exons->[$i]->end . " - " . $exons->[$i]->strand . "\n";
        foreach my $group ( keys %intron_groups ) {
          ...
          if ( scalar( @{ $intron_groups{$group} } ) > 1 ) {
            ...
            print STDERR "Eliminating " . $intron->hseqname . " " . $intron->score . "\n";
    ...
    } ## end if ( $exon_intron->[$i...])
  } ## end for ( my $i = 0; $i < scalar...)

  if ( $strict & !$removed ) {
    warn( "Cannot simplify this gene any more " .
          "EXON 0:" . $exons->[0]->start . " - " . $exons->[0]->end . " - " . $exons->[0]->strand . "\n" );
    if ( $self->recursive_limit < $self->MAX_RECURSIONS ) {
      ...
      warn( "Giving up on " . "EXON 0:" .
            $exons->[0]->seq_region_name . " - " . $exons->[0]->start . " - " . $exons->[0]->end . " - " . $exons->[0]->strand . "\n" );
      return "Give up";
    }
  }
...
sub model_cluster {
  ...
  foreach my $model (@models) {
    ...
    foreach my $cluster (@clusters) {
      # do they overlap?
      if ( $model->[0]->start <= $cluster->{'end'} && $model->[-3]->end >= $cluster->{'start'} ) {
        # Expand the cluster
        $cluster->{'start'} = $model->[0]->start if $model->[0]->start < $cluster->{'start'};
        $cluster->{'end'}   = $model->[-3]->end  if $model->[-3]->end > $cluster->{'end'};
...
sub merge_exons {
  ...
  foreach my $key ( keys %$extra_exons ) {
    ...
    foreach my $exon (@exons) {
      if ( $start_anchor <= $exon->end && $start_anchor >= $exon->start ) {
        $start_anchor = -1;
      }
      if ( $end_anchor <= $exon->end && $end_anchor >= $exon->start ) {
        $end_anchor = -1;
      }
    }
  ...
  } ## end foreach my $key ( keys %$extra_exons)
  ...
  while ( $ec != scalar(@exons) ) {
    ...
    for ( my $i = 1; $i < scalar(@exons); $i++ ) {
      ...
      # do they overlap
      if ( $left_exon->start <= $right_exon->end && $left_exon->end >= $right_exon->start ) {
        # merge them
        if ( $right_exon->end >= $left_exon->end && $right_exon->start <= $left_exon->start ) {
          $left_exon->{"_extra"} = 0;
        }
        $left_exon->start( $right_exon->start ) if $right_exon->start < $left_exon->start;
        $left_exon->end( $right_exon->end ) if $right_exon->end > $left_exon->end;
  ...
  for ( my $i = 1; $i <= $#exons; $i++ ) {
    ...
    foreach my $intron (@$introns) {
      ...
      if ( $intron->start > $prev_exon->start && $intron->end < $exon->end && $intron->strand == $strand ) {
        $intron_count++;
      }
    }
...
READ: while ( my $read = $iterator->next_seq ) {
    ...
    for ( my $i = 0; $i <= $#mates; $i++ ) {
      ...
      next if $i == $#mates;
      my $unique_id = $name . ":" . $mates[$i]->[3] . ":" . $mates[ $i + 1 ]->[2] . ":" . $strand;
      $id_list{$unique_id}++;
      #    print "$unique_id";
    }
    #   print "\n";
  } ## end READ: while ( my $read = $iterator...)
  ...
  foreach my $key ( keys %id_list ) {
    ...
    my $if = Bio::EnsEMBL::DnaDnaAlignFeature->new( -start        => $data[1],
                                                    -end          => $data[2],
                                                    -strand       => $data[3],
                                                    -hstart       => 1,
                                                    -hend         => $length,
    ...
    my $left_splice  = $slice_adaptor->fetch_by_region( 'toplevel', $if->seq_region_name, $if->start + 1, $if->start + 2, $if->strand );
    my $right_splice = $slice_adaptor->fetch_by_region( 'toplevel', $if->seq_region_name, $if->end - 2,   $if->end - 1,   $if->strand );
    ...
    push @ifs, $if;
  } ## end foreach my $key ( keys %id_list)
  # sort them
  ...
  if ( $self->FILTER_ON_OVERLAP ) {
    ...
    if ( $array_length > 1 ) {
      ...
      } ## end for ( my $j = 0; $j < $array_length...)
      @ifs = @tmp_array;
    } ## end if ( $array_length > 1)
  } ## end if ( $self->FILTER_ON_OVERLAP)
...
sub ungapped_features {
  ...
  } ## end foreach ( my $i = 0; $i < scalar...)
  return \@ugfs;
} ## end sub ungapped_features
...
sub dna_2_extra_exons {
  ...
  my $small_intron_slice =
    $self->small_intron_slice_adaptor->fetch_by_region( 'toplevel', $self->chr_slice->seq_region_name, $start, $end, );
  ...
  ROUGH: foreach my $rough (@$rough_genes) {
    ...
    for ( my $i = 0; $i < scalar(@ugfs) - 1; $i++ ) {
      ...
      $name =~ s/\./*/g;
      my $unique_id = $name . ":" . $ugfs[$i]->end . ":" . $ugfs[ $i + 1 ]->start . ":" . $read->strand . ":$type";
      $id_list{$unique_id}++;
      if ( $i > 0 && $i < scalar(@ugfs) - 1 ) {
        ...
        my $name = $read->seq_region_name;
        $name =~ s/\./*/g;
        my $unique_id = $name . ":" . $ugfs[$i]->start . ":" . $ugfs[$i]->end . ":" . $read->strand . ":$type";
        $exon_list{$roughid}{$unique_id} = $ugfs[$i];
      }
    }
  } ## end while ( scalar @extra_reads...)
...
sub dna_2_intron_features {
  ...
  my %id_list;
  my $rough_genes = $self->prelim_genes;
  my $intron_slice = $self->intron_slice_adaptor->fetch_by_region( 'toplevel', $self->chr_slice->seq_region_name, $start, $end, );
...
INTRON: foreach my $intron (@chosen_sf) {
    if ( $intron->hseqname =~ /non canonical/ ) {
      ...
      if ( $intron->score > 1 && $intron->end - $intron->start < 150000 ) {
        ...
        say STDERR "Discarding canonical INTRON with score" .
          $intron->score . " from " . $intron->start . " to " . $intron->start . " on " . $intron->seq_region_name;
      }
    }
  } ## end INTRON: foreach my $intron (@chosen_sf)
  return ( \@filtered_introns, $index );
} ## end sub fetch_intron_features
...
sub make_exon {
  ...
  my $padded_exon = create_Exon( $start - 20, $end + 20, -1, -1, -1, $self->analysis, undef, undef, $self->chr_slice, );
  # dont let it fall of the slice because of padding
  $padded_exon->start(1) if $padded_exon->start <= 0;
  $padded_exon->end( $self->chr_slice->length - 1 ) if $padded_exon->end >= $self->chr_slice->length;

  my $feat = new Bio::EnsEMBL::DnaDnaAlignFeature( -slice        => $self->chr_slice,
